import type { ActionContext, CompiledActionNode, ExpressionCompiler, RendererEnv } from '@nop-chaos/flux-core';
...
describe('ajax action messages', () => {
  it('shows messages.success via direct executeRuntimeAjaxAction call', async () => {
    ...
    const result = await executeRuntimeAjaxAction(
      ...
      } as unknown as CompiledActionNode,
      { scope: { id: 'scope-1' } } as unknown as ActionContext,
      undefined,
      {
  ...
  it('supports template evaluation in messages', async () => {
    ...
    const result = await executeRuntimeAjaxAction(
      ...
      { scope } as unknown as ActionContext,
      undefined,
      {
        getEnv: () => ({ notify } as unknown as RendererEnv),
        expressionCompiler: {} as unknown as ExpressionCompiler,
  ...
  it('shows confirm dialog and cancels when user declines', async () => {
    ...
    const result = await executeRuntimeAjaxAction(
      ...
      undefined,
      {
        getEnv: () => ({ notify, confirm } as unknown as RendererEnv),
        expressionCompiler: {} as unknown as ExpressionCompiler,
        evaluate: <T>(target: unknown) => target as T,
        executeApiRequest,
      },
  ...
  it('proceeds with ajax when user confirms', async () => {
    ...
    const result = await executeRuntimeAjaxAction(
      ...
      undefined,
      {
        getEnv: () => ({ notify: vi.fn(), confirm } as unknown as RendererEnv),
        expressionCompiler: {} as unknown as ExpressionCompiler,
        evaluate: <T>(target: unknown) => target as T,
        executeApiRequest,
      },
