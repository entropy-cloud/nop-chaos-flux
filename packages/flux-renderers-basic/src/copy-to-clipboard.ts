...
function fallbackExecCommand(text: string): boolean {
  ...
  try {
    ...
    if (isDevRuntime()) {
      console.warn('[TextRenderer] copy-to-clipboard fallback failed', error);
    }
    return false;
  }
}
...
export async function copyToClipboard(text: string): Promise<CopyToClipboardResult> {
  ...
  if (
    ...
    try {
      ...
      if (isDevRuntime()) {
        console.warn(
          '[TextRenderer] navigator.clipboard.writeText rejected; falling back to execCommand',
          error,
        );
      }
    }
  ...
  if (isDevRuntime()) {
    console.warn('[TextRenderer] copy-to-clipboard denied (HTTPS / permissions required)');
  }
