...
这就是为什么 Flux 可以被理解为一个"执行 DSL 的虚拟机"：它执行的是**最终执行切片**，不是整个平台的全部实现。对 schema 作者可见的输入输出，尽量通过统一边界进出；超出这层的宿主与领域复杂性，则保持在 core 之外。
...
Base Tree 可以近似理解为 schema 编译后的结构树或模板图。它描述的是程序的静态结构：有哪些节点、节点之间的父子关系、每个节点的类型和属性。

关键点：Base Tree 不负责开放式 runtime 组装。运行时允许的是对已进入 execution model 的结构做**受控激活或实例化**，例如 `when` 的结构参与、`loop` 的重复实例化、`dynamic-renderer` 的受控延迟片段装配；它不是重新打开 authoring/loader 阶段的继承或模块合并。
...
它不是"数据"，而是**值的生产过程**。API 请求是最常见的例子：它发起请求、等待响应、返回结果。请求过程中它有状态（loading/success/error），请求完成后它有值。
...
- **Capability** 决定“有没有权力触达外部世界、由谁执行”
- **Resource** 决定“这个交互在 Flux 内部如何被生命周期化、状态化，并发布为值”

以 API 请求为例：
- 能不能发、怎么发、由谁发，是 `Capability` / 宿主 transport boundary 的职责
...
### 一个有用但必须收住的类比：把 Resource 近似看成 computed 的运行时扩展
...
这种理解帮助你看清 Flux 响应式系统的演化路径：从简单的响应式绑定（`${a + b}`），到同步派生值类比，再到 runtime-owned 的值生产单元，是一条自然的扩展线。
...
这些特性让 Resource 成为一个完整的"runtime-owned 值生产者"抽象，而不仅仅是同步派生值的放大版。
...
关键点：Reaction 的生命周期由**词法作用域**决定，而不是 React 组件的挂载/卸载。更准确地说，它遵循 lexical ownership：当某个 scope 对应的 runtime sidecar bucket 被销毁时，该 scope 拥有的 Reaction 也会被清理。

这是一个重要的设计选择：Flux 的响应式系统在语义上独立于 React 组件生命周期。当前实现通常仍通过 renderer/runtime wiring 注册这些 sidecar，但 React 只是 Flux 的一个渲染后端，不是它的核心本体。
...
Host Projection 最容易被误读。它不是“把 Flux 内部状态暴露给宿主的调试接口”，而是**把宿主拥有的只读快照投影进 schema 可见环境**。
...
### 最直接的理解：依赖变化后，运行时对它们的处理不同

最关键的不是哲学化地证明三者有多“优雅”，而是理解：**依赖变化发生后，runtime 对它们的处理完全不同**。
...
再强调一次：这里的含义不是把 `ScopeRef` 设计成"数据 + 行为 + 生命周期"的大对象。`ScopeRef` 仍然保持纯数据词法环境；source/reaction 的生命周期所有权是在 runtime sidecar 中按 scope 归属实现的。
...
但必须记住：**三棵树是工程直觉，不是比七原语更高优先级的本体论**。更精确的规范仍然是 `Base Tree + ScopeRef + Value/Resource/Reaction + Capability + Host Projection`。三棵树只是帮助你把这些东西先分层看清。
...
> - ComponentTree ≈ `Base Tree` 这层结构直觉
...
但要始终记住本文的角色：**它是解释文档，不是最高优先级的规范文档。** 当本文与 `docs/architecture/frontend-programming-model.md` 或更窄 owning doc 冲突时，应以后者为准。
...
1. **Base Tree**：先确定这里有一个 `container`、一个 `text`、一个 `data-source`、一个 `reaction`。
...
5. **Capability**：真正发请求时，transport 仍然经由 `env.fetcher` 这类 capability boundary。
...
然后看 `packages/flux-runtime/src/schema-compiler.ts`，理解 execution compiler 如何把 final execution schema 降到 runtime template；它展示的是执行侧编译，不是重新执行 platform / loader assembly。
