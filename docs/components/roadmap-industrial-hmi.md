...
- `2026-08-05-2129-multi-audit-industrial-hmi.md` `[P2-5]`（dim 19）— `handler-error` 诊断丢原始 Error（`scada-canvas.tsx:176-177` 仅取 message）且无 `monitor.onError` telemetry 路径（`reportDiagnostic` 仅对 `flux-*` 码转发 monitor）。建议传原始 error 第三参 + 扩 monitor typing admit `'action'` phase 或加 `onActionError` 通道。
- `2026-08-05-2129-multi-audit-industrial-hmi.md` `[P2-6]`（dim 19）— `findCircularDependencyError`（`dirty-collector.ts:137-148`）cause-chain 深度上限 10，对 10+ 层 expression chain（工业过程管线现实场景）的 cycle 会误判为 generic eval 失败。建议移除人工上限或提至 ~1000（`cause===current`/`undefined` 守卫已防无限循环）。
