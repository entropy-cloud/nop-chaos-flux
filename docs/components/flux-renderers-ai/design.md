...
>
> **投影重建触发条件（P1-6 2026-08-10 multi-audit + FIND-06 2026-08-11 补盲区）**：投影快照不仅在 turn-boundary（`isProcessing` true→false）重建——engine 替换（会话切换 / `clear()` / `setMessages` 复水）为双 idle，无翻转，快照会无限期显示上一会话。现触发 = ① `isProcessing` 翻转（boundary 克隆）；② `requestState` 进入终态（completed/aborted/error）；③ messages **数组引用**变化（clear/setMessages/engine swap 在 mutate recipe 内整体替换数组）——**无条件**（FIND-06 盲区 B：交换到后台流式会话 B(idle→processing) 时立即克隆，streaming 期间数组引用稳定不会误触发）；④ 空闲期 messages **length** 变化（engine 原地变更——abort 空产物 drop 的 `splice` 不换引用，此信号使 P2-14 幽灵在 abort 边界重建后被二次 idle 重建清除）；⑤ 空闲期消息集**元素恒等指纹**变化（FIND-06 盲区 A：last-message id + finishReason + tool_calls length——abort-mid-executor 后 `cleanDanglingAssistantAt` 的 strip-keeps-text 原地等长元素替换同引用同长度，仅指纹可见，修复前 pre-strip ghost tool_calls 保留到下一轮次）。成本纪律保持：streaming 期间不克隆（`snapSourceRef`/`snapLength`/`snapFingerprint` 恒等收敛，守卫不重触发）。proof 见 `ai-chat-projection.test.tsx` P1-6 用例集（engine swap / clear / 复水 / abort-ghost）+ FIND-06 用例集（strip 盲区 / swap-to-streaming 盲区）。
...
export interface BubbleContentRendererMatch {
  ...
  priority?: number;
  /** P1-9（2026-08-10）：消息级渲染器标记——matcher 检查消息级字段（如
   * tool_calls / reasoning_content / metadata.isError），渲染器每消息运行
   * 一次，与切片级 pass 并行（见下"消息级 vs 切片级"）。可选字段，默认
   * false（切片级、first-match-wins 语义不变）。消息级渲染器收到
...
**消息级 vs 切片级（P1-9，2026-08-10 multi-audit 修复）**：`ai-bubble` 渲染装配拆两个 pass 并行运行——**消息级 pass**（`messageLevel: true`：tools / reasoning / error）每消息运行一次（matcher 检查消息级字段），**切片级 pass**（markdown / image / data-part / text）按 `resolveContentSlices` 切片各自 first-match-wins。历史缺陷：markdown（NORMAL=0）按切片遮蔽 tools/reasoning（CONTENT=10），混合消息（文本 + reasoning_content / 文本 + tool_calls）只渲染文本——工具卡 / 推理面板永不出现。修复后混合消息**同时**渲染文本与工具卡 / 推理面板 / error 提示。约束：① 流式中（`message.loading === true`，首 chunk 前）消息级 pass 跳过，LOADING(-1) 切片渲染器独占（spinner 语义不变）；② `content:''` 分支零回归（空文本切片仍落回消息级 matcher）。
...
| **`engine`（外部注入）**                  | resolved prop / host page-data（`ai-chat.engine` schema 表达式，典型 `${engine}`） | host 经 `useConversation.activeEngine` 切换时重算 page-data → schema 重渲染 → `ai-chat` 拿到新 engine |
...
| 单会话 `engine` 实例                                                  | **域内部**                          | `useConversation` 内部 Map                                                                                                                                                                  | 不可进 scope（含函数/handler）。但**可经 schema `engine` prop 注入到 `ai-chat`**（resolved value，非 scope 快照）——与 `connector`/`conversationController` 同为 host 注入对象，不违反本行约束（§11.5 line 467 约束的是反应式 scope 快照，不是 resolved prop 值） |
...
| 消息编辑态（`message.state.editing`）                                 | **域内部**                          | `MessageEngine.setMessageEditing()` 写入 `message.state.editing`（`ai-bubble/user-edit.tsx` 读 `active` + `draft`，`ai-bubble/index.tsx` 派生 `isEditing`）                                 | 通过 `onAction: 'edit'/'resubmit'` event 触发；renderer 经 `engine.setMessageEditing()` 写入、经 `message.state.editing` 快照读取。引擎持有使 A-8 虚拟滚动回收编辑行后，重新挂载能恢复编辑器 + 草稿（不投影到 scope）                                            |
| 消息 metadata.usage（P4 token 用量）                                  | **域内部**（跟随 message.metadata） | connector 填充                                                                                                                                                                              | 不投影（消息级跟随 messages 快照）                                                                                                                                                                                                                               |
| 自动滚动位置                                                          | **local**                           | `useState` / `useRef`                                                                                                                                                                       | 不投影                                                                                                                                                                                                                                                           |
| 折叠态（thinking / tool 展开）                                        | **renderer-local + 域内部可选字段** | `ai-tool-call.tsx` / `reasoning.tsx` 本地 `useState`；`ChatToolCallUIState.open` / `state.thinking.open` 为**可选**字段，engine 插件**不写**（undefined-absent，P1-7/P1-8 2026-08-10 修复） | 不投影。合并语义：`state.open !== undefined ? state.open : internalOpen`——插件路径恒本地展开；host 显式写入 `open` 时随引擎快照存活（虚拟化行回收不丢），本地展开态随行回收复位（方案 A 记录成本）                                                               |
...
| ai-citations     | `nop-ai-citations`     | Widget                     |
| ai-token-usage   | `nop-ai-token-usage`   | Widget                     |
| ai-voice-input   | `nop-ai-voice-input`   | Widget                     |
| ai-suggestions   | `nop-ai-suggestions`   | Widget                     |
...
**`onResponseComplete` / `onError` / `onAbort` 引擎 scope 边界（2151 P2 doc hardening）**：订阅 effect（`ai-chat.tsx` deps `[engine]`）在 engine 引用变化（典型：`useConversation.activeEngine` 切换会话）时**先 cleanup 旧订阅再重订新 engine**——即旧（后台）engine 的后续 `processing→completed` 过渡**不会**触发 `onResponseComplete`。这是 documented keep-alive trade-off：切走的会话引擎仍可在后台流式（`useConversation` 的 processing-cache），但其完成事件对当前 `ai-chat` 不可见。Host 若需感知后台 turn 完成，应在 engine owner 侧（如 `useConversation` 的 turn 跟踪）订阅，而非依赖 `ai-chat` 的 schema event。
