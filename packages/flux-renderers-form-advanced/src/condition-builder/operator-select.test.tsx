...
describe('OperatorSelect', () => {
  ...
  it('renders single operator as static label', () => {
    render(
      <OperatorSelect
        operators={[{ label: 'Equal', value: 'equal' }]}
        value="equal"
        onChange={() => {}}
      />,
    );
  ...
  it('renders select for multiple operators', () => {
    render(<OperatorSelect operators={operators} value="equal" onChange={() => {}} />);
    expect(screen.getByTestId('mock-select')).toBeTruthy();
  });

  it('calls onChange when value changes', () => {
    const onChange = vi.fn();
    render(<OperatorSelect operators={operators} value="equal" onChange={onChange} />);
  ...
  it('passes current value to select', () => {
    render(<OperatorSelect operators={operators} value="like" onChange={() => {}} />);
    expect(screen.getByTestId('mock-select').dataset.value).toBe('like');
  });

  it('passes empty string when value is undefined', () => {
    render(<OperatorSelect operators={operators} value={undefined} onChange={() => {}} />);
