...
describe('insert/delete row/column', () => {
  ...
  it('should insert row', async () => {
    ...
    expect(snap.document.workbook.sheets[0].cells?.['A2']).toBeUndefined();
    expect(snap.document.workbook.sheets[0].cells?.['A3']?.value).toBe('A2');
  });
  ...
  it('should delete row', async () => {
    ...
    const snap = core.getSnapshot();
    expect(snap.document.workbook.sheets[0].cells?.['A1']?.value).toBe('A2');
    expect(snap.document.workbook.sheets[0].cells?.['B1']).toBeUndefined();
  });
  ...
  it('should delete column', async () => {
    ...
    const snap = core.getSnapshot();
    expect(snap.document.workbook.sheets[0].cells?.['A1']?.value).toBe('B1');
  });
