...
- **字段写同步、无 write-debounce、故无 flush 契约（B4.1 / I2+I5）**：与 input-text 一致，input-number 的字段写同步（`onChange`→`setValue` 同步入 store，无 debounced field-write）。`form.reset()`/`component:reset` 后 DOM `input.value` 经 `useBoundFieldValue` 订阅同步跟随 runtime 值，无需 flush。`numberAdapter.out`（`value-adapter.ts:234`）把空输入归一为 `undefined`（清空表单值），故 clear→submit 序列化无该数值字段。刻意不引入 debounced write（同 input-text）。回归锚见 `input-reset-resync.test.tsx`、`input-controlled-value.test.tsx`。
