...
- [x] 创建包骨架：`package.json`（name `@nop-chaos/flux-renderers-3d`，manifest 整体对齐 industrial 模板：deps `three@0.186.0` + `@nop-chaos/flux-core`/`flux-react`/`@nop-chaos/ui` workspace:\*，peerDeps react/react-dom，devDeps `@types/three` + react/react-dom + flux-formula/flux-runtime；scripts 对齐 industrial）、`tsconfig.json`、`tsconfig.build.json`、`vitest.config.ts`（happy-dom + coverage 90% 阈值，`src/test-support/**` 排除）、`src/index.tsx` 空导出。
- [x] 四处注册：`vite.workspace-alias.ts` 增 `@nop-chaos/flux-renderers-3d`；`tsconfig.base.json` paths 增映射；root `tsconfig.json` references 增；确认 pnpm workspace glob 捕获。
- [x] `pnpm install` 后包级 `typecheck`/`build`/`test`/`lint` 四绿（骨架冒烟：definitions 或 index 的最小可执行测试 1 条）。

Exit Criteria:

- [x] `pnpm --filter @nop-chaos/flux-renderers-3d typecheck && ... build && ... test && ... lint` 全过。
- [x] 根 `pnpm typecheck` 不因新包退化（39→40 tasks 全绿）。
...
- [x] `schemas.ts`：按 design-renderer.md §1 落 `ThreeCanvasSchema`/`ThreeSceneConfig`/`ModelConfig`/`LightConfig`/`DataBinding`/`ThreeCanvasEvents` 类型（`AnimationConfig`、`DataBinding.transform`/`condition` 均仅类型声明，消费在 I2.2——管线内 transform 阶段为 identity 接缝）。
- [x] `renderer-definitions.ts`：按 §2 落 definition（fields：scene/bindings/animations/events 四个 `kind: 'prop'` + loading/empty region；propContracts 四项）+ `registerThreeRenderers(registry)` 入口。
- [x] Meta-contract 测试：definition 注册后 registry 查询、fields 通道断言（对齐 scada-canvas-meta-contract 测试形态）。

Exit Criteria:

- [x] `registerThreeRenderers` 后 `registry.get('three-canvas')` 返回 definition；fields 断言过。
...
- [x] 先写失败测试（红）：`extractExpressionDepsViaProbe` 五态（ok/compile-failed/create-state-failed/evaluate-failed/deps-empty，含 wildcard 与非 leaf-state 分支）、`analyzeBindingSubscriptions`（纯路径直取/复杂表达式 probe/deps-empty 嫌疑集/非 `${}` 形态 normalize）、`createPrivateEvalScope`（get/has/只读边界）。
- [x] 实现 `binding/flux-eval.ts`（语义对齐 industrial 同名模块 + design-data-binding.md §2）：`compileValue` → `.kind === 'dynamic'` 判定 → `createState` → 宽容 Proxy probe scope → `evaluateWithState` → `state.root` leaf-state/wildcard/paths 判别；`expressionReadsScope` 启发式（全局名误报为可接受）。
- [x] 测试转绿；用真实 `createExpressionCompiler`（flux-formula）。

Exit Criteria:

- [x] flux-eval 单测全绿（含五态穷举 + deps-empty 嫌疑去重）。
...
- [x] 先写失败测试（红）：updateProperty 类型分派（Vector3/Euler/Color/visible）、pending buffer 回放与丢弃、generation 判弃、dispose 链、queue drain——用 stub renderer 工厂 + fake rAF/timers；`GLTFLoader` 经 vi.mock 或注入 stub，OrbitControls 必要时同法（happy-dom 无 GL）。
- [x] `SceneManager`：renderer 工厂注入（测试 stub）；`init`（camera/lights 五类/environment/fog/rAF 启动/OrbitControls damping）、`setFrameUpdateQueue`、`onPick`/`onHover` 订阅（射线拾取 + interactive 门控）、`loadModels`（ModelLoader generation-guard）、`updateProperty`（点分导航 + `.set()` 类型分派 + `modelId::path` pending buffer 回放 + 失败丢弃）、`updateAnimations(delta)`、`dispose` 全链。
- [x] `ModelLoader`：`loadAsync(url, onProgress)` 2 参 + generation 只读捕获/cancel bump（D5）。
- [x] 引擎单测转绿（stub renderer 下覆盖红测所列行为 + 灯光组装；three 真实类型参与断言）。

Exit Criteria:

- [x] 引擎单测全绿（先红后绿；GLTFLoader/OrbitControls 的 mock 或注入方式在测试文件内注明）。
...
- [x] 先写失败测试（红，真实 flux-formula compiler + 受控 scope + stub 引擎）：scope 变化 → 引擎收到期望值的端到端断言、deps-empty 上报、模型未就绪初值回放、事件派发 payload。
- [x] `use-scene-manager`：visible 入参驱动 true→false dispose / false→true init；ResizeObserver 尺寸同步；webgl-unavailable 捕获。
- [x] `use-binding-bridge`：analyze（useMemo）→ `useScopeSelector` paths 订阅 → 求值 + `Object.is` 比较（lastValues ref）→ pendingUpdatesRef → `setFrameUpdateQueue` 注册（deps：bindings + sceneManager identity；重建时清 pending/lastValues）→ `reportOnce` 去重上报（compile/evaluate/deps-empty）。
- [x] `use-three-events`：`createNormalizedActionEvent` + `helpers.dispatch`（object:click/object:hover/ready/error），onPick/onHover 订阅清理。
- [x] `three-canvas.tsx` 组件壳：meta.visible/className/testid、loading/empty region、`data-three-scene-state` 状态埋点。
- [x] 组件测试转绿（红测所列四类断言全过）。

Exit Criteria:

- [x] 绑定桥接端到端组件测试全绿（含初值回放与错误去重断言）。
- [x] 包级 coverage ≥ 90%（骨架阈值先例）；headless 不可测分支按仓库既有惯例在 vitest coverage 配置显式列名 exclude（沿用 industrial `src/test-support/**` 格式），不降阈值；任何新增 exclude 在 plan 收口记录注明，若需调整阈值本身须先停下按计划修订流程处理。
...
- [x] 根 `pnpm typecheck`/`build`/`lint`/`test`/`check` 全绿；`node scripts/three-perf/bench-scene-graph.mjs` 仍可跑（three 版本未动）。
- [x] 实现与 v5 设计的漂移核对：分册回写（如有）+ plan 记录。

Exit Criteria:

- [x] 五项根验证全绿；漂移记录（或「无漂移」声明）在案。
...
- [x] 所有 in-scope confirmed contract drifts 已收敛（实现 vs v5 设计漂移：groundColor 字段落纸为 schemas/引擎同步修改，无其他漂移）
- [x] 行为/契约结果已达成（three-canvas 可注册、绑定端到端可用）
...
- [x] `pnpm test` 73/73 任务 12,268 passed / 0 failed
- [x] `pnpm check` exit 0（新增 3 命中已按流程注册：3d 包灯光默认色 ×2 + 结构化诊断 message ×1，EXEMPTIONS 登记 reason/source）
