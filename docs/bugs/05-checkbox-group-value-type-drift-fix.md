...
- `packages/amis-renderers-form/src/renderers/input.tsx` routed checkbox-group updates through `JSON.stringify(...)` and `JSON.parse(...)` to fit a string-only shared field handler
- `packages/amis-renderers-form/src/field-utils.tsx` also normalized checkbox-group reads to strings, so the UI compared coerced string values against original option values
...
- updated shared field handlers in `packages/amis-renderers-form/src/field-utils.tsx` to pass through typed values instead of forcing string-only updates
- updated checkbox-group in `packages/amis-renderers-form/src/renderers/input.tsx` to read the bound raw array value and write arrays directly
...
- `packages/amis-renderers-form/src/index.test.tsx` - verifies non-string checkbox-group values stay intact in form state and submit payloads
- `packages/amis-renderers-form/src/index.test.tsx` - verifies plain scope updates keep checkbox-group values as arrays instead of JSON strings
