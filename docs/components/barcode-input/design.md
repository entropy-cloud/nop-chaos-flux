...
- `type: 'barcode-input'`
- `sourcePackage: '@nop-chaos/flux-renderers-scheduling'`（调度组件层）
...
- 扫码覆盖层（portal 到 body）：
  ...
  - `<div data-slot="barcode-scanner-status-text">`（状态文字：「正在打开摄像头...」/「请对准条码」/「识别中」/「识别失败，请重试」）
...
- 点击"批量确认"后逐条调用后端接口（通过 `onScan` 事件）
