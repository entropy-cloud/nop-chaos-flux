...
>
> **投影重建触发条件（P1-6，2026-08-10 multi-audit 修复）**：投影快照不仅在 turn-boundary（`isProcessing` true→false）重建——engine 替换（会话切换 / `clear()` / `setMessages` 复水）为双 idle，无翻转，快照会无限期显示上一会话。现触发 = ① `isProcessing` 翻转（boundary 克隆）；② `requestState` 进入终态（completed/aborted/error）；③ 空闲期 messages **数组引用**变化（clear/setMessages/engine swap 在 mutate recipe 内整体替换数组）；④ 空闲期 messages **length** 变化（engine 原地变更——abort 空产物 drop 的 `splice` 不换引用，此信号使 P2-14 幽灵在 abort 边界重建后被二次 idle 重建清除）。成本纪律保持：streaming 期间不克隆（`snapSourceRef`/`snapLength` 恒等收敛，守卫不重触发）。proof 见 `ai-chat-projection.test.tsx` P1-6 用例集（engine swap / clear / 复水 / abort-ghost）。
