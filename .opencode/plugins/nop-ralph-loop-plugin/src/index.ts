...
export function createRalphLoopHook(ctx: PluginInput, config?: RalphLoopConfig): RalphLoopHook {
  ...
  return async (input, output): Promise<void> => {
    ...
    if (input.tool === 'task') {
      ...
      if (shouldInjectOracleVerification) {
        ...
        log(`[${HOOK_NAME}] Injected Oracle verification prompt`, {
          sessionID,
...
function createChatMessageHandler(
  ...
  return async (input, output): Promise<void> => {
    ...
    const isUlwLoopTemplate =
      ...
      promptText.includes('<nop-user-task>');
    const isCancelRalphTemplate = promptText.includes('Cancel the currently active NOP Ralph Loop');
    ...
    if (isRalphLoopTemplate || isUlwLoopTemplate) {
      ...
      log(`[${HOOK_NAME}] Started via chat message template`, {
        sessionID,
        isUlw: isUlwLoopTemplate,
      });
