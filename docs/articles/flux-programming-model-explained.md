...
> 本文的目标读者：已经快速浏览过 `docs/architecture/flux-design-principles.md` 与 `docs/architecture/frontend-programming-model.md`、但仍没把各份规范拼成完整执行模型的开发者或 AI。
>
> 如果你带着"这是又一个低代码渲染框架"的预期来阅读 Flux 代码，你会困惑于很多设计选择。

**相关文档导航：**

- 如果你想了解 Flux 的设计决策和工程动机，先阅读 [`docs/articles/flux-design-introduction.md`](./flux-design-introduction.md)
- 如果你需要先确认文档层级与阅读顺序，先看 `docs/index.md` 与 `docs/architecture/README.md`
- 如果你需要查阅具体接口和契约，阅读 `docs/architecture/` 目录下的规范文档
- 本文的目标是帮助你在继续深入窄领域规范文档之前建立统一心智模型
- 如果本文与规范文档冲突，以 `docs/architecture/frontend-programming-model.md` 为顶层优先级文档，`docs/architecture/flux-core.md` 为代码库高层基线，对应窄领域 owner 文档保有局部优先级

## 1. 理解 Flux 的第一步：它首先是前端 DSL 运行时

从产品定位看，Flux 当然服务于低代码平台；但从架构理解上，Flux 不是"浏览器里做动态组装的低代码框架"。
...
这些功能在 Flux 之外，在平台层。Flux 的"功能不完整"正是它的设计目标：**单一职责，只做执行**。

### 声明式边界继续前移，但不是把一切都塞进 schema

**哪些东西应该进入 schema，哪些东西不应该进入 Flux core**。
...
初次阅读时，很容易把它们当作"功能模块的分类"：这个管渲染、这个管数据、这个管副作用。

规范文档把七原语定义为一个闭合 primitive set。**你必须把这七类语义角色分开看**，否则就会把结构、数据可见性、值派生、值生产、观察后果、效果权限和宿主快照混成一团。

同时也要记住：**不是所有重要系统都是原语。** `Action Algebra`、`Operation Control`、`FormRuntime` / `PageRuntime` / `SurfaceRuntime`、validation owner 这些都很重要，但它们在最新规范里属于由七原语组合出来的派生运行时系统，而不是第八、第九个原语。
...
### Value — 统一求值层，而不只是“表达式结果”

如果严格对照最新规范，Value 不应被理解得太窄。它不只是“`${...}` 表达式求值的结果”，而是 Flux 的**统一求值层**。

进入 Value 语义的，不只有表达式，还包括：literal、`${expr}`、模板字符串、array、object，以及以 `name` / field path 作为绑定锚点的读取场景。

所以更准确的说法是：Value 回答的是**“此处的值如何被读取、构造或派生出来”**。`${1 + 2}` 是 Value，`${user.name}` 是 Value，`"Hello, ${name}"` 是 Value，`{ label: "${user.name}", disabled: "${readonly}" }` 这种对象值同样走统一 Value 编译/求值路径。

简单说，Value 是**纯计算与依赖收集**，没有副作用。给定相同的可见作用域状态，Value 总是返回相同的结果；求值过程中读取了哪些路径，也会被 runtime 自动记录为依赖。

这里还要补一个最新架构里非常重要的边界：**Value 负责读取、构造、派生和依赖收集，不负责统一一切“写回”语义。**

- 用户编辑字段、`FormRuntime.setValue`、`ScopeRef.update` 这类 owner-local 值写入，属于数据 owner 的写入轴线
- `Resource` 发布值，属于运行时拥有的生产者发布轴线
- `setValue` 这种 action 虽然最后也会改值，但它是经由 `Capability` 进入 owner-local 写入路径的 command/effect 入口，不反过来定义全部写入模型

这条区分很重要，因为最新规范已经明确把“值读取/派生”“owner-local 值写入”“生产者发布”“命令/效果触发”拆成不同轴线，而不是都塞进一个模糊的 binding 概念里。

另外，`name` / field path 在这里也容易被误读。它们经常出现在字段绑定、validation 规则参数、Resource 发布目标这些地方，但**它们只是 Value 读取或绑定锚点，不是新的原语，也不是写入 owner 本身。**
...
在最新 `api-data-source` 文档里，这个边界又被进一步收窄了：`data-source` 是非渲染的 named source owner，`name` 和 `statusPath` 是**结构发布路径**，不是通用动态表达式；而远程 producer 的可见 effect 入口，统一收敛到 `ajax` action 这条 command/effect 路径上。

如果你熟悉 Effect 系统（如 ZIO、Cats Effect），可以把 Resource 粗略类比为“被运行时拥有和管理的值生产描述”；但它的语义重点不是一般化 effect 编排，而是**发布一个 Logical Value**。

### 一个有用的类比：Resource ≈ computed 的异步扩展
...
最新规范里还有一个很容易混淆的点：**validation 依赖重校验不属于 Reaction。**

字段间依赖、`requiredWhen`、`equals`、hidden policy、错误状态、级联重校验，属于 validation owner / data owner 这一侧的派生运行时系统。它们当然会响应值变化，但不是通过 `Reaction` 原语来建模。
...
这里的“只读”在最新规范里强调的是**单向读语义**，不等于“必须 defensive clone 一份副本”。很多公开读面默认允许 by-reference 的只读视图；关键约束是不能越过只读纪律原地改写，而不是强制复制。
...
这里说的 Value，也不是只指“某个表达式节点”。它覆盖的是统一求值层：literal / `${expr}` / template / array / object，以及 field `name` / field path 这类读取锚点所参与的值构造过程。
...
| 原语     | 依赖变化时的行为                                 |
| -------- | ------------------------------------------------ |
| Value    | 同步重新计算，返回新值并刷新其依赖集合           |
| Resource | 可能取消当前生产过程，重新开始生产               |
| Reaction | 重新观察，并在满足条件时通过 Capability 调度后果 |

再加一条经常被忽略的补充：**validation owner 也会对依赖变化作出反应，但它走的是 owner-local 重校验路径，而不是上述三者之一的别名。** 这也是为什么最新文档把 validation 明确放回 value axis + owner axis 的组合模型中，而不是把它硬塞成 `Reaction` 的一个特例。

换句话说：字段当前值的读取、规则参数里的可求值部分，复用的是 Value；但错误状态、字段参与、依赖反查、触摸/提交流程，不属于 Value 本身。
...
**Template/Instance 分离已经是当前架构基线。** 但阅读当前代码时，仍然要区分规范中的本体模型和实现里的某些过渡形态，不要把临时兼容对象误认成最终架构本体。

## 5. 词法所有权：生命周期由作用域决定，但 owner 语义另有边界
...
这里的含义不是把 `ScopeRef` 设计成"数据 + 行为 + 生命周期"的大对象。`ScopeRef` 仍然保持纯数据词法环境；source/reaction 的生命周期所有权是在 runtime sidecar 中按 scope 归属实现的。

但在最新架构里，还要再补上另一层边界：**词法作用域不等于 Data Domain Owner。**

- `ScopeRef` 回答的是“这里能读到什么”
- `Data Domain Owner` 回答的是“这份值归谁拥有、谁保存 validation state、谁决定 publish/commit/submit 边界”

所以，scope creation、surface creation、data-domain owner creation 在语义上是三件相关但不等价的事情。比如 `dialog` 可以创建 own scope 和 surface owner，但只有在其中承载了明确的数据域与 validation/publish 边界时，才构成真正的数据 owner。
...
## 6. 三棵树是有用的工程直觉，但优先级低于七原语与 owner 文档
...
**三棵树是工程直觉，不是比七原语更高优先级的本体论**。更精确的规范仍然是 `Template + ScopeRef + Value/Resource/Reaction + Capability + Host Projection`。在最新文档里，还可以把它再压缩理解成 `Template + semantic overlays`：数据 lexical overlay、capability lexical overlay，以及与它们正交的 resource/reaction/host projection/instance targeting 等运行时语义层。三棵树只是帮助你把这些东西先分层看清。
...
并且要再加一句：**三棵树也不能替代 owner 文档。** 当你在讨论 validation、staged/live 编辑、surface open-state、page/root owner、row-local owner 时，真正应该回看的不是“三棵树”，而是 `data-domain-owner.md`、`form-validation.md`、`scope-ownership-and-isolation.md` 这些窄领域 owner 文档。
...
但要注意：StateTree 不是 owner tree。谁能“看见”数据，不等于谁“拥有”这份数据的 validation 和 publish 语义。
...
先读 `docs/index.md` 与 `docs/architecture/README.md`，再读 `docs/architecture/flux-design-principles.md` 和 `docs/architecture/frontend-programming-model.md`。

它们分别告诉你：文档地图、架构层级、设计原则、什么是七原语、什么属于 core、什么属于 derived runtime system、什么留在 host/domain 外部。没有这层 baseline，后面看代码时很容易把局部实现误当成整体定义。
...
然后配合 `docs/architecture/scope-ownership-and-isolation.md` 一起看，避免把 `data`、默认继承、`isolate`、row scope、loop item scope 的语义混成一个“所有子树都一样”的模型。
...
### 第三步半：理解 Resource / Reaction 与 owner 语义的运行时边界
...
然后继续读 `docs/architecture/data-domain-owner.md` 与 `docs/architecture/form-validation.md`。这样你才能把“依赖变化导致谁重算、谁刷新、谁重校验、谁决定提交边界”区分开，而不会把 validation、Resource、Reaction、字段写回全都误判成同一个响应式机制。
...
如果你想理解“为什么同样是 scope，可见性、validation ownership、publish 边界却不能混成一个东西”，把 `docs/architecture/data-domain-owner.md`、`docs/architecture/scope-ownership-and-isolation.md`、`docs/architecture/form-validation.md` 连起来读。
...
3. **Value/Resource/Reaction 的区分是必要的**，因为统一求值、值生产和 watch-to-effect 在运行时的响应方式不同。

4. **生命周期由词法所有权决定**，不是 React 组件。它说明这些语义边界不由 React 生命周期定义。

5. **Scope、owner、surface 是不同边界。** `ScopeRef` 负责可见性，`Data Domain Owner` 负责 validation/publish/commit 语义，surface owner 负责 open/close 等界面生命周期。

6. **三棵树是有用的工程直觉**：组件结构、数据流向、行为解析各自独立；但更高优先级的规范仍然是七原语模型与对应 owner 文档。

7. **Capability 是单一效果 authority path**：所有作者可见副作用都必须通过 Capability 解析出的路径发生，schema 不能直接执行副作用。

8. **Host Projection 是只读宿主快照进入 schema 的入口**，不是第二条写入路径，也不是把整个宿主 runtime 暴露进 scope。

9. **不是所有前端复杂性都属于 Flux core**：host shell、协作协议、领域算法、高频手势循环等仍应留在 core 之外。

10. **规范文档 ≠ 解释文档**：理解设计意图后，再阅读规范文档；如有冲突，以规范文档为准。
