...
| --- | ----------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | c3-5-host-surfaces.spec.ts:27（Tiptap editor edit + submit echo） | **6/6 全绿**                                                                                                          | 时序敏感（全量并行下触发）；与 w3d-editor 同根因（TipTap label-activation 竞态）                                                                                                                               | ✅ **已修复（2026-08-08，DR plan Phase 3）**——editor 面 `frameRootTag: 'div'` 修复 + 30/30 复跑绿（含 3 轮 repeat）；从 watch-only 清单移除 |
| 2   | c3-5-host-surfaces.spec.ts:81（Tiptap link button）               | **6/6 全绿**                                                                                                          | 同上                                                                                                                                                                                                           | ✅ **已修复（2026-08-08，DR plan Phase 3）**——同上；从 watch-only 清单移除                                                                  |
| 3   | w3d-editor.spec.ts:28（applying bold）                            | **3/15 绿（12/15 失败）**——首次 1-3 个 keystroke 丢失（"hel"、"hello edi"、"ell" 等变体），点击后立即键入的字符被丢弃 | **判真缺陷**：TipTap label-activation 竞态——字段 frame `<label>` 的隐式 control = 首个 labelable 后代（bold 工具栏按钮），点击内容区激活按钮 → toggleBold 存 stray mark + 焦点跳转丢 keystroke（bug note 117） | ✅ **已修复（2026-08-08，DR plan Phase 3）**——`frameRootTag: 'div'` + 回归测试 + 30/30 复跑绿；从 watch-only 清单移除                       |
