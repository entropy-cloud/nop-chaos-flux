import React from 'react';
import {
  PageLabPage,
  ContainerLabPage,
  FragmentLabPage,
  FlexLabPage,
  DialogLabPage,
  DrawerLabPage,
  CommandPaletteLabPage,
  TabsLabPage,
  LoopLabPage,
  RecurseLabPage,
  TextLabPage,
  IconLabPage,
  BadgeLabPage,
  ButtonLabPage,
  ScopeDebugLabPage,
  DynamicRendererLabPage,
  ReactionLabPage,
  KeyboardLabPage,
  FormLabPage,
  InputTextLabPage,
  InputEmailLabPage,
  InputPasswordLabPage,
  InputNumberLabPage,
  TextareaLabPage,
  SelectLabPage,
  CheckboxLabPage,
  SwitchLabPage,
  RadioGroupLabPage,
  CheckboxGroupLabPage,
  ButtonGroupSelectLabPage,
  FieldsetLabPage,
  HiddenLabPage,
  InputTreeLabPage,
  TreeSelectLabPage,
  TagListLabPage,
  KeyValueLabPage,
  ArrayEditorLabPage,
  ComboLabPage,
  InputTableLabPage,
  TransferLabPage,
  PickerLabPage,
  IconPickerLabPage,
  ConditionBuilderLabPage,
  ObjectFieldLabPage,
  ArrayFieldLabPage,
  VariantFieldLabPage,
  DetailFieldLabPage,
  DetailViewLabPage,
  CrudLabPage,
  QueryFilterLabPage,
  BatchBarLabPage,
  TableLabPage,
  TreeLabPage,
  DataSourceLabPage,
  ChartLabPage,
  EChartsLabPage,
  ListLabPage,
  PaginationLabPage,
  StatisticsLabPage,
  StatTileLabPage,
  SparklineLabPage,
  DashboardFilterLabPage,
  InputDateLabPage,
  InputDatetimeLabPage,
  InputTimeLabPage,
  DateRangeLabPage,
  InputMonthLabPage,
  InputQuarterLabPage,
  InputYearLabPage,
  MarkdownEditorLabPage,
  InputFileLabPage,
  InputImageLabPage,
  EditorLabPage,
  GridLabPage,
  CollapseLabPage,
  WizardLabPage,
  ButtonGroupLabPage,
  DropdownButtonLabPage,
  StepsLabPage,
  TimelineLabPage,
  ResponsiveLabPage,
  MarkdownLabPage,
  HtmlLabPage,
  JsonViewLabPage,
  LinkLabPage,
  ImageLabPage,
  CardLabPage,
  CardsLabPage,
  EmptyLabPage,
  ResultLabPage,
  ProgressLabPage,
  SpinnerLabPage,
  SeparatorLabPage,
  AlertLabPage,
  MappingLabPage,
  StatusLabPage,
  AudioLabPage,
  VideoLabPage,
  CarouselLabPage,
  QrcodeLabPage,
  DiffViewLabPage,
  PullRefreshLabPage,
  InfiniteScrollLabPage,
  SwipeCellLabPage,
  CountdownLabPage,
  NoticeBarLabPage,
  AiChatLabPage,
  AiMessageListLabPage,
  AiBubbleLabPage,
  AiSenderLabPage,
  AiConversationsLabPage,
  AiToolCallLabPage,
  AiAttachmentsLabPage,
  AiCitationsLabPage,
  AiFeedbackLabPage,
  AiTokenUsageLabPage,
  AiPromptsLabPage,
  AiSuggestionsLabPage,
  AiVoiceInputLabPage,
  AiWelcomeLabPage,
  GanttLabPage,
  KanbanLabPage,
  CalendarLabPage,
  BarcodeInputLabPage,
} from './renderers';

export const RENDERER_LAB_REGISTRY: Record<string, React.ComponentType> = {
  page: PageLabPage,
  container: ContainerLabPage,
  fragment: FragmentLabPage,
  flex: FlexLabPage,
  dialog: DialogLabPage,
  drawer: DrawerLabPage,
  'command-palette': CommandPaletteLabPage,
  tabs: TabsLabPage,
  loop: LoopLabPage,
  recurse: RecurseLabPage,
  text: TextLabPage,
  icon: IconLabPage,
  badge: BadgeLabPage,
  button: ButtonLabPage,
  'scope-debug': ScopeDebugLabPage,
  'dynamic-renderer': DynamicRendererLabPage,
  reaction: ReactionLabPage,
  keyboard: KeyboardLabPage,
  form: FormLabPage,
  'input-text': InputTextLabPage,
  'input-email': InputEmailLabPage,
  'input-password': InputPasswordLabPage,
  'input-number': InputNumberLabPage,
  textarea: TextareaLabPage,
  select: SelectLabPage,
  checkbox: CheckboxLabPage,
  switch: SwitchLabPage,
  'radio-group': RadioGroupLabPage,
  'checkbox-group': CheckboxGroupLabPage,
  'button-group-select': ButtonGroupSelectLabPage,
  fieldset: FieldsetLabPage,
  hidden: HiddenLabPage,
  'input-tree': InputTreeLabPage,
  'tree-select': TreeSelectLabPage,
  'tag-list': TagListLabPage,
  'key-value': KeyValueLabPage,
  'array-editor': ArrayEditorLabPage,
  combo: ComboLabPage,
  'input-table': InputTableLabPage,
  transfer: TransferLabPage,
  picker: PickerLabPage,
  'icon-picker': IconPickerLabPage,
  'condition-builder': ConditionBuilderLabPage,
  'object-field': ObjectFieldLabPage,
  'array-field': ArrayFieldLabPage,
  'variant-field': VariantFieldLabPage,
  'detail-field': DetailFieldLabPage,
  'detail-view': DetailViewLabPage,
  crud: CrudLabPage,
  'query-filter': QueryFilterLabPage,
  'batch-bar': BatchBarLabPage,
  table: TableLabPage,
  tree: TreeLabPage,
  'data-source': DataSourceLabPage,
  chart: ChartLabPage,
  echarts: EChartsLabPage,
  list: ListLabPage,
  pagination: PaginationLabPage,
  statistics: StatisticsLabPage,
  'stat-tile': StatTileLabPage,
  sparkline: SparklineLabPage,
  'dashboard-filter': DashboardFilterLabPage,
  'input-date': InputDateLabPage,
  'input-datetime': InputDatetimeLabPage,
  'input-time': InputTimeLabPage,
  'date-range': DateRangeLabPage,
  'input-month': InputMonthLabPage,
  'input-quarter': InputQuarterLabPage,
  'input-year': InputYearLabPage,
  'markdown-editor': MarkdownEditorLabPage,
  'input-file': InputFileLabPage,
  'input-image': InputImageLabPage,
  editor: EditorLabPage,
  grid: GridLabPage,
  collapse: CollapseLabPage,
  wizard: WizardLabPage,
  'button-group': ButtonGroupLabPage,
  'dropdown-button': DropdownButtonLabPage,
  steps: StepsLabPage,
  timeline: TimelineLabPage,
  responsive: ResponsiveLabPage,
  markdown: MarkdownLabPage,
  html: HtmlLabPage,
  'json-view': JsonViewLabPage,
  link: LinkLabPage,
  image: ImageLabPage,
  card: CardLabPage,
  cards: CardsLabPage,
  empty: EmptyLabPage,
  result: ResultLabPage,
  progress: ProgressLabPage,
  spinner: SpinnerLabPage,
  separator: SeparatorLabPage,
  alert: AlertLabPage,
  mapping: MappingLabPage,
  status: StatusLabPage,
  audio: AudioLabPage,
  video: VideoLabPage,
  carousel: CarouselLabPage,
  qrcode: QrcodeLabPage,
  'diff-view': DiffViewLabPage,
  'pull-refresh': PullRefreshLabPage,
  'infinite-scroll': InfiniteScrollLabPage,
  'swipe-cell': SwipeCellLabPage,
  countdown: CountdownLabPage,
  'notice-bar': NoticeBarLabPage,
  'ai-chat': AiChatLabPage,
  'ai-message-list': AiMessageListLabPage,
  'ai-bubble': AiBubbleLabPage,
  'ai-sender': AiSenderLabPage,
  'ai-conversations': AiConversationsLabPage,
  'ai-tool-call': AiToolCallLabPage,
  'ai-attachments': AiAttachmentsLabPage,
  'ai-citations': AiCitationsLabPage,
  'ai-feedback': AiFeedbackLabPage,
  'ai-token-usage': AiTokenUsageLabPage,
  'ai-prompts': AiPromptsLabPage,
  'ai-suggestions': AiSuggestionsLabPage,
  'ai-voice-input': AiVoiceInputLabPage,
  'ai-welcome': AiWelcomeLabPage,
  gantt: GanttLabPage,
  kanban: KanbanLabPage,
  calendar: CalendarLabPage,
  'barcode-input': BarcodeInputLabPage,
};
