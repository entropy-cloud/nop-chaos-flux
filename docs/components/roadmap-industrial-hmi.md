...
- **2026-08-05-2129 post-remediation audit P1 remediation plan `2026-08-05-2129-3` 执行完成（2026-08-06，2026-08-05-2129-3-industrial-hmi-audit-p1-remediations，✅）**：两份 2129 审计（open-ended adversarial + multi-dimensional）共 4 条 P1 全部收口。四 Phase 全部落地——Phase 1 拆分 2 个超限测试文件（`serialization.test.ts` 749→parse-serialize/diff/validate+fixtures、`refresh-pipeline.test.ts` 726→core/state/animation-lifecycle+fixtures，8 兄弟均 <700 行，恢复 oversized 硬门禁 industrial 包 0 失败）；Phase 2 修复 multi P1-1 binding-expression scope 订阅断裂（`use-scada-points-bridge.ts` 新增 `collectBindingExpressionScopePaths` 扫 `symbols[].bindings[].expression` 经 probe 提取 scope 路径，`paths` 并集 variables + binding-expression → `variables:[]` + binding.expression 直连 scope 场景 `useScopeSelector` enabled、`scopeData` 含 scope 成员、binding 正确求值；failing-first integration test 挂真实桥接边界断言 text=42→60 reactive + nested=6，关闭 variables-optional-contract 场景 ① false-green）；Phase 3 修复 multi P1-2 RefreshPipeline.onError 生产未接线（`createBindingDomain` 增 `onError` 参数、`UseScadaEngineArgs.onPipelineError` ← `scada-canvas reportDiagnostic`，mount + reloadBindings 两路径透传；`evaluateFlux` 改返 discriminated `FluxEvalOutcome` 区分 compile-failed/evaluate-failed 使 `reportError` emit 对称码 `flux-compile-failed`/`flux-evaluate-failed`，三个声明源 source:flux/binding.expression/scale.expression observably symmetric；failing-first integration test 断言坏 binding.expression 错误到达 monitor.onError + 对称性 proof）；Phase 4 修复 open P1-1 shadow revert 完整性（`STYLE_RESET_DEFAULTS` 改 `Record<keyof ScadaSymbolStylePatch, unknown>` 穷尽守卫 + 补 `shadow` reset 值 `{x:0,y:0,blur:0,color:'transparent'}` + hoist `applied.delete` 到 revert 分支守卫之外，翻转既有缺陷测试断言退出 alarm 后 shadow 清除 + 重入再应用）。包级 687 tests / 54 files 全绿、workspace 全量验证（typecheck/build/lint 32/32 + test 59/59）全绿、`pnpm check:oversized-code-files` industrial 包 0 失败（14 ERROR 文件全在其他包，out-of-scope）。open P1-1 + multi P1-1/P1-2/P1-3 四项 confirmed live defect 已修复并各带 focused regression / integration proof（断言结果值/可见性/错误到达，非仅 not.toThrow 或 call-count）。closure-audit 由独立 fresh-session sub-agent（task `ses_02bb6a641ffe7eR3pBVTw6MA5v`）执行，verdict `pass`（零 Blocker / 零 Major）。源审计 `Audit Status: planned → closed`（两份）。25 P2 已在 Follow-up Backlog「2026-08-05-2129 post-remediation audit P2」子节追踪，按 mission 节奏择期处理。

- **2026-08-05-2129 post-remediation audit P2（doc-drift 子集）plan `2026-08-06-0746-1` 执行完成（2026-08-06，2026-08-06-0746-1-industrial-hmi-doc-drift-cleanup，✅）**：2129 multi-audit dim 16 三条纯文档漂移 P2（P2-1/P2-2/P2-3）收口——I18 表达式一元化 + 历次模块迁移后设计文档未同步回写造成。单 Phase 全部落地——P2-1：`design-renderer.md §11` 实现树重写对齐 live 布局（`schemas.ts`/`renderer-definitions.ts` 从 `renderer/` 上移 `src/` 顶层；`serialization/` 补 `equality.ts`；`renderer/hooks/` 补 `use-scada-handles.ts`；`renderer/` 补 `scada-errors.ts`）；P2-2：`design-engine.md §9:270` `$xxx` → `${expr}`（I18 唯一语法）+ 指向 `design-data-binding.md §4.2`；P2-3：`editor-initiation.md:60` row 8 binding 域模块清单对齐 `design-data-binding.md §11` post-I18 拆分（移除 `expression-evaluator`/`@{pointId}`，补 `bind-resolver.ts`/`flux-eval.ts`/`reverse-index.ts`/`dirty-collector.ts`/`value-to-state.ts`/`animator.ts`）。纯文档计划，零代码变更——Test Strategy 档位「不适用」，build/test/lint/typecheck gate 按 plan guide「纯文档计划」条款豁免。3 处编辑面经 rg 逐项核验零漂移（`design-engine.md §9` 无 `$xxx`；`design-renderer.md §11` + `editor-initiation.md:60` row 8 无 `expression-evaluator`/`@{pointId}`，row 8 含 `flux-eval`），§11 树文件路径经 live `ls` 逐项存在。closure-audit 由独立 fresh-session sub-agent（task `ses_02b9a5e09ffeRPgEm8CsYAk3tG`）执行，verdict `pass`。Follow-up：no remaining plan-owned work（其余 2129 P2 归 sibling plan `2026-08-06-0746-2`/`2026-08-06-0746-3` 或后续 mission 节奏）。

- **2026-08-05-2129 post-remediation audit P2（validator/equality 子集）plan `2026-08-06-0900-1` 执行完成（2026-08-06，2026-08-06-0900-1-industrial-hmi-validator-equality-robustness，✅）**：2129 audit 5 条 validator/equality 层 fail-closed / 判等正确性 P2（multi P2-9 dim 15 + open P2-2/P2-3/P2-5/P2-6）收口。五条共享同一结果面：组态 JSON 进入运行时的最后一道防线（`serialization/validate.ts` + `serialization/equality.ts`）的健壮性与判等正确性。三 Phase 全部落地（每 Phase failing-first Proof 红→绿）——Phase 1（P2-2+P2-9-deepEqual）：`equality.ts deepEqual` 加 `Array.isArray(a)!==Array.isArray(b)→false` 守卫 + 数组按 index 递归分支（object 分支保留 own-keys 递归），`deepEqual([1,2],{0:1,1:2})===false`；增内部 `depth` 参数（默认 0 保持二参公开签名兼容），`depth > MAX_DEEP_EQUAL_DEPTH`（~100）返 `false` fail-closed，~10k 层不再 stack overflow；新建 `serialization-equality.test.ts`（全仓首个 equality 直测文件）。Phase 2（P2-3+P2-9-validateSymbolNode）：`checkNumberField` 收紧 `typeof==='number' && Number.isFinite`（NaN/±Infinity 被拒），`deadband` 路由 `checkNumberField` 享 finite 守卫；`validateSymbolNode` 增 `depth` 参数 + `MAX_VALIDATE_DEPTH`（~100）cap 溢出 push 深度错误并 return。Phase 3（P2-5+P2-6）：`scanLegacyAtSyntax` 提取 `scanSymbolLegacy` 递归 helper 镜像 validateSymbolNode children 遍历（嵌套子图元 `@{pointId}` 方言 warn，对称加 depth cap 防深嵌套 crash）；新增 `assertShape(value, schema, scope, errors)` helper 接入 shadow/animation from-to/background/declaration scale.k,b/init 子形状校验。`design-renderer.md §4.3` 校验行同步。包级 718 tests / 55 files 全绿、workspace 全量验证（typecheck/build/lint 32/32 + test 59/59）全绿。closure-audit 由独立 fresh-session sub-agent（task `ses_02b501b1bffeMJryRe3w656jt2`）执行，verdict `pass`（零 Blocker/Major/Minor；特别核验三处深度 cap——equality/validateSymbolNode/scanSymbolLegacy 全 capped，执行期一度遗漏的 scanSymbolLegacy cap 已由 Phase 2 deep-children 测试驱出补齐）。open-audit P2-2/P2-3/P2-5/P2-6 + multi-audit P2-9 五项 confirmed live defect 已修复并各带 focused failing-first proof（断言结果值/判等/拒绝/警告，非仅 not.toThrow）。Follow-up：no remaining plan-owned work（其余 2129 P2 归 sibling plan `2026-08-06-0900-2` symbol 几何 / `2026-08-06-0900-3` engine-lifecycle 或后续 mission 节奏）。

- **2026-08-05-2129 post-remediation audit P2（symbol 几何/属性 子集）plan `2026-08-06-0900-2` 执行完成（2026-08-06，2026-08-06-0900-2-industrial-hmi-symbol-geometry-property-consistency，✅）**：2129 audit 5 条 symbol 族几何/属性一致性 P2（open P2-4/P2-7/P2-8/P2-9/P2-10）收口。五条共享同一结果面：`src/symbols/**` 图元的 create-time 几何 / update-time（applyProps）几何 / 读写属性名对称性。三 Phase 全部落地（每 Phase failing-first Proof 红→绿）——Phase 1（P2-4 复合族 resize）：`composite.ts` `CompositeParts` 增 `resize?` hook + `applyCompositeProps` EXTENT_FIELDS 分支无 extent 时回落 resize；device 4（motor/pump/valve/fan）+ gauge + sensor-control 4（indicator/button/sensor/switch）共 9 个无 extent 族各在 build 注册 resize hook（重算 body 容器 + 子形状相对锚点）；Phase 2（P2-7/P2-8/P2-9 create/update 一致）：`video.ts` stroke/strokeWidth 改 guarded（对齐 fill guard）；`pipe-junction.ts` stub strokeWidth 从 `props.strokeWidth ?? 4` 派生 + applyProps 路由；`thermometer.ts` 模块级 `BULB_RESERVE=-24` 统一 create/applyProps，首帧 8px 跳变消除；Phase 3（P2-10 读写对称）：`symbol-factory.ts` 新增 `fromNodeAttrs`（`toNodePatch` 逆映射），`scada-engine.ts getSymbolProps` 改返 schema 键名（textSize/scale/strokeDash/textColor/align），host 往返不再喂错键；e2e `scada-demo.spec.ts` 同步 `fontSize→textSize`。owner doc `design-symbols.md` §4 + `design-engine.md` §8.3 + `flux-guide/design-patterns/scada.md` 同步。包级 736 tests / 55 files 全绿、workspace 全量验证（typecheck/build/lint 32/32 + test 全绿）全绿。closure-audit 由独立 fresh-session sub-agent 执行，verdict `pass`。open-audit P2-4/P2-7/P2-8/P2-9/P2-10 五项 confirmed live defect 已修复并各带 focused failing-first proof（断言几何响应/样式保留/常量一致/键名对称）。Follow-up：no remaining plan-owned work（其余 2129 P2 归 sibling plan `2026-08-06-0900-3` engine-lifecycle 或后续 mission 节奏；复合族未来新增 symbol 带 width/height binding 按 resize hook 模式注册）。
...
| I18.3 | **迁移与兼容**：demo/playground 生产配置 `@{}`/`$xxx` 已迁移（实测 demo 4 处 `$xxx`→`${}`、6 个测试文件 ~42 处迁移为 flux-formula parity）；validator 对旧 `@{}` warn（不 fail，错误码 `legacy-at-syntax`，指向迁移）+ 配置 codemod（`scripts/scada-expression-codemod.mjs`）；单元测试改写（expression-evaluator.test.ts 删除、reverse-index.test.ts / bind-resolver.test.ts 重写为 flux `${}` parity）；文档同步（design-data-binding.md §4.1/§4.2/§9.1/决策表/§11/§12.2、design-renderer.md §8.3）                                      | I18.1, I18.2 |
...
设计债收口：删除自建表达式方言（`@{pointId}`），统一为平台唯一 `${expr}` 语法；点表降为可选间接层。4 项交付：（1）binding 域核心语法一元化（删 expression-evaluator.ts、reverse-index 改 flux 依赖收集、bind-resolver evaluate 改 flux 编译求值、环检测保留）；（2）点表可选（`variables` 经 plan 核实**已可选**（类型 `variables?:` + validator `if (config.variables !== undefined)`），本计划交付 contract-lock 5 场景单测 + doc-sync，非 phantom 改动；绑定直连 scope/data-source、点值注入求值 scope 复用 createPrivateEvalScope）；（3）迁移与兼容（实测 demo 4 处 `$xxx`→`${}` + 6 测试文件 ~42 处迁移、validator 旧 `@{}` warn（不 fail）+ codemod、单测改写为 flux-formula parity）；（4）验证（Must automate 档，failing-first，包级 + workspace + scada e2e 回归）。设计依据与决策项（D1-D6）：`docs/discussions/2026-08-05-industrial-hmi-expression-convergence-discussion.md`。**高频遥测 INV-4 out-of-band 通道保持不变**。
...
### 2026-08-05-2129 post-remediation audit P2（multi + open audit）

> 来源：`docs/audits/2026-08-05-2129-multi-audit-industrial-hmi.md`（15×P2）+ `docs/audits/2026-08-05-2129-open-audit-industrial-hmi.md`（10×P2）。这些条目均经两审计独立复核（orchestrator 手工 walk-through live code），P1（multi 3 + open 1）已由 plan `2026-08-05-2129-3` 全部收口（2026-08-06 closure-audit `pass`，源审计 `Audit Status: closed`）；下列 P2 为 non-blocking polish / 窄 foot-gun / debuggability residual，按 mission 节奏择期处理，暂无 owner plan。每条带源审计文件 + 代码定位可追溯。

**Multi-audit（15×P2）**

- `2026-08-05-2129-multi-audit-industrial-hmi.md` `[P2-1]`（dim 16）— `design-renderer.md §11:303-313` 实现树过期：`schemas.ts`/`renderer-definitions.ts` 误置于 `renderer/`（live 在 `src/` 顶层）；缺 `use-scada-handles.ts`/`scada-errors.ts`/`serialization/equality.ts`。建议按 live 布局重写 §11 树。（已由 plan `2026-08-06-0746-1` P2-1 收口，✅）
- `2026-08-05-2129-multi-audit-industrial-hmi.md` `[P2-2]`（dim 16）— `design-engine.md §9:270` 仍引用 I18 后已弃用的 `$xxx` flux 语法（live 仅认 `${expr}`，`$xxx` 走 `dollar-without-brace` Failure Path 求值 undefined）。建议改为 `${expr}` 并指向 `design-data-binding.md §4.2`。（已由 plan `2026-08-06-0746-1` P2-2 收口，✅）
- `2026-08-05-2129-multi-audit-industrial-hmi.md` `[P2-3]`（dim 16）— `editor-initiation.md:60` 三态清单 row 8 引用已删除的 `expression-evaluator` 模块 + 已弃用 `@{pointId}` 语法（live 已拆为 `bind-resolver.ts`+`flux-eval.ts`）。建议对齐 `design-data-binding.md §11` post-I18 模块拆分。（已由 plan `2026-08-06-0746-1` P2-3 收口，✅）
- `2026-08-05-2129-multi-audit-industrial-hmi.md` `[P2-4]`（dim 19）— `evaluateFlux`（`dirty-collector.ts:384-424`）把非 cycle 错误塌缩为字面串 `'expression evaluation failed'`，丢弃原始 Error（syntax position/evaluator reason/cause 链）。即使 P1-2 接通 onError，host 监控仍无法区分 `${1+}`(syntax) vs `${a.b.c}`(runtime TypeError)。建议扩 `onError(code, message, error?)` 并经 sentinel/重抛透传原始 error。与 P1-2 互补（P1-2=通道缺失，本条=通道签名丢上下文）。
- `2026-08-05-2129-multi-audit-industrial-hmi.md` `[P2-5]`（dim 19）— `handler-error` 诊断丢原始 Error（`scada-canvas.tsx:176-177` 仅取 message）且无 `monitor.onError` telemetry 路径（`reportDiagnostic` 仅对 `flux-*` 码转发 monitor）。建议传原始 error 第三参 + 扩 monitor typing admit `'action'` phase 或加 `onActionError` 通道。（已由 plan `2026-08-06-0746-3` Phase 1 收口，✅）
- `2026-08-05-2129-multi-audit-industrial-hmi.md` `[P2-6]`（dim 19）— `findCircularDependencyError`（`dirty-collector.ts:137-148`）cause-chain 深度上限 10，对 10+ 层 expression chain（工业过程管线现实场景）的 cycle 会误判为 generic eval 失败。建议移除人工上限或提至 ~1000（`cause===current`/`undefined` 守卫已防无限循环）。（已由 plan `2026-08-06-0746-3` Phase 2 收口，✅）
- `2026-08-05-2129-multi-audit-industrial-hmi.md` `[P2-7]`（dim 21）— fill 分支缺 `1e-6` width/height floor（`use-scada-config-sync.ts:113-127`），`clampScale(Infinity)=MIN_SCALE`（zoom OUT），与 `fit`/contain 分支（`viewport.ts:65-67` 有 floor → MAX_SCALE zoom IN）对零尺寸内容方向相反。建议补 `Math.max(1e-6, bounds.width)` floor 使 fill 零尺寸收敛到 MAX_SCALE。
- `2026-08-05-2129-multi-audit-industrial-hmi.md` `[P2-8]`（dim 21）— `handlePluginZoom`（`scada-engine.ts:425-445`）除零：`rawScale===0` 时 `clamped/rawScale=Infinity` 传 `zoomLayer.scaleOfWorld`，矩阵 corrupt 不可恢复。建议早退守卫 `if (rawScale===0 || !Number.isFinite(rawScale)) { syncViewportFromZoomLayer(); return; }`。
- `2026-08-05-2129-multi-audit-industrial-hmi.md` `[P2-9]`（dim 15）— `validateSymbolNode`（`validate.ts:264-280` children 递归）与 `deepEqual`（`equality.ts:17-32`）无深度限制，~10k 层嵌套（恶意/损坏 host JSON）stack overflow，fail-closed 边界缺口。建议加 `depth` 参数（cap ~100）+ 溢出返结构化错误/sentinel。（已由 plan `2026-08-06-0900-1` Phase 1/2 收口，✅）
- `2026-08-05-2129-multi-audit-industrial-hmi.md` `[P2-10]`（dim 07）— `engine.reset()`（`scada-engine.ts:191-199`，`importConfig`/version-change 全量重建路径）不清 `InteractionOverlay`，旧 hover 高亮残留；`use-scada-events.ts:77` `lastHoverSymbolRef` 不重置。建议 `engine.reset` 末尾 `this.interaction?.clear()` + events hook config-change effect 重置 `lastHoverSymbolRef`。
- `2026-08-05-2129-multi-audit-industrial-hmi.md` `[P2-11]`（dim 22）— `useScadaHandles`（`use-scada-handles.ts:45-156`）effect deps 含 `runtime`，但 `invoke` 经 ref 读最新 runtime → 每次 config reload（`setRuntime`）冗余重注册 handle（无正确性影响）。建议从 deps 移除 `runtime` + 加 reload 不重注册回归 test。
- `2026-08-05-2129-multi-audit-industrial-hmi.md` `[P2-12]`（dim 23）— codemod test（`expression-codemod.test.ts:21-37`）内联了生产 codemod patterns 的**子集**（缺 `QUOTED_DOLLAR_PATTERN`），验的是等价副本非生产函数。建议改为 `import { migrateContent } from '../../../../../scripts/scada-expression-codemod.mjs'`。（已由 plan `2026-08-06-0746-2` P2-12 收口，✅）
- `2026-08-05-2129-multi-audit-industrial-hmi.md` `[P2-13]`（dim 23）— `binding-expression-unification.test.ts:459-473` 标题声称验 `?? ''` 兜底（point `blank`），但断言的是无关点 `getPointValue('a')===1`，`blank` 从未被断言 → false-green。建议补 `expect(pointStore.getPointValue('blank')).toBe('${}')`。（已由 plan `2026-08-06-0746-2` P2-13 收口，✅）
- `2026-08-05-2129-multi-audit-industrial-hmi.md` `[P2-14]`（dim 23）— `scada-canvas-smoke.test.tsx:35-52` 标题声称 "scene build" 但仅验 `handles.length>0`（等价 not.toThrow），未断言图元入树/`data-status==='ready'`。建议补 `getSymbols()` 含 rect-1 或 `data-status` 断言。（已由 plan `2026-08-06-0746-2` P2-14 收口，✅）
- `2026-08-05-2129-multi-audit-industrial-hmi.md` `[P2-15]`（dim 14）— `variables-optional-contract.test.ts:235-248` 空 config 测试唯一断言 `not.toThrow()`，未验 flushFrame 返值或 applyAttrs 未被调。建议补 `expect(pipeline.flushFrame(...)).toBe(false)` + `vi.fn()` 断言 applyAttrs 未调。（已由 plan `2026-08-06-0746-2` P2-15 收口，✅）

**Open-ended audit（10×P2）**

- `2026-08-05-2129-open-audit-industrial-hmi.md` `[P2-1]` — animation 增量 vs binding 同属性非确定性闪烁 + 动画 CPU 永久空耗；precedence 未文档化未测试（`use-scada-engine.ts:62-65,85` animator wiring + `dirty-collector.ts:261-265` flushFrame 先 collectBindings 后 collector.flush → 脏帧 binding 覆盖 animation 增量）。建议文档化 {binding,state,animation} 同属性优先级 + validate/registration 时 warn，或给 animation "decorative override only when no binding" 语义。
- `2026-08-05-2129-open-audit-industrial-hmi.md` `[P2-2]` — `equality.ts:17-32` `deepEqual` 把数组与 array-shaped 的对象混同（`deepEqual([1,2],{0:1,1:2})===true`），违反自身注释「数组按 index 比较」。对 `custom`/`fillStyle` 等无类型内省字段，host 往返翻转 array↔object 形态时产 empty diff patch / 丢 serialize override。建议加 `Array.isArray(a)!==Array.isArray(b)→false` 守卫 + 数组按 index 比较分支。（已由 plan `2026-08-06-0900-1` Phase 1 收口，✅）
- `2026-08-05-2129-open-audit-industrial-hmi.md` `[P2-3]` — `validate.ts:18-27` `checkNumberField` 接受 `NaN`/`Infinity`/`-Infinity`（`JSON.parse('1e400')→Infinity`），`x/y` 仅 `?? 0`（Infinity 留 Infinity），`strokeWidth/textSize/period/loop/deadband` 无下游 clamp。建议收紧为 `typeof n==='number' && Number.isFinite(n)`。（已由 plan `2026-08-06-0900-1` Phase 2 收口，✅）
- `2026-08-05-2129-open-audit-industrial-hmi.md` `[P2-4]` — 复合 device/instrument/sensor-control 11 symbols 在 create 算 width/height 几何但 update（`applyProps` 路由到不存在的 `parts.extent`）丢弃 → diff-resize 与 width/height binding 零几何响应（0653 P2-8 B2 修了 line/arrow/pipe，复合族未修）。建议各 symbol `applyProps` 重算子几何（镜像 B2 修复）或给 `createCompositeGroup` per-symbol `resize` hook。（已由 plan `2026-08-06-0900-2` Phase 1 收口，✅）
- `2026-08-05-2129-open-audit-industrial-hmi.md` `[P2-5]` — `scanLegacyAtSyntax`（`validate.ts:390-432`）只扫顶层 `config.symbols`，不递归 `scada-group` children → 组态最典型形态（group 嵌套子图元）的 legacy `@{pointId}` 方言静默不 warn。建议 walk children（镜像 `validateSymbolNode:276-278`）。（已由 plan `2026-08-06-0900-1` Phase 3 收口，✅）
- `2026-08-05-2129-open-audit-industrial-hmi.md` `[P2-6]` — validator 只校验声明对象 surface 类型，`shadow`/animation `from,to`/`background`/declaration `scale.k,b`/`init`/binding-`scale` 子形状 malformed 全过 → 消费者读声明子字段 raw 产 NaN/undefined（`init` 与 `scale.k/b` 后果最重，静默 corrupt 点表）。建议一个 `assertShape(value, schema, scope)` helper 覆盖声明对象形状。（已由 plan `2026-08-06-0900-1` Phase 3 收口，✅）
- `2026-08-05-2129-open-audit-industrial-hmi.md` `[P2-7]` — `video.ts:34-36` create 无条件覆盖 author `stroke`/`strokeWidth`（与上方 guarded `fill` 行不对称）。建议 `if (attrs.stroke===undefined) attrs.stroke=...`。（已由 plan `2026-08-06-0900-2` Phase 2 收口，✅）
- `2026-08-05-2129-open-audit-industrial-hmi.md` `[P2-8]` — `pipe-junction.ts:85` stubs 硬编码 `strokeWidth:4` 且 `applyProps` 不路由 → strokeWidth 变更加粗 body 但 stubs 留细。建议 stub `strokeWidth` 从 `props.strokeWidth ?? 4` 派生 + applyProps 路由 stubs。（已由 plan `2026-08-06-0900-2` Phase 2 收口，✅）
- `2026-08-05-2129-open-audit-industrial-hmi.md` `[P2-9]` — `thermometer.ts:42`(create `-24`) vs `:78`(update `-16`) liquid-anchor 常量分歧，空液位首次 binding tick 跳 8px（sibling `level.ts:42,66` 两路径一致，thermometer 离群）。建议统一单一 `BULB_RESERVE` 常量。（已由 plan `2026-08-06-0900-2` Phase 2 收口，✅）
- `2026-08-05-2129-open-audit-industrial-hmi.md` `[P2-10]` — `getSymbol` 读 handle 返回 leafer-internal 属性名（`fontSize`/`scaleX+scaleY`/`dashPattern`/`fill`-for-textColor）却 `as ScadaSymbolProps` 强转 → read/write field-name 不对称，host getSymbol→setSymbolProps 往返喂错键。建议 `getSymbolProps` 反映射回 schema 名（`toNodePatch` 逆），或文档化为 raw leafer attrs 去 cast。（已由 plan `2026-08-06-0900-2` Phase 3 收口，✅）
