...
> Plan Status: active（draft → active：独立子 agent 两轮审查，首轮 revised（1 Major）已修订解决，复检 pass，零 Blocker/零 Major，共识达成）
...
- [ ] 新增 region label 测试用例：`label: { type: 'text', text: '自定义' }` 形式下 variant-field 渲染 label 文本（先红：当前 `schemaProps.label` 被编译器替换为 region key → 断言失败）。
- [ ] **red 用例需手工模拟编译产物**：单测不跑编译器（编译管线行为 `node-compiler-helpers.ts:270` 在单测外），red 用例必须直接构造 `props.label = region key 占位串 + regions.label render handle` 模拟编译后输入。
...
- [ ] 值 label 回归用例确认存在（既有字符串 label 用例保持绿）。

Exit Criteria:

- [ ] region label 用例先红（断言失败复现 12-01，且失败根因不是 mock 链返回 undefined，而是 label 解析通道缺失）；既有字符串 label 用例仍绿。
- [ ] 测试文件的 FieldFrame mock 与 resolve 通道 mock 已按「green 可观测」要求更新（FieldFrame mock 渲染 label、resolve 委托 regions）。
...
- [ ] 12-01：**按组件既有先例走 prop threading，而非在 view 内直接调用 helper**——`VariantFieldView` 只接收 `schemaProps`（`props.props`）与 `meta`（`variant-field-view.tsx:59-75`），没有 `regions` 也没有完整 `props`，而 `resolveFieldLabelContent` 签名要求 `Pick<RendererComponentProps, 'props'|'meta'|'regions'>`（`field-reading.tsx:38-40`）。照抄 `variant-field.tsx:72-75` 解析 hint/description 的先例（`:82-84` 作为 prop 下传）：在 `variant-field.tsx` 用 `resolveFieldLabelContent` 或 `resolveRendererSlotContent(props, 'label')` 解析 label（region/值双形式），新增 `VariantFieldViewProps.labelContent` prop 传入，`variant-field-view.tsx:222` 改用它（或传 `regions` 进 view 由 view 内解析）。
- [ ] 12-01 回归：Phase 1 region label 用例转绿（含 mock 更新后）。
- [ ] 12-02：`scripts/audit/rules.mjs:16-18` allowlist 路径更新为 `variant-field-view.tsx`（或双登记），跑 `pnpm check:audit-fieldframe-bypasses` 零候选命中。
- [ ] 12-03：`docs/architecture/field-frame.md:97-116` 补 `renderer?: string`（说明：渲染为 `data-renderer`，供 debugger 锚点；`node-frame-wrapper.tsx:62` 实传 `renderer={props.templateNode.rendererType}`）。

Exit Criteria:

- [ ] region label 用例转绿；form-advanced 包 `pnpm --filter @nop-chaos/flux-renderers-form-advanced test` 全绿。
- [ ] `pnpm check:audit-fieldframe-bypasses` 零命中（不再输出 variant-field 3 处候选）。
- [ ] `field-frame.md` 接口列表含 `renderer?: string`，与 `field-frame.tsx:47` 一致。
...
- [ ] 12-01 region label 已修复且有测试（先红后绿）
- [ ] 12-02 allowlist 路径已更新，`check:audit-fieldframe-bypasses` 零命中
- [ ] 12-03 `field-frame.md` 已补 `renderer` prop
- [ ] roadmap Follow-up Backlog 12-01/12-02/12-03 三行已翻转 `[x]` 附收口注记
- [ ] daily log 已记录（`docs/logs/2026/08-07.md`）
- [ ] 不存在被静默降级到 deferred 的 in-scope confirmed live defect
- [ ] 受影响的 owner docs 已同步（field-frame.md 已更新；无其他 owner-doc 变更面则核实时注明）
