...
`Context-Engineering` 的定位是上下文工程资料库。这里的上下文工程主要指面向一次或一组模型调用的上下文构造：模型在推理时看到的不只是用户的一句话，还包括示例、记忆、检索结果、工具、状态和控制流。它主要回答：怎样组织这些信息，才能让模型在当前任务中更稳定地理解、推理和验证。

`attractor-guided-engineering-template` 的定位是项目模板。它当然也服务于“给 AI 准备合适上下文”，但它处理的是更上游的问题：仓库如何长期保存事实、权威和证明关系，使任何一次 AI 会话都能从仓库中恢复正确上下文。它主要回答：当人和 AI 反复修改同一个仓库时，哪些信息必须留在文件中，哪些文件拥有解释权，一个任务怎样才算真正完成。

差异不在于 AGE 是否属于广义上下文工程。广义上，AGE 也是上下文工程：它为未来 AI 会话准备可恢复、可路由、可审计的上下文。差异在处理层级：`Context-Engineering` 更关注会话内或跨少数轮次的上下文如何组织，AGE Template 更关注仓库如何成为长期上下文生成器。
...
`Context-Engineering` 的实用价值更分散，而且不能直接等同于可落地做法。它适合用来学习会话级上下文工程的概念，或从中抽取少量提示词结构、审计步骤和实验问题；但这些材料通常还需要被重新写成具体项目的 prompt、checklist、agent workflow 或测试脚本，才能进入工程实践。代码示例多是 toy model、template 或 placeholder，距离生产级“语义场系统”还有距离。

如果要实际使用它，比较稳妥的方式不是“照着 field/attractor 概念实现一个系统”，而是做小范围提炼：选一个具体任务，例如 bug diagnosis、方案评审或长上下文问答；从 `Context-Engineering` 中抽取一段可执行步骤，例如先列候选解释、再检查证据权重、再用反例扰动判断；把它写成团队自己的 prompt 或 skill；最后用真实案例比较使用前后的误判率、遗漏率和修正成本。离开这种二次提炼和验证，它更多是概念资料库，而不是开箱即用的方法论。
...
| 维度       | Context-Engineering                                              | AGE Template                                                                      |
| ---------- | ---------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| 实用价值   | 概念资料库和素材来源；需二次提炼成具体 prompt/checklist/workflow | 可直接复制到项目的协作治理模板，并把完成条件落到 owner/proof/closure              |
| 理论深度   | 覆盖范围广，概念体系丰富，但形式化和实现验证不足                 | 范围较窄，但对象清晰，可继续展开为语义承诺、`q/p`、`J-flow/R-flow` 和 repo 坐标系 |
| 实际创新性 | 把会话级上下文工程组织成 field/attractor/protocol 的统一语言     | 把仓库级上下文工程明确建模为轨迹收敛和保结构问题                                  |
| 主要风险   | 概念过多，容易把隐喻误读为可测理论                               | 模板过轻，复杂框架项目需要扩展 owner-doc 和验证体系                               |
| 最适合场景 | 学习、研究、prompt/agent workflow 设计                           | 中小应用项目的 AI 协作落地                                                        |

如果只看“是否能马上改变团队工作方式”，AGE Template 更强。如果看“是否提供一套理解会话级上下文组织的概念语言”，`Context-Engineering` 更丰富，但它的实践价值主要来自二次加工，而不是原样执行。如果看“实际创新性”，AGE Template 的贡献更集中：它把上下文工程从“给当前调用准备材料”推进到“让仓库持续生成正确上下文”，并把 AI 协作中的软件开发漂移问题落到仓库文件、语义权威拓扑和关闭机制上；`Context-Engineering` 的贡献更像知识整合和概念框架，把许多会话级上下文技术组织到 field/attractor 语言下。
...
AGE Template 则把上下文工程推进到仓库级。即使 prompt 和 retrieval 设计得很好，如果仓库没有 owner-doc precedence，模型仍可能把旧日志、旧计划、当前设计和源码片段混成同等事实。AGE 让这些材料在仓库内有固定职责，使后续 session 能从文件恢复“应该向哪里收敛”。

换句话说，`Context-Engineering` 解决的是“当前和跨少数轮次的上下文状态如何收敛”，AGE Template 解决的是“仓库如何长期产生正确上下文，并让软件开发轨迹如何收敛”。作为简写可以说：前者的 attractor 是会话语义引力，后者的 attractor 是仓库工程方向。
