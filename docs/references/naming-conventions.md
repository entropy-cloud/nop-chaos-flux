...
| **chart 用 recharts，扩展图表用 echarts** | 普通图表用 `chart`（recharts）；扩展/复杂自定义图表新增 `echarts` 渲染器（双渲染器并存，见 `docs/backlog/echarts-integration-roadmap.md`）；`chart` 渲染器上不做 echarts config 透传。                          |
...
| **echarts 相关**    | chart（recharts）上的 echarts config 透传、echarts 扩展（wordcloud/stat/bmap）                                                                                                                            | echarts 能力以独立 `echarts` 渲染器提供（双渲染器并存，见 `docs/backlog/echarts-integration-roadmap.md`），不污染 chart schema；geo 地图/`registerTheme`/`registerMap` 归 echarts 渲染器 schema 能力。         |
...
- 相关分析：`docs/bugs/15b-component-level-initfetch-analysis-and-fix.md`
