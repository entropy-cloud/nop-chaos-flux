...
- **日历 locale 随 flux 语言（2026-08-03 C2.4 收敛）**：共享日期控件（input-date/input-datetime/date-range 共用 `date-field-control` / date-range 面板）按当前 flux 语言（`useFluxTranslation().i18n.language`）将 react-day-picker locale 映射为 `enUS`/`zhCN` 传入 Calendar——月/周标题与导航文案随语言本地化（zh-CN 为周一起始）。
