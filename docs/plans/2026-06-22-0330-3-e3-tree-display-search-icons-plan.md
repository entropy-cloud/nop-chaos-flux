...
- [x] **Fix**：`tree/design.md` §2 新增 Flux 决策表子节（保留现有「与 AMIS 的关系」叙述，按 X5 模式补决策表，列：能力 / 采纳 / 不采纳 / 理由），列：`searchable`（实现）、`showIcon`/`iconField`（实现）、`showGuideLine`（实现）、选择/值绑定/勾选（不采纳 + 归 input-tree/tree-select 理由，引用 §8/§11）、节点拖拽（不采纳/后续 + 无共享 dnd 工具 + 层级 DnD 复杂 + 理由）、节点 CRUD（后续 + 理由）、异步懒加载（后续 + 归 input-tree 范围）、虚拟滚动（不采纳 + E2d 已 Deferred）、远程搜索（不采纳 + 归 data-source）。
- [x] **Decision**：裁定搜索行为 —— 本地子串过滤 `labelField`；匹配节点自动展开祖先链（非全展开）；高亮匹配文本（`<mark>` 或 marker span）；清空搜索恢复搜索前展开快照。**open-state 管理要点**：当前 `TreeNodeRenderer` 各节点独立 `useState` 管开合（`tree-renderer.tsx:170`），搜索自动展开 + 快照恢复需从父 `TreeRenderer` 下发「search-forced-open」信号或 matched-paths 集合覆盖子节点本地态；本 Decision 需裁定该机制（受控展开覆盖 vs 提升开合态到父）。结论写入 `tree/design.md`。
- [x] **Decision**：裁定 `showIcon` 与 `node` region 的关系 —— 若 `node` region 已自定义渲染图标，`showIcon`/`iconField` 是否叠加（裁定：`showIcon` 是便捷快捷方式，`node` region 自定义优先；两者同时存在时 region 优先，design.md 注明）。结论写入 design.md。
- [x] **Decision**：裁定引导线实现 —— 按 depth 渲染垂直 `border-l`（Tailwind），`showGuideLine:true` 启用。结论写入 design.md。
- [x] **Decision**：裁定拖拽 —— Deferred（`optimization candidate`）。理由：无共享 dnd 工具（E1c table 行拖拽 table 专属不可复用）；tree 拖拽是层级 DnD（跨层级移动、父子重排）比扁平列表复杂；design.md §8 L117 已明确延后。non-blocking：当前 tree 是展示组件，无值绑定，拖拽不影响展示契约成立。结论写入 design.md Deferred 节 + 本 plan Deferred But Adjudicated。

Exit Criteria:

- [x] `tree/design.md` 含 §2 Flux 决策表节（live repo 可读，列含采纳/不采纳/理由）。
- [x] 搜索行为/icon 与 region 关系/引导线/拖拽四条 Decision 结论明确，无歧义。
...
- [x] 测试文件存在，运行全部 RED。
- [x] 用例覆盖 Goals 中搜索/图标/引导线所有可观测行为 + 五条 Failure Path。
...
- [x] `schemas.ts`：`TreeSchema` 新增 `searchable?: boolean`、`showIcon?: boolean`、`iconField?: string`、`showGuideLine?: boolean`。
- [x] `tree-renderer.tsx`：TreeRenderer 新增搜索框（`searchable`）+ 本地过滤逻辑（子串匹配 labelField）+ 匹配节点祖先链自动展开 + 高亮 + 清空恢复快照（按 Phase 1 Decision）。
- [x] `tree-renderer.tsx`：TreeNodeRenderer 新增节点图标（`showIcon`/`iconField`，经 `resolveLucideIcon`，按 Phase 1 Decision 与 region 优先级）+ 引导线（`showGuideLine`，按 depth border-l）。
- [x] `data-renderer-definitions.ts`：tree renderer definition fields 注册新 prop（searchable/showIcon/iconField/showGuideLine 为 prop 非 region）。
- [x] Phase 2 RED 用例全部转 GREEN。

Exit Criteria:

- [x] Phase 2 全部用例 GREEN；既有 flux-renderers-data tree 测试套件无回归。
- [x] live repo 核对：TreeRenderer/TreeNodeRenderer 真实读新字段（grep 非空），搜索过滤/图标/引导线 runtime 路径调用渲染逻辑（非空壳）。
...
- [x] `tree/design.md` §4（schema）/§5（字段分类）/§9（node template，注 icon 与 region 关系）/§10（Empty State，注搜索无匹配提示）同步落地内容，与 runtime 一致；新增 DOM marker 说明（搜索框/图标/引导线/高亮 marker 记入 §9 或专用 marker 小节，design.md 当前无独立 DOM marker 节）。
- [x] playground 新增「树展示 UX 增强」示例页（演示搜索过滤 + 高亮、节点图标、引导线），注册路由。
- [x] `examples.manifest.json` 登记新示例。
- [x] **e2e**：新增 `tests/e2e/tree-display-ux.spec.ts`，覆盖搜索输入过滤 + 自动展开匹配 + 高亮、清空恢复、节点图标渲染、引导线的关键交互路径（满足 roadmap Cross-Cutting「每个工作项必须有 e2e」硬约束）。

Exit Criteria:

- [x] `tree/design.md` §4/§5/§9/§10 与 runtime 一致（live repo 可读）。
...
- [x] tree 搜索/过滤（searchable + 自动展开 + 高亮 + 清空恢复）已落地且 focused 测试 GREEN
- [x] tree 节点图标（showIcon/iconField）已落地且 focused 测试 GREEN
- [x] tree 引导线（showGuideLine）已落地且 focused 测试 GREEN
- [x] tree/design.md 含 Flux 决策表（X5 扩展完成）
- [x] 拖拽裁定已落地（Deferred + non-blocking 理由记录）
- [x] 缺省回退无回归（既有 flux-renderers-data tree 测试套件全过）
- [x] playground 示例 + `examples.manifest.json` 登记
- [x] `tests/e2e/tree-display-ux.spec.ts` 存在并覆盖关键交互路径
- [x] 不存在被静默降级到 deferred 的 in-scope live defect 或 contract drift
- [x] 受影响 owner docs（design.md §2/§4/§5/§9/§10）已同步到 live baseline
...
Status Note: E3 tree display UX（搜索/过滤 + 节点图标 + 缩进引导线）全 4 Phase 执行完成，Plan Status → completed。三项能力（`searchable`/`showIcon`+`iconField`/`showGuideLine`）落地于 `packages/flux-renderers-data/src/tree-renderer.tsx` + `schemas.ts` + `data-renderer-definitions.ts`，缺省回退无回归。`tree/design.md` §2 Flux 决策表 + §2.1 关键裁定 + §4/§5/§9/§10 同步落地。拖拽裁定 Deferred（`optimization candidate`，`Successor Required: no`）已记录于 design.md 与本 plan Deferred But Adjudicated。Closure Gates 全 `[x]`（含独立 closure-audit，见下方证据）。`examples.manifest.json` 裁定：该 manifest 跟踪 renderer type（`tree` 已在 runtime 列表 L44），本 plan 未引入新 renderer type，仅增强既有 type 的 props，故无需新增条目（裁定记于 `docs/logs/2026/06-22.md`）。

Closure Audit Evidence:

- Auditor / Agent: 独立 closure auditor sub-agent（fresh session，不复用执行者上下文，mission-driver [MISSION_DRIVER] closure-audit 流）
- Audit Scope: Phase status/items consistency、Exit Criteria vs live repo、Anti-Hollow、Five-point consistency、Deferred honesty、Docs sync
- Findings: **approved**（零 Blocker / 零 Major）
  - Phase status/items consistency: 4 Phase 全 `completed`，所有 in-scope checklist item `[x]`（closure-audit gate 由本次独立审计勾选）。
  - Exit Criteria vs live repo（逐条核对 `packages/`）:
    - `schemas.ts:166-169` TreeSchema 新增 `searchable?`/`showIcon?`/`iconField?`/`showGuideLine?` ✓
    - `tree-renderer.tsx` `computeTreeSearch` L145-198（DFS，matchedNodeIds + forcedOpenNodeIds 祖先链 + visibleNodeIds）、`renderHighlightedLabel` L200-235（`<mark data-slot="tree-search-highlight">`）、`renderNodeIcon` L237-251（getIn + resolveLucideIcon，null-safe）、TreeNodeRenderer `effectiveOpen`/`searchForcedOpen` L311-312、guide-line spacer L406-410（按 depth）、TreeRenderer `searchQuery` state L605 + searchState L606-607 + 搜索框 L726-733 ✓
    - `data-renderer-definitions.ts:466-469` 注册 4 prop ✓
    - `packages/flux-renderers-data/src/__tests__/tree-display-ux.test.tsx` 8 用例覆盖 5 条 Failure Path ✓
    - `tests/e2e/tree-display-ux.spec.ts` 6 用例 ✓
  - Anti-Hollow: 无空壳/return null 占位/swallowed exception；computeTreeSearch/renderHighlightedLabel/renderNodeIcon 为真实实现且被 TreeNodeRenderer/TreeRenderer 调用。
  - Deferred honesty: 拖拽（`optimization candidate`，无共享 dnd 工具 + 层级 DnD 复杂 + design.md §8 延后）、CRUD（`out-of-scope improvement`）、异步/虚拟（`out-of-scope improvement`，E2d 已 Deferred）—— 均附 non-blocking 理由，无 in-scope live defect/contract drift 被静默降级。
  - Docs sync: `docs/components/tree/design.md` §2 Flux 决策表 + §2.1 四条裁定 + §4/§5/§9（§9.1 icon-region + §9.2 DOM markers 表）/§10（搜索无匹配）；`docs/logs/2026/06-22.md` 全 4 Phase + full-green 记录 ✓
- Executor Evidence（执行者自报，已经独立 auditor 抽样核对一致）:
  - 代码：`schemas.ts`/`tree-renderer.tsx`/`data-renderer-definitions.ts` 同上。
  - 验证（full-green）：`pnpm typecheck`=49/49、`pnpm build`=26/26、`pnpm lint`=26/26、`pnpm test`=49/49 tasks（flux-renderers-data 50 files/439 tests、playground route-matrix 19 files/88 tests 全绿）；e2e tree-display-ux 6/6 GREEN。
  - Playground：`apps/playground/src/pages/tree-display-ux-demo.tsx` + App.tsx + route-model.ts（id `tree-display-ux`）。
