...
`@nop-chaos/flow-designer-renderers` root entry 不再把 `createFlowDesignerRegistry()` 这种已知命名漂移 alias、`DesignerXyflow*` bridge、`DesignerCanvasContent`、`DesignerPaletteContent`、`DefaultInspector`、或 `designer-context` hooks 当作稳定公共 API。需要遗留 alias 或这些实现叶子时，使用 `@nop-chaos/flow-designer-renderers/unstable`。
