...
- **E4 包基建与依赖引入（2026-08-06，✅ 方案 A 裁定 + 空壳注册 + closure-audit PASS）**：plan `2026-08-06-2118-2` 激活 E4: `todo` → `planned`。**E4.1 包结构裁定**：引入 `@leafer-in/editor@2.2.9`（版本对齐 spike lockfile + runtime leafer-ui@2.2.9 + MIT），实测 `editor.esm.min.js` ~50KB；**模块图隔离证明**（`grep` 确认主入口 `src/index.ts` 零 editor 引用 + `src/` 无 `@leafer-in/editor` runtime import）→ **裁定方案 A**（不新建包，编辑器放入既有 `flux-renderers-industrial` 的 `src/editor/` subpath），综合 trade-off 四维度（bundle 经模块图证明可隔离 + 双态隔离 module/subpath 级足够 + 命令面零复制 + 维护成本最低），`bundle-isolation-fail` Failure Path 不触发；隔离策略：subpath `/editor` export + 独立注册函数 `registerScadaEditorRenderers`（不并入 `registerScadaRenderers`）+ playground subpath import（禁主入口 re-export），对齐 `@nop-chaos/flux-renderers-ai/rich-text` 先例。裁定结论回写 `design-architecture.md §4.4`（新增 §4.4.1）+ trade-off 表 + §3 + §11、`design-renderer.md` §3 + §11、4 sibling design 包归属、roadmap总览。**E4.2 空壳注册**：`src/editor/` 骨架（schemas.ts 最小 config/width/height + scada-editor-canvas.tsx 空壳组件 + renderer-definitions.ts 空壳定义 + index.ts `registerScadaEditorRenderers` 独立函数 + styles.css）+ 工程接线三通道（tsconfig.base.json paths + vite.workspace-alias.ts + package.json exports，`/editor` subpath 置于 main 之前防前缀误匹配）+ `examples.manifest.json` runtime 含 `scada-editor-canvas` + playground App.tsx subpath import 注册；renderer 注册单测（7 用例：定义结构 + 最小 fields + 注册无冲突 + 独立于 registerScadaRenderers + 幂等）+ 空壳 smoke 单测（data-slot + data-status="shell" 渲染不抛错）。**人工确认阈值：未触发**（roadmap「人工确认阈值」不含「新建包」+ 本裁定采方案 A 不新建包 + roadmap总览已显式授权 E4.1 裁定）。**workspace 全量验证**：typecheck 32/32 + build 32/32 + lint 32/32（0 errors）+ test 59/59 green（industrial 57 files / 754 tests 含 13 新 editor 测试，src/editor 覆盖率 100%）。**closure-audit PASS**：独立 fresh-session sub-agent `ses_0289e205afferYBEI18enNPCdC` verdict = **pass**（0 Blocker / 0 Major / 2 Minor 已处理：① daily log 表述先于状态 finalize 后一致；② design-architecture.md:219 残留「待 E4.1 裁定」历史引述已改写消除 grep 误报）。Phase Status E4 `planned` → `done`；Plan Status → `completed`；closure-audit gate ticked。
...
- **E5. M1 MVP 编辑器实现** (`todo`) <!-- 双态切换 + 图元库面板 + 拖拽放置 + 属性面板 schema（几何/样式/绑定）+ 保存/加载 + 编辑期校验 -->
