...
1. 当前 Flux 是 lexical read + own-store write
2. 子 scope 可以沿父链读取
3. 但写入不会自动路由到父 scope，而是在当前 own scope 形成遮蔽

这一点与 v6 的 `WriteRoute` 差异非常大。v6 追求的是显式写路由与 cell 归属；当前 Flux 则优先保持简单、局部、可解释的 own-scope 写语义。
...
5. 当表达式发生顶层 lexical scope 枚举或整体 materialize 时，会退化为 wildcard 依赖；但已绑定对象的枚举仍只锚定到其 lexical root，见 `packages/flux-formula/src/scope.ts:137-146`, `:179-201`
...
3. 一旦触发 wildcard，局部命中会退化为保守全命中
4. 更难为全局调度器提供统一、细粒度可预测输入
...
这里的“更适合”仅指设计潜力，不代表在当前项目中已经被实现或证明更优。
...
换句话说，当前 Flux 在动作编排这一块已经明显接近 v6 的一部分设计方向。

但这种接近只限于“结构化控制流程序”和 branch semantics，不等于当前 Flux 已经拥有 v6 所设想的统一 effect graph 内核。
...
这一维度上，两者在 action control-flow 语义上的差距没有看起来那么大。
...
4. reaction 在注册时会先求值一次建立初始依赖，`immediate` 控制的是是否立刻触发动作，不是是否先建立 watch 基线，见 `packages/flux-runtime/src/reaction-runtime.ts:340-347`
5. reaction 带有 `MAX_REACTION_FIRE_COUNT` 熔断，超限后会告警并自动 dispose，见 `packages/flux-runtime/src/reaction-runtime.ts:43-49`, `:191-225`

同时，本次采样代码只能证明 source/reaction 的 runtime substrate 与 registry 已经落地；还不能据此断言通用 schema 渲染主路径已经对所有 source/reaction 统一自动挂载。
...
当前 Flux 在校验上的优势很明显，但主要体现在设计基线成熟度，而不是“所有能力都已完整实现”。
...
但从当前状态看，更准确的说法是：Flux 在校验上的设计基线比 v6 更完整，且部分关键能力已进入分阶段实现；v6 仍主要停留在统一内核方案层。
...
见 `docs/architecture/frontend-programming-model.md:171-172`, `:219-225`, `:290-297` 与 `docs/architecture/flux-design-principles.md:155-160`, `:171-187`。

其中 `Capability` 的现行解析面并不只是一层抽象名字，还包括:

1. built-in platform actions
2. `ComponentHandleRegistry` 的实例定向调用
3. `ActionScope` 的词法命名空间查找
...
## 15. 哪些地方 v6 在设计上更有潜在优势

在纯架构设计上，v6 的明显潜在优势主要有六点。
...
如果要做一个明显超出当前 Flux 的全新底层，这六点确实是最有价值的探索方向。
...
3. 当前以词法所有权组织 scope/resource/reaction sidecar 的实现方式与若干派生规则
...
这里还需要再收敛一个关键判断: 真正值得统一的通常不是全部 runtime 语义，而是共享 substrate。
...
2. 基础依赖追踪与失效传播
3. 结构化写入发布
4. action 结果代数与 effect continuation
5. async ownership、取消、重试、超时、trace id

更适合独立演化的能力:

1. validation runtime
2. surface/dialog/drawer owner family
3. table/row 高频特化 runtime
4. flow designer / spreadsheet / report / word editor 等复杂域 runtime
5. React renderer adapter 与其他宿主适配层

因此，最稳妥的问题不是“要不要 graph kernel”，而是:

1. 哪些共享机制值得下沉到底层
2. 哪些 owner 语义必须保持独立
3. 哪些复杂领域绝不能因为追求统一而反灌核心
...
这些改动如果进一步扩大 author-visible runtime surface、改变 primitive category 或削弱当前 host/domain 边界，就会冲击当前 Flux 的规范基线。若只是内部 substrate 层面的局部收敛，则未必天然违背现行规范。

更现实的改写方式是:

1. 不追求“大一统 graph kernel”
2. 先追求“minimal execution kernel”
3. 让 validation、surface、table、domain runtime 继续独立演化，但逐步共享更强的底层 substrate
...
4. 若未来真的借鉴 v6，更合理的落点也更像“最小执行内核 + 独立 owner family 演化”，而不是把所有 runtime 语义统一成一个大 kernel。
