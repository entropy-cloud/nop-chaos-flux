...
| E1d    | crud 数据生命周期：轮询刷新（走 data-source）+ 可折叠查询区 + 无限滚动 + cards/list 模式（✅ 全部落地；cards/list 模式经 successor plan `2026-06-24-2358-2` 收口，CRUD `listMode` 字段已实现；跨页选择保留已由 E0c 实现） | crud   |
