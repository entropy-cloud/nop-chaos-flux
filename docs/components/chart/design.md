...
| echarts config 透传                                        |                       | ✅     | 归独立 `echarts` 渲染器承载（双渲染器并存，见 `docs/backlog/echarts-integration-roadmap.md`）；透传到 chart 破坏 schema 边界，引入与 recharts 渲染路径冲突的双轨。                                         |
| echarts 扩展（wordcloud/stat/bmap）                        |                       | ✅     | 扩展图表由独立 `echarts` 渲染器按需引入承载，超出 chart 表现层定位。                                                                                                                                  |
| geo 地图                                                   |                       | ✅     | 地图需独立数据协议与地图资源（geojson）管理，归 `echarts` 渲染器承载（双渲染器并存）。                                                                                                                  |
| echarts 主题（`chartTheme`/`registerTheme`/`registerMap`） |                       | ✅     | chart 主题由 CSS 变量 + Tailwind design token 管理（见 `docs/architecture/theme-compatibility.md`、`styling-system.md`）；echarts 主题体系归 `echarts` 渲染器 schema 能力。                            |
