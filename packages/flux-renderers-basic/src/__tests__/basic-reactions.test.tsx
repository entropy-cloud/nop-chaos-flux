...
describe('basicRendererDefinitions reaction and action behavior', () => {
  it('rerenders scope-debug when the current scope changes', async () => {
    ...
    render(<SchemaRenderer schemaUrl="test://basic/reactions" schema={{ type: 'page', body: [{ type: 'button', label: 'Increment', onClick: { action: 'setValue', args: { path: 'count', value: '${count + 1}' } } }, { type: 'scope-debug', title: 'Probe', defaultExpand: true }] }} data={{ count: 0 }} env={env} formulaCompiler={formulaCompiler} />);
  ...
  it('runs reaction actions immediately when configured', async () => {
    const SchemaRenderer = createBasicSchemaRenderer();
    render(<SchemaRenderer schemaUrl="test://basic/reactions" schema={{ type: 'page', body: [{ type: 'reaction', watch: '${count}', immediate: true, actions: { action: 'setValue', args: { path: 'message', value: 'count:${count}' } } }, { type: 'text', text: '${message}' }] }} data={{ count: 3 }} env={env} formulaCompiler={formulaCompiler} />);
  ...
  it('gates reaction execution with when and once', async () => {
    const SchemaRenderer = createBasicSchemaRenderer();
    render(<SchemaRenderer schemaUrl="test://basic/reactions" schema={{ type: 'page', body: [{ type: 'reaction', watch: '${status}', immediate: true, once: true, when: 'value === "ready" && prev !== value', actions: { action: 'setValue', args: { path: 'message', value: 'triggered:${status}' } } }, { type: 'button', label: 'Ready', onClick: { action: 'setValue', args: { path: 'status', value: 'ready' } } }, { type: 'button', label: 'Done', onClick: { action: 'setValue', args: { path: 'status', value: 'done' } } }, { type: 'text', text: '${message}' }] }} data={{ status: 'idle', message: 'initial' }} env={env} formulaCompiler={formulaCompiler} />);
    expect(screen.getByText('initial')).toBeTruthy();
  ...
  it('coalesces reaction triggers during debounce windows', async () => {
    const SchemaRenderer = createBasicSchemaRenderer();
    render(<SchemaRenderer schemaUrl="test://basic/reactions" schema={{ type: 'page', body: [{ type: 'reaction', watch: '${count}', debounce: 20, actions: { action: 'setValue', args: { path: 'message', value: 'count:${count}' } } }, { type: 'button', label: 'Inc', onClick: { action: 'setValue', args: { path: 'count', value: '${count + 1}' } } }, { type: 'text', text: '${message}' }] }} data={{ count: 0, message: 'start' }} env={env} formulaCompiler={formulaCompiler} />);
  ...
  it('prevents unbounded self-trigger cascades in reactions', async () => {
    const notify = vi.fn();
    const SchemaRenderer = createBasicSchemaRenderer();
    render(<SchemaRenderer schemaUrl="test://basic/reactions" schema={{ type: 'page', body: [{ type: 'reaction', watch: '${count}', immediate: true, actions: { action: 'setValue', args: { path: 'count', value: '${count + 1}' } } }, { type: 'text', text: '${count}' }] }} data={{ count: 0 }} env={{ ...env, notify }} formulaCompiler={formulaCompiler} />);
    await waitFor(() => {
  ...
  it('does not overwrite mount-time reaction writes with the initial page data sync effect', async () => {
    const SchemaRenderer = createBasicSchemaRenderer();
    render(<SchemaRenderer schemaUrl="test://basic/reactions" schema={{ type: 'page', body: [{ type: 'reaction', watch: '${count}', immediate: true, actions: { action: 'setValue', args: { path: 'message', value: 'count:${count}' } } }, { type: 'text', text: '${message}' }] }} data={{ count: 1, message: 'initial' }} env={env} formulaCompiler={formulaCompiler} />);
