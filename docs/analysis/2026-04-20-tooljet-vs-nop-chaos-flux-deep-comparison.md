...
const store = createStore<{ snapshot: Record<string, any>; lastChange: ScopeChange }>(
  () => ({ snapshot: initialData, lastChange: createDefaultChange() })
);
...
const subscribe = useMemo(
  () => isStatic
    ...
    : ((listener) => props.scope.store?.subscribe((change) => {
...
export const buttonConfig = {
  ...
  styles: { /* color, border, icon */ },
  exposedVariables: { buttonText: 'Button', isVisible: true },
  actions: [ /* click, setText, setVisibility, ... */ ],
};
```
