...
| `submitForm`    | none                      | implicit `ctx.form`                        | no payload   | Canonical baseline is current-form submit only. Cross-form submit should use `component:submit`. `action: 'submit'` is accepted as a compatibility alias.                                                               |
...
| `refreshSource` | none                      | `targetId`                                 | no payload   | runtime-owned source entry，`targetId` 指向 canonical data-source `name`                                                                                                                                                |
| `setValue`      | `args: { path?, value }`  | none                                       | canonical    | Canonical baseline is a current-scope write. `args.path` selects the path inside the current dispatch scope. Cross-component or cross-form writes should use `component:setValue` instead of built-in targeting fields. |
| `setValues`     | `args: { path?, values }` | none                                       | canonical    | Canonical baseline is a current-scope multi-write. `args.path` makes `values` keys relative to the current dispatch scope. Cross-component or cross-form writes should use `component:setValues`.                       |
...
| plain named action   | `args`                   | lexical `ActionScope`                                                    | already aligned | Simple action names without a colon (e.g. `action: 'myAction'`) resolve through `XUI_ACTIONS_NAMESPACE` (`__xui_actions__`) via `runNamedAction()`. This is a third dispatch path alongside `component:<method>` and `namespace:method`. |
...
3. `submitForm` 是语义型 form submit command，不要求业务 payload；新 schema 默认应理解为“提交当前 form”。如需明确 targeting 其他 form，优先使用 `component:submit`
...
4. 二者的正式语义是当前 dispatch scope 内的写操作；若需要实例 targeting，应改用 `component:setValue` / `component:setValues`
...
2. `action: 'submit'` is accepted as a compatibility alias and falls through to the same handler
3. `packages/flux-action-core/src/action-dispatcher/built-in-actions.ts` 对 `submitForm` 生成无 payload invocation
4. `packages/flux-runtime/src/action-adapter.ts` 对 `submitForm` 直接调用 `ctx.form.submit(...)`
