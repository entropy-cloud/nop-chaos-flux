...
>
> **anchor-epoch 注记（FIND-18 校准，2026-08-11，plan `2026-08-11-0335-3`）**：本文件 `文件:行` 引用为 **2026-08-09/10 时点锚点**；1606-1 模块抽取（autoSave → `use-conversation-autosave.ts`）与后续修复使部分行号漂移，已按 live 校准（K-⑩-3 autoSave 引用改 `use-conversation-autosave.ts:56/:101`；W-E/W-⑨-a/W-⑨-c create-engine 锚点按 live 更新）。历史锚点以 live 复核为准（multi-audit 交叉模式 4「anchor-epoch + live-verify」建议）。
...
- **位置**：`create-engine.ts:579-580`（runOnce catch `commitOrDropResidue`；定义 `:495-507`）× `use-conversation-autosave.ts:56`（autoSave `isDone` 含 'error'）→ `:101` `saveMessages` 落盘（1606-1 模块抽取后迁移；FIND-18 校准）
...
> 2026-08-10 更新（plan `2026-08-10-0925-2` Phase 5 附带评估）：**W-⑨-b 随 K-⑥-3 修复自然收敛，从 watch-only 移除**；W-E 评估后不收敛维持登记——新 watch-only 实为 **3 条**（W-E / W-⑨-a/c）。

| #     | 观察（文件:行）                                                                                                                                                                       | 分类                | 复触发条件 / 不阻塞理由                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| W-E   | `onAfterRequest` 对 abort-before-first-chunk 轮仍触发（`create-engine.ts:552-554`，FIND-18 校准），回调收到空 assistant（`content:''`）——aborted 轮被当作「请求完成」通知插件         | watch-only residual | 触发：abort 于首 chunk 前 + 插件实现 onAfterRequest（计数/记录类插件把空轮记为完成）；结果：插件侧状态失真，engine 状态无损坏。**I4 附带评估（2026-08-10，plan `2026-08-10-0925-2` Phase 5）**：K-⑩-1 修复后 **W-E 不自然收敛**——`onAfterRequest` 仍在空产物 drop 之前触发（`create-engine.ts:552-554` 无条件调用，`commitOrDropResidue` 在 `:555`；FIND-18 校准，修复仅把空产物从列表中 drop，未重排/跳过 hook 面）→ **维持 watch-only 登记**（如实记录；不承诺修复） |
| W-⑨-a | connector-missing 早退（`create-engine.ts:215-238`，FIND-18 校准）不触发 onTurnStart/onTurnEnd——`plugin.onError` 无配对 start/end（生命周期不对称）                                   | watch-only residual | 触发：无 connector 的 engine + 实现生命周期计数/日志的插件；结果：插件计数失步。设计面（早退不入正常生命周期），低严重度                                                                                                                                                                                                                                                                                                                                               |
| W-⑨-b | bootstrap-active 会话的 chat 面回退自建引擎不在 engineCache → 不挂 autoSave → 默认会话消息在首次 switch 前不持久化（`use-conversation.ts:242-246` 伴生）                              | watch-only residual | 触发：storage 模式 + 首挂载直接发消息（不先 switch）；结果：消息仅内存。**I4 附带评估（2026-08-10，plan `2026-08-10-0925-2` Phase 5）**：K-⑥-3 修复后 **W-⑨-b 自然收敛**——bootstrap 选中 active 时经 `buildEngineFor` 建引擎（挂 autoSave 订阅）+ 加载存储消息，默认会话消息在首挂载即持久化 → **从 watch-only 移除**（findings §3.3 行更新；adjudication §3.4 维持登记注记已被 I4 收口评估覆盖）                                                                      |
| W-⑨-c | `clear()`/`setMessages()` 不重置 `lastError`（`create-engine.ts:150-162`（setMessages）/`:646-660`（clear），FIND-18 校准）——reset 后 stale 错误在下一 turn 启动（`:247` 清除）前可见 | watch-only residual | 触发：错误轮后 host 调 clear/setMessages 再读 state.lastError；结果：UI 残留 stale 错误气泡。低严重度（下一 turn 自动清除）                                                                                                                                                                                                                                                                                                                                            |
...
- **watch-only**：新 3 条（W-E / W-⑨-a/c，**W-⑨-b 已随 K-⑥-3 修复自然收敛移除**）+ Cycle 1 W1-W4 维持
