...
| **G. 复杂交互组件的功能正确性** | 21       | 显示与定位正确性         | 定位算法、坐标/行高对齐、日期边界、渲染结构、CSS marker 命中（含复杂交互渲染器时必选） |
|                                 | 22       | 集成接线与可操作性       | schema→store→DOM→事件 全链路通断、controlled/uncontrolled、交互闭环                    |
|                                 | 23       | 测试有效性与假绿         | 固化缺陷断言、mock 掉被测边界、死代码带测试、覆盖有效性                                |
...
├── 20-accessibility.md
├── 21-display-positioning.md      # 条件：含复杂交互渲染器时
├── 22-integration-wiring.md       # 条件：同上
├── 23-test-effectiveness.md       # 条件：同上
└── summary.md
...
> 本组维度针对 gantt / kanban / calendar / 任意 designer / scheduler 等"复杂交互渲染器"。
> **历史教训**：scheduling 包曾通过多维度审计（维度 05/06/19/20）+ 600+ 单测全绿，却仍带着 12 个 P0 发布（时间线空白、看板拖拽无反应、日历视图切换失效、扫码器黑屏）。根因是既有维度只查"代码质量结构"，不查"渲染结果是否正确 / 可操作 / 测试是否有效"。详见 `docs/bugs/71-scheduling-deep-audit-blind-spot-display-operability-test-effectiveness.md` 与 `docs/analysis/2026-07-22-scheduling-display-operability-deep-analysis.md`。
> **何时启用**：审计对象包含复杂交互渲染器（带 canvas/时间线/矩阵/拖拽/相机等"看得见摸得着"的交互面）时，本组三维度为必选；纯表单/展示组件可选。
> **轻量替代**：若不需要跑完整深度审计，可用独立 skill `docs/skills/complex-component-display-operability-audit-prompt.md`（本组三维度的轻量单 pass 版）。**勿与该 skill 重复执行同一审计**。
> **维护提示**：本组维度内联的 `曾发现…` 历史案例在对应 bug 修复后转为"历史模式"参考，不代表当前缺陷。
...
目标：核查组件渲染出的视觉结果是"正确的"，而非"能跑的"。重点查定位算法、坐标/行高对齐、日期/单位边界、渲染数量、与 design.md 契约及开源参考的一致性。既有维度 09 查"是否按 RendererComponentProps 模式写"，本维度查"写出来的东西渲染后是否正确"。

必读文档：
- 被审组件的 docs/components/<type>/design.md（契约基线：布局范式、marker、定位公式、默认值）
- 对应开源参考（如 ~/sources/complex-controls/ 下的 SVAR/DHTMLX/react-kanban-kit/Schedule-X/react-zxing）
- docs/architecture/styling-system.md（marker 视觉契约）

执行步骤：

1. **定位算法核对**：对每个"由数据算坐标/尺寸"的纯函数（layout.ts / scale.ts / *-layout-utils.ts / *-time-utils.ts），独立推算 2-3 个代表输入的正确输出（手算），再对照实现。重点查：
   a. 日期/单位的 inclusive vs exclusive 边界（end 当 inclusive 会导致 off-by-one；同日任务塌成最小宽）。
   b. 并发/重叠分配是否按"实际并发数"而非"配置上限"算宽（曾发现单事件只占 1/maxConcurrent 宽，75% 留白）。
   c. 行高/列宽在"左面板"与"右面板"之间是否一致（曾发现网格行高随内容撑开，而时间线用固定行高，两侧纵向错位）。
2. **渲染数量与结构核对**：对照 design.md 的布局范式，核查实际渲染的行列数。曾发现"资源×日期矩阵"组件实际渲染成 42 列通用日历（每格 ~28px 标签截断为空）；或虚拟列表行缺 position:absolute 致行距翻倍。
3. **刻度/标签/格式核对**：时间刻度的格式 token（如 strftime 的 %V/%W/%q）是否真正实现；未知 token 是否原样输出乱码。
4. **特殊元素核对**：里程碑（零宽）、多日拆分块（is-split）、今日/周末标记、依赖线（4 类型路由）、跨日连接线——逐一确认定位与可交互性（pointer-events）。
5. **时区/日期运算核对**：日期是否统一用 UTC 构造却用 local getter 格式化（toISODateString/isToday/isSameDay），导致非 UTC 时区错位。CI 常在 UTC 下掩盖。
6. **CSS marker 与视觉契约核对**：design.md §10 列出的每个 data-slot/marker 是否在 CSS 中有对应规则；hover/active/拖拽反馈（ghost 透明度/scale、drop indicator 指示线）是否实现且选择器能命中（曾发现 data-dragging 放在根节点、CSS 用后代选择器导致永不匹配）。
   - **与维度 10 的边界**：marker 命名/纯度/间距/主题归维度 10；本维度只查"交互反馈视觉是否实现且选择器是否命中"这一结果性事实。
7. **空/加载态核对**：design.md 规定的骨架/空态是否实现（非仅"不崩"）。

输出格式（内联，与维度 05/09/14/20 一致）：

对每个发现：
### [维度21] 简短标题
- **文件**: packages/xxx/src/yyy.tsx:行号
- **严重程度**: P0/P1/P2/P3
- **类别**: 定位算法/渲染结构/刻度标签/特殊元素/时区/CSS marker/空态
- **现状**: 一句话描述
- **应为之值**: 对照 design.md / 开源参考 / 数学定义
- **建议**: 修复方向

误报排除：
- 设计上允许的相邻月灰格（通用日历）不算缺陷；但若组件被 design.md 明确定义为"矩阵"而非"通用日历"，则 42 列结构是缺陷。
- "我觉得不好看"不算缺陷；必须有数学/契约/开源对照证据。
- 挂载/effect 时序导致的显示失败（如 video 黑屏）属维度 22（集成接线），不属本维度。
```
...
```
以下为"维度 22"的维度正文。派发时必须与上文"共享提示词前缀"拼接。

审核维度 22：集成接线与可操作性

目标：核查"schema → store → DOM → 事件"全链路是否真的接通，以及核心交互是否可操作。既有维度 04（状态所有权）查"事实源是否单一"，维度 09（渲染器契约）查"是否按 RendererComponentProps 模式写、local state 是否构成第二事实源"；本维度查"接线是否通、交互是否成立"——含"只有一份 state 但 JSX 读错变量"（render 分支读 schema 值而非内部 hook 值）、"store bump 的 revision 无组件订阅"这类 04/09 查不出的"接线失灵"情形。

必读文档：
- 被审组件 design.md（§7 状态归属、§8 事件/句柄、§9 数据源）
- docs/architecture/renderer-runtime.md（regions/events/helpers 通道）
- 被审组件的 scheduling-renderer-definitions.ts / schemas.ts（声明面）

执行步骤：

1. **schema→store 接线核对**：凡 store/config 接受的字段，核对顶层组件是否真的从 resolved props 透传了（曾发现 zoomLevels/ownership 字段/regions 全部声明却漏传给 store，致时间线全空）。
2. **内部 state 是否驱动渲染核对**：顶层组件若用 hook 维护交互态（activeView/currentDate/expandedSet），核查渲染分支读的是"hook 返回值"还是"schema resolved 值"。曾发现 calendar 渲染分支读 schema view 而非内部 activeView，切换完全失效；gantt 展开/收起 bump 的 revision 无任何生产组件订阅。
3. **controlled/uncontrolled 核对**：是否把"prop 已提供"等同于"外部受控"而无回推机制（曾发现 kanban 传 data 即进入 controlled、setBoardData 恒空操作，所有拖拽/增删失效）。三态 ownership（local/controlled/scope）是否在顶层落地，*StatePath/*Ownership 字段是否被消费。
4. **事件派发核对**：schema 声明的每个 event（onXxx）是否在对应交互点真的 dispatch（曾发现 onTaskDragEnd/onLinkDragEnd/onTaskClick/onEventCreate 等全部声明却从不派发，致拖拽无法持久化）。
5. **句柄/region 接线核对**：component:xxx reaction 是否接入 useImperativeHandle（曾发现 exportPNG/importICal 声明为 reaction 但 handle 未实现）；声明的 region（taskBar/columnHeader/cardTemplate 等）是否真的下传给子组件渲染（曾发现 board 从不把 regions 传给 column）。
6. **核心交互通断核对**（最高优先）：拖拽（源注册/目标注册/edge 检测/落位/视觉反馈）、键盘导航（焦点移动/箭头语义不冲突）、缩放/滚动（滚对元素、锚定）、打开/关闭编辑器/覆盖层——逐条确认"用户操作 → DOM 响应 → 状态更新 → 事件外抛"闭环。曾发现 scrollTo* 滚错元素（网格而非时间线）、ArrowLeft/Right 语义相同、video 在 phase=scanning 才挂载但流在 loading 阶段已尝试挂到 null ref（永久黑屏）。
7. **降级与错误反馈核对**：失败路径（相机不可用/权限拒绝/数据为空/WASM 加载失败）是否给用户可见反馈，而非静默黑屏/空白。
...
- **文件**: packages/xxx/src/yyy.tsx:行号
- **严重程度**: P0/P1/P2/P3
- **类别**: 接线/ownership/事件/句柄/region/交互闭环/降级
- **现状**: 一句话描述
- **用户可见症状**: 如"点切换无反应""拖拽后卡片弹回"
- **建议**: 修复方向

误报排除：
- 设计上明确为"只读展示 v1"的组件，拖拽不实现不算缺陷（但需 design.md 明确）。
- "悲观更新"（等数据源确认才更新 DOM）与"controlled no-op"（永不更新）是不同问题，后者是缺陷。
- 响应式订阅 key 不匹配（mutation 不 bump 其 listener 选中的 revision）本质属维度 05（响应式订阅精度）；本维度只在"订阅完全缺失导致交互失效"时报告，精细订阅范围交维度 05。
- needs-runtime 项（如 mid-drag 适配器销毁是否硬断裂）标注清楚，不要当确定结论。
```
...
```
以下为"维度 23"的维度正文。派发时必须与上文"共享提示词前缀"拼接。

审核维度 23：测试有效性与假绿

目标：识别"测试全绿却保护不了正确行为"的假绿模式。既有维度 14（测试覆盖与质量）看覆盖率与数量，本维度看"断言的是正确值还是缺陷值、是否 mock 掉被测边界、死代码是否带测试"。假绿比"没测试"更危险——它主动阻碍修复（修了 bug 反而让测试变红）。

必读文档：
- docs/skills/unit-test-logic-and-contract-coverage-audit-prompt.md
- docs/bugs/71-scheduling-deep-audit-blind-spot-display-operability-test-effectiveness.md（本维度起源）

执行步骤：

1. **"固化缺陷"断言识别**（最高危）：对每个断言实际计算值/渲染值的测试，核对：
   a. 断言值是否等于"当前实现值"而非"正确值"（曾发现 layout.test 断言同日任务 $w===4、calendar-layout-utils.test 标题写 full width 却断言 width===25、drag-create.test 断言 start===end）。
   b. 测试标题/描述与断言是否矛盾。
   c. 该断言若被"修正为正确值"，测试是否反而失败——若是，即为固化缺陷。
2. **集成边界 mock 识别**：核查顶层组件/集成测试是否 mock 掉被测对象本身：
   a. 测 X 组件却 mock 掉 X 的全部子组件与 hook，仅断言"壳渲染"（曾发现 gantt.test mock 全部子组件+4 hook、calendar.test mock useCalendarState 恒返固定值、barcode-overlay.test mock 两个核心 hook——使全部 P0 不可测）。
   b. 测 DnD 却 mock 掉 DnD 库后只验"回调被调"，不验"卡片实际落位"。
   c. store/纯函数测试用直接构造绕过顶层组件，掩盖"顶层漏传字段"（曾发现 zoom 测试全绿但 gantt.tsx 从不透传 zoomLevels）。
3. **同义反复/零断言识别**：`expect(true).toBe(true)`、仅 `not.toThrow()` 验"不崩"、调用函数后无任何关于结果的断言。
4. **死代码带测试识别**：被测模块在生产中无任何导入方（死代码），却带完整测试套件——形成"测了死代码"的假覆盖（曾发现 useKanbanAdder/useKanbanCollab/KanbanWipBadge/CalendarResourceGroup/CalendarResourceHeader/GanttCompact/ResourceLoadView 全是死代码带测试）。
5. **时区/环境敏感测试识别**：依赖本地时区的日期测试只在 CI(UTC) 下通过，未强制非 UTC 时区。
6. **覆盖率 vs 有效覆盖核对**：高覆盖率文件是否真的测了关键路径（曾发现 kanban-board.tsx 覆盖率 48%、column-header 27%、resource-load 组件 0%，而 DnD/视图切换/落位等核心路径无有效断言）。

输出格式（内联，与维度 05/09/14/20 一致）：

对每个发现：
### [维度23] 简短标题
- **文件**: packages/xxx/src/yyy.test.tsx:行号
- **严重程度**: P0/P1/P2/P3
- **类别**: 固化缺陷/边界 mock/同义反复/死代码测试/环境敏感/覆盖无效
- **现状**: 贴出矛盾断言原文（标题 vs expect）
- **建议**: 修复方向

误报排除：
- 合理的 mock（mock 外部依赖如 getUserMedia/html2canvas/pragmatic-dnd 底层）不算缺陷；缺陷是"mock 掉被测对象本身"。
- 纯工具函数的隔离测试是合理的；缺陷是"仅有隔离测试、无任何集成冒烟测试"。
- not.toThrow() 作为补充断言可接受；缺陷是"它作为唯一断言"。
- **与维度 14 §8 的边界**：维度 14 §8 已就"测试名 vs 断言""可见性通道"做的检查，本维度不重复报告；本维度独有价值在"固化缺陷断言（断言值=缺陷值）""死代码带测试""覆盖有效性（关键路径断言了什么）"这三类 14 不覆盖的深度。完全无测试的关键路径遗漏归维度 14（覆盖洞），不属本维度。
```
...
|            | 18 跨包模式一致性     | 收尾完善                                                                             |        |
|            | 20 可访问性 (WCAG)    | 交互质量保障                                                                         |        |
| **第六批** | 21 显示与定位正确性   | **条件**：审计对象含复杂交互渲染器（gantt/kanban/calendar/designer/scheduler）时必选 | 3 并行 |
| （条件）   | 22 集成接线与可操作性 | 同上                                                                                 |        |
|            | 23 测试有效性与假绿   | 同上                                                                                 |        |
...
```
阶段一（迭代深挖）—— 按 5~6 个批次顺序派发各维度第 1 轮初审，每批次内并行：
  ...
  批次 6（条件：审计对象含复杂交互渲染器 gantt/kanban/calendar/designer/scheduler 时）:
    Task(21-R1) + Task(22-R1) + Task(23-R1)
