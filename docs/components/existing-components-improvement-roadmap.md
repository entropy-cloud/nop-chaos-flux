...
> Last Updated: 2026-06-22 (E3 P2 closed — all sub-items done or adjudicated; condition-builder async metadata loading abandoned per 请求下沉原则)
...
- X1 doAction 命令族统一: `done`（`docs/plans/2026-06-21-2146-1-x1-doaction-command-family-unification-plan.md`，closed；successor 既有 runtime 复合编辑器子集已收口：`docs/plans/2026-06-22-1137-1-x1-successor-composite-editor-handles-plan.md`，array-editor/key-value addItem/removeItem/moveItem handle 落地；不存在组件归 main roadmap）
- X2 可阻止事件: `done`（`docs/plans/2026-06-21-2146-2-x2-preventable-events-plan.md`，completed）
- X4 data-source 请求层增强: `done`
- E3 P2 体验完善（按需启动）: `done`（全部子项已收口。condition-builder formula 子项 ✅ done（`docs/plans/2026-06-22-0149-1-e3-condition-builder-formula-completion-plan.md`）；布尔控件 trueValue-falseValue 子项 ✅ done（`docs/plans/2026-06-22-0149-2-e3-boolean-control-value-contract-plan.md`）；text+icon 视觉字段 ✅ done（`docs/plans/2026-06-22-0149-3-e3-basic-display-visual-fields-plan.md`，completed）；flex/page/tabs 布局族 ✅ done（`docs/plans/2026-06-22-0330-1-e3-layout-family-flex-page-tabs-plan.md`，completed）；input-number 长按步进 + array-editor/key-value min/max+reorder + component:addItem/removeItem/moveItem handle ✅ done（`docs/plans/2026-06-22-0330-2-e3-form-input-number-array-keyvalue-plan.md` + `docs/plans/2026-06-22-1137-1-x1-successor-composite-editor-handles-plan.md`，completed）；tree display 搜索/图标/引导线 ✅ done（`docs/plans/2026-06-22-0330-3-e3-tree-display-search-icons-plan.md`，completed）；dynamic-renderer autoLoad gate + component:refresh ✅ done（`docs/plans/2026-06-22-0528-1-e3-dynamic-renderer-autoload-gate-plan.md`，completed）；chart 视觉配置（area/legend/stacked/grid/colors）✅ done（`docs/plans/2026-06-22-0528-2-e3-chart-visual-configuration-plan.md`，completed）；select option-template region ✅ done（`docs/plans/2026-06-22-0740-1-e3-select-option-template-region-plan.md`，completed）；code-editor colorize 只读高亮 ✅ done（`docs/plans/2026-06-22-0740-2-e3-code-editor-colorize-plan.md`，completed）；input autocomplete (data-source suggestions, E2a deferred successor) ✅ done（`docs/plans/2026-06-22-0901-1-e3-input-autocomplete-data-source-suggestions-plan.md`，completed）；table popOver 单元格 (E1c deferred successor) ✅ done（`docs/plans/2026-06-22-0901-2-e3-table-popover-cell-plan.md`，completed）；text maxLine toggle + icon size token (E3 basic-display deferred residuals) ✅ done（`docs/plans/2026-06-22-1137-2-e3-text-maxline-toggle-icon-size-token-plan.md`，completed — `maxLineToggle?: boolean` toggle + 溢出检测 + a11y；`icon.size` 扩展为 `number | 'sm' | 'md' | 'lg'` token `{ sm:12, md:16, lg:20 }`）；condition-builder async metadata loading 子项 ⛔ abandoned（违反「请求必须下沉」原则，见 `docs/bugs/15b-component-level-initfetch-analysis-and-fix.md`）—— **无残留未实现项**）
...
所有子项已按 Phase Status 标注收口。详情见各子项的 closure plan。condition-builder async metadata loading 因违反「请求必须下沉」原则被废弃（`docs/bugs/15b-component-level-initfetch-analysis-and-fix.md`）。**本 roadmap 无待实现项残留。**
...
| 单测                | 每个改进配 focused 单测；契约漂移修复必须配验证字段生效的测试                                                    |
| **Playground 示例** | **每个工作项完成后，必须在 `apps/playground/src/` 下有对应可交互示例页面，注册到 playground 路由，演示新增能力** |
| **E2E 测试**        | **每个工作项完成后，必须在 `tests/e2e/` 下有对应 e2e 测试文件，覆盖新增能力的关键交互路径，确保回归安全**        |
