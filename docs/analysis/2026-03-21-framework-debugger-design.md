...
- `packages/flux-react/src/index.tsx` 已经会通过 `env.monitor` 发出 render 相关事件
- `packages/flux-runtime/src/action-runtime.ts` 已经会发出 action 和部分 api 相关事件
- `packages/flux-runtime/src/request-runtime.ts` 已经具备请求执行层的切入点
- `packages/flux-core/src/index.ts` 已经定义了 `RendererMonitor`、`RendererPlugin`、`ErrorMonitorPayload` 等契约
...
- `packages/flux-core/` 定义契约层
- `packages/flux-runtime/` 负责编译、动作、请求、页面、表单运行时
- `packages/flux-react/` 负责 React 集成和渲染边界
- `packages/flux-renderers-*` 是具体 renderer 实现
...
- “全局禁用”: 由 `window.__NOP_FLUX_DEBUGGER__` 决定
...
- ~~节点 inspect~~ → 已实现 data-cid DOM 属性 + inspectByCid API
- 节点级 render 统计
- 选中节点最近事件过滤
- ~~作用域快照查看~~ → 已实现，通过 inspectByCid 返回 scope 数据
- 编译结果摘要

### 12.4 暂不建议放入第一版

- ~~完整 DOM inspect 选择器~~ → 已通过 data-cid 属性实现轻量版（无 UI 选择器，但支持 document.querySelector('[data-cid]') 反查）
- 通过表达式执行器直接运行任意 JS
- 深度订阅内部 form/page store 私有状态
- 远程上传日志
- action replay

## 12.5 DOM cid 反查机制（2026-03-29 新增）

### 设计动机

原始设计中 DOM → 组件 → Store 的反查链路被列为"暂不建议"。但实际调试场景中，从 DOM 节点出发找到对应的组件状态是最高频的需求之一。本方案以最小侵入性实现此能力。
...
```ts
interface NopComponentInspectResult {
  cid: number;
  handleId?: string;
  handleName?: string;
  handleType?: string;
...
interface NopDebuggerAutomationApi {
  // ... existing methods
...
反查能力依赖于 ComponentHandleRegistry，该 registry 已在 SchemaRenderer 根级别创建，并通过 ComponentRegistryContext 向下传递。debugger controller 持有对 root ComponentRegistry 的引用，通过 controller.setComponentRegistry(registry) 注入。
...
建议 `@nop-chaos/nop-debugger` 暴露一套明确的 automation API：
...
- `__NOP_FLUX_DEBUGGER_API__` 指向当前活动 controller 的 automation API
- `__NOP_FLUX_DEBUGGER_HUB__` 用于多实例场景，按 `controllerId` 管理多个调试器实例
...
- playground 改为接入 `@nop-chaos/nop-debugger`
...
1. 新增独立 package `@nop-chaos/nop-debugger`。
2. 以 `SchemaRenderer` 宿主根边界作为唯一主接入点。
3. 通过 `window.__NOP_FLUX_DEBUGGER__` 作为全局启用开关。
