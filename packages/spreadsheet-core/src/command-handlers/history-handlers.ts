...
export const handleBeginTransaction: CommandHandler<BeginSpreadsheetTransactionCommand> = (store) => {
  const state = store.getState();
  store.setState({ transactionDoc: state.document });
  return { ok: true, changed: false };
};

export const handleCommitTransaction: CommandHandler<CommitSpreadsheetTransactionCommand> = (store) => {
  const state = store.getState();
...
export const handleRollbackTransaction: CommandHandler<RollbackSpreadsheetTransactionCommand> = (store) => {
  const state = store.getState();
  if (state.transactionDoc) {
