...
- timeline-v2. timeline 受控当前事件扩展: `done`（plan: `docs/plans/2026-08-04-2030-2-timeline-v2-controlled-current-event-plan.md`（closure audit 通过）；timeline 由展示型扩展为支持受控当前事件——value/defaultValue/valueOwnership(local|controlled|scope)/valueStatePath/onChange 五字段落地于 `flux-renderers-layout`（schemas.ts + process-display-definitions.ts + timeline-renderer.tsx），value 解析链与 steps 同构（key 匹配 → 数字 clamp），渲染层裁定未匹配→defaultValue→无 active（不回退首项，与 steps →0 兜底显式不同）；点击 seek 仅 onChange 声明时可点（data-clickable + tabindex + Enter/Space），controlled 只派发不 mutate、scope 写 valueStatePath；active marker `data-state="active"`；reverse 下 active 按逻辑顺序解析；focused 单测 10 条 + playground 演示 + e2e 5 条；未声明 v2 字段零行为回归）
...
- G1. graph 图查看器（1）: `done`（plan: `docs/plans/2026-08-04-2030-1-g1-graph-viewer-plan.md`（closure audit 通过）；`graph` renderer 落地于新包 `@nop-chaos/flux-renderers-graph`——只读交互式图查看器（@xyflow/react 复用 flow-designer 版本 + dagre 分层布局 + 单选模型 + 7 句柄 + 搜索 + 畸形数据硬契约），playground `#/graph-demo` 演示页 + e2e 8 条 + 单测 42 条；`examples.manifest.json` graph 已 `runtime`）
...
> 当前无 proposed 工作项（G1 已确认并完成，见顶部 Phase Status）。
...
| `graph`              | G1        | `flux-renderers-graph`         | `docs/components/graph/design.md`              |
...
| 请求下沉            | 新组件**不得声明挂载时自动加载数据的 schema 字段**（如 `initFetch`/`api`/`source` 等挂载触发语义）。数据请求必须通过 `data-source` + action graph 下沉到请求层。用户交互驱动的按需加载（如 tree `childrenSource` 展开加载、input `suggestSource` 输入建议）不在此限。违反示例与裁定见 `docs/bugs/15b-component-level-initfetch-analysis-and-fix.md` |
