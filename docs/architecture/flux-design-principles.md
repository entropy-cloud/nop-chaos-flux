...
**FormRuntime** 组合了 Action Algebra（分支调度）、Value（name / field path 读取和规则参数求值）、owner-local 值写入、validation owner 状态、Capability（提交动作和 async rule action），以及自身特有的子树契约、触摸策略、错误显示策略等逻辑。
...
3. Schema-authored command/effect 只通过 Capability；owner-local 值写入不是任意 action 触发。
