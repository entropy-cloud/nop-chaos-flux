...
**内容**: 将所有副作用集中到可拦截通道。注意各版本的机制差异：v2/v5 的 EffectDispatcher 是效应数据模型（可序列化纯数据）；v8 的 CapabilityDispatcher 是权限控制导向；v9 的 Effect Channel 是可测试/可审计导向的中间件链。
**价值**: 审计、Scope 销毁时自动取消、效应排序、可测试性。
**路径**: 扩展 `ActionRuntimeAdapter` 增加效应通道语义；内部表示，不改外部 API。

### 主题 5：Draft Scope / 表单草稿隔离
**出现在**: v2, v4（DraftSession 冲突检测）, v5（draftMode API）, v6, v7, v8（完整 DraftScope 语义）, v9, v10（Undo/Redo）, Final
**内容**: 可复用的 `DraftScope` 原语，带 `commit()` / `discard()`，替代渲染器层级的临时 FormRuntime。v4 的 DraftCommitOutcome 四结果（committed/conflicted/rejected/replay-required）和 v8 的独立校验状态设计最为完备。
**价值**: 更干净的复合字段隔离；Undo/Redo 基础。
**路径**: 校验计划 Phase 3。从渲染器模式提取到共享基底。
...
| 10 | EffectChannel 形式化（可序列化纯数据） | 主题 4 | v2, v5, v9 |
...
| # | 改进项 | 主题 | 来源 |
|---|-------|------|------|
| 13 | DraftScope 原语（commit/discard + 冲突检测） | 主题 5 | v2, v4, v8 |
