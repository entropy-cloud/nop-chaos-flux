...
- `2026-08-05-2129-multi-audit-industrial-hmi.md` `[P2-12]`（dim 23）— codemod test（`expression-codemod.test.ts:21-37`）内联了生产 codemod patterns 的**子集**（缺 `QUOTED_DOLLAR_PATTERN`），验的是等价副本非生产函数。建议改为 `import { migrateContent } from '../../../../../scripts/scada-expression-codemod.mjs'`。
- `2026-08-05-2129-multi-audit-industrial-hmi.md` `[P2-13]`（dim 23）— `binding-expression-unification.test.ts:459-473` 标题声称验 `?? ''` 兜底（point `blank`），但断言的是无关点 `getPointValue('a')===1`，`blank` 从未被断言 → false-green。建议补 `expect(pointStore.getPointValue('blank')).toBe('${}')`。
- `2026-08-05-2129-multi-audit-industrial-hmi.md` `[P2-14]`（dim 23）— `scada-canvas-smoke.test.tsx:35-52` 标题声称 "scene build" 但仅验 `handles.length>0`（等价 not.toThrow），未断言图元入树/`data-status==='ready'`。建议补 `getSymbols()` 含 rect-1 或 `data-status` 断言。
- `2026-08-05-2129-multi-audit-industrial-hmi.md` `[P2-15]`（dim 14）— `variables-optional-contract.test.ts:235-248` 空 config 测试唯一断言 `not.toThrow()`，未验 flushFrame 返值或 applyAttrs 未被调。建议补 `expect(pipeline.flushFrame(...)).toBe(false)` + `vi.fn()` 断言 applyAttrs 未调。
