...
| U8  | `engine.applyDiff` 半途抛错致画布与 working copy 永久背离     | **防护**：`applyUndoRedoDiff` catch 经 `engine.build(beforeWorking)` 全量重建回滚引擎场景（leafer 无事务，全量重建是唯一可靠回滚）+ 同步 `synced.config=beforeWorking` 闭合「steady state 下 diff 为空 → 引擎永不愈合」的永久背离路径；entry 未 commit 保留在原栈，`editor-internal-error` 派发 |
