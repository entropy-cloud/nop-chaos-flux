...
落到工程里，仓库在当前约束下所有可能长成的代码、文档、测试组合，就是状态空间；人、AI、review、CI、文档更新这些持续作用，就是演化规则；它们叠加出来的 live repo 历史，就是轨迹；attractor 则是这套系统在长期迭代中会被反复拉回的稳定结构。
...
*Lorenz attractor：局部轨迹高度复杂，整体仍被稳定结构约束。混沌不等于随机；局部不可预测，不等于整体失控。*
