...
- Flux 不是扩展性的主要发生地
- Flux 不是把设计器平台能力上提为 core primitive 的那一层
...
Flow Designer、Report Designer、Word Designer 从 Flux core primitive 的视角看，不应被视为需要单独发明第二套核心本体的平台实体。
...
- 该控件可以拥有自己的 shell
- 该控件可以消耗特定数据模型和行为模型
- 其静态执行配置通过 schema 注入，动态运行时状态通过受约束的 host projection / capability boundary 接入

因此：

- 设计器不是 Flux core primitive taxonomy 中需要额外升格的一类本体
- 设计器不需要一套厚重的、独立于组件系统之外的第二运行时协议
...
因此，旧文里的“三棵树分离”可以继续作为工程直觉，但不应再被当作 Flux primitive ontology 的完整定义。
更高优先级的规范见 `docs/architecture/frontend-programming-model.md`。
...
设计器不单独定义一套脱离统一组件模型的特殊平台协议，而是落在统一的组件模型上，并在 platform-extension architecture 中通过受约束的 host boundary 获得扩展能力。
...
### 5.3 为什么不设计厚重的“设计器平台协议”

因为一旦给设计器单独设计厚重且脱离统一组件模型的平台协议，就会自然走向：
...
- 允许存在极薄的 shared host protocol，用于 snapshot subscription、command dispatch、session summaries、namespace wiring 这类宿主接线问题
- 但这类协议属于 host architecture，不是 Flux core primitive，也不是设计器平台化的许可
...
- 二者作为静态执行配置可以沿不同输入边界进入最终执行 schema
...
但更精确的 primitive closure 仍以 `docs/architecture/frontend-programming-model.md` 的七类 primitive 为准：
...
设计器或复杂控件的静态执行输入与动态运行时数据必须分开理解。

- 静态执行输入可以通过 ordinary scope data、schema props、或静态配置字段进入最终执行 schema
- 动态 workbench/session/document snapshot 必须通过 host projection 进入系统，而不是被当成普通可写 scope 数据
...
- 复杂控件自己的静态执行数据模型可以映射到当前 scope 或专用子 scope
- 不要求 runtime 看见“designer model”这一类特殊对象
- 允许数据模型在不同 region/子树中以不同投影暴露

推荐规则：

- 若复杂控件需要稳定宿主运行时视图，应通过 host projection 实现
- 若复杂控件需要普通执行期静态输入，应通过 ordinary scope data 或 schema props 实现
- 若复杂控件需要局部高频状态，应保持在控件内部状态或其局部 store 中
- 只把确实需要被 schema 表达式消费的数据投影进 scope

### 6.4 行为模型注入接口

行为模型统一通过 capability boundary 注入，当前主路径是 built-in actions、`ActionScope` namespaced action，以及 `ComponentHandleRegistry` 实例能力调用。
...
- `behavior` 只表示该控件需要的静态 capability wiring / import / namespace 配置描述，不是额外的动态行为通道
- `shell` 只表示壳层布局或专用外观配置
- `body/toolbar/inspector/dialogs` 仍然是普通 schema region
- 这些字段属于最终执行 schema 中的静态配置输入，不是额外的动态宿主通道
- 运行期动态 workbench/session/doc state 仍然必须通过 host projection 与 capability boundary 进入系统，而不是塞回 `model` / `behavior` / `shell`
