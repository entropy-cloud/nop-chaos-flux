...
- **S10. Playground 测试页面** (`partial — S10.1–S10.4 done, S10.5 done`)
...
| S3.9 | done   | **多选 + 批量操作**：`selectionMode: 'multiple'`，Shift+Click 范围选择，批量修改任务属性/拖动                                            | `design-multi-select-batch.md`                 | S2.6       |
...
| S9.6 | done   | **大文件虚拟滚动**：`diff-virtual-list.tsx`——`virtualizationThreshold: 500`，超过启用 `FixedSizeList`（行高 24px），`dangerouslySetInnerHTML` 模板渲染           | `design.md §12` | S9.4 |
