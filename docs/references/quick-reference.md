...
- **Component Handles**（合并注册 18 方法，plan 2026-08-07-1835-2 Phase 2 / P1-06）：**runtime 9** `fit`/`center`/`getSymbols`/`getSymbol`/`setPointValue`（编辑态 not-supported）/`getPointTable`（空）/`exportConfig`/`importConfig`/`destroy`（置 `data-status="destroyed"`）+ **编辑扩展 9** `save`/`load`/`addSymbol`/`removeSymbol`/`updateSymbol`/`group`/`ungroup`/`undo`/`redo`（均入 undo 栈，不派发 `symbol:*`）。失败路径：`invalid-config`/`invalid-node`/`symbol-not-found`/`editor-mount-failed`/`not-visible`/`not-a-group`/`destroyed`。
