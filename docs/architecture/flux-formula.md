...
4. **`|` 的主语义仍是位运算** — `${5 | 3}` 这类表达式必须按位或求值；仅对兼容层识别出的 amis 风格 `${x | filter:arg}` filter-pipe 语法做预处理重写
...
- 预处理层仍支持 amis 风格的 filter-pipe 兼容输入，例如 `${name | wrap:"[":"]"}`
- 该兼容重写只在顶层 `|` 链被识别成合法 filter 调用段时触发，并按左结合顺序重写为普通函数调用
- 如果右侧不满足 filter 调用段形状，则保留原始表达式，让 parser 按位运算等原生语义处理
