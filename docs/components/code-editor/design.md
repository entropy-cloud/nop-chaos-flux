...
| 只读代码高亮显示（colorize）                                                                               | **实现**（E3） | colorize 是独立只读渲染范式：触发条件为 schema 新增 `colorize?: boolean`（独立标志，与 `readOnly`/`diffValue` 正交——`readOnly` 仍实例化完整 EditorView，colorize 短路 EditorView 创建，渲染 `@lezer/highlight` `highlightTree` + `HighlightStyle` 静态 DOM）。何时用 colorize vs `readOnly` editor：**纯展示、无光标/选择/折叠/补全诉求** 用 colorize（更轻量、不实例化 editor）；**需要光标定位、文本选择、折叠、component:getEditorView 句柄** 用 `readOnly:true` editor。language→Lezer parser 映射走 `getLanguageParser(language): Parser | null`（与 `createLanguageExtension`12 语言一一对齐，SQL 走`StandardSQL.language.parser`），plaintext/未知语言 → null → 纯文本降级。DOM marker `data-colorize`。E2h 显式 deferred，由 `docs/plans/2026-06-22-0740-2-e3-code-editor-colorize-plan.md` 收口 |
...
- `colorize?: boolean`：只读静态语法高亮开关。`colorize:true` 且 `diffValue` 未设置时，渲染器**短路 EditorView/MergeView 创建**，改为用 `@lezer/highlight` `highlightTree` + `@codemirror/language` `HighlightStyle` 把 `value` 渲染为带高亮 token 的静态 `<pre>`（根节点带 `data-colorize` marker）。与 `readOnly`/`mode` 正交：`readOnly` 只把 editor 切到只读态但仍实例化完整 EditorView，`colorize` 则完全不实例化 editor。

**colorize vs `readOnly` editor 选择指引**：

| 场景                                                       | 推荐                           | 理由                                              |
| ---------------------------------------------------------- | ------------------------------ | ------------------------------------------------- |
| 纯展示代码片段（无交互诉求）                               | `colorize:true`                | 不实例化 EditorView，更轻量；静态 DOM 一次性渲染  |
| 需要光标定位 / 文本选择 / 折叠 / 行号                      | `readOnly:true`（完整 editor） | 这些能力依赖 EditorView 内核                      |
| 需要 `component:getEditorView` 句柄或 `onEditorMount` 事件 | `readOnly:true`（完整 editor） | colorize 不发布 EditorView 句柄（无 editor 实例） |
| diff 模式（`diffValue` 有值）                              | MergeView（`diffValue` 路径）  | colorize 不与 diff 组合（见 Out Of Scope）        |
...
- colorize 只读静态语法高亮（E3 新增）：`colorize:true` 短路 EditorView/MergeView 创建，用 `@lezer/highlight` `highlightTree` + `@codemirror/language` `HighlightStyle` 渲染静态高亮 `<pre>`（DOM marker `data-colorize`）；language→parser 映射走 `getLanguageParser`，与 12 语言一一对齐（SQL 走 `StandardSQL.language.parser`），plaintext/未知语言 → 纯文本降级；light/dark 与既有 `editorTheme` 对齐（light 走 `defaultHighlightStyle`，dark 走 `oneDarkHighlightStyle`）
...
- colorize 只读代码高亮（E3 已落地：`colorize:true` 短路 EditorView，走 `highlightTree` 静态高亮）
