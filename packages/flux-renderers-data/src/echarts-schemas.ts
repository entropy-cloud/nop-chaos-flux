...
export interface EChartsMapSchema extends SchemaObject {
  /** 注册到 echarts 的地图名；`option.series[].map` 由作者声明同名引用。 */
  name: string;
  /** GeoJSON 数据（scope 表达式绑定或已解析对象）——裁决 A2 内置加载机制例外。 */
  geoJson: SchemaValue;
}
...
export interface EChartsSchema extends BaseSchema {
  ...
  /** map 类型的 GeoJSON 注册桥接（仅注册地图数据，不生成 series）。 */
  map?: EChartsMapSchema;
}
