...
- [x] (Fix) 产出 `docs/components/stat-tile/design.md` + `example.json`（含同比/环比 + sparkline 示例）。
- [x] (Fix) 更新 `docs/analysis/2026-08-09-bi-control-support-analysis.md` 状态标注（stat-tile landed）。

Exit Criteria:

- [x] `stat-tile` 单测全绿（KPI 格式化/涨跌色/null 占位/sparkline 空数据/表达式求值）。
- [x] `docs/components/stat-tile/design.md` + `example.json` 已存在且与 live renderer 行为一致。
...
- [x] (Decision) 裁定预设模型：`presets?: Array<{ label: string; value: { start: string; end: string } | { relative: 'today' | 'yesterday' | 'last7days' | 'last30days' | 'thisMonth' | 'lastMonth' } }>`，相对档位运行时解析为绝对区间写入值。
- [x] (Proof) 先写 failing 单测：相对档位解析（last7days 边界、跨月/跨年）、预设选中后 value 输出为绝对 ISO 区间、非法预设回退无预设。
- [x] (Fix) 实现预设 UI（预设 chip 行）+ 解析逻辑，保持与现有 `date-range` 值协议兼容。
- [x] (Fix) 更新 `docs/components/date-range/design.md`（预设 schema + 示例）。

Exit Criteria:

- [x] 相对预设解析单测全绿（含跨月/跨年边界）。
- [x] 预设选择产物与现有 `date-range` value 协议兼容（focused 单测断言）。
...
- [x] (Decision) 裁定筛选模型：复用 `crud` queryForm 的"表单 → summary"模式，但筛选值写入**共享 scope**（约定 key 前缀如 `filter.*`），消费端（chart/table 的 `source` 表达式、data-source）经 `${filter.xxx}` 读取；提交动作显式触发消费端 data-source `refresh`。
- [x] (Proof) 走查单测：共享 scope 写入 → `data-source` 因 `useRenderScope` 订阅自动重载（不依赖手动 refresh 也可联动）；key 失配时 dev warn（Failure Path dashboard-filter-no-link）。
- [x] (Fix) 产出 `docs/components/dashboard-filter/design.md`：约定（scope key 命名、提交/重置语义、与 crud 的复用边界）+ 最小可运行 `example.json`（2 个筛选 + 1 chart + 1 table 联动）。
- [x] (Fix) 在 playground 增加 `dashboard-filter` 示例页（并入 `apps/playground/src/data-route-entries.ts`；若后续出现 BI/仪表盘示例目录则优先放入），验证人工可操作。
- [x] (Follow-up) 记录 `panel-chrome` 组合基线（card regions + 刷新按钮 + filter 联动）到 design.md，作为后续独立控件的裁决依据。

Exit Criteria:

- [x] scope 联动走查单测全绿（写入 scope → 消费端重载；key 失配 dev warn）。
- [x] `docs/components/dashboard-filter/design.md` + `example.json` 存在；playground 示例页可运行。
...
- [x] (Decision) 裁定 card 是否增加约定字段（如 `refreshAction?: ActionSchema` 渲染 header 刷新按钮，dispatch 到 data-source `refresh` handle）vs 纯组合（header region 放 Button + onClick）——以示例 schema 体积与复用性为判据。
- [x] (Proof) 若采纳字段：failing 单测（refresh 按钮存在且点击 dispatch 正确 action；无 data-source 时按钮禁用或隐藏）。
- [x] (Fix) 按裁定落地（字段或示例）；产出"图表卡片 + 刷新 + 筛选联动"完整示例并入 dashboard-filter example。
- [x] (Fix) 更新 card / dashboard-filter 文档。

Exit Criteria:

- [x] 裁定记录 + 示例落地（组合示例可运行，含刷新触发 data-source 重载）。
- [x] 若新增 card 字段：对应单测全绿。
...
- [x] (Proof) 先写 failing 单测：双轴 series 按 `yAxisId` 归轴、无 `yAxisId` 回退默认轴；brush 选区在数据更新后按索引保留（in-place，延续 DD2 契约）；heatmap 空/畸形数据降级（延续 DD1 空态硬契约）。
- [x] (Fix) 实现 chart-schemas.ts 扩展 + chart-renderer.tsx 渲染路径。
- [x] (Fix) 更新 `docs/components/chart/design.md`（新字段 + 决策表增行 + 示例）。
- [x] (Follow-up) 记录后续候选：`map`（独立评估 geojson 资源管理）、tooltip/legend 自定义 slot。

Exit Criteria:

- [x] 双轴/brush/heatmap 单测全绿（含回退与降级路径）。
- [x] `docs/components/chart/design.md` 与 live schema 一致（字段表 + 示例更新）。
...
- [x] `stat-tile`、`date-range` 预设、chart 增强三项 focused 单测全绿（无被静默降级的 in-scope 行为）。
- [x] `dashboard-filter` 编排约定落地（文档 + 示例 + scope 联动走查通过）。
- [x] card 刷新约定裁定记录与示例落地。
- [x] `docs/components/stat-tile/design.md`、`docs/components/dashboard-filter/design.md`、`docs/components/chart/design.md`、`docs/components/date-range/design.md` 与 live baseline 一致。
- [x] 不存在被静默降级到 deferred / follow-up 的 in-scope 行为缺口。
- [x] 受影响的 owner docs 已同步到 live baseline。
- [x] `docs/logs/` 已按 AGENTS.md 写入收口记录（含 full-green 验证状态，如有）。
- [x] 由独立子 agent（fresh session）执行的 closure-audit 已完成并记录证据；执行 session 不得自审勾选本项。
- [x] `pnpm typecheck`
- [x] `pnpm build`
- [x] `pnpm lint`
- [x] `pnpm test`
- [x] `pnpm check`（零新增未登记 red）
...
Status Note: 已执行完成（2026-08-09）——5 Phase 全 completed + Closure Gates 13/13 [x] + Plan Status → `completed`。

Closure Audit Evidence:

- Auditor / Agent: 独立 fresh-session closure-audit sub-agent（执行 session 不自审）
- Evidence:
  - 首轮 `ses_018012121ffelsb2uD5Es5UWTe` verdict `revise`（1 Blocker：`card-refresh-convention.test.tsx` 内联 card 定义 TS2322——修复（region render 显式 `React.ReactNode` cast）后 `pnpm typecheck` 33/33 复绿；1 Major：docs/logs 收口记录缺失——已写入 `docs/logs/2026/08-09.md` 顶部收口条目，覆盖 5 Phase + 测试计数 + full-green 数字 + 审计轨迹）。
  - Re-audit `ses_017f9abaeffehFEpNdwi2KQUnW` verdict `pass`（两项 finding 均确认 resolved；`pnpm --filter @nop-chaos/flux-renderers-data typecheck` exit 0、workspace `pnpm typecheck` 33/33、card-refresh 测试 2/2 绿、docs/logs 收口条目在案、git status 无杂散文件）。
  - 执行验证：`pnpm typecheck`/`build`/`lint` 33/33、`pnpm test` 60/60 tasks 全绿、`pnpm check` exit 1 仅 = 既有登记 red（`check:audit-event-dispatch-ctx` industrial 6 hits 已登记；standalone `check:ai-engine-invariants` = 注册命中 ⑧×1+⑥×3；oversized 仅 2 exempt locale；docs-garbled 零新增）——零新增未登记 red。

Follow-up:

- 由 closure-audit 通过后的 finalization 轮执行 commit（AGENTS.md：NEVER commit unless explicitly asked；mission driver 下由收口轮决定）。
