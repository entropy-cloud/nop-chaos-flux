...
> 更新于 2026-04-04，基于 `packages/report-designer-renderers/src/` 和 `packages/report-designer-core/src/` 实际代码

本节明确区分"当前代码已落地"和"设计目标（尚未落地）"，避免后续工作建立在错误基线上。

### 14.1 已落地

- `@nop-chaos/report-designer-core` 存在，含 `createReportDesignerCore`、`ReportDesignerSnapshot`、`core.dispatch`
- `@nop-chaos/report-designer-renderers` 存在，`ReportDesignerPageRenderer` 已实现 schema-driven 的 `toolbar`、`fieldPanel`、`inspector`、`dialogs` region（由 plan 32 落地）
- `report-designer:*` namespace 已注册（`page-renderer.tsx:73-79`）
- `report-designer:dropFieldToTarget`、`report-designer:updateMeta`、`report-designer:replaceMeta`、`report-designer:openInspector`、`report-designer:closeInspector`、`report-designer:preview`、`report-designer:importTemplate`、`report-designer:exportTemplate` 命令已在 core 定义（`report-designer-core/src/commands.ts`）
- `ReportDesignerBridge extends SpreadsheetBridge` 继承关系已定义（`report-designer-renderers/src/bridge.ts`）

### 14.2 尚未落地

**默认 canvas 仍是 fallback：**

`page-renderer.tsx:132` 当 `body` region 未提供时回退到 `renderFallbackCanvas(snapshot)`，不渲染真实 spreadsheet host。第 2 节架构图中"canvas region → spreadsheet renderer"路径当前不存在于默认渲染路径。

**toolbar 与 core 命令面未对齐：**

`report-designer-toolbar-defaults.ts` 暴露了 `report-designer:undo`、`report-designer:redo`、`report-designer:save`、`report-designer:stopPreview`，但 `report-designer-core/src/commands.ts` 没有这四个命令的定义。当前状态为"默认 toolbar 按钮存在，但 dispatch 时 core 会报未知命令"。

**host scope 未注入（与 Flow Designer 不同）：**

当前 region 渲染调用形如 `props.regions.toolbar?.render({ data: hostData })`，传入的是 `data` 对象而非 child scope。Flow Designer 使用 `render({ scope: designerScope, actionScope })` 将快照字段投影进 child scope；Report Designer 还没有采用这种模式。因此 region 内 schema 表达式**无法**通过 `${activeCell.address}` 等方式直接读取 designer 状态。

**session/dirty/leave-guard 未接入共享协议：**

`ReportDesignerBridge` 虽然有 `dirty` 字段，但 page-renderer 没有把它接入共享的 `WorkbenchSessionState` 或 leave-guard 机制。

### 14.3 Phase 3 完成状态（2026-04-04）

所有四项目标均已落地并通过验证（typecheck ✅ build ✅ lint ✅ test 116/116 ✅）：

1. **canvas 路径** — `page-renderer.tsx` 默认 body 现在挂载 `<ReportSpreadsheetCanvas core={core} snapshot={snapshot} />`，不再回退到 `renderFallbackCanvas`。新文件 `report-spreadsheet-canvas.tsx` 在内部组合 `createSpreadsheetCore` + `createSpreadsheetBridge` + `createReportDesignerBridge` + `useSpreadsheetInteractions` + `SpreadsheetGrid` + `SheetTabBar`。

2. **core 命令面对齐** — `report-designer-core/src/commands.ts` 已补齐 `StopPreviewCommand`、`UndoCommand`、`RedoCommand`、`SaveCommand`；`core.ts` 实现了 `undoStack`/`redoStack` 和 `pushUndoEntry`；`ReportDesignerRuntimeSnapshot` 新增 `canUndo`/`canRedo`。

3. **child scope 注入** — 所有 region 的 `render()` 调用已从 `{ data: hostData }` 切换到 `{ scope: reportDesignerScope, actionScope }`。`host-data.ts` 新增 `buildReportDesignerScopeData` 和 `useReportDesignerHostScope`，与 Flow Designer 的 `useDesignerHostScope` 模式对齐。

4. **host scope 数据派生** — `buildReportDesignerScopeData` 从 `core` + `snapshot` 派生完整 scope record，包含 `designer`、`runtime`、`fieldSources`、`fieldDrag`、`meta`、`preview`、`inspectorPanels`、`selectionTarget`、`reportDocument`、`workbook`、`activeSheet`、`canUndo`、`canRedo`、`documentName`、`fieldCount` 等字段，schema 表达式可直接通过 `${activeCell.address}` 等路径读取。
