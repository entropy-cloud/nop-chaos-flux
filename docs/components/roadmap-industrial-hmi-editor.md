...
- **post-remediation plan {2} 契约接线/结构/UI/测试/性能收口（2026-08-08，✅ 5 Phase 全交付 + workspace full-green）**：plan `2026-08-07-1835-2` Phase 2–5 执行完成（Phase 1 oversized 拆分由前序完成）。11 条 P1 全部收口——**P1-04** save/load dispatch（runtime-mutators save/load 经 onSave/onLoad 派发 scada-editor:save/load + canvas 透传）；**P1-05** commitPolicy auto-save（方案 A：notifySession 在 commitPolicy='auto' 且非事务期触发 save+onSave）；**P1-06** runtime 9 句柄合并注册（fit/center/getSymbols/getSymbol/setPointValue/getPointTable/exportConfig/importConfig/destroy 全部从 editor instance 可达，destroy 置 data-status=destroyed）；**P1-08** mode 同步（mountEditorEngine + load 后校正 engine.mode→session.mode）；**P1-09** controlled pushback（方案 A：useEffect watcher 监听 config/mode prop 变化→load/switchMode）；**P1-B** delete/group/ungroup 默认 toolbox 按钮 + canvas 键盘层（Delete/Ctrl+Z/Y/Ctrl+G/Ctrl+Shift+G/arrows）；**P1-10** raw `<textarea>`→`@nop-chaos/ui` `<Textarea>`；**P1-11** palette drop 落指针处（getBoundingClientRect+getWorldPoint）；**P1-12** false-green 修复（programmaticSelect 补 expect + onReady/onError 注入 dispatch spy 断言 mock.calls + move 几何断言 + group/ungroup 真实 mutation + 反向验证记录）；**P1-13** recomputeLinkagesForMovedNode O(n+k)（Map<id,node> O(1) lookup + collectSymbolBounds 复用跨 junction + applyPatch 直接写预解析 node ref）+ trailing syncWorkingCopy（microtask 批处理去掉 s 乘子）。workspace full-green：typecheck/build/lint 32/32 + test 59/59（industrial 95 files / 1254 tests，coverage ≥90% branches）+ check:oversized-code-files industrial editor 0 ERROR。两份源审计 Audit Status → `closed`。owner docs（design-renderer §8 / flux-guide design-patterns/scada-editor.md / quick-reference.md）同步到 live baseline。Closure-audit gate（独立 fresh session）按 AGENTS.md 待触发。
...
### E11 — 逐组件审计（component-audit mission 交叉审计）

> 2026-08-08 立项（component-audit mission C10 work item，仿照 component-audit 18 维 checklist 对 `scada-editor-canvas` 进行逐组件审计）。审计卡：`docs/audits/per-component/scada-editor-canvas.md`（状态 fixed-pending-closure）。

| ID    | 内容                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | 依赖  |
| ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- |
| E11.1 | **scada-editor-canvas 18 维审计 + 自动修复**：① **P1-1 schema 类型漂移**——`ScadaEditorCanvasSchema` 缺 `loading`/`empty`/`error` region 声明（`editor/schemas.ts` 消费 `props.regions.*` 但 interface 未声明）→ 补齐 `SchemaInput` 声明；② P2-1 静态元数据缺失——editor definition 缺 `propContracts`/`eventContracts`（runtime 定义有，editor 无）→ 补齐 7 propContracts + 7 eventContracts 含 payload shape；③ P2-2 a11y——canvas 交互面缺 `role`/`aria-label` → 加 `role="application"` + `aria-label`；④ P2-3 disabled meta 未响应 → 加 `aria-disabled` + `inert` + drop/keyboard guard；⑤ **P2-4（shared）error code 语义混淆**——`invalid-config`（命令句柄失败不升级）vs `config-invalid`（校验失败升级），editor parseAndValidateConfig 误用 → 统一为 `config-invalid`（CX-9 共性重构）；⑥ P3-1 palette title 用 def.type → 改为 def.name。验证：typecheck/build/lint/test 全绿（1302 tests）                                                                                                                                                                                                                               | E10.2 |
| E11.2 | **Bug & Lesson 总结**（component-audit 审计发现归档）：**Bug-A**（P1-1，已修复）——`editor/schemas.ts` ScadaEditorCanvasSchema 缺 loading/empty/error region 声明，consumed-but-undeclared 类型漂移；**Bug-B**（P2-4/shared，已修复）——error code `invalid-config` vs `config-invalid` 语义混淆，parseAndValidateConfig 误用命令句柄码用于校验升级路径，host 监控收到不一致 error code；**Bug-C**（P2-3，已修复）——`props.meta.disabled` 未响应，host 设 disabled 时编辑器仍可操作（drop/keyboard 全可用）；**Lesson-A**——schema interface 声明必须与 renderer-definitions fields 同步（region 注册但 TypeScript interface 未声明 = 编译期类型漂移，框架运行时从 regions 通道提供但类型不安全）；**Lesson-B**——error code 设计必须区分"升级码"（→ canvas error status + onError event）与"不升级码"（→ 诊断通道/command handle failure），两个码集不可混用；**Lesson-C**——canvas 交互面 a11y 需在 wrapper 声明 `role="application"`（与 runtime scada-canvas 同型，I19.1 Lesson-A 一致）；**Lesson-D**——`props.meta.disabled` 是四态契约的一部分，instance-renderer 必须消费（与 CX-8 复合字段族 readOnly/disabled 传播同型模式） | E11.1 |
