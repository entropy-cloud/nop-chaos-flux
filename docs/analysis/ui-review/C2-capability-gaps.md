...
### 回写 ⑮ — D1 G-D 网格编辑深度语义件族产品化终态（2026-08-31，plan `2026-08-31-0721-2-d1-gd-grid-editing-semantic-components.md`）

> 授权链: roadmap Cross-Cutting 5（追加式回写，初版裁决表零改动）+ D1 work item 本体（G-D 终态由本 plan Phase 4 承载）。HEAD 见当日 dev log。执行顺序约束履行：本 plan（N=2）于 G-C plan（N=1）收口后执行，共写面 `renderer-interfaces.md`/flux-guide/C2 回写追加区冲突消除（⑭ 先落、本回写后落 ⑮）。

**G-D（网格编辑深度：单元格编辑器矩阵/列菜单/分组/行高，L2——级别锚定以初版裁决表 L2 为准）已产品化（本 plan）——终态证据**:

- **分组/聚合渲染语义落地（回写 ⑦ 候选 1 并入项消解）**：`table` 级 `group: TableGroupConfig`（`field` 声明即启用 + `aggregates`（sum/avg/min/max/count + label）+ `missingLabel`）——排序/过滤后行集上的 client 全量分组，组序按数据首现序；组头行 `data-slot="table-group-header"`（折叠 chevron `aria-expanded` + 组名 + 成员数 + 聚合文本 `{label ?? fn}: {value}` `·` 连接，kanban 列头聚合同构）+ 成员行 `data-row-group` 标记；聚合按组内全量成员求值；缺字段行归 `missingLabel ?? '-'` 兜底组 + 一次性 dev warn（gd-group-missing-field）行不丢；折叠集按组键记忆跨数据刷新保持。兼容矩阵逐项锁定：无 group 逐字节等价 / tree 优先惰性（gd-group-tree-clash）/ drag 优先抑制（gd-group-drag-clash）/ 组头不可选不渲染 checkbox / `selectAllMode:'page'` 页全选作用于当页组员行集 / 分页对「组头+组员」交错显示序列切片（组头计入页槽位，页首组头可零组员渲染；totalPages 按序列长度计，分页条 totalRows 维持行数口径）。**「table renderer 无内建分组/聚合渲染语义维持为 G-D 子能力缺口」就此消解**；复刻页 mock 预聚合姿势退役为 retrofit 候选（Follow-up 登记）。
- **单元格原位编辑双态 + 型别分派编辑器矩阵落地（回写 ⑦「零承载面」缺口消解）**：列级 `editable?: boolean | TableCellEditableConfig`（`editor` 五型 text/number/select/date/checkbox 映射既有 input 族控件，ui 包零新增导出——ask-first 门禁未触发）与既有 `quickEdit` 通道**共存分层**（Phase 1 三候选裁定：共存分层胜出，演进 quickEdit 本体否决——常驻控件语义与双态语义互斥原地改写将翻转既有 schema 可见行为；替换否决——quickEdit 非死配置无废弃依据）。双态状态机：导航态（普通格同款显示文本 + `tabIndex=0`）→ click/Enter/F2 进入编辑态（编辑器挂载自动聚焦）→ Enter/失焦(值变化)提交、Esc 取消零写入零派发（gd-cell-edit-cancel）、required/number 校验失败拦截提交编辑态保持（gd-cell-edit-invalid）；checkbox 编辑器 toggle 即提交两态折叠。写入通道 CX-10 合规：`quickSaveItemAction ?? quickSaveAction` 在场 → 字段覆写 draft scope 派发（quickEdit 控制器保存语义同构复用：`createDraftScopeStore` 共享、save-generation 守卫、record 快照防换行污染、显式 `ok:false` 失败检查、成功 `rowScope.merge`；失败 notify + 草稿编辑态保持 gd-cell-edit-save-fail）；不在场 → `rowScope.update` 纯客户端写通道。共存矩阵：editable 声明优先 quickEdit 控件不渲染（gd-cell-edit-quickedit-coexist 一次性 warn 不双控件叠渲染）/ 无编辑器回退只读（gd-cell-edit-no-editor）/ 无列 name 只读（gd-cell-edit-no-name）/ 无声明逐字节等价；可编辑格 click/keydown stopPropagation 与行级手势隔离。**回写 ⑦「单元格原位编辑与同格双态零通道」就此消解**（基点口径修正：quickEdit 通道在库、深度缺口在三——同格双态/型别矩阵/键盘进入——本 plan 逐项落地）。
- **键盘面落地（P6b 十五键位终态表的 G-D 义务）**：格级键位 Enter/F2 进入、Enter 提交、Esc 取消、blur 提交全数落地（先红后绿键位矩阵锁定）；**方向键漫游显式 deferred**（需格级坐标模型 + 虚拟滚动同步 + 行级焦点仲裁，无 consuming 复刻页——十五键位表登记的是缺口证据而非消费证据；successor: D1 输入池/本语义件族后续轮次）。
- **三维度裁定终态（G-B2 Decision 4 successor 义务 + G-B2 Decision 5① 边界登记 + 回写 ⑦ 候选 2，全部显式落字无静默）**：①「fill-handle 编辑器选区模型」= Deferred But Adjudicated（`optimization candidate`；新理由 = **无消费证据**——「依赖未就位」旧理由随 G-B2+G-B3 双前置 done 失效；P6b A 系列 fill-handle 零覆盖维持，零 consuming 复刻页时点实现属投机基础设施；Successor Required: yes——D1 输入池，出现消费诉求时在本 plan 双态状态机与编辑器矩阵之上叠加）；②「共享 roving helper 抽取」= 维持不抽取（not adopted——键盘漫游 deferred 落入 G-B2 Decision 5① 预登记条件分支，采纳方计数仍 <2；kanban ad-hoc roving 回归采纳评估顺延至漫游环立项）；③「动态列模型」= Deferred But Adjudicated（`optimization candidate`；`columnSettings` 勾选显隐 + `columns` 表达式动态求值 T28/T29 + 会话排序端点姿势已完整可达，真实诉求场景「用户自定义列」无 consuming 复刻页；Successor Required: yes——D1 输入池/独立 plan 候选，沿 tabs 集合管理先例做 columns 轴）。**G-B2 回写 ⑫ 登记的两项 successor 重评义务就此闭环**。
- **初版 G-D 行「列菜单」「行高」两子能力 Non-Goal 显式登记**：live 复核两者 schema 层近似可达（列设置/排序/固定经 `columnSettings` 与列头菜单形态已有承载先例——回写 ③⑦；行高四档经 className 表达式状态驱动实测可达——回写 ⑦ A9），本 plan 不立项，归 D1 输入池按 C2 后续轮次评估（不留隐含 debt）。
- 契约文档：`docs/references/renderer-interfaces.md` §Table Group And Aggregate Contract + §Table Cell In-Place Edit Contract（landed 终稿）；flux-guide `design-patterns/table.md` §13 分组与聚合 + §14 单元格原位编辑双态（作者条目）。
- 先红后绿单测汇总：分组矩阵 `table-grouping.test.ts` 14 条（resolve/聚合函数/兜底/交错序列）+ 渲染矩阵 `table-group-render.test.tsx` 14 条（零回归/组头输出/折叠保持/兜底/聚合兜底/selectAllMode 页作用域/tree·drag 冲突/排序共存/optionRow 共存/schema 校验）+ 编辑矩阵 `table-editable-cell.unit.test.tsx` 15 条（resolve/双态状态机/全编辑器/CX-10 派发载荷/save-fail）+ `table-cell-edit-render.test.tsx` 8 条（零回归/共存/只读 fallback/行手势隔离/校验）；连带回归全包 136 files / 1027 tests 全绿。
- styling-system 核查结论：组头行为 `data-slot="table-group-header"`/`data-group-key`/`data-collapsed`/`data-row-group` marker 输出 + 内部布局类（widget 内部视觉面，非布局 marker 违约——组头行是 table widget 自身结构）；editable 格 `data-slot="table-editable-cell"`/`data-editing`/`data-saving`/`data-editor` + ui 组件消费；均符合「widget renderer 自样式 + data-slot + data-\*/aria-\* 状态」既有规则，styling-system.md 零改动（无凑条目）。
