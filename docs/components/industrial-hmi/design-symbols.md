...
- **复合族 resize 契约（plan 2026-08-06-0900-2 P2-4 Decision）**：复合族（device/instrument/sensor-control）的 `CompositeParts` 可注册 `resize?(key:'width'|'height', value, parts)` hook。`applyCompositeProps` 的 EXTENT_FIELDS（width/height）分支：有 extent part（level/thermometer/progress 的 liquid/bar 长度件）→ width/height 路由到 extent（既定语义，resize 填充柱/液柱非容器 body）；无 extent part（motor/pump/valve/fan/gauge/indicator/button/sensor/switch）→ 回落 `parts.resize` hook 重算 body 容器尺寸 + 子形状（rotor/core/blades/label 等）相对锚点，使 width/height 经绑定或 applyProps 变更产出可见几何响应（B2 points-based 修复的复合族镜像）。**拒绝**「width/height 加入 BODY_FIELDS 直接写 body 节点」方案——body 是 Rect/Ellipse，写其 width/height 不动子形状相对布局，仍产几何不一致。
- **create/update 几何一致契约**：复合/基础图元的 create-time 几何与 update-time（applyProps）几何须共用同一常量/锚定逻辑，禁出现「create 用 `-24`、applyProps 用 `-16`」类常量分歧（thermometer `BULB_RESERVE` 经此统一，消除首帧 binding tick 8px 跳变）；create 的 author 声明样式（stroke/strokeWidth/fill）须 guarded（`if (attrs.x === undefined)`），禁无条件覆盖（video create stroke guard 经此对齐）；派生子尺寸（pipe-junction stub strokeWidth）须从声明 prop 派生（`props.strokeWidth ?? fallback`）且 applyProps 路由变更，使 body/stub 粗细一致。
- **`getSymbolProps` 读写对称契约（plan 2026-08-06-0900-2 P2-10 Decision）**：`getSymbolProps` 返回 `ScadaSymbolProps` schema 键名，经 `fromNodeAttrs`（`toNodePatch` 的逆映射，同文件对偶）把 leafer 节点属性面反映射——`scaleX+scaleY→scale`、`fontSize→textSize`、`textAlign→align`、`dashPattern→strokeDash`、Text 节点 `fill→textColor`/非 Text `fill→fill` 透传（消解 `fill` 多对一歧义）。读返回与写期望（`setSymbolProps`/`toNodePatch`）键名对称，host `getSymbolProps(id)`→`setSymbolProps(id, roundtrip)` 往返不再喂错键。仅发射 schema 键，mock/leafer 内部键跳过防往返污染节点。**拒绝**「文档化为 raw leafer attrs 去 cast」方案——host 工具链仍需手动映射，往返喂错键未根治。
