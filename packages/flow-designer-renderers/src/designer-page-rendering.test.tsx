...
describe('designer-page rendering contracts', () => {
  ...
  it('passes meta.disabled through designer-field control variants', () => {
    ...
    expect((view.container.querySelector('input[type="text"]') as HTMLInputElement)?.disabled).toBe(
      true,
    );
    expect((view.container.querySelector('textarea') as HTMLTextAreaElement)?.disabled).toBe(true);
    expect((view.container.querySelector('input[type="number"]') as HTMLInputElement)?.disabled).toBe(
      true,
    );
  });
  ...
  it('renders title as a supported value-or-region contract above the toolbar', () => {
    ...
    expect(
      designerPageDef?.fields?.some((field) => field.key === 'title' && field.regionKey === 'title'),
    ).toBe(true);
  });
