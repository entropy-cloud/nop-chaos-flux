...
- A `run()` reachable more than once within a single effect lifecycle (e.g. an initial autoload plus an exposed `refresh` capability) MUST capture its abort controller per invocation (`const myController = new AbortController()`) and check `myController.signal.aborted` after each `await`. A shared effect-scope `let controller` is unsafe: a later run reassigns it, so an aborted earlier run reads the wrong controller post-`await` and its stale result or spurious error wins (regression: `dynamic-renderer` F4).
- An in-flight guard ref (a concurrency marker such as `inFlightKeyRef`, distinct from a success marker such as `lastInitKeyRef`) MUST be cleared in the same cleanup that aborts its controller. Refs outlive the effect body; if cleanup aborts but leaves the marker, a mid-init change to a non-activation dependency strands the marker and the next effect body bails, silently dropping init for that activation key. When a promise's settlement also clears the marker, gate that clear on controller identity (`abortRef.current === controller`) so a stale aborted promise cannot clear a fresh re-run's marker (regression: `FormRenderer` F5).
...
- See `docs/archive/plans/90-form-store-per-path-subscription-plan.md` for the per-path subscription
  implementation history and `docs/archive/plans/91-form-field-state-normalization-refactor-plan.md`
