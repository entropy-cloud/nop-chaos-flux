...
| D4       | 真实按钮副作用（`component:setSenderDraft` + `ai:regenerate` + feedback 写 metadata）                          | `planned` | `packages/flux-renderers-ai/src/adapters/{ai-component-handle,ai-action-provider}.ts` + `ai-sender.tsx` + `schemas.ts` + `ai-widgets-demo.tsx`                        | D1+D2+D3 | `docs/plans/2026-08-24-2317-1-d4-real-button-side-effects.md`  |
| D5       | Showcase 完整性（扩 widgets demo 含 Reasoning / Tool-call / Citations widget 实例）                            | `planned` | `apps/playground/src/pages/ai-widgets-demo.tsx` + `ai-widgets-fixture.ts`                                                                                             | D1       | `docs/plans/2026-08-24-2237-2-d5-showcase-completeness.md`     |
| D6       | LaTeX 公式渲染（`remark-math` + `rehype-katex` + `katex` peer dep）+ 代码高亮（lowlight 复用）+ fence 边界收紧 | `planned` | `packages/flux-renderers-ai/src/renderers/ai-bubble/renderers/markdown.tsx` + `markdown-buffer.ts` + `package.json` + `styles.css` + `apps/playground/src/styles.css` | D2       | `docs/plans/2026-08-24-2237-3-d6-latex-code-highlight.md`      |
