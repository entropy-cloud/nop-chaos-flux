...
export const flowDesignerRendererDefinitions: RendererDefinition[] = [
  {
    ...
    propContracts: {
      ...
      treeDocumentEpoch: {
        shape: { kind: 'number' },
        displayName: 'Tree Document Epoch',
        description: 'Monotonically increasing host epoch that authorizes tree document replacement.',
        editorType: 'number',
      },
      treeDocumentAckSessionId: {
        shape: { kind: 'string' },
        displayName: 'Tree Document Ack Session',
        description: 'Host acknowledgement session id for the pending tree writeback head.',
        editorType: 'string',
      },
      treeDocumentAckDispatchId: {
        shape: { kind: 'number' },
        displayName: 'Tree Document Ack Dispatch',
        description: 'Host acknowledgement dispatch id for the pending tree writeback head.',
        editorType: 'number',
      },
      treeDocumentChangeAction: {
        shape: { kind: 'object', fields: {} },
        displayName: 'Tree Document Change Action',
        description: 'Single-root action dispatched for every committed tree change with treeDocument bindings.',
        editorType: 'action',
      },
      config: {
        shape: { kind: 'object', fields: {} },
    ...
    fields: [
      ...
      { key: 'treeDocumentEpoch', kind: 'prop' },
      { key: 'treeDocumentAckSessionId', kind: 'prop' },
      { key: 'treeDocumentAckDispatchId', kind: 'prop' },
      { key: 'treeDocumentChangeAction', kind: 'prop' },
