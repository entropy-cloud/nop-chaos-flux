...
| -------- | -------------------------------------------------------------------------------------------------------------- | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | -------------------------------------------------------------- |
| D0       | 产品标准基线（fixture 设计与 typography 决策）                                                                 | `planned` | `docs/analysis/2026-08-23-ai-widgets-vs-tiny-robot-comparison.md` §6-§7                                                                                               | —        | `docs/plans/2026-08-24-1045-1-d0-product-standard-baseline.md` |
| D1       | Mock Connector 富 markdown Fixture（含 11 种元素 + 6 preset + 关键词分发 + env 工厂 option 化）                | `planned` | `apps/playground/src/ai/mock-ai-env.ts`、`apps/playground/src/ai/ai-widgets-fixture.ts`（新）                                                                         | D0       | `docs/plans/2026-08-24-1045-2-d1-rich-markdown-fixture.md`     |
| D2       | Markdown Typography 自定义 CSS（取代 `prose`）                                                                 | `planned` | `packages/flux-renderers-ai/src/renderers/ai-bubble/renderers/markdown.tsx` + `styles.css`                                                                            | D0       | `docs/plans/2026-08-24-1045-3-d2-markdown-typography-css.md`   |
| D3       | 气泡 Avatar（lucide + 圆形 + 32px）                                                                            | `todo`    | `packages/flux-renderers-ai/src/renderers/ai-bubble/index.tsx` + `styles.css`                                                                                         | D0       | _(待 plan 起草)_                                               |
| D4       | 真实按钮副作用（`component:setSenderDraft` + `ai:regenerate` + feedback 写 metadata）                          | `todo`    | `packages/flux-renderers-ai/src/adapters/{ai-component-handle,ai-action-provider}.ts` + `ai-sender.tsx` + `schemas.ts` + `ai-widgets-demo.tsx`                        | D1+D2+D3 | _(待 plan 起草)_                                               |
| D5       | Showcase 完整性（扩 widgets demo 含 Reasoning / Tool-call / Citations widget 实例）                            | `todo`    | `apps/playground/src/pages/ai-widgets-demo.tsx` + `ai-widgets-fixture.ts`                                                                                             | D1       | _(待 plan 起草)_                                               |
| D6       | LaTeX 公式渲染（`remark-math` + `rehype-katex` + `katex` peer dep）+ 代码高亮（lowlight 复用）+ fence 边界收紧 | `todo`    | `packages/flux-renderers-ai/src/renderers/ai-bubble/renderers/markdown.tsx` + `markdown-buffer.ts` + `package.json` + `styles.css` + `apps/playground/src/styles.css` | D2       | _(待 plan 起草)_                                               |
