...
describe('ai-chat Layer C ComponentHandle (registration lifecycle + dispatch)', () => {
  it('registers a component handle on mount and the handle is resolvable by componentId', async () => {
    ...
    // handle is registered and resolvable by componentId with the 5 methods.
    await waitFor(() => {
  ...
  it('unregisters the component handle on unmount (register -> unregister lifecycle)', async () => {
    ...
    const unregisterSpy = vi.fn(() => undefined);
    const registerSpy = vi
      .spyOn(registry, 'register')
      .mockReturnValue(unregisterSpy as unknown as () => void);
    ...
    await waitFor(() => {
      ...
      expect(registerSpy.mock.calls[0][0]?.id).toBe('chat-a');
    });

    unmount();

    // The unregister fn returned by `register` is invoked on cleanup.
  ...
  it('component:sendMessage action routes through the registry and reaches the engine', async () => {
    ...
    // The dispatched text appears as a user bubble.
    await waitFor(() => {
      const userBubble = document.querySelector('[data-slot="ai-bubble"][data-role="user"]');
