...
- `selector` 是真实核心步骤类型，不能漏掉。
- `fork` / `fork-n` 是单一 body 被动态复制执行，不是用户手工维护多条 branch。
...
type TaskFlowStepProps =
  | { type: 'simple'; bean: string }
  | { type: 'step'; source?: string; legacyDslTag?: 'xpl'; customType?: string }
  | { type: 'script'; lang: string; source?: string }
  | { type: 'invoke'; bean: string; method: string }
  | { type: 'invoke-static'; method: string }
  | { type: 'call-task'; taskName?: string; taskVersion?: number; taskModelPath?: string }
  | {
      type: 'call-step';
      libName?: string;
      libVersion?: number;
  ...
  | { type: 'sleep'; sleepMillisExpr: string }
  | { type: 'suspend'; resumeWhen?: string }
  | { type: 'exit'; source?: string }
...
Authoring schema 只暴露 `step` 类型，不再暴露 `xpl` 类型。导入旧 `<xpl>` 时可用 `step.props.legacyDslTag = 'xpl'` 记录原始 tag；新建节点一律输出 `<step>`。目前需要特殊转换的是 `loop-n`：JSON/可视节点使用 hyphenated id，Java/model step type 使用 `loopN`。
...
| 步骤类型        | 可视节点     | 类型专属属性                                                                         | 子编辑器                         |
| --------------- | ------------ | ------------------------------------------------------------------------------------ | -------------------------------- |
| `simple`        | 叶子节点     | `bean`                                                                               | 无                               |
| `step`          | 叶子节点     | `source`, `legacyDslTag`, `customType`                                               | 无                               |
| `script`        | 叶子节点     | `lang`, `source`                                                                     | 无                               |
| `invoke`        | 叶子节点     | `bean`, `method`                                                                     | 无                               |
| `invoke-static` | 叶子节点     | `method`                                                                             | 无                               |
| `call-task`     | 叶子节点     | `taskName`, `taskVersion`, `taskModelPath`                                           | 无                               |
| `call-step`     | 叶子节点     | `libName`, `libVersion`, `libModelPath`, `stepName`                                  | 无                               |
| `delay`         | 叶子节点     | `delayMillisExpr`                                                                    | 无                               |
| `sleep`         | 叶子节点     | `sleepMillisExpr`                                                                    | 无                               |
| `suspend`       | 叶子节点     | `resumeWhen`                                                                         | 无                               |
| `exit`          | 终止节点     | `source`                                                                             | 无                               |
| `end`           | 终止节点     | `source`                                                                             | 无                               |
| `sequential`    | 容器节点     | 无                                                                                   | `dingflow` body                  |
| `selector`      | 容器节点     | 无                                                                                   | `dingflow` body                  |
| `graph`         | 容器节点     | active graph container 的 `enterStepRefs`, `exitStepRefs`                            | `workflow` body                  |
| `parallel`      | 容器节点     | `joinType`, `autoCancelUnfinished`, `aggregator`                                     | `dingflow` body                  |
| `fork`          | 容器节点     | `varName`, `indexName`, `producer`, `joinType`, `autoCancelUnfinished`, `aggregator` | single `dingflow` body           |
| `fork-n`        | 容器节点     | `indexName`, `countExpr`, `joinType`, `autoCancelUnfinished`, `aggregator`           | single `dingflow` body           |
| `loop`          | 容器节点     | `varName`, `indexName`, `itemsExpr`, `maxCount`, `varType`, `while`, `until`         | single `dingflow` body           |
| `loop-n`        | 容器节点     | `varName`, `indexName`, `beginExpr`, `endExpr`, `stepExpr`                           | single `dingflow` body           |
| `if`            | 分支容器节点 | `condition`                                                                          | `then` / `else` branch body      |
| `choose`        | 分支容器节点 | `decider`                                                                            | `case` / `otherwise` branch body |
| `custom`        | 兼容节点     | `customType`                                                                         | 无，默认隐藏                     |
...
- `steps[]` 按顺序输出到 `<steps>`。
- `sequential` / `selector` / `parallel` / `fork` / `fork-n` / `loop` / `loop-n` 的 `step.body` 输出为该 step 的内部 `<steps>`。
...
- `fork` / `fork-n` 只有一个 body container，不允许手工多 branch。
- `custom` 默认只读导入，不默认允许新建。
...
4. 补齐 `selector`、`parallel`、`fork`、`fork-n`、`loop`、`loop-n`。
...
- 所有核心步骤，包括 `selector`，都有明确节点类型、属性 schema、子编辑器归属和 lowering 规则。
