...
- 如果时间有限，可以减少初审维度数量，但**不能跳过维度复核阶段**。
...
- 对于“零发现”的维度，仍需一个独立复核 agent 复查并明确输出“未发现需报告问题”。
...
1. 每个维度文件包含该维度的初审结果 + 复核结论（保留/降级/驳回标注）
2. 汇总报告必须包含复核统计（初审总数、保留/降级/驳回数）
...
- 覆盖的包：[列表]
- 审核日期：YYYY-MM-DD
- 执行方式：每个维度一个初审子 agent + 一个维度复核子 agent + 若干高风险逐项复核 / 低风险批量复核子 agent，共 N 个子 agent
...
4. **独立复核是强制步骤**: 每个维度必须经过“初审 → 维度复核”；高风险或不确定发现还必须继续做子项复核。
...
维度复核：
- 输入：该维度初审全文
- 要求：重新读取 live code / 当前文档，不把初审结论当事实；输出保留 / 降级 / 驳回清单，并指出哪些条目需要继续逐项复核
...
批次 4: Task(03) + Task(10) + Task(13) + Task(19)
批次 5: Task(11) + Task(12) + Task(16) + Task(17) + Task(18) + Task(20)
```
