...
- [ ] P0-01: Add keyboard drag/resize interaction to Gantt bars (Arrow keys for resize, Space/Enter for drag start/end, Tab for bar focus).
- [ ] P0-02: Implement keyboard alternative for Calendar event drag-and-drop (Arrow keys move event; Enter/Space to pick up/place; Escape to cancel).
- [ ] P0-03: Make Kanban column drag handle keyboard focusable (`tabIndex={0}`, Arrow keys for reorder).
- [ ] P0-04: Add focus trap + `role="dialog"` + `aria-modal="true"` to Barcode scanner overlay.
- [ ] P0-05: Add focus trap + `aria-modal="true"` to Calendar overlays (type selector, confirm dialog).

Exit Criteria:

- [ ] Gantt bars can be resized and dragged using only keyboard (Arrow keys + Enter/Space).
- [ ] Calendar events can be moved, resized, and created using only keyboard.
- [ ] Kanban columns can be reordered using keyboard (Tab to handle, Arrow keys to move).
- [ ] Barcode scanner overlay traps focus and announces role to screen readers.
- [ ] All Calendar overlays trap focus and are announced as dialogs.
...
- [ ] P1-20: Make Gantt layout resize handle keyboard accessible (`tabIndex`, Arrow key handlers).
- [ ] P1-21: Add `role="grid"` to Calendar month/week/day tables; `role="gridcell"` to cells.
- [ ] P1-22: Add `aria-live="polite"` regions for dynamic content updates in all 4 sub-domains.
- [ ] P1-23: Fix Calendar batch scheduler radio inputs — unhide from a11y tree (use visually-hidden instead of `display:none`).
- [ ] P1-24: Add keyboard DnD path for Kanban cards (move via Arrow key + Space/Enter).
- [ ] P1-25: Add `aria-label` to Barcode scanner video element describing camera feed purpose.
- [ ] P1-26: Add `role="dialog"` + focus trap to Kanban activity log side panel.
- [ ] P1-27: Add keyboard alternative for Gantt link-draw interaction.
- [ ] P2-34: Add `aria-expanded` to Gantt expand/collapse toggles.

Exit Criteria:

- [ ] Gantt layout resize handle focusable and operable via keyboard.
- [ ] Calendar grid cells have correct ARIA roles; navigation via Arrow keys works.
- [ ] Dynamic content changes (task updates, card moves, event changes) announced by screen readers.
- [ ] Batch scheduler radio inputs visible to accessibility tree but visually hidden.
- [ ] Kanban cards movable via keyboard (independent of column reorder).
- [ ] Barcode scanner video element has descriptive `aria-label`.
- [ ] Activity log panel has dialog semantics and focus trap.
- [ ] Gantt link edges can be initiated via keyboard.
- [ ] Expand/collapse state reflected via `aria-expanded` attribute.
...
- [ ] P2-35: Add `aria-pressed` to Kanban tag filter buttons.
- [ ] P2-36: Add associated `<label>` or `aria-label` to Kanban search input.
- [ ] P2-37: Replace DOM querySelector-based ARIA management in Gantt grid rows with React render-time attributes.
- [ ] P2-38: Link scheduler config error/success messages to controls via `aria-describedby`.
- [ ] P2-39: Add `aria-label` to Kanban column containers.
- [ ] P2-40: Add non-color indicators (icons, patterns, text) to Calendar weekend/today states.
- [ ] P2-41: Add accessible labels to Calendar day-view time slots.

Exit Criteria:

- [ ] Kanban tag filter buttons announce pressed state to screen readers.
- [ ] Kanban search input has an associated visible or aria-label.
- [ ] Gantt grid rows have stable, render-time ARIA attributes (not DOM-manipulated).
- [ ] Scheduler config form controls announce error/success messages via `aria-describedby`.
- [ ] Kanban columns have distinct `aria-label` values.
- [ ] Calendar weekend/today states convey meaning through structure/icons, not just color.
- [ ] Calendar time slots have labels readable by screen readers.
...
- [ ] All 5 P0 keyboard blockers resolved, verified via keyboard-only interaction walkthrough.
- [ ] All 9 P1 accessibility issues resolved, verified via automated a11y checks and keyboard testing.
- [ ] All 10 P2 accessibility issues resolved.
- [ ] `aria-live` regions present in all sub-domains, verified via screen reader testing.
- [ ] No color-only indicators remain — every state differentiated by structure, icon, or text.
- [ ] No in-scope a11y findings downgraded to deferred/follow-up.
- [ ] Relevant owner docs updated or `No owner-doc update required` statement recorded.
