...
- [x] `packages/flux-core/src/types/actions.ts` — 新增 `XuiActionDefinitions` 类型
- [x] `packages/flux-core/src/types/node-identity.ts` — `TemplateNode` 增加 `namedActionPlans` 字段
- [x] `packages/flux-compiler/src/schema-compiler.ts` — `compileSingleNode()` 中提取 `xui:actions`（直接从 schema 读取），两阶段编译（key 提取 → 符号表推入 → region 编译 → action chain 编译）
- [x] `packages/flux-compiler/src/schema-compiler/diagnostics.ts` — 注册 `xui` namespace validator，处理 `xui:actions` 诊断
- [x] `packages/flux-compiler/src/schema-compiler/symbol-helpers.ts` — 新增 `pushNamedActionSymbols()`
- [x] 单元测试：正常编译、嵌套继承符号表、命名冲突诊断、与 built-in 同名警告、value 类型校验、自引用警告
...
- [x] `packages/flux-core/src/named-action-provider.ts` — 新增 `createNamedActionProvider()` 工具函数
- [x] `packages/flux-react/src/node-renderer.tsx` — NodeRenderer 中处理 `namedActionPlans`：创建合成 namespace provider、合并祖先 named actions、注册到 ActionScope、卸载时清理
- [x] `packages/flux-action-core/src/action-dispatcher/action-runners.ts` — 新增 `runNamedAction()` 步骤（步骤 3）
- [x] `packages/flux-action-core/src/action-dispatcher/action-execution.ts` — 在 component 和 namespaced 之间插入 named action 步骤
- [x] 单元测试：provider 查找/清理生命周期、遮蔽、嵌套继承
...
- [x] 编译测试：多 action chain 复用（schema-compiler-xui-actions.test.ts）
- [x] 编译测试：子节点继承父节点定义
- [x] 编译测试：子节点定义遮蔽父节点
- [x] 诊断测试：命名冲突、类型校验、自引用警告（schema-compiler-diagnostics.test.ts）
- [x] Provider 测试：注册/查找/遮蔽/继承（named-action-provider.test.ts）
- [x] 回归测试：现有 action 分发行为不受影响（flux-action-core, flux-runtime 全量测试通过）
...
Status Note: All 3 phases completed and verified. All 10 acceptance criteria satisfied. Independent closure audit PASS.

Closure Audit Evidence:

- Reviewer / Agent: Independent sub-agent (closure audit session)
- Evidence: All 10 acceptance criteria verified against live code — types, compiler logic, diagnostics, runtime registration/cleanup, provider fallback, resolution order, tests all confirmed present and correct. 248 compiler tests, 119 core tests, 142 action-core tests, 682 runtime tests all pass.

Follow-up:

- E2E tests for `xui:actions` referencing `xui:imports` namespace actions (requires playground integration, deferred)
- Debugger/UI visualization of `xui:actions` (explicitly out of scope, future plan)
