...
- inspector schema 使用固定宿主 scope 读取 `selection` / `target`、`activeSheet`、`meta`
...
- `designer` — 主投影，包含 `selectionTarget`、`inspector`、`inspectorPanels`、`fieldDrag`、`preview`、`activeMeta`、`fieldSources`、`fieldSourceCount`、`fieldCount` 等
...
顶层便利字段 (`inspector`, `inspectorPanels`, `meta`, `canUndo`, `canRedo`, `documentName`, `fieldCount`) 是 `designer.*` 或 `runtime.*` 的镜像，保留给现有 consumer。
