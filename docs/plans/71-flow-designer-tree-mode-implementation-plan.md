...
- [ ] `pnpm lint` 全量通过
- [ ] `pnpm test` 全量通过
- [ ] `flow-designer-core` tree-projection 单元测试覆盖：链式、条件分支、并行分支、嵌套、空 tree
- [ ] 钉钉审批流 domain 集成测试通过
- [ ] Action flow domain 集成测试通过
- [ ] `flow-designer-renderers` designer-page tree 模式渲染测试通过
- [ ] playground 三种示例可切换且渲染正确
- [ ] graph 模式行为无回归
