...
| amis `addable` / `closable` / `draggable`          | **schema 暴露（typecheck）** | schema 层（`TabsSchema` / `TabsItemSchema`）已暴露 `addable` / `closable` / `draggable` 字段对齐 amis 配置面；renderer 行为级实现（拖拽重排、关闭、新增）仍为后续 feature plan。仅 `editable` / `hash` / `source` / `swipeable` 等仍为 Non-Blocking Follow-up（§3/§17/§19）。 |
