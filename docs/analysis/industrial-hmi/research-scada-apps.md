...
| 9   | 帧动画模型（帧=Pen 属性子集，插值写 calculative）                      | meta2d `render.ts:3319-3540`（`setNodeAnimate/Process`）          | leafer 无动画引擎（I0.2 结论）：需在 leafer 之上自建 Animator，逐帧 setAttr 或 patch 矩阵    |
