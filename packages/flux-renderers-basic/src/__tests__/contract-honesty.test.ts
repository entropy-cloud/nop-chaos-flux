...
const resolver = buildPerRendererSourceResolver(
  files,
  basicRendererDefinitions.map((d) => d.type),
  factories,
);

describe('flux-renderers-basic contract honesty guard', () => {
  it('every declared event / capability contract is referenced by the renderer implementation (per-renderer isolation)', () => {
    const violations = findUnreferencedContracts(basicRendererDefinitions, resolver);

    expect(violations).toEqual([]);
  });

  it('per-renderer isolation: a probe is flagged even when a sibling wires its contract (H7)', () => {
    const real = basicRendererDefinitions.find(
      (d) =>
    ...
    const violations = findUnreferencedContracts([...basicRendererDefinitions, probe], resolver);
