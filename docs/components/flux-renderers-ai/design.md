...
| 消息编辑态（`message.state.editing`，P2 增强项）                      | **local**（**当前实现**）           | 组件 `useState`（`ai-bubble/user-edit.tsx` + `ai-bubble/index.tsx` 的 `isEditing` 区域） | 通过 `onAction: 'edit'/'resubmit'` event 触发；引擎**无** editing-state setter。**后果**：`ai-message-list` 的 A-8 虚拟滚动对 >200 消息做虚拟回收时，被回收行持有的 `editing` 局部 state 会丢失（边界 trade-off；迁移到引擎属未来增强，见 plan `2026-07-25-0117-2` Deferred） |
