...
需要注意的是，Base Tree 不负责开放式 runtime 组装。运行时允许的是对已进入 execution model 的结构做**受控激活或实例化**，例如 `when` 的结构参与、`loop` 的重复实例化、`dynamic-renderer` 的受控延迟片段装配；它不是重新打开 authoring/loader 阶段的继承或模块合并。
...
5. **Capability**：真正发请求时，schema 先走 capability authority path；再往下，runtime 会通过 `env.fetcher` 这类 host wiring 完成具体 transport。
