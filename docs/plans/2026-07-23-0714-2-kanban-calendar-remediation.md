...
- [ ] 04-01 / 07-015: Fix props-to-state sync — prevent `resolved.data` overwriting local mutations in `local` ownership mode
- [ ] F-41 / 04-04: Fix `use-kanban-filter.ts` — add `useEffect` to sync `externalFilterText` → `localText`
- [ ] 22-22: Fix DnD registration — memoize registration callbacks to prevent per-render adapter destruction
- [ ] 07-016: Fix keyboard DnD listener — stabilize effect deps to avoid re-attachment on every `boardData` change

Exit Criteria:

- [ ] Kanban local mutations survive prop changes in `local` ownership mode
- [ ] `use-kanban-filter` syncs `externalFilterText` changes to local text state
- [ ] DnD adapters not destroyed/recreated on unrelated renders
- [ ] Keyboard DnD listener not re-attached on every `boardData` change
...
- [ ] F-80: Declare `onColumnAdd` in `KanbanSchema`, `KanbanEvents`, renderer definitions, and `schemas.ts`
- [ ] F-77: Fix `confirmAddColumn` — use `structuredClone` (consistent with mutation helpers)
- [ ] F-78: Fix `data-dnd-column-header` — use `{dndEnabled || undefined}` pattern or conditional spread
- [ ] Renderer contract: Add `void` prefix before all 8 event handler calls in `kanban-board.tsx`
- [ ] 15-6: Replace `document.querySelector` with scoped ref-based DOM access for drag visual feedback
- [ ] F-44 (kanban part): Remove or justify 3 redundant `useCallback` in `kanban-board.tsx`

Exit Criteria:

- [ ] `onColumnAdd` typed in all declaration surfaces — TypeScript resolves with correct payload
- [ ] `confirmAddColumn` uses `structuredClone` — no shallow spread
- [ ] `data-dnd-column-header` absent from DOM when DnD disabled (not `"undefined"`)
- [ ] All event handler calls prefixed with `void`
- [ ] Drag visual feedback uses container-scoped refs, not unscoped `document.querySelector`
- [ ] No unnecessary `useCallback` without annotation
...
- [ ] 23-1: Fix `changeCard` test assertion — match correct merge behavior (priority assert)
- [ ] F-73: Fix DnD integration test CSS selector — `[data-slot="column-drag-handle"]` → `[data-slot="kanban-column-drag-handle"]`
- [ ] 14-4 / 23-5: Expand `use-kanban-collab.test.ts` — add branch coverage (WebSocket event simulation, disconnect/reconnect)
- [ ] 06-4: Add concurrency guard to `kanban-export.ts` — per-instance not global

Exit Criteria:

- [ ] `changeCard` test asserts correct expected priority
- [ ] DnD integration test actually queries drag handles and performs meaningful assertions
- [ ] `use-kanban-collab` branch coverage >50%; hook tested with real WebSocket event simulation
- [ ] Kanban export uses per-instance concurrency guard
...
- [ ] F-72: Add `locale` prop to `CalendarDayView`; use it instead of hardcoded `'en-US'`
- [ ] F-79: Replace hardcoded `aria-label="Previous"`/`"Next"` with `t('scheduling.previous')`/`t('scheduling.next')` via `flux-i18n`
- [ ] 07-022 / 15-5: Scope `document.querySelector` in drag visual to calendar container ref
- [ ] F-75: Remove `latestViewRef`/`latestDateRef` — use hook return values (`activeView`/`currentDate`) directly in callbacks
- [ ] 22-31: Handle events with explicit `resourceId`s when no resources array provided
- [ ] 07-023: Stabilize drag effect dependencies beyond `dragState.active`

Exit Criteria:

- [ ] `CalendarDayView` accepts `locale` prop and renders localized date header
- [ ] Calendar header Previous/Next `aria-label` uses `flux-i18n` translations
- [ ] Drag visual DOM access scoped to calendar container ref
- [ ] No `latestViewRef`/`latestDateRef` — callbacks use hook state directly
- [ ] Events with explicit `resourceId`s rendered correctly without resources array
- [ ] Drag effect has complete dep array
...
- [ ] 06-3: Fix `toBlob` abort signal check — ensure it responds to abort at any point, not just synchronously at start
- [ ] 14-1: Add baseline tests for 3 Calendar 0% coverage dialog/overlay components
- [ ] 14-5: Improve `calendar.tsx` statement coverage from 42% (>60% target)
- [ ] 23-3: Fix Calendar top-level test — stop mocking all 8 hooks; verify event/resource rendering
- [ ] 23-11: Fix `calendar-timezone.test.ts` — test actual timezone-sensitive operations

Exit Criteria:

- [ ] Calendar export abort signal properly observed throughout async operation
- [ ] All 0% calendar dialog/overlay components have baseline rendering tests
- [ ] `calendar.tsx` statement coverage ≥60%
- [ ] Calendar test verifies event/resource rendering (not just DOM markers)
- [ ] Calendar timezone tests verify non-UTC timezone rendering
...
- [ ] 07-006: Consolidate focus trap — ensure all 3 files (`calendar.tsx`, `kanban-board.tsx`, `barcode-scanner-overlay.tsx`) use shared `useFocusTrap`
- [ ] 07-029: Fix `onMount`/`onUnmount` lifecycle event kind for Kanban and Calendar — align with BarcodeInput pattern (declared as `events`, not `meta`)

Exit Criteria:

- [ ] All 3 dialog/overlay components use shared `useFocusTrap` hook
- [ ] `onMount`/`onUnmount` declared consistently as `events` across all scheduling renderers
- [ ] `pnpm typecheck && pnpm test -- --coverage` passes for scheduling package
...
- [ ] All in-scope confirmed live defects fixed (F-41, F-72, F-73, F-75, F-77, F-78, F-79, F-80, 04-01, 04-04, 06-3, 06-4, 07-006, 07-015, 07-016, 07-022, 07-023, 07-029, 15-5, 15-6, 22-22, 22-31, 23-1, F-44 kanban)
- [ ] No live defects silently downgraded to deferred/follow-up
- [ ] Kanban DnD, filter, event contract, DOM correctness verified
- [ ] Calendar i18n, drag, ref sync, resource handling verified
- [ ] `pnpm typecheck`
- [ ] `pnpm build`
- [ ] `pnpm lint`
- [ ] Coverage thresholds maintained (current: branches 50, functions 60, lines 63, statements 60)
