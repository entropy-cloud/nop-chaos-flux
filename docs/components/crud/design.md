...
| `quickSaveAction` / `quickSaveItemAction`（quick edit — per-row save）           | **实现**（per-row save 范式）                                                                                                                                                                                                                                                                                                                          | —                                                                                       | 当前基线。行级 `useRowQuickEditDraft` 维护整行 draft，`quickSaveItemAction` 作为行保存入口。操作列（`type: 'operation'`）渲染保存/取消按钮。per-cell 保存按钮移除（向后兼容：无操作列时降级为 per-cell） |
...
| `selection.toggleOnRowClick`（点击行切换选中）                                   | **实现**：点击行（排除 input/textarea/button/a/checkbox/switch 等交互控件）切换选中；`maxSelectionLength` 已满时点击未选行不触发；仅在实际 toggle 时 `preventDefault`（改进 amis 始终 preventDefault 阻止文本选择的缺陷）；自定义 `onRowClick` 先执行，toggle 链式追加；行 `data-row-toggleable="true"` marker 驱动 `cursor: pointer`                  | —                                                                                       | amis `checkOnItemClick`；ERP 批量选择高频需求                                                                                                                                                            |
...
| cards/list 模式                                                                  | **实现**：`listMode?: 'table' \| 'cards' \| 'list'`（缺省 `'table'`，零回归）。`'cards'`/`'list'` 下 CRUD 以对应 renderer 为行渲染载体（经 `helpers.render` 按 type 解析，不与 carrier 包产生硬依赖），selection 由 CRUD 自持（carrier `selectionMode:'none'` + 模板控件绑定 `selectionStatePath`），pagination 按载体能力承载（见 §4.1 carrier 边界） | —                                                                                       | Q2 决策已裁定 cards/list 为 table 之外的可选行渲染载体；本 plan（`2026-06-24-2358-2`）收口 E1d deferred 项                                                                                               |
...
#### 4.1.2 行渲染载体边界（`listMode`）

`listMode` 选择行渲染载体，缺省 `'table'`（与改动前逐字节一致，零回归）。非 table 模式下 CRUD 始终拥有 selection 与 pagination 状态（与 table 模式同源 scope state path），carrier 只负责"把行集合画出来"：

| 载体            | 渲染方式                                                                                                    | selection 所有权                                                                                  | pagination 所有权                                                                                                                                          |
| --------------- | ----------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `table`（缺省） | 内部 `<TableRenderer>`（直组件装配）                                                                        | scope-owned，透传给 table runtime（`useTableSelection` 三态全 wired）                             | scope-owned，透传给 table runtime                                                                                                                          |
| `cards`         | 经 `helpers.render({ type:'cards', ... })` 按 type 解析渲染（不与 `flux-renderers-content` 产生包级硬依赖） | CRUD 自持：carrier `selectionMode:'none'`，选择经 `card` 模板内控件绑定 `selectionStatePath` 表达 | CRUD 自载：CRUD 预切片当前页 `filteredRows` 喂给 carrier `items`，footer 渲染分页（`PaginationPrevious`/`PaginationNext` 绑定 CRUD `paginationStatePath`） |
| `list`          | 经 `helpers.render({ type:'list', ... })` 按 type 解析渲染（`list` 与 CRUD 同包）                           | CRUD 自持：carrier `selectionMode:'none'`，选择经 `item` 模板内控件绑定 `selectionStatePath` 表达 | 复用 list runtime：`paginationOwnership:'scope'` + `paginationStatePath`（list 按 scope 分页切片），footer 仍由 CRUD 渲染分页控件                          |

裁定要点（来自 E1d deferred 收口依据，区分「schema 声明」与「runtime 落地」）：

- **不要求 cards/list renderer 新增能力。** cards renderer 当前无 pagination 字段、selection 仅 local（schema 声明 `selectionOwnership` 但 runtime 零引用）；list renderer pagination runtime 落地（支持 `paginationOwnership:'scope'`）但 selection 仅 local。因此非 table 模式下 CRUD 自持 selection（经模板控件写同一 `selectionStatePath`，`$crud` 摘要 / selection-aware `listActions` 因读同一 scope state 而继续生效）。
- **请求下沉不变**：cards/list 模式复用 CRUD 已有的 `filteredRows`（来自 `source`），不引入组件级请求。
- **operation 列不自动 lowering**：cards/list 用 `card`/`item` 模板表达行内容（含 operation 等价物，如模板内 `button`）；table operation 列到 cards/list 的自动等价属 feature 级设计，显式 deferred（见 plan `Deferred But Adjudicated`）。
- 模板内 selection 控件示例：因 `checkbox` renderer 是字段双向绑定（写 boolean 到 `name` 路径，不支持数组隶属切换），且 carrier item 作用域的 `setValue` 写只落在 item 本地 store（不回传 CRUD scope），非 table 模式 selection 经模板内 `button` 调用 CRUD 句柄的 `toggleSelection` capability（`{ action: 'component:toggleSelection', componentId, args: { key: '${$slot.item.id}' } }`）。该 capability 在 CRUD 自身 scope 写 `selectionStatePath` 数组（与 table 模式同一 scope state path）。模板内 selection 显示态（如 button label）应直接读 `selectionStatePath`（store-backed，reactive），而非 `$crud.selectedRowKeys`——后者是只读摘要 overlay，仅对 CRUD 直接子节点 reactive（经 CRUD 重渲染刷新），对经 `helpers.render` 嵌套的 carrier item 节点不保证 reactive（尤其在 React Compiler 自动 memo 下 carrier schema 内容不变不重渲染时）。`$crud` 摘要 / selection-aware `listActions` 仍因读同一 scope state path 而继续生效。
...
- `columns`
- `empty`
- `listMode`（`'table' | 'cards' | 'list'`，缺省 `'table'`；cards/list 模式分别消费 `card`/`item` 模板，见 §4.1.1）
- `card` / `item`（cards/list 模式的行模板 region，`item`/`index` 参数）
- `rowKey`
...
per-row quick edit 要求用户在 `columns` 中声明一个 `type: 'operation'` 列，其 `buttons` 中包含保存/取消按钮：
...
```json
{
  ...
  "label": "操作",
  "buttons": [
    { "type": "button", "label": "保存", "actionType": "saveQuickEditRow" },
    { "type": "button", "label": "取消", "actionType": "cancelQuickEditRow" }
  ]
}
```

无操作列时自动降级为 per-cell 保存行为（向后兼容）。
...
### B3.1 契约收口（行身份 / 数据收缩钳制 / nullish 归一化 / falsy filter）

> 来源：`docs/plans/2026-06-26-0520-1-b31-table-row-identity-pagination-clamp-sort-selection-plan.md`。下列契约与 live code 一致。

- **T5 render-time 钳制（透传底层 table）**：CRUD 的分页钳制由内部 `table` 在渲染期承担（`currentPage` 钳到 `[1, totalPages]`，totalPages 基于传入 `source` 行数）。CRUD 把 `filteredRows` 作为 `source` 透传给 table，table 据此切片；当删行 / 批量动作使 `source` 收缩到总行数 `< (currentPage-1)*pageSize` 时，table 自动落到末页（非空），CRUD 不卡空页。该钳制是纯渲染期派生，不回写 scope；server-side「钳制后页码进入下次请求」属请求 owner（data-source / action）职责。`$crud.pagination.currentPage` 仍读 scope owner 原值（钳制前的展示结论见 `table/design.md` §7 B3.1）。
- **T27 nullish 归一化契约（已落地，显式化）**：`normalizeCrudSourceValue` 对 source 归一化——`items` / `rows` / `records` / `list` 任一为数组即取用，否则 `[]`（故 `{items:null}` / `{items:undefined}` 得 `[]`，空网格无 toast）；`total` 回退序为 `total` → `count` → `rows.length`（故 `{total:null}` 得 `rows.length`）。空行时 `totalPages = max(1, ceil(0/pageSize)) = 1`，无无限分页循环。
- **F1 falsy-but-present filter 语义（已落地，显式化）**：client-mode（`clientMode.loadDataOnce` / header filter）下，filter 值 `0`（number）被视为**有效 filter**而非「无 filter」丢弃——keep-filter 判定的 `else return true` 分支保留 number `0`，match 阶段 `cell === value` 匹配 `=== 0` 的行。string `"0"`（trim 后 length > 0）同理保留。仅 `null` / `undefined` / 空串 / 空数组被视为「无 filter」。
- **F2 cleared-field query 语义（已落地，显式化）**：两层语义需区分——(1) **client-mode 行匹配**（`applyQueryToRows`）按 F1 判定丢弃 `null`/`undefined`/空串/空数组字段（清空字段不约束 client 结果）；(2) **发布的 query payload**（`onQuerySubmit` 的 `query`）反映 queryForm `getValues()` 经 `toRecord` 归一化的原值——**清空字段不自动省略 key**，而是按其在表单中的清空值（`""`/`null` 等）透传进 `query`。这是默认契约（CRUD 不引入 per-field send-empty 开关）；若后端要求「清空 = 省略 key」，经 `transformOutAction` submit-transformer 或 query-adaptor 在请求层剔除空值（与 Flux「请求下沉」一致，不把请求语义塞进 queryForm）。
- **T3 pageSize 不变量（透传底层 table）**：见 `table/design.md` §7 B3.1——翻页写回始终携带当前 pageSize，CRUD scope pagination 的 pageSize 独立解析、不被默认值覆盖。

### B3.3 裁定（嵌套 crud 状态隔离 / id 唯一性契约）

> 来源：`docs/plans/2026-06-26-0830-1-b33-table-advanced-tree-aggregate-perf-plan.md`。下列裁定与 live code 一致，非叙事。

- **T24 嵌套 crud containment / ID 命名空间隔离（已落地，显式化）**：每个 CRUD 的 owner state 按 id/cid 命名空间隔离——`createCrudOwnerPaths`（`crud-renderer-ownership.ts`）把 query/pagination/sort/filter/selection state 各自落到 `$_crud.${id ?? cid ?? 'crud'}.<slice>` 独立 path。因此多个 CRUD 共存（sibling 或嵌套于 expandable row / dialog / 嵌套 scope）时，刷新/翻页/选择 A 不覆盖 B，B 分页独立——隔离 by construction（不依赖运行时 guard）。B3.3 T24 锚（`crud-b33-nested-isolation.test.tsx`）：(1) `createCrudOwnerPaths` unit——distinct id → distinct paths、same id → 命名空间碰撞；(2) 集成——sibling crud A/B 各自 per-instance `$crud` 摘要，选 A 行 → A `selectionCount=1` 而 B `=0`；翻 A 页 → A `currentPage=2` 而 B `=1`。嵌套 crud-in-expandable-row 复用同一 owner-path 机制。
- **T24 id 冲突防御 guard 裁定（裁定：doc-only non-goal）**：隔离仅在嵌套 CRUD 有 **distinct id/cid** 时成立；两个 CRUD 共享同一 `id`（authoring 失误）会导致 owner-path 命名空间碰撞（状态互相覆盖）。裁定为 **doc-only non-goal**（不实现运行时 guard）：(1) 运行时 id-collision 检测需要在某个中心 registry 跟踪所有活跃 CRUD 的 owner-path（含 mount/unmount/HMR/strict-mode double-mount），跨 ownership 边界且易误报；(2) 本 roadmap 是测试/文档边界债，不引入新的运行时检测层。契约改为**显式文档化「CRUD id 必须在其共享 scope 内唯一」**：作者须为共存 CRUD 提供 distinct `id`（或 `cid` 自动区分），否则状态会别名。这是 authoring 契约，非运行时保证。
...
| `quickSaveApi`                         | `quickSaveAction`                                                              | 批量 quick edit 保存（per-row save 范式中为降级 fallback，当无 `quickSaveItemAction` 时使用）                                                                                                                                                                 |
| `quickSaveItemApi`                     | `quickSaveItemAction`                                                          | 行级保存入口（per-row save 首选）。配合 `type: 'operation'` 列中的保存按钮使用                                                                                                                                                                                |
...
| quick edit              | 已支持    | 已支持 per-row save：行级 `useRowQuickEditDraft` 共享 draft + 操作列保存按钮，覆盖 inline / dialog / custom body。保留 per-cell 降级路径（无操作列时）。visible saving feedback（spinner + saving text）                                                                                       | 已覆盖                                              |
