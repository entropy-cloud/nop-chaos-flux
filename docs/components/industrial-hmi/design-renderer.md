...
| 反序列化  | `parseScadaConfig(input: string \| object): ScadaConfig`——字符串先 JSON.parse（错误归入 onError）；**fail-closed 纯对象窄化 + 深隔离**（plan 2026-08-08-1931-2 Phase 1 / F5）：非纯对象 input（null/array/原始值）抛 `scada config must be an object`（不再 `as ScadaConfig` cast 类型谎言，与 validate 广度对齐），纯对象经 `structuredClone` 深隔离返回（与调用方不共享嵌套 symbols/variables 引用，mutate 不回流）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | `serialization/parse.ts`                                                |
...
- 序列化函数导出归属（plan 2026-08-04-1558-1 Phase 3 Decision + plan 2026-08-08-1931-2 Phase 3 / P2-1 修订）：serialization 契约函数 **全部经包入口导出**——`serializeScadaConfig`/`parseScadaConfig`/`validateScadaConfig`/`diffScadaConfig` + 类型 `ScadaValidationResult`，供 host 侧工具链（config 迁移/校验/审计/入口校验）直接调用；`industrialRendererDefinitions`（P2-3）亦导出供 host 自定义注册。运行期消费经 `component:exportConfig`/`component:importConfig` 句柄（内部转发至 `engine.exportConfig()`），renderer 不直调 `serializeScadaConfig`——故该函数无 live 内部消费者，保留导出为契约诚实（非死代码）。`parseScadaConfig` 为 fail-closed 纯对象窄化 + 深隔离（Phase 1 / F5）。
