...
export function createCompilerPluginAppliers(plugins?: RendererPlugin[]) {
  function applyBeforeCompilePlugins(schema: SchemaInput): SchemaInput {
    return (plugins ?? []).reduce(
      (current, plugin) => plugin.beforeCompile?.(current) ?? current,
      schema,
    );
  }
  ...
  function applyAfterCompilePlugins(template: CompiledTemplate): CompiledTemplate {
    return (plugins ?? []).reduce(
      (current, plugin) => plugin.afterCompile?.(current) ?? current,
      template,
    );
  }
...
export async function prepareSchemaImports(
  schema: SchemaInput,
  options: CompileSchemaOptions = {},
) {
  ...
  const resolveImportUrl = (
    options as CompileSchemaOptions & {
      resolveImportUrl?: (
        schemaUrl: string,
        from: string,
