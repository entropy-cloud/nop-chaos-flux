...
### 7.6 `disabled` umbrella 扇出（CB1 锁定）

`disabled` 是**单一 umbrella 开关**：顶层派生一次（`condition-builder.tsx:160`，`disabled = presentation.effectiveDisabled || fieldState.submitting`），再下传到每个 `ConditionGroup` / `ConditionItem`，扇出到全部 mutation affordance，**不存在部分禁用语义**。
...
- `disabled` umbrella（见 §7.6）不改变 marker 命名：被禁用的 NOT/AND-OR/add 按钮保留其 data-slot/title，仅置 `disabled`；item delete、group delete、drag handle 在 `disabled:true` 时**不渲染**（无 marker），这是测试与主题定位的稳定约定。
