...
- **plan 2026-08-07-1835-1 Phase 3 live baseline 同步（2026-08-07）**：open P1-E（z-order undo 全量替换违反 R4 增量约束）+ multi P1-01（grouped-child undo 丢失，根因 `cloneConfigSnapshot` 浅克隆）落地修复。**§4.1.2 forward/inverse 增量语义**经两项新增严格满足：① `cloneConfigSnapshot` 改深克隆（递归 children），grouped-child 编辑产生非空 diff 入栈（此前浅克隆致 children 数组在 prev/working 间共享 → diff 为空 → 不入栈，数据丢失）；② `ScadaConfigDiff` 新增 `reordered?: string[]` 字段承载 z-order 增量（forward.reordered = 新顺序 id 列表 / inverse.reordered = 旧顺序 id 列表），reorderZOrderFn forward 不再用全量 added/removed；computeInverse + applyDiffToConfig + engine.applyDiff（新增 applyReorder）三处消费 reordered。R4「栈元素只持 forward + inverse 增量，无全量快照」对 z-order op 严格成立（栈条目 O(n) strings 而非 O(n) 全量节点对象，10 万图元单次 z-order 内存从 ~MB 级降至 KB 级）。focused proof 见 `scada-editor-canvas-undo-integrity.test.tsx`（grouped-child undo 往返 + z-order reordered round-trip + R4 增量断言）。
...
| U8  | `engine.applyDiff` 半途抛错致画布与 working copy 永久背离                   | **防护**：`applyUndoRedoDiff` catch 经 `engine.build(beforeWorking)` 全量重建回滚引擎场景（leafer 无事务，全量重建是唯一可靠回滚）+ 同步 `synced.config=beforeWorking` 闭合「steady state 下 diff 为空 → 引擎永不愈合」的永久背离路径；entry 未 commit 保留在原栈，`editor-internal-error` 派发。**plan 2026-08-08-1931-4 Phase 2 / P1-4 backport**：同一 failure-class 的另一 catch（`syncWorkingCopy` catch，覆盖所有编辑路径——updateWorkingNode / addWorkingSymbol / removeWorkingSymbol / group / ungroup / connection-write）原仅回滚 working copy 不回滚 engine，下轮 syncWorkingCopy diff 为空 → 引擎永不愈合；现 backport 同型 `engine.build(synced.config)` 全量重建 + 两 catch 共享 `reconcileEditorTargets` helper 对齐 `editor.target`。两 catch 同 failure-class 同防护，无不对称 |
| U9  | editor.target 在节点 identity 变化后 dangle（leafer Editor 强引用销毁节点） | **防护**（plan 2026-08-08-1931-4 Phase 2 / P1-2 / P1-3）：所有可能替换 LeafNode 对象的代码路径后无条件重解析 `editor.target` —— `syncWorkingCopy` 成功路径（applyUpdate patch.children 子树重建，覆盖通用编辑流）+ `applyUndoRedoDiff` 成功路径（去除旧长度门控，identity 可在 id 集不变时变）+ 两 catch（engine.build 全量重建后）。统一经 ctx helper `reconcileEditorTargets`：`session.selection` 经 `engine.getSymbol(id)?.node` 重解析到 live registry，装配到 `editor.target`（或 clearEditorSelection）                                                                                                                                                                                                                                                                                 |
