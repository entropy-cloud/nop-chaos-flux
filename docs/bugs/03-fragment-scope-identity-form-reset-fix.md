...
- `packages/amis-react/src/index.tsx` created a new child scope on every `RenderNodes` pass when `props.options.data` was present
...
- added fragment-scope caching in `packages/amis-react/src/index.tsx` for the `options.data` render path
...
- `packages/amis-react/src/index.test.tsx` - verifies a form rendered through fragment helpers keeps its field value after a host rerender recreates the fragment data object
