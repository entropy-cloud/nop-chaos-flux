...
| `selection.keepOnPageChange`                                                     | **实现**             | 跨页选择保留是高频 CRUD 批量操作需求（如多页选完统一删除）。翻页不清空 selection；`keepOnPageChange: true` 时 `handleSelectAll` 合并当前页而非替换，`handleSelectAll(false)` 仅移除当前页而非清空。与 `autoClearSelectionOnRefresh` 协同：翻页（pagination）永不触发清空，仅刷新（refresh）受 `autoClearSelectionOnRefresh` 控制。命名清晰表达意图，与 data-source/action 下沉原则不冲突 |
| `selection.maxSelectionLength`                                                   | **实现**             | 选择上限是常见业务约束（如"最多选 3 项"）。达上限后未选行 checkbox disabled；`handleSelectRow` 拒绝新增；`handleSelectAll` 截断到上限（按当前页顺序取前 N 个可勾选行）。不与 radio 单选语义冲突（radio 忽略此字段）                                                                                                                                                                      |
| `selection.maxKeepSelectionLength`                                               | **不采纳（删字段）** | 仅与 `keepOnPageChange: true` 联用才有意义。超限需要一整套策略裁定（阻止新选 / LRU 淘汰最早 / FIFO），属于 feature 级设计而非契约漂移修复。Non-Goals 已明确排除 `selectionRetentionStrategy` 扩展字段。如未来有跨页保留上限需求，应由 E1d 连同策略字段一起设计后重新引入                                                                                                                 |
| `selection.checkableWhen`                                                        | **实现**             | 按行可勾选条件（如"仅 active 行可选"）是高频业务需求。按行求值表达式，falsy 行 checkbox disabled 且不可被全选纳入。表达式求值失败按 Flux 现行 `when` 语义视为 falsy（disabled）。与 Flux 统一表达式机制一致，不引入散落条件属性                                                                                                                                                          |
| 自动轮询刷新                                                                     | **计划实现（E1d）**  | 走 data-source `interval`，不开组件级 api                                                                                                                                                                                                                                                                                                                                                |
| 可折叠查询区（`filterTogglable`）                                                | **计划实现（E1d）**  | 当前 queryForm 永远内联可见                                                                                                                                                                                                                                                                                                                                                              |
| 无限滚动分页                                                                     | **计划实现（E1d）**  | 当前仅分页                                                                                                                                                                                                                                                                                                                                                                               |
| cards/list 模式                                                                  | **计划实现（E1d）**  | 依赖主 roadmap W1c/W2a                                                                                                                                                                                                                                                                                                                                                                   |
| `syncLocation`（URL 同步查询参数）/ `parsePrimitiveQuery`                        | **不采纳**           | 偏路由/宿主导航职责，不在组件（与分析报告 §5 一致）                                                                                                                                                                                                                                                                                                                                      |
| `clientMode.matchFunc`                                                           | **暂不实现**         | 契约已定义（DESIGN-ACK-NOT-IMPL），runtime deferred                                                                                                                                                                                                                                                                                                                                      |
| `autoGenerateQueryForm`                                                          | **暂不实现**         | schema 存在，runtime 未实现（DESIGN-ACK-NOT-IMPL）                                                                                                                                                                                                                                                                                                                                       |
| 列拖拽排序（`draggable`）                                                        | **暂不实现**         | 当前为最小上下移动（DESIGN-ACK-NOT-IMPL，依赖 table E1c）                                                                                                                                                                                                                                                                                                                                |
| 服务端分页完整 request owner                                                     | **暂不实现**         | 已落地 source-result 消费，完整 owner 待补（DESIGN-ACK-NOT-IMPL）                                                                                                                                                                                                                                                                                                                        |
| 组件级 `api`/`initFetch`/`sendOn`                                                | **不采纳**           | 请求下沉 data-source + action（见 analysis §5）                                                                                                                                                                                                                                                                                                                                          |
| 导出 `export-csv`/`export-excel`                                                 | **不采纳**           | 后台职责，前端不做（见 analysis §5）                                                                                                                                                                                                                                                                                                                                                     |
