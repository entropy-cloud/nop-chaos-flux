...
- **安全性**：`from` 的值通过宿主提供的 `env.importLoader` 解析，框架本身不执行任何 URL 解析或脚本加载。安全边界的划分是明确的：框架负责管理导入的词法可见性和生命周期（引用计数、teardown），宿主负责决定哪些库可以被加载以及如何加载。建议宿主实现时采用白名单机制，只允许加载预先注册的可信库标识符，而非接受任意 URL。在默认情况下（宿主未实现 importLoader），`xui:imports` 声明将被静默忽略，不会产生安全风险。命名空间的隔离保证了导入的库不能覆盖内置平台动作（`setValue`、`ajax` 等），命名空间动作的解析优先级始终低于内置动作
...
**开发工具**：`nop-debugger` 提供浮动调试面板，可查看 compile、render、
action、api 四类事件的时间线，以及单个组件的 scope 数据和 form state
（通过 node tab 和 element picker）。ActionScope 的命名空间列表和
ComponentHandleRegistry 的内部索引在面板中直接可视。动态节点
（expression-node、template-node）在编译后保留 source 字段，可在调试
时追溯到原始表达式文本；静态节点没有 source 字段。
