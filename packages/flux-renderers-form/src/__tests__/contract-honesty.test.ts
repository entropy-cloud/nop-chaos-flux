...
/**
 * The form renderers delegate their ComponentHandle to shared factories in
 * `@nop-chaos/flux-runtime` (`createFormComponentHandle` for forms,
 * `useInputComponentHandle` for input fields). A factory is mixed into a
 * renderer's capability source ONLY when that renderer references its hook
 * (per-renderer selection), so a sibling renderer's factory cannot mask a
...
describe('flux-renderers-form contract honesty guard', () => {
  ...
  it('per-renderer isolation: a probe is flagged even when a sibling wires its contract (H7)', () => {
    ...
    const violations = findUnreferencedContracts([...formRendererDefinitions, probe], resolver);
  ...
  it('guard mechanism is effective: an injected unreferenced event contract is flagged', () => {
    const probe: RendererDefinition = {
      type: '__contract_probe__',
      component: () => null,
      eventContracts: {
