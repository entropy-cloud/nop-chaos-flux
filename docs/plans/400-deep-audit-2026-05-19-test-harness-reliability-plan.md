...
- [x] Stop globally suppressing real `WebSocket connection` failures in shared E2E entry paths, or replace the suppression with a targeted, honest gate.

Exit Criteria:

- [x] `14-06` through `14-09` and `R01-04` are fixed.
- [x] Focused proof covers cleanup, zero-error discipline, and the final shared error-gate behavior.
...
Status Note: Original `14-06` through `14-09` reliability fixes remain landed, and the widened in-scope open-ended finding `R01-04` is now also closed. The shared E2E fixture no longer globally suppresses `WebSocket connection` failures, focused hard-gate proof now verifies those errors fail `assertTrackedPageErrors(page)`, and repo-wide `pnpm typecheck`, `pnpm build`, `pnpm lint`, and `pnpm test` are green. No remaining plan-owned work remains.

Closure Audit Evidence:

- Reviewer / Agent: `general` subagent `ses_1bf1d762fffedGQRE4o3TxVrTW`
- Evidence: Independent closure audit returned acceptable for closure. Live evidence includes `tests/e2e/fixtures.ts` with the broad `WebSocket connection` suppression removed, regression proof in `tests/e2e/fixtures-hard-gate.spec.ts` (`3 passed`), focused supported E2E proof in `tests/e2e/flux-basic-row-inspect.spec.ts` (`1 passed`), bug note `docs/bugs/62-e2e-shared-websocket-error-suppression-fix.md`, and green workspace `pnpm typecheck`, `pnpm build`, `pnpm lint`, and `pnpm test`.
