...
- [ ] Dim03-01/P1-03: Add `onMount`/`onUnmount` to Gantt/Calendar fields arrays OR remove from schemas if never dispatched.
- [ ] Dim03-02/P1-04/P2-02: Wire or remove GanttSchema `childrenField`, `initiallyExpanded`, `className` fields, and all 4 reaction fields.
- [ ] Dim03-03/P2-03: Wire or remove CalendarSchema ownership fields (`viewOwnership`, `dateStatePath`, etc.).
- [ ] Dim03-05/P2-04: Remove `as any` cast from BarcodeInputRenderer registration — use proper typed reference.
- [ ] Dim03-07/P1-04: Wire all 8 reaction fields via `props.reactions` or remove from schema.
- [ ] Dim03-08/P2: Resolve `GanttTask`/`GanttLink` type name collision between schemas.ts and gantt.types.ts.
- [ ] Dim04-01/P1-05: Fix Kanban `boardData` — derive from `resolved.data` or add re-sync mechanism.
- [ ] Dim04-02/P1-06: Fix Barcode `inputValue` — read initial value from form store on mount; use form store as single source.
- [ ] Dim04-03/P1-07: Wire or remove all ownership/statePath schema fields across Kanban and Calendar.
- [ ] Dim04-04/P1-08: Fix GanttStore — re-initialize when resolved props change (e.g., key-based remount or `useEffect` sync).
- [ ] Dim04-05/P2-05: Sync Calendar date/view when initial props change.
- [ ] Dim04-06/P2-06: Wire `collapsedStatePath`/`collapsedOwnership` or remove from schema; derive `collapsedMap` from resolved data.
- [ ] F-09: Remove unused `currentBoard` parameter from `kanban-undo-stack.ts` `undo`/`redo` functions.
- [ ] F-08: Either wire `controlledDate`/`controlledView` through Calendar component or remove hook parameters.

Exit Criteria:

- [ ] Every field in every schema type is either wired at runtime or removed from the type.
- [ ] No renderer maintains local state that duplicates store/scope data without a sync mechanism.
- [ ] GanttStore re-initializes when props change.
- [ ] `undo`/`redo` functions no longer accept unused parameters.
- [ ] Calendar controlled-mode parameters either functional or removed from hook signature.
...
- [ ] Dim10-01/P1-14: Add `@import` of scheduling CSS to playground `styles.css`.
- [ ] Dim10-02/P1-15: Refactor Calendar.tsx inline styles (drag ghost, type selector, confirm dialog) to CSS classes.
- [ ] Dim10-03/P1-16: Refactor CalendarBatchScheduler from 100% inline styles to CSS classes + `cn()`.
- [ ] Dim10-04/P1-17: Refactor CalendarTimezoneSelector from 100% inline styles + imperative mutations to CSS classes.
- [ ] Dim10-05/P2-13: Replace hardcoded color literals in styles.css with CSS variable references.
- [ ] Dim10-06/P2: Replace hardcoded fill/stroke colors in Gantt SVG elements with CSS variables.
- [ ] Dim10-07/P2-14: Scope bare `[data-slot]` selectors under root marker classes.
- [ ] Dim11 (P2): Replace raw `<input>`, `<button>`, `<select>`, `<option>` with `@nop-chaos/ui` equivalents across Kanban, Gantt, and Calendar.
- [ ] F-19/F-27: Add `t()` from `@nop-chaos/flux-i18n` to all user-facing strings in Gantt, Calendar, and Kanban sub-domains.
- [ ] F-28: Consume existing locale keys (`scheduling.today`, `scheduling.gantt.*`, etc.) in the corresponding components.
- [ ] F-19: Replace Calendar hardcoded `zh-CN` locale with locale prop; pass locale from Calendar root through view component hierarchy.
- [ ] F-30: Fix Kanban test assertions — use test-specific locale context or check for dynamic text.

Exit Criteria:

- [ ] Scheduling CSS loaded in playground — visual styles apply to all scheduling components.
- [ ] Zero inline styles with hardcoded colors in Calendar components.
- [ ] Zero bare `[data-slot]` selectors — all scoped under root marker classes.
- [ ] Zero raw `<input>`, `<button>`, `<select>` in scheduling package (except performance-exempted host surfaces).
- [ ] All user-facing strings in Gantt/Calendar/Kanban use `t()` or a locale prop.
- [ ] All existing locale keys under `scheduling.*` consumed by runtime code.
- [ ] Calendar components accept and pass a `locale` prop through the component hierarchy.
- [ ] Kanban tests pass with i18n added.
...
- [ ] Dim06-1/P1-09: Add AbortSignal to barcode scanner overlay init async IIFE.
- [ ] Dim06-2/P1-10: Replace stale `enabled` closure in detection poll with AbortController.
- [ ] Dim06-3/P1-11: Fix FilterBar debounce — use useEffect cleanup, not useCallback return value.
- [ ] Dim06-4/P2-07: Add stale guard/concurrency key to Calendar iCal import/export.
- [ ] Dim06-5/P2-08: Add concurrency guard (ensure single operation) to Gantt export handles.
- [ ] Dim06-6/P2-09: Add retry mechanism for WASM fetch failure.
- [ ] Dim06-7/P2-10: Clean up Calendar drag-create timer on unmount.
- [ ] Dim19-1/P1-19: Add `console.error` to WebSocket connection failure.
- [ ] Dim19-2/P2-29: Preserve original error in WS message parse error.
- [ ] Dim19-4/P2-30: Log torch toggle failure with original error.
- [ ] Dim19-5/P2-31: Log dynamic import failure; return error shape instead of null.
- [ ] Dim19-6/P2-32: Preserve original error in Calendar export catch.
- [ ] Dim19-8/P2-33: Log Calendar iCal dynamic import failure.
- [ ] Dim14-2/P1-18: Add tests for all 12 zero-coverage Calendar source files (6 view components, 4 hooks).
- [ ] Dim14-1/P2-21/Dim14-3/P2-22: Add tests for Gantt hooks (4) and main renderer.
- [ ] Dim14-16/P2-24: Add integration tests for DnD interactions in kanban/gantt/calendar.
- [ ] Dim14-15/P2-23: Verify coverage thresholds; add tests to reach ≥60% calendar subdomain.

Exit Criteria:

- [ ] All async operations use AbortSignal/useEffect cleanup — no dangling timers or stale callbacks.
- [ ] Every async failure path logs original error — zero silent error swallows.
- [ ] Calendar subdomain has ≥60% file-level coverage; zero source files untested.
- [ ] Gantt hooks and main renderer have focused tests.
- [ ] Integration DnD tests exist for kanban, gantt, and calendar.
...
- [ ] Dim01-1/P1-01: Remove phantom `@nop-chaos/flux-renderers-form` dependency.
- [ ] Dim01-2/P1-02: Either import `@zxing/library` as JS module or remove from dependencies (keep only CDN reference).
- [ ] Dim01-3/4/5 (P2): Move `html2canvas`, `jspdf`, `xlsx`, `ical.js` to optional peer dependencies or dynamic-import-only.
- [ ] Dim15-1/P2-25: Refactor O(n^2) calendar conflict detection to O(n log n) interval tree.
- [ ] Dim15-4/P2-28: Replace `cardIds.indexOf()` in KanbanColumn render hot path with `Set.has()`.
- [ ] Dim15-5/P2-12: Fix GanttStore in-place mutation pattern (use immutable updates).
- [ ] Dim15-6/P3: Remove redundant `useCallback`/`useMemo` across scheduling package (30+ files).
- [ ] Dim15-7/P3: Remove `React.memo(KanbanCard)` — redundant under React Compiler.
- [ ] F-06: Document inconsistent undo systems; choose one pattern (command or snapshot) for future consolidation; add TODO/FIXME in code.
- [ ] Dim03-06/P3: Remove unnecessary `schedulingRendererDefinitions` array export (internal only).

Exit Criteria:

- [ ] Zero phantom dependencies in package.json.
- [ ] Zero hard deps used only via dynamic import — all moved to peer/optional.
- [ ] Calendar conflict detection uses O(n log n) or better.
- [ ] Kanban column render has no O(n) lookup per card.
- [ ] GanttStore uses immutable update patterns.
- [ ] No redundant `useCallback`/`useMemo` or `React.memo` in scheduling package.
- [ ] Undo system inconsistency documented.
...
- [ ] All dead schema contracts either wired or removed from types.
- [ ] Zero dual-state patterns in any scheduling renderer.
- [ ] All async operations properly cleaned up; zero timer/callback leaks.
- [ ] Styling system compliant: zero hardcoded inline colors; CSS scoped; playground imports present.
- [ ] Zero raw HTML elements where `@nop-chaos/ui` alternatives exist.
- [ ] Calendar subdomain coverage ≥60%; all source files have at least one test.
- [ ] Zero silent error swallows — every async failure path logs original error.
- [ ] i18n: all user-facing strings in Gantt/Calendar/Kanban use `t()` or locale prop.
- [ ] Phantom deps removed; optional deps correctly classified.
- [ ] Performance red lines addressed (O(n^2) conflict detection, hot-path indexOf, redundant memo).
- [ ] Undo system inconsistency documented with successor path.
- [ ] No in-scope finding downgraded to deferred/follow-up without explicit adjudication.
- [ ] Relevant owner docs updated or `No owner-doc update required` recorded.
- [ ] By independent sub-agent (fresh session) executed closure audit; execution session did not self-audit.
- [ ] `pnpm typecheck`
- [ ] `pnpm build`
- [ ] `pnpm lint`
- [ ] `pnpm test`
...
- Auditor / Agent: _(to be filled)_
- Evidence: _(to be filled)_

Follow-up:

- GanttStore Zustand migration successor plan
- Diff-view CSS definition plan
