...
- P4a. Linear 风格 issue tracker 复刻 — 分析与静态复刻: `planned`（plan `docs/plans/2026-08-29-1819-2-p4a-linear-tracker-static-replica.md`，2026-08-29 draft review 通过（fresh session `ses_fb2ecb628ffe9AVbcoSat7Gxin`，2 轮，R1 fail（1 Blocker：avatar 非 renderer 型别 + 3 Major：testid slug 口径/CSS 文件名 slug 化/Display 载体 popover 不可达）→ 修复后 R2 pass-with-minors 零 Blocker/Major；执行时序受 P3b 虚线约束已写入 plan））
