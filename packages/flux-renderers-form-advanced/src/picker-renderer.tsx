...
import { Button, cn, Empty, Input } from '@nop-chaos/ui';
...
export function PickerRenderer(props: RendererComponentProps<PickerSchema>) {
  ...
  const openDialog = React.useCallback(() => {
    ...
    setPending(new Set(multiple ? selectedValues : []));
    setQuery('');
    setOpen(true);
  ...
  const confirmSelection = React.useCallback(() => {
    if (multiple) {
      ...
      // single-select: pick the last toggled entry (or keep existing if none toggled)
      const nextSingle = Array.from(pending).pop();
      writeValue(nextSingle);
    }
    setOpen(false);
    void props.events.onPick?.();
  }, [multiple, pending, props.events, writeValue]);
          ...
          <DialogBody className="flex flex-col gap-2">
            ...
            <div className="max-h-72 min-h-32 overflow-y-auto rounded border border-border">
              {filteredOptions.length === 0 ? (
                ...
                <ul role="listbox" aria-multiselectable={multiple || undefined}>
                      ...
                      <li key={String(option.value)}>
                        ...
                        >
                          <input
                            ...
                            onChange={() => togglePending(option.value)}
