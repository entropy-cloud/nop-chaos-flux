...
- `selection`、`target` — `selectionTarget` 的兼容别名；仍作为 schema-visible convenience aliases 保留
...
- retained convenience mirrors:
  - top-level `workbook`, `activeSheet`, `canUndo`, `canRedo`, `documentName`, `fieldCount`, `inspector`, `meta`, `preview`
...
顶层便利字段 (`inspector`, `inspectorPanels`, `meta`, `canUndo`, `canRedo`, `documentName`, `fieldCount`) 是 `designer.*` 或 `runtime.*` 的镜像。
