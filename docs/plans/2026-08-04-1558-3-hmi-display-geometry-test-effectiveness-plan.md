...
- [x] `Fix` — `computeSymbolBounds`：对含 `custom.points` 的节点（line/arrow/polygon 等 points 几何族）从 points 数组计算 min/max 包围盒；无 width/height 但有 points 时不再退化为 0 尺寸。消费链（`applyInitialViewport` + 句柄 fit/center）自动收敛。
- [x] `Fix` — `dirty-collector.ts` `collectStates`：主绑定 scale 转发——经 reverse-index 取 primary binding，`resolveState(declaration, raw, { scale: binding.scale })`，且按 F4 语义只在声明级无 scale 时生效（或以同一 scale 对象为准）。
- [x] `Fix` — `text.ts` + instrument 数值标签：**以显式 width 为主路径**（自动宽下 autoSizeAlign/无 layoutWidth 使 textAlign 偏移不生效，已核实 installed leafer-ui@2.2.9 dist）——居中 Text 按内容测量设置 width（measureText 或标签固定宽度，取可验证者）；`~/sources/industrial-hmi-research/` leafer 源码核对仅作确认轮（ground truth 以已安装 dist 为准），不作为前置依赖。
- [x] `Fix` — canvas slot 落点：`ScadaCanvasEngine` 创建后把 `data-slot="scada-canvas-canvas"`（及 marker 语义）落到真实 canvas DOM 元素（Proof：leafer App 结构定位 canvas 元素——`app.canvas.view` 面已确认可用），wrapper div 保留 marker class 且 **`styles.css:9` 规则目标复核**（slot/marker 移动后 CSS 选择器命中面需一致，F8）；`design-renderer.md §10` 表同步（canvas 行 marker `nop-scada-canvas-canvas`、删除或注记永不渲染的 `scada-canvas-overlay` 行——覆盖物在 leafer sky 层）。

Exit Criteria:

- [x] ④ 组前置回归测试全绿入库；polygon fit 精确断言、scale 判定一致性断言、Text 居中断言、slot DOM 断言均为 repo-observable。
- [x] `design-renderer.md §10` 表与 live DOM marker/slot 一致（canvas marker 与 slot 落点相符、overlay 行移除或注记）。
- [x] 包级既有单测无回退（Phase 局部 typecheck + 相关 spec 复跑）。
...
- [x] `Proof` — 前置覆盖用例（对缺口清单逐项写断言）：`cacheImage`/`measureAddStrategies` 投影（scada-engine.ts:322-324,378）、`setPointValues` 注入通道（dev/test handle，**限于 mount 路径注入**——reload 后注入命中既有 stale-closure P2（plan `{2}` Phase 1 修复面），若覆盖用例先红于 stale-closure，该 finding 路由 plan `{2}` 而非本计划扩 scope，F5）、`onBuildError` catch（config 构建失败路径）、`center`-no-config（句柄无 bounds）、`validate.ts` 17 错误分支逐分支、`expression-evaluator.ts` 14 错误分支逐分支、switch OFF 位 applyProps。
- [x] `Fix` — `batch-add-probe` 单测：断言 probe 产物 **count + 返回 shape**（mock 下 `add()` 零耗时使 perNodeMs 舍入为 0、ratio 退化——**timing>0 断言保留在既有 e2e probe 真实计时处（scada-perf.spec.ts:367-369 断言 count/perNodeMs>0/batchMs>0）；ratio 维持观察项（e2e 不作 pass/fail 门禁）**，F6）。
- [x] `Fix` — `scada-canvas-smoke.test.tsx` 补 `resetLeaferMock()`（对齐唯一漏调消费者文件）。
- [x] `Proof` — 覆盖率验证：包级 `pnpm --filter @nop-chaos/flux-renderers-industrial test` coverage 阈值 90+ 维持（或缺口闭合后不低于基线），缺口清单逐项在 coverage 报告中映射。

Exit Criteria:

- [x] 缺口清单全部有断言覆盖（逐项可在 coverage 报告/测试文件中定位）；batch-add-probe 不再 0% 覆盖。
- [x] smoke 测试文件含 `resetLeaferMock()`；包级测试全绿、coverage ≥ 90 达标。
...
- [x] 显示几何收口：polygon bounds、scale 判定一致性、Text 居中、slot 落点（Phase 1 回归全绿 + §10 表一致）。
- [x] e2e 有效性收口：pan 断言非恒真、canvas 存在性断言、无 blanket console allowance（Phase 2 e2e 全绿）。
- [x] 覆盖缺口闭合：清单逐项有断言、batch-add-probe 覆盖、smoke 补 reset（Phase 3 全绿 + coverage ≥ 90）。
- [x] 不存在被静默降级到 deferred / follow-up 的 in-scope live defect 或 contract drift。
- [x] 必要 focused verification 已完成；受影响的 owner docs 已同步（design-renderer.md §10、roadmap-industrial-hmi.md）。
...
Status Note: 执行 session 完成全部三 Phase（Phase 1 由前轮收口、本轮执行 Phase 2/3）；workspace typecheck/build/lint/test 全绿；roadmap + design-renderer.md §10 已同步；closure-audit 交独立 fresh-session 子 agent 完成后方可将 Plan Status 改为 completed。

Closure Audit Evidence:

- Auditor / Agent: independent fresh-session subagent (closure-audit) — APPROVED.
  - Findings summary: 三 Phase 落地与 diff 摘要逐项核对一致（spot-check 非 trust）。Phase 2：`helpers/scada-canvas-assert.ts` 硬门禁（`<canvas>` tag + 非零尺寸 + `forceRender` 帧计数 > 0）+ best-effort 像素探测（SecurityError/全零 fallback F2，仅 log 不失败）逻辑正确；`scada-perf.spec.ts` 已无 `allowConsoleErrors` 调用（仅注释提及），pan 断言实读 `tree.zoomLayer.x/y` 前后偏移并断言 `viewportChanged`（TE-1 非恒真）；`playground-entry-pages.spec.ts` `ROUTES_WITH_KNOWN_ERRORS` 已移除 `scada-perf-scale`、保留其余 8 条（m3-r2）。Phase 3：`batch-add-probe.test.ts`（count/shape/双 Group.add 路径 instanceof 断言）、`scada-canvas-lifecycle-hardening.test.tsx`（onBuildError('config-build-failed') 经 applyDiff-throw 触发 + setPointValues 销毁后 no-op）、`scada-engine.test.ts`（cacheImage/resolveImageUrl/exportConfig-no-config/measureAddStrategies 投影）、`serialization.test.ts`（validate 9 错误分支）、`expression-evaluator.test.ts`（错误分支矩阵）、`sensor-control-symbols.test.ts`（switch true→false OFF 位）、`scada-canvas-smoke.test.tsx`（resetLeaferMock）均为真实断言非桩。docs：`design-renderer.md §10` canvas 行 marker/slot 一致（落在真实 `<canvas>`）、overlay 行注记移除；`roadmap-industrial-hmi.md` 11 处收口注记齐备。
  - Deferred 审定：唯一 deferred 项「leafer 自动宽 Text 居中的浏览器级像素验证」分类 `watch-only residual`、`Successor Required: no`（mock↔真实漂移候选面，非 in-scope live defect）；残余 uncoverage（`validate.ts:91-92`/`expression-evaluator.ts:288,303`/`use-scada-handles.ts:118`）均为防御性死守卫，非 live defect——无静默降级。
  - Re-run verification：`pnpm --filter @nop-chaos/flux-renderers-industrial test` → 562/562 全绿、coverage 97.29 stmts / 91.89 branches（≥90 阈值）；`typecheck` 清洁。Plan 文本内部一致（三 Phase/Exit/Closure Gates 全 `[x]`，仅 closure-audit gate 本项由本次审计勾选）。
- Executor Evidence（供审计复核）:
  - Phase 2 e2e：`npx playwright test scada-{demo,edge-cases,perf,pressure-demo}.spec.ts` 25/25 全绿 + `playground-entry-pages.spec.ts -g scada-perf-scale` 1/1 全绿；pan 断言经 `tree.zoomLayer.x/y` 视口变化证据（TE-1 实测 `zoomLayer offset changed across pan windows`）；`allowConsoleErrors(100)` ×5 与 `ROUTES_WITH_KNOWN_ERRORS` 的 `scada-perf-scale` 条目已移除，无 console.error/pageerror 失败。
  - Phase 3 包级：`pnpm --filter @nop-chaos/flux-renderers-industrial test` 562/562 全绿；coverage 97.29 stmts / 91.89 branches / 98.46 funcs / 99.37 lines（阈值 90 达标）；`batch-add-probe.ts` 0%→100%、`switch.ts` branch 87.5→100%、`validate.ts` 91.5→99、`expression-evaluator.ts` 93.19→98.95、`scada-engine.ts` 97.53→99.38；`scada-canvas-smoke.test.tsx` 已补 `resetLeaferMock()`。
  - Workspace：`pnpm typecheck` 32/32、`pnpm build` 32/32、`pnpm lint` 32/32、`pnpm test` 59/59 全绿。
  - 残余 uncoverage（防御性死代码/不可达守卫）：`validate.ts:91-92`（validateStateDeclaration 入口 isPlainObject 守卫，经 validateSymbolNode:233 前置守卫不可达）、`expression-evaluator.ts:288,303`（exhaustive switch 后的 default throw，不可达）、`use-scada-handles.ts:118`（exportConfig-no-config 守卫，Phase 3 author-less fallback 后不可达）—— 均非 in-scope live defect。

Follow-up:

- watch-only residual：leafer 自动宽 Text `textAlign:'center'` 的浏览器级像素验证（mock 面已验证 attrs，真实浏览器像素级复核属 mock↔真实漂移候选面，视觉对齐范畴、无契约损坏；随未来 e2e 像素级断言通道或 I16 编辑器轮自然覆盖）。无其余 plan-owned 残余工作。
