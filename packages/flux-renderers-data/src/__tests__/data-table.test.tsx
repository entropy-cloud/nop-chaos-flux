...
describe('dataRendererDefinitions table behavior', () => {
  it('renders row-scope actions that open dialogs with row data', async () => {
    ...
    render(<SchemaRenderer schema={{ type: 'page', body: [{ type: 'table', source: [{ id: 1, name: 'Alice' }, { id: 2, name: 'Bob' }], columns: [{ label: 'Name', name: 'name' }, { type: 'operation', label: 'Actions', buttons: [{ type: 'button', label: 'Inspect', onClick: { action: 'dialog', dialog: { title: 'Record details', body: [{ type: 'text', text: 'User: ${$slot.record.name}' }] } } }] }] }] }} env={env} formulaCompiler={formulaCompiler} />);
  ...
  it('dispatches row click events against the row scope', async () => {
    cleanup();
    const SchemaRenderer = createDataSchemaRenderer([buttonRenderer]);
    render(<SchemaRenderer schema={{ type: 'page', body: [{ type: 'table', source: [{ id: 1, name: 'Alice' }, { id: 2, name: 'Bob' }], onRowClick: { action: 'dialog', dialog: { title: 'Row click', body: [{ type: 'text', text: 'Selected ${record.name}' }] } }, columns: [{ label: 'Name', name: 'name' }] }] }} env={env} formulaCompiler={formulaCompiler} />);
  ...
  it('renders header, footer, and schema-based empty content through normalized regions', async () => {
    ...
    render(<SchemaRenderer schema={{ type: 'page', body: [{ type: 'table', header: [{ type: 'text', text: 'Table header' }], footer: [{ type: 'text', text: 'Table footer' }], empty: { type: 'text', text: 'No rows for ${team}' }, columns: [{ label: 'Name', name: 'name' }], source: [] }] }} data={{ team: 'Ops' }} env={env} formulaCompiler={formulaCompiler} />);
  ...
  it('renders plain-value empty content through value-or-region fallback', () => {
    cleanup();
    const SchemaRenderer = createDataSchemaRenderer();
    render(<SchemaRenderer schema={{ type: 'page', body: [{ type: 'table', empty: 'Nothing here', columns: [{ label: 'Name', name: 'name' }], source: [] }] }} env={env} formulaCompiler={formulaCompiler} />);
  ...
  it('renders schema-based column labels through compiled column regions', async () => {
    cleanup();
    const SchemaRenderer = createDataSchemaRenderer();
    render(<SchemaRenderer schema={{ type: 'page', body: [{ type: 'table', columns: [{ label: { type: 'text', text: 'Member ${team}' }, name: 'name' }], source: [{ id: 1, name: 'Alice' }] }] }} data={{ team: 'Roster' }} env={env} formulaCompiler={formulaCompiler} />);
  ...
  it('uses local pagination state by default', async () => {
    cleanup();
    const SchemaRenderer = createDataSchemaRenderer();
    render(<SchemaRenderer schema={{ type: 'page', body: [{ type: 'table', columns: [{ label: 'Name', name: 'name' }], source: [{ id: 1, name: 'Alice' }, { id: 2, name: 'Bob' }, { id: 3, name: 'Carol' }], pagination: { currentPage: 1, pageSize: 1, pageSizeOptions: [1] } }] }} env={env} formulaCompiler={formulaCompiler} />);
    expect(screen.getByText('Alice')).toBeTruthy();
  ...
  it('uses controlled pagination when configured and waits for external prop updates', async () => {
    cleanup();
    const SchemaRenderer = createDataSchemaRenderer();
    const controlledSchema = { type: 'page', body: [{ type: 'table', paginationOwnership: 'controlled', onPageChange: { action: 'setValue', componentPath: 'pageState', value: '${page}' }, columns: [{ label: 'Name', name: 'name' }], source: [{ id: 1, name: 'Alice' }, { id: 2, name: 'Bob' }, { id: 3, name: 'Carol' }], pagination: { currentPage: '${pageState || 1}', pageSize: 1, pageSizeOptions: [1] } }] } as const;
    const { rerender } = render(<SchemaRenderer schema={controlledSchema} data={{ pageState: 1 }} env={env} formulaCompiler={formulaCompiler} />);
    fireEvent.click(document.querySelector('[data-slot="table-pagination"] [aria-label="Go to next page"]')!);
    rerender(<SchemaRenderer schema={controlledSchema} data={{ pageState: 2 }} env={env} formulaCompiler={formulaCompiler} />);
    await waitFor(() => expect(screen.getByText('Bob')).toBeTruthy());
  });
  ...
  it('uses scope-backed pagination when configured and updates through scope state', async () => {
    cleanup();
    const SchemaRenderer = createDataSchemaRenderer();
    render(<SchemaRenderer schema={{ type: 'page', body: [{ type: 'table', paginationOwnership: 'scope', paginationStatePath: 'tableState.pagination', columns: [{ label: 'Name', name: 'name' }], source: [{ id: 1, name: 'Alice' }, { id: 2, name: 'Bob' }, { id: 3, name: 'Carol' }], pagination: { currentPage: 1, pageSize: 1, pageSizeOptions: [1] } }, { type: 'text', text: 'Page state: ${tableState.pagination.currentPage}' }] }} data={{ tableState: { pagination: { currentPage: 1, pageSize: 1 } } }} env={env} formulaCompiler={formulaCompiler} />);
    fireEvent.click(document.querySelector('[data-slot="table-pagination"] [aria-label="Go to next page"]')!);
    await waitFor(() => {
  ...
  it('uses local row selection state by default', async () => {
    cleanup();
    const SchemaRenderer = createDataSchemaRenderer();
    render(<SchemaRenderer schema={{ type: 'page', body: [{ type: 'table', rowSelection: { type: 'checkbox', selectedRowKeys: [] }, columns: [{ label: 'Name', name: 'name' }], source: [{ id: 1, name: 'Alice' }, { id: 2, name: 'Bob' }] }] }} env={env} formulaCompiler={formulaCompiler} />);
  ...
  it('uses controlled row selection and waits for external prop updates', async () => {
    ...
    const { rerender } = render(<SchemaRenderer schema={controlledSchema} data={{ selectedKeys: [] }} env={env} formulaCompiler={formulaCompiler} />);
    const initialCheckboxes = document.querySelectorAll('[data-slot="checkbox"]');
    fireEvent.click(initialCheckboxes[1]!);
    rerender(<SchemaRenderer schema={controlledSchema} data={{ selectedKeys: ['1'] }} env={env} formulaCompiler={formulaCompiler} />);
    await waitFor(() => {
  ...
  it('exposes table selection through component handle actions', async () => {
    cleanup();
    const SchemaRenderer = createDataSchemaRenderer([buttonRenderer]);
    render(<SchemaRenderer schema={{ type: 'page', body: [{ type: 'table', id: 'users-table', rowSelection: { type: 'checkbox', selectedRowKeys: [] }, columns: [{ label: 'Name', name: 'name' }], source: [{ id: 1, name: 'Alice' }, { id: 2, name: 'Bob' }] }, { type: 'button', label: 'Select Alice', onClick: { action: 'component:setSelection', componentId: 'users-table', args: { selectedRowKeys: ['1'] } } }, { type: 'button', label: 'Read Selection', onClick: { action: 'component:getSelection', componentId: 'users-table', then: { action: 'setValue', componentPath: 'selectionResult', value: '${result.data}' } } }, { type: 'text', text: '${selectionResult}' }] }} env={env} formulaCompiler={formulaCompiler} />);
  ...
  it('renders schema-based column cells through compiled cell regions with row scope', async () => {
    cleanup();
    const SchemaRenderer = createDataSchemaRenderer();
    render(<SchemaRenderer schema={{ type: 'page', body: [{ type: 'table', columns: [{ label: 'Summary', name: 'name', cell: { type: 'text', text: 'Member ${$slot.record.name}' } }], source: [{ id: 1, name: 'Alice' }] }] }} env={env} formulaCompiler={formulaCompiler} />);
  ...
  it('keeps table root marker non-visual and merges schema className onto the root', () => {
    cleanup();
    const SchemaRenderer = createDataSchemaRenderer();
    render(<SchemaRenderer schema={{ type: 'page', body: [{ type: 'table', className: 'stack-sm border', columns: [{ label: 'Name', name: 'name' }], source: [] }] }} env={env} formulaCompiler={formulaCompiler} />);
  ...
  it('propagates repeated table row instancePath into row child nodes', async () => {
    ...
    render(<SchemaRenderer schema={{ type: 'page', body: [{ type: 'table', columns: [{ type: 'operation', buttons: [{ type: 'button', label: 'Row action' }] }], source: [{ id: 1, name: 'Alice' }] }] }} env={env} formulaCompiler={formulaCompiler} onComponentRegistryChange={onComponentRegistryChange} />);
  ...
  it('passes row instancePath through helpers.dispatch action context', async () => {
    ...
    render(<SchemaRenderer schema={{ type: 'page', body: [{ type: 'table', columns: [{ label: 'Dispatch', cell: { type: 'dispatch-probe' } }], source: [{ id: 1, name: 'Alice' }] }] }} env={env} formulaCompiler={formulaCompiler} onActionScopeChange={registerProbeNamespace(observedLocators)} />);
  ...
  it('uses schema rowKey as stable repeated identity instead of source index', async () => {
    ...
    render(<SchemaRenderer schema={{ type: 'page', body: [{ type: 'table', rowKey: '__rowKey', columns: [{ label: 'Probe', cell: { type: 'node-instance-probe' } }], source: [{ id: 99, __rowKey: 'client-a', name: 'Alice' }] }] }} env={env} formulaCompiler={formulaCompiler} />);
  ...
  it('reuses one stable row scope per materialized row key', async () => {
    ...
    const { rerender } = render(<SchemaRenderer schema={{ type: 'page', body: [{ type: 'table', rowKey: 'id', columns: [{ label: 'Scope', cell: { type: 'row-scope-id-probe' } }], source: [{ id: 1, name: 'Alice' }] }] }} env={env} formulaCompiler={formulaCompiler} />);
    const initialScopeId = (await screen.findByTestId('row-scope-id-probe')).textContent;
    rerender(<SchemaRenderer schema={{ type: 'page', body: [{ type: 'table', rowKey: 'id', columns: [{ label: 'Scope', cell: { type: 'row-scope-id-probe' } }], source: [{ id: 1, name: 'Alice updated' }] }] }} env={env} formulaCompiler={formulaCompiler} />);
