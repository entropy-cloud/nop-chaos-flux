...
- `packages/flux-renderers-data/src/crud-renderer.tsx` 当前仍是 thin shell：只渲染 query form、toolbar/listActions、内部 table，发布最小 `$crud` 摘要，并桥接 `refresh/getSelection/clearSelection` 三个 component methods。
- 当前 `crud-renderer.tsx` 的 source 读取仅直接接受数组；没有把 `queryForm`、`source`、分页、排序、过滤、刷新参数真正接成完整数据工作流。
- 当前 `crud-renderer.tsx` 没有渲染 `footerToolbar`，也没有消费 `toolbarLayout`、`selection`、`pageField`、`pageSizeField`、`defaultParams` 等正式字段的运行时语义。
- 当前 `$crud` 摘要只发布 `loading/refreshing/itemCount/total/hasSelection/selectionCount/selectedRowKeys`；设计里已经定义的 `query/pagination/sort/filters/visibleColumnNames` 还没有接线到 live runtime。
- `packages/flux-renderers-data/src/schemas.ts` 与 table renderer 已有分页、排序、过滤、selection、expandable 的基础能力；`operation` 列按钮也已经有 live rendering，但 fixed columns、column settings、responsive more-columns expansion、header search/filter UI、quick edit 仍未全部收口。
...
- 本计划明确只收口 CRUD workflow 主路径，并把 `operation` 列稳定性与 fixed columns 这两个直接影响 CRUD 主列表可用性的依赖一并收口；其余 table-heavy parity 与 editing-heavy parity 仍保留给 successor plans。
...
- 不在本计划中收口 column settings、responsive more-columns expansion、header search/filter UI。
...
- column settings、responsive more-columns expansion、header search/filter UI
...
- [ ] 让 `queryForm` 提交和重置真正驱动 CRUD refresh 参数，而不是仅渲染内部 form shell
- [ ] 让 refresh 路径统一消费 query/default params/`pageField`/`pageSizeField`，而不是只刷新本地数组 table
- [ ] 明确并实现本计划内的最小数据工作流基线：至少支持数组型 `source` 加上 CRUD 内部 query/pagination/sort/filter 参数流，并用 focused tests 证明 refresh 输入会随之变化
- [ ] 补齐 query submit、query reset、refresh 的 focused tests

Exit Criteria:

- [ ] 至少一条 focused test 证明 query submit 会影响下一次 refresh 输入
- [ ] 至少一条 focused test 证明 query reset 会清理下一次 refresh 输入，而不是只清 UI
...
- [ ] 打通 `source`、分页、排序、过滤、selection 与 `$crud/statusPath` 摘要的联动发布
- [ ] 补齐 `footerToolbar`、`toolbarLayout`、`listActions` 的 live render 与标准块协作
- [ ] 让 selection-driven list actions 的 enable/disable 与 refresh 后清理策略具备 focused tests
- [ ] 确认 `operation` 列在 CRUD 场景下继续保持 live behavior，并补充 focused regression coverage

Exit Criteria:

- [ ] `$crud` 能稳定发布 query/pagination/sort/filters/selection 摘要，并能被 toolbar/listActions 消费
- [ ] `footerToolbar` 不再只是 schema 字段，而是 live render behavior
- [ ] `toolbarLayout` 至少有一条 focused test 或 playground-observable 结果，证明标准 toolbar block 已能稳定渲染
- [ ] 至少一条 focused test 证明 CRUD 表格中的 `operation` 列按钮仍可稳定渲染与交互
...
- [ ] 让 `table`/`crud` 真正消费 `columns[].fixed`，而不是只在 schema surface 暴露字段
- [ ] 支持 left/right fixed columns 的 header/body 对齐、selection/expand 列并存场景，以及 operation 列作为 fixed right 常见用法
- [ ] 增加 focused tests，证明左侧固定列和 operation 固定列在 CRUD 主列表中具备稳定 DOM/behavior 基线

Exit Criteria:

- [ ] 至少一条 focused test 证明 `fixed: 'left'` 在 CRUD/table live DOM 中可观察成立
- [ ] 至少一条 focused test 证明 `type: 'operation'` 列可与 fixed columns 共存而不回退成普通列
- [ ] playground 或 example 能人工复核 fixed columns 在 CRUD 主列表中的可用性
...
- [ ] 把 `filter -> queryForm`、`primaryField -> rowKey`、`perPageField -> pageSizeField` 加入 `crud` authoring transform
- [ ] 只在本计划已具备 live workflow 语义的字段上增加 alias；不为未实现能力预埋误导性 canonicalization
- [ ] 增加 focused compiler tests，证明 alias 输入会 lower 到当前 live canonical fields

Exit Criteria:

- [ ] 常见 AMIS CRUD workflow 字段在当前支持范围内可直接 canonicalize 到 Flux schema
- [ ] 迁移 alias 与 live runtime 支持范围严格一致
...
- [ ] 在 playground 增加可人工复核的 CRUD workflow 示例页，包括 `operation` 列和 fixed columns 的可观察结果
- [ ] 重写 CRUD 设计文档中的特性对比表，只把本计划已真正落地的 workflow 项标成已实现
- [ ] 同步 example 和 migration-example，使其与 live behavior 和 authoring transform 一致

Exit Criteria:

- [ ] playground/examples/docs 能证明 CRUD workflow 主路径已可用，且包含 fixed columns / operation 列基线
- [ ] 文档不再把 table-heavy parity 或 editing-heavy parity 误写成本计划已完成
...
- [ ] 扩充 CRUD focused tests，至少覆盖查询、重置、刷新、分页/排序/过滤摘要发布、selection-driven list actions、fixed columns、operation regression、migration alias
- [ ] 运行完整验证并更新 daily log
...
- [ ] CRUD 查询、刷新、分页、排序、过滤、selection 工作流具备 focused behavior tests
- [ ] `operation` 列与 fixed columns 具备 focused behavior tests
- [ ] `authoringTransform` 已承接关键 AMIS/legacy CRUD workflow alias，且有 compiler tests
- [ ] `docs/components/crud/design.md`、`docs/components/crud/example.json`、`docs/components/crud/migration-example.json`、playground 示例已同步
- [ ] 特性对比表已按 live behavior 重写，不再把“契约已定义”误写成“已实现”
