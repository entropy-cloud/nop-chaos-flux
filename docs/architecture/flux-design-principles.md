...
Flux 的核心执行模型是响应式的，而且是声明式的。作者不需要显式搭建命令式联动过程——只要一个动态值通过 name 或 `${expr}` 读取了 scope 中的路径，它就自动落入依赖图，依赖变化时被重新求值。
...
**FormRuntime** 组合了 Action Algebra（分支调度）、Value（name 绑定的值读写和值约束）、Capability（提交动作），以及自身特有的子树契约、触摸策略、错误显示策略等逻辑。
