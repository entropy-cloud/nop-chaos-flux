...
帧尾批量写入（rAF/节流窗口结束）：
  ...
  - **{binding,state} > animation 同属性优先级契约**（plan 2026-08-06-0900-3 Phase 3 open P2-1）：当同一图元同一属性同时被 binding/state 合帧与 animation 增量写入时，**binding/state 胜出**。事实源：animator 经 `collect:(entry)=>collector.collect(entry)` 把当帧增量写入**同一个** `pending` Map（`Map<symbolId, Map<property, value>>`，last-write-wins per `(symbolId, property)`）；`flushFrame` 内 `collectBindings`/`collectStates` **先于** `collector.flush` 执行（dirty-collector.ts flushFrame：collectBindings → collector.flush），故脏帧 binding/state 条目覆盖同属性的 animator 当帧增量。该 precedence **确定且可预期**（固定 collect 顺序 + last-write-wins Map，非 RNG）。animation 仅在**无 binding/state 覆盖的属性**上正常生效（如 `dashOffset` flow 动画）。author 按此契约组态：同属性的 binding 与 animation 会产 inter-frame 视觉跳变（animation 增量在脏帧被 binding 覆盖、在纯 animation 帧生效），若需 animation 不被 binding 覆盖应使用不同属性。focused 回归守护：`refresh-pipeline-animation-lifecycle.test.ts` 三组（同属性 binding 覆盖 + 无 binding 属性 animation 正常 + 多帧跨帧稳定性）。**不新增** validate/registration warn（precedence 确定可预期，warn 反而 noisy）。未来若引入 animation "decorative override only when no binding" 语义改写（含无 binding 属性短路径性能优化）需重开 Decision 经人工确认（Non-Goal）。
  │
  ▼
