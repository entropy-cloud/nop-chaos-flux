...
- [ ] (Decision) 裁定内核 API（对齐 `ScadaEditorSession` 模式，hmi 迁移兼容）：
...
- [ ] (Fix) 实现 `editor-core` 包（骨架：package.json/tsconfig/build 脚本/graph 包模板），不含任何 DOM/leafer/领域类型。
- [ ] (Fix) 注册别名与根 tsconfig references；`docs/architecture/editor-core.md` 描述最终 API（会话/undo/适配器/双态）与拒绝的替代方案（如复用 ScadaEditorSession 原样、meta2d 历史栈直搬）。
- [ ] (Follow-up) 对照 `editor-session.ts`/`undo-stack.ts` 逐项核对 API 覆盖（迁移就绪矩阵初版，Phase 5 完善）。

Exit Criteria:

- [ ] `editor-core` 单测全绿（会话/undo/适配器/双态契约）。
- [ ] `docs/architecture/editor-core.md` 与 live API 一致。
...
- [ ] (Decision) 裁定运行态布局方案：复用 `grid` renderer（CSS grid flow，语义简单但无绝对坐标）vs 自研绝对定位 + 网格对齐渲染（对齐编辑态坐标模型，编辑/运行同构）——以"编辑态坐标 → 运行态渲染零转换"为判据，倾向自研（DashboardLayout 坐标模型单一来源）。
- [ ] (Decision) 裁定 `DashboardLayoutSchema`：`panels: Array<{ id; type; title?; x; y; w; h; props?: SchemaValue; source?: SchemaValue }>` + `cols?`/`rowHeight?`/`gap?` + `empty`/`height`。
- [ ] (Proof) failing 单测：布局坐标模型（x/y/w/h 网格换算、越界 clamp、重叠检测）、面板渲染映射（type → renderer、props/source 表达式求值）、空布局 empty slot。
- [ ] (Fix) 实现运行态 `dashboard` renderer（type `dashboard`，category `layout`）+ definitions 注册（包级 `registerDashboardRenderers`）。

Exit Criteria:

- [ ] 布局计算单测全绿；`dashboard` 运行态在 playground 以静态布局 JSON 可渲染。
...
- [ ] 编辑态走查通过：拖拽/缩放/吸附/undo/redo/删除/保存/预览切换全链路可用。
- [ ] 坐标纯函数 + editor-core 集成单测全绿。
...
- [ ] (Fix) INV-1~5 审计（无新 IO；内部 state 域内持有；RendererComponentProps 契约）。

Exit Criteria:

- [ ] playground 示例可运行（手测清单全过）；design.md + example.json 与 live 行为一致。
- [ ] 分析报告与 daily log 同步；INV 审计无遗留。
...
- [ ] (Proof) API 覆盖走查：`editor-core` 会话/undo/提交语义逐一对照 `ScadaEditorSession`/`UndoStack`/`cloneConfigSnapshot`/`ScadaCommitPolicy`，产出覆盖矩阵（直接映射/需适配/缺口）。
- [ ] (Decision) 裁定迁移路径：a) 全量迁移（editor-core 替代 ScadaEditorSession，SCADA 域实现 `ScadaDomainAdapter`）；b) 双轨（先 adapter 包装，后替换）；给出建议与风险（30+ 测试文件回归面）。
- [ ] (Fix) 迁移实施 = successor plan（记录路径、范围、验证策略；本计划不做实施）。
- [ ] (Follow-up) 记录 workbench 外壳统一（hmi editor 三段式对齐 WorkbenchShell）为 successor 范围项。

Exit Criteria:

- [ ] 覆盖矩阵完成且无未标注缺口；迁移 successor plan 路径记录在案。
- [ ] 本计划内无 hmi 代码变更（实施归 successor）。
...
- [ ] `editor-core` 单测全绿（会话/undo/适配器/双态）；dashboard 布局与编辑交互 focused 单测全绿。
- [ ] playground BI 看板示例（编辑↔运行、保存还原、数据联动）手测通过。
- [ ] hmi 迁移覆盖矩阵完成；迁移路径 successor plan 已记录。
- [ ] `docs/architecture/editor-core.md` + `docs/components/dashboard-editor/design.md` + example.json 与 live baseline 一致。
- [ ] INV-1~5 审计通过。
- [ ] 不存在被静默降级到 deferred / follow-up 的 in-scope 行为缺口。
- [ ] 受影响的 owner docs 已同步（分析报告、daily log）。
