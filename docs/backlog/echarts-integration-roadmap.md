...
| ID   | Status | 内容                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | 设计文档                                                | 依赖 |
| ---- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- | ---- |
| E1.1 | done   | `echarts` 渲染器骨架：基础组件（init/setOption/resize/dispose 生命周期）、schema 类型定义、结构验证、渲染器注册（与 `chart` 并存）；ECharts 作为可选依赖按需引入。**决策已裁决：按需引入粒度 = 按功能模块**（echarts/core + charts + components + renderers 官方 tree-shaking 入口集中注册于单一 setup 模块，渲染器懒加载）。落地：`flux-renderers-data/src/echarts-*.ts(x)`；plan `docs/plans/2026-09-13-2343-echarts-e1-1-renderer-skeleton-plan.md`（closure audit approved 2026-09-13） | `analysis/echarts-migration-analysis.md`（§二/§三/§四） | E0.1 |

### E2 — 核心图表

| ID   | Status | 内容                                                                                                                                                                                                                                                                                                                                                                                                                                           | 设计文档                                            | 依赖 |
| ---- | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------- | ---- |
| E2.1 | done   | dataset + encode 数据映射（对象数组 / 列式 / 二维数组，表达式绑定）、基础图表（bar/line/pie/scatter）dataset 形态落地、tooltip/legend 原生透传 + legendselectchanged 事件通道、`empty` 插槽与空 dataset 显式空态。**决策已裁决：统一主题 token = CSS 变量 → `flux` 注册主题**（默认应用，指定透传）。事件桥接（A3）一并落地。plan `docs/plans/2026-09-13-2350-echarts-e2-1-dataset-binding-theme-plan.md`（closure audit approved 2026-09-13） | `analysis/echarts-migration-analysis.md`（§三/§四） | E1.1 |
...
| ID   | Status  | 内容                                                                                                                               | 设计文档                                                | 依赖 |
| ---- | ------- | ---------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- | ---- |
| E4.1 | planned | 地理图 (map) + GeoJSON 注册、K 线图 (candlestick)、力导向图 (graph)、旭日图 (sunburst)、主题河流 (themeRiver)、自定义图表 (custom) | `analysis/echarts-migration-analysis.md`（§五 Phase 4） | E2.1 |
