...
- `detail-field` / `detail-view` 当前是 compiler-classified detail child-owner baseline：打开 detail session 时由 renderer 实例化 child `FormRuntime`，但 owner boundary 与 parent child-contract mode 已由 compiler/runtime contract 承载
...
- 这是 target performance baseline，不是当前已严格落地的实现事实
- 当前 live implementation 在打开时仍会基于适配后的值创建 temporary draft `FormRuntime` initial values snapshot
...
在 `docs/architecture/data-domain-owner.md` 的术语下，它应被理解为：

- current staged child-domain baseline
- child owner 默认 `seed-on-open`
- child owner 对 parent 默认执行 `commit-only` 或 owner-specific `publish-patch`
- 当前 live implementation 是 detail renderer 触发实例化 child `FormRuntime`，但 owner boundary 与 parent coordination 已按 compiler/runtime child-owner baseline 工作
- 当前 live writeback target 仍然要求 parent `FormRuntime`，也就是当前更准确的说法是 form-backed staged child owner baseline
...
- 当前 live `detail-field` 在 `readOnly=true` 时允许打开 detail surface 查看内容
- 但只读 surface 不提供 confirm/writeback，仍不应误读为“只读编辑”
...
在 `Data Domain Owner` 语义下，`detail-view` 不是“又开一个 scope”那么简单，而是：

- 对一组局部数据建立 staged child-domain boundary
- 在 confirm 前保持 child-owned working state 与 validation state
- confirm 后只把 owner-defined commit result 写回 immediate parent owner
- dialog / drawer 只是承载这个 child owner 的 surface，不是 owner 本身
...
- 当前 live `detail-view` 在 `readOnly=true` 时同样允许打开 detail surface 查看内容
- 但只读 surface 不提供 confirm/writeback，仍不应误读为“只读编辑”
...
- 上面是更严格的 target contract
- 当前 live `detail-view` 更宽：如果没有 `patch` / `updates`，仍会 fallback 到 whole-object replacement、root-level key merge，或直接应用 raw draft value
- 因此不要把“只允许显式 commit result”误读成当前已完全执行的限制
...
- draft form 是 renderer-local temporary `FormRuntime`
