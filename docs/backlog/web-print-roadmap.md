...
| ---- | ------ | ------------------------------------------------------ | ------------------------------------- | ---- |
| P0.1 | done   | 阅读 `~/sources/print/` 中所有项目的 README 和核心代码 | —                                     | —    |
| P0.2 | done   | 提取各项目的技术栈、架构设计、核心功能对比             | 调研报告                              | P0.1 |
| P0.3 | done   | 分析本项目现有设计器框架可复用的部分                   | 调研报告                              | P0.2 |
| P0.4 | done   | 设计 flux-json 打印模板 schema                         | `docs/components/print/design.md`     | P0.3 |
| P0.5 | done   | 编写调研报告文档                                       | `docs/analysis/web-print-research.md` | P0.2 |

### P1 — 基础设施搭建

| ID   | Status | 内容                                                                                                            | 设计文档                          | 依赖 |
| ---- | ------ | --------------------------------------------------------------------------------------------------------------- | --------------------------------- | ---- |
| P1.1 | done   | 创建 `flux-print-core` 包（package.json、tsconfig、vitest、schemas.ts、index.ts）                               | —                                 | P0.4 |
| P1.2 | done   | 创建 `flux-print-renderers` 包（package.json、tsconfig、vitest、schemas.ts、renderer-definitions.ts、index.ts） | —                                 | P1.1 |
| P1.3 | done   | 配置包依赖和构建，注册到 workspace                                                                              | —                                 | P1.2 |
| P1.4 | done   | 实现打印模板 schema 编译器                                                                                      | `docs/components/print/design.md` | P1.1 |

### P2 — 打印设计器

| ID   | Status | 内容                                           | 设计文档                          | 依赖       |
| ---- | ------ | ---------------------------------------------- | --------------------------------- | ---------- |
| P2.1 | done   | 实现打印设计器画布组件（纸张、标尺、网格）     | `docs/components/print/design.md` | P1.2       |
| P2.2 | done   | 实现组件面板（文本、图片、表格、条码、二维码） | `docs/components/print/design.md` | P2.1       |
| P2.3 | done   | 实现属性面板（位置、尺寸、样式、数据绑定）     | `docs/components/print/design.md` | P2.1       |
| P2.4 | done   | 实现打印预览功能                               | `docs/components/print/design.md` | P2.2, P2.3 |
| P2.5 | done   | 实现撤销/重做（复用 editor-core）              | —                                 | P2.1       |

### P3 — 打印渲染器

| ID   | Status | 内容                                     | 设计文档                          | 依赖 |
| ---- | ------ | ---------------------------------------- | --------------------------------- | ---- |
| P3.1 | done   | 实现打印渲染器（模板 + 数据 → HTML）     | `docs/components/print/design.md` | P1.2 |
| P3.2 | done   | 实现浏览器打印 API 调用                  | —                                 | P3.1 |
| P3.3 | done   | 实现 PDF 导出功能（html2canvas + jspdf） | —                                 | P3.1 |
| P3.4 | done   | 支持分页和表格分组                       | `docs/components/print/design.md` | P3.1 |

### P4 — Playground 集成与 E2E 测试

| ID   | Status | 内容                       | 设计文档 | 依赖       |
| ---- | ------ | -------------------------- | -------- | ---------- |
| P4.1 | done   | 创建打印设计器演示页面     | —        | P2.4, P3.2 |
| P4.2 | done   | 添加路由入口到 playground  | —        | P4.1       |
| P4.3 | done   | 编写 e2e 测试用例          | —        | P4.2       |
| P4.4 | done   | 运行测试验证，确保全量通过 | —        | P4.3       |
...
| 3   | **myprint**            | Apache 2.0  | [gitee](https://gitee.com/MyPrintDesign/myprint)              | Vue 3, TypeScript, Vite                | 一站式方案，支持多级表头、SVG、手画板                         | 架构设计参考（实测 LICENSE.txt 为 Apache 2.0；可参考代码，2026-09-05 修正原"无 LICENSE"误记） |
