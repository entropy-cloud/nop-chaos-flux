...
| P3-1 | P3       | `switch.ts:60-62`                                  | resize hook 用 `lever.x !== 3` 推断 on/off，width===height 时 on 位 x=3 误判 off（边缘 case）                                                                      | residual-adjudicated at HCA-CR（watch-only，默认 48×28 不触发，Successor: no）                            |
| P3-2 | P3       | `level.ts:42`/`thermometer.ts:50`/`progress.ts:44` | extent 族容器 width/height 变更丢失 create-time 内边距（cosmetic，容器 resize 非主绑定用例）                                                                       | residual-adjudicated at HCA-CR（cosmetic watch-only，Successor: no）                                      |
| P3-3 | P3       | `polygon.ts:41`                                    | `custom.points=[]` 空数组 → 空图形无 fallback（author-controlled）                                                                                                 | residual-adjudicated at HCA-CR（author-controlled watch-only，Successor: no）                             |
| P3-4 | P3       | `composite.ts:53-56`/`common.ts:20-22`             | rotation/几何 NaN 无守卫直传 leafer（defense in depth，binding 层应产合法值）                                                                                      | residual-adjudicated at HCA-CR（纵深冗余，validate finite 已守，Successor: no）                           |
