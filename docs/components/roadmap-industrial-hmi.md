...
- `2026-08-05-2129-open-audit-industrial-hmi.md` `[P2-7]` — `video.ts:34-36` create 无条件覆盖 author `stroke`/`strokeWidth`（与上方 guarded `fill` 行不对称）。建议 `if (attrs.stroke===undefined) attrs.stroke=...`。
- `2026-08-05-2129-open-audit-industrial-hmi.md` `[P2-8]` — `pipe-junction.ts:85` stubs 硬编码 `strokeWidth:4` 且 `applyProps` 不路由 → strokeWidth 变更加粗 body 但 stubs 留细。建议 stub `strokeWidth` 从 `props.strokeWidth ?? 4` 派生 + applyProps 路由 stubs。
- `2026-08-05-2129-open-audit-industrial-hmi.md` `[P2-9]` — `thermometer.ts:42`(create `-24`) vs `:78`(update `-16`) liquid-anchor 常量分歧，空液位首次 binding tick 跳 8px（sibling `level.ts:42,66` 两路径一致，thermometer 离群）。建议统一单一 `BULB_RESERVE` 常量。
- `2026-08-05-2129-open-audit-industrial-hmi.md` `[P2-10]` — `getSymbol` 读 handle 返回 leafer-internal 属性名（`fontSize`/`scaleX+scaleY`/`dashPattern`/`fill`-for-textColor）却 `as ScadaSymbolProps` 强转 → read/write field-name 不对称，host getSymbol→setSymbolProps 往返喂错键。建议 `getSymbolProps` 反映射回 schema 名（`toNodePatch` 逆），或文档化为 raw leafer attrs 去 cast。
