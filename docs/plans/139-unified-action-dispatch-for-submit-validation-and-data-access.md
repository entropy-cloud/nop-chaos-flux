...
- [ ] 在 `validation.ts` 中将 `{ kind: 'async'; api: ApiSchema; ... }` 替换为 `{ kind: 'async'; action: ActionSchema; debounce?: number; message?: string }`
- [ ] 在 runtime validation 执行中，`kind: 'async'` 统一走 `runtime.dispatch(action)` 而非直接调 `executeApiSchema`
- [ ] 更新所有使用 `kind: 'async'` + `api` 的测试，改为使用 `action: { action: 'ajax', args: { url, method } }` 或自定义 action
- [ ] 更新 `docs/architecture/form-validation.md` 中 async validation 相关描述

Exit Criteria:

- [ ] `validation.ts` 中不再有 `{ kind: 'async'; api: ApiSchema }` 变体
- [ ] 所有 async validation 走 `runtime.dispatch(...)` 执行
- [ ] 所有 validation 相关测试通过
...
- [ ] 从 `SubmitFormInput` 接口中删除 `submitApiCall` 字段
- [ ] 从 `form-runtime-submit-flow.ts` 的 `executeFormSubmit` 中删除 `api` 参数和 `submitApiCall` 分支，只保留 `submitAction` 和 fallback-to-values 两条路径
- [ ] 从 `form-runtime.ts` 的 `submit()` 中删除 `api` 参数
- [ ] 从 `runtime-factory.ts` 中删除 `submitApi` 工厂注入和相关 lambda
- [ ] 从 `form-runtime-types.ts` 中删除 `submitApi` 相关类型
- [ ] 更新 form renderer（`form.tsx`）中的 lifecycle handler 映射
- [ ] 更新所有 submit 相关测试，从 `form.submit(api)` 改为通过 `submitAction` 配置 ajax action

Exit Criteria:

- [ ] `executeFormSubmit` 不再有 `api` 参数
- [ ] `form.submit()` 不再接受 `api` 参数
- [ ] submit flow 只有 `submitAction`（有就用）和 return-values（没有就返回）两条路径
- [ ] 所有 submit 相关测试通过
...
- [ ] 从 `ActionDataSourceSchema` 中删除 `api: ApiSchema` 字段
- [ ] 从 `DataSourceSchema` 的其他承载位置删除 `api`
- [ ] 从 `FormLifecycleHandlers` 相关的 runtime factory input 中删除 `submitApi` 字段
- [ ] 更新 data-source renderer 和相关 runtime 代码，不再处理 `api` 字段
- [ ] 更新所有 data-source 测试，从 `api: { url }` 改为 `action: "ajax", args: { url }`
- [ ] 更新所有 playground 示例和 docs 中的 `api` 写法

Exit Criteria:

- [ ] schema 类型中不再有 `api: ApiSchema` 字段（`ApiSchema` 类型定义本身保留）
- [ ] 所有 data-source 测试通过
- [ ] playground 示例使用 `action: "ajax"` 写法
...
- [ ] 更新 `docs/architecture/flux-runtime-module-boundaries.md` 中 form-runtime 的模块描述
- [ ] 确认 `docs/architecture/api-data-source.md` 中所有示例使用 `action` 写法，无残留 `api` 属性
- [ ] 确认 `docs/architecture/form-validation.md` 中 async validation 描述使用 `action` 写法
- [ ] 确认 `docs/components/*/design.md` 中无残留 `api` 属性引用
- [ ] `pnpm typecheck && pnpm build && pnpm lint && pnpm test` 全部通过

Exit Criteria:

- [ ] 所有公开类型中不再有 `submitApi` / `submitApiCall` / schema-level `api` 字段
- [ ] `ApiSchema` 不再出现在 submit / validation / data-source 的公开 API 签名中
- [ ] 所有文档示例使用 `action` 写法
- [ ] `pnpm typecheck && pnpm build && pnpm lint && pnpm test` 通过

## Validation Checklist

- [ ] Form submit 走 action dispatch 一条路径，无旁路
- [ ] Async validation 走 action dispatch，本地/远程统一
- [ ] Data-source 不再有 `api` 字段，统一用 `action: "ajax"`
- [ ] `ApiSchema` 只被 `ajax` action 内部消费
- [ ] 所有 runtime 测试通过
- [ ] 所有 form-renderer 测试通过
- [ ] 所有 data-source 测试通过
- [ ] 所有文档示例与实现一致
