...
describe('basicRendererDefinitions reaction and action behavior', () => {
  it('rerenders scope-debug when the current scope changes', async () => {
    ...
    render(<SchemaRenderer schemaUrl="test://basic/reactions" schema={{ type: 'page', body: [{ type: 'button', label: 'Increment', onClick: { action: 'setValue', componentPath: 'count', value: '${count + 1}' } }, { type: 'scope-debug', title: 'Probe', defaultExpand: true }] }} data={{ count: 0 }} env={env} formulaCompiler={formulaCompiler} />);
  ...
  it('runs reaction actions immediately when configured', async () => {
    const SchemaRenderer = createBasicSchemaRenderer();
    render(<SchemaRenderer schemaUrl="test://basic/reactions" schema={{ type: 'page', body: [{ type: 'reaction', watch: '${count}', immediate: true, actions: { action: 'setValue', componentPath: 'message', value: 'count:${count}' } }, { type: 'text', text: '${message}' }] }} data={{ count: 3 }} env={env} formulaCompiler={formulaCompiler} />);
  ...
  it('gates reaction execution with when and once', async () => {
    const SchemaRenderer = createBasicSchemaRenderer();
    render(<SchemaRenderer schemaUrl="test://basic/reactions" schema={{ type: 'page', body: [{ type: 'reaction', watch: '${status}', immediate: true, once: true, when: 'value === "ready" && prev !== value', actions: { action: 'setValue', componentPath: 'message', value: 'triggered:${status}' } }, { type: 'button', label: 'Ready', onClick: { action: 'setValue', componentPath: 'status', value: 'ready' } }, { type: 'button', label: 'Done', onClick: { action: 'setValue', componentPath: 'status', value: 'done' } }, { type: 'text', text: '${message}' }] }} data={{ status: 'idle', message: 'initial' }} env={env} formulaCompiler={formulaCompiler} />);
    expect(screen.getByText('initial')).toBeTruthy();
  ...
  it('applies repeated button setValue actions against the latest scope value', async () => {
    const SchemaRenderer = createBasicSchemaRenderer();
    render(<SchemaRenderer schemaUrl="test://basic/reactions" schema={{ type: 'page', body: [{ type: 'button', label: 'Inc', onClick: { action: 'setValue', componentPath: 'count', value: '${count + 1}' } }, { type: 'text', text: '${count}' }] }} data={{ count: 0 }} env={env} formulaCompiler={formulaCompiler} />);
    const button = screen.getByText('Inc');
    fireEvent.click(button);
    fireEvent.click(button);
    fireEvent.click(button);
  ...
  it('prevents unbounded self-trigger cascades in reactions', async () => {
    const notify = vi.fn();
    const SchemaRenderer = createBasicSchemaRenderer();
    render(<SchemaRenderer schemaUrl="test://basic/reactions" schema={{ type: 'page', body: [{ type: 'reaction', watch: '${count}', immediate: true, actions: { action: 'setValue', componentPath: 'count', value: '${count + 1}' } }, { type: 'text', text: '${count}' }] }} data={{ count: 0 }} env={{ ...env, notify }} formulaCompiler={formulaCompiler} />);
    await waitFor(() => {
  ...
  it('does not overwrite mount-time reaction writes with the initial page data sync effect', async () => {
    const SchemaRenderer = createBasicSchemaRenderer();
    render(<SchemaRenderer schemaUrl="test://basic/reactions" schema={{ type: 'page', body: [{ type: 'reaction', watch: '${count}', immediate: true, actions: { action: 'setValue', componentPath: 'message', value: 'count:${count}' } }, { type: 'text', text: '${message}' }] }} data={{ count: 1, message: 'initial' }} env={env} formulaCompiler={formulaCompiler} />);
