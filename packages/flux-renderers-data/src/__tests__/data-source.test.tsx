...
describe('dataRendererDefinitions data-source behavior', () => {
  ...
  it('evaluates formula sources into explicit name bindings', async () => {
    cleanup();
    const SchemaRenderer = createDataSchemaRenderer();
    render(<SchemaRenderer schema={{ type: 'page', body: [{ type: 'data-source', name: 'total', formula: '${4 * 6}' }, { type: 'text', text: 'Total: ${total}' }] }} env={env} formulaCompiler={formulaCompiler} />);
  ...
  it('fetches data and injects into scope', async () => {
    ...
    const SchemaRenderer = createDataSchemaRenderer();
    render(<SchemaRenderer schema={{ type: 'page', body: [{ type: 'data-source', api: { url: '/api/user/1' }, name: 'user' }, { type: 'text', text: 'Hello, ${user?.name}' }] }} env={{ ...env, fetcher }} formulaCompiler={formulaCompiler} />);
  ...
  it('uses initialData before fetch completes', async () => {
    ...
    const SchemaRenderer = createDataSchemaRenderer();
    render(<SchemaRenderer schema={{ type: 'page', body: [{ type: 'data-source', api: { url: '/api/user/1' }, name: 'user', initialData: { name: 'Initial' } }, { type: 'text', text: 'Hello, ${user?.name}' }] }} env={{ ...env, fetcher }} formulaCompiler={formulaCompiler} />);
  ...
  it('shows error message on fetch failure', async () => {
    ...
    const notify = vi.fn();
    const SchemaRenderer = createDataSchemaRenderer();
    render(<SchemaRenderer schema={{ type: 'page', body: [{ type: 'data-source', api: { url: '/api/error' }, name: 'data' }] }} env={{ ...env, fetcher, notify }} formulaCompiler={formulaCompiler} />);
  ...
  it('suppresses error notification when silent is true', async () => {
    ...
    const notify = vi.fn();
    const SchemaRenderer = createDataSchemaRenderer();
    render(<SchemaRenderer schema={{ type: 'page', body: [{ type: 'data-source', api: { url: '/api/error' }, name: 'data', silent: true }] }} env={{ ...env, fetcher, notify }} formulaCompiler={formulaCompiler} />);
