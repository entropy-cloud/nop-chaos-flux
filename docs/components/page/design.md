...
| `asideResizable` / `asideMinWidth` / `asideMaxWidth`（可调整侧边栏宽度）     | **实现**               | amis `asideResizor`；`asideResizable: true` 时 aside 内渲染 drag handle `data-slot="page-aside-resize-handle"`，使用 pointer events + `setPointerCapture`（原生触摸+笔支持，无需 document-level listeners），宽度存 React local state，clamp 到 `[asideMinWidth ?? 200, asideMaxWidth ?? 600]`；`asidePosition: 'right'` 时 dx 反转；右键忽略。改进 amis：仅鼠标事件→pointer events，imperative cssText→React state。 |
| `asideSticky`（侧边栏粘性定位）                                              | **实现**               | `asideSticky: true` 时 aside 应用 `position: sticky; top: 0; max-height: 100vh; overflow-y: auto`，侧栏随页面滚动保持可见。                                                                                                                                                                                                                                                                                           |
