...
- [ ] 为 `valueAdaptationOwnerHelper` 的三个函数编写 characterization tests，覆盖：无 action 时的 fallback 行为（返回 rawValue/workingValue）、有 action 时的 payload 构造（`injectDefaultArgs`）、action 失败时的 fallback、`publishValidateResultErrors` 的 error 映射
- [ ] 为 `input-text` 的 `null → ''` 和 `checkbox` 的 `coerceBooleanString` 编写 snapshot behavior tests（记录当前行为，用于 Phase 2 回归验证）

Exit Criteria:

- [ ] `valueAdaptationOwnerHelper` characterization tests 全部通过
- [ ] 简单字段值转换 snapshot tests 全部通过
...
- [ ] 定义 `ValueAdapter<TExternal, TInternal>` 接口
...
- [ ] 定义 `AdapterValidationResult` 和 `AdapterValidationIssue`
...
- [ ] 定义 `actionAdapter` 的 default payload 规则与 `value-adaptation-and-detail-field.md` 对齐：无显式 `args` 时注入 `{ value, name, readOnly }`，有显式 `args` 时 replace 不 merge
- [ ] 定义 adapter 失败时的 fallback 语义：`in` 失败返回原始值；`out` 失败返回原始工作值；`validate` 失败返回 `{ valid: false, issues: [...] }`。与当前 `valueAdaptationOwnerHelper` 的 fallback 行为一致
- [ ] 更新 `docs/architecture/value-adaptation-and-detail-field.md` 的 "Shared Wrapper" 章节，将 `ValueAdapter` 接口定位为所有值适配的统一协议

Exit Criteria:

- [ ] `ValueAdapter` 接口已从 `@nop-chaos/flux-core` 导出且类型正确
- [ ] 内置 adapter 工厂函数有 focused tests
- [ ] `actionAdapter` 的 payload 规则测试与 `value-adaptation-and-detail-field.md` 一致
- [ ] adapter fallback 行为有 focused tests
- [ ] 文档已更新
...
- [ ] 扩展 `useFormFieldController` 接受 `adapter?: ValueAdapter` 参数
- [ ] 当 `adapter` 存在时，读路径使用 `adapter.in(rawValue, ctx)` 替代 JSX 层散落逻辑；写路径使用 `adapter.out(uiValue, ctx)` 替代 `toFormValue`
- [ ] 当 `adapter` 不存在时，保持当前 `toFormValue` 回退行为（向后兼容）
- [ ] `input-text` 迁移：声明 `stringAdapter`（`in: v == null ? '' : String(v)`, `out: identity`）
- [ ] `textarea` 迁移：同 `input-text`，声明 `stringAdapter`
- [ ] `select` 迁移：声明 `stringAdapter`
- [ ] `radio-group` 迁移：声明 `stringAdapter`
- [ ] `checkbox` 迁移：声明 `booleanStringAdapter`（`in: Boolean(v)`, `out: Boolean(v)`）。注意：当前 `onCheckedChange` 发送 `String(Boolean(checked))`（字符串 "true"/"false"），经 `coerceBooleanString`（`v === 'true'`）转为 boolean。迁移时需同步修改 `onCheckedChange` 直接发送 boolean，否则 `Boolean("false")` = `true` 会破坏行为。Phase 0 snapshot tests 兜底回归检测。
- [ ] `switch` 迁移：同 checkbox（共享同一 `booleanStringAdapter`）
- [ ] `checkbox-group` 迁移：声明自定义 adapter（`in: Array.isArray(v) ? v : []`, `out: identity`）
- [ ] 保留 `toFormValue` 作为 fallback，不破坏已有自定义字段

Exit Criteria:

- [ ] `input-text` 不再在 JSX 中做 `null → ''` 转换，改用 `adapter.in()`
- [ ] `checkbox` 不再使用 `coerceBooleanString` 作为 `toFormValue`，改用 adapter
- [ ] Phase 0 的简单字段 snapshot tests 全部通过（行为不变）
- [ ] 无 `adapter` 的字段行为不变（向后兼容验证）
...
- [ ] `object-field`：当 schema 声明了 `transformInAction` 时，在 `createObjectFieldChildScope` 初始化阶段用 `actionAdapter.in(rawValue, ctx)` 得到编辑工作值（替代直接透传 rawValue）。当 schema 声明了 `transformOutAction` 时，在 projected form proxy 的 setValue 路径上拦截父表单写回，先经过 `actionAdapter.out(workingValue, ctx)` 转换。没有声明 transform 的 object-field 行为不变（直接 live-edit 透传）
- [ ] `variant-field`：variant switch 时 per-option 的 `transformInAction` 调用迁移为 `actionAdapter.in()`，复用共享 default payload 规则（替代当前内联的 `injectVariantTransformInArgs` + `props.helpers.dispatch`）。当前 variant-field 使用 `{ value, variant, readOnly }` payload（`variant-field.tsx:142-146`），其中 `variant` 携带目标 option key。迁移决策：将 `variant` 映射到 `AdapterContext.name`（variant option key 在语义上就是当前正在选择的字段名），保持 `AdapterActionContext` 不增加额外字段。adapter 内部通过 `ctx.name` 获取 variant key
- [ ] `array-field`：**不在本 phase 迁移**。其 scalar item `{ value }` 包装是 scope projection 机制（由 `array-field-runtime.ts` 的 `projectValues` 管理），不是值适配。`transformInAction`/`transformOutAction` 支持作为 follow-up
...
- [ ] `object-field` 当声明 `transformInAction` 时，子 scope 收到的是转换后的值而非原始值（有 focused test）
- [ ] `object-field` 当声明 `transformOutAction` 时，写回父表单的值经过转换（有 focused test）
- [ ] `object-field` 未声明 transform 时行为不变（回归测试通过）
- [ ] `variant-field` switch migration 使用 `actionAdapter` 和共享 payload 规则
- [ ] variant-field 的 per-option `variant` 上下文在 adapter 迁移后正确传递（有 focused test）
- [ ] playground schemas 中不存在因 renderer 忽略 transform 而依赖 no-op 语义的 object-field/variant-field schema（已 grep 验证）
- [ ] 现有复合字段测试全部通过
...
- [ ] 重构 `valueAdaptationOwnerHelper` 内部实现：`runTransformIn` 委托给 `actionAdapter.in()`，`runTransformOut` 委托给 `actionAdapter.out()`，`runValidate` 委托给 `actionAdapter.validate()`。保留 `valueAdaptationOwnerHelper` 的公开接口不变（接受 `ActionSchema` + `ActionRunner`），detail-field/detail-view 调用方无需修改
- [ ] staged lifecycle 调度（`runTransformIn → createDraft → validate → runTransformOut → commit`）继续由 `detail-field.tsx` 和 `detail-view.tsx` 管理，不在 helper 内
- [ ] 保留当前 fallback 语义：`runTransformIn` 失败返回 `input.rawValue`；`runTransformOut` 失败返回 `input.workingValue`。与 `actionAdapter` 的 fallback 行为一致
- [ ] Phase 0 的 `valueAdaptationOwnerHelper` characterization tests 全部通过

Exit Criteria:

- [ ] `valueAdaptationOwnerHelper` 内部基于 `actionAdapter` 实现
- [ ] `valueAdaptationOwnerHelper` 公开接口不变，detail-field/detail-view 调用方无修改
- [ ] Phase 0 的 characterization tests 全部通过（行为与重构前一致）

## Validation Checklist

- [ ] `ValueAdapter` 接口已从 `@nop-chaos/flux-core` 导出，内置工厂函数有 focused tests
- [ ] `AdapterContext` / `AdapterActionContext` 分层正确，同步 adapter 不被迫接受 scope/form/dispatch
- [ ] `actionAdapter` 的 dispatch 机制在构造时捕获，不依赖运行时注入
- [ ] adapter fallback 行为与当前 `valueAdaptationOwnerHelper` 一致
- [ ] 简单字段（input-text、textarea、select、checkbox、switch、radio-group、checkbox-group）已迁移到声明式 adapter，JSX 层不再有值转换逻辑
- [ ] 复合字段（object-field、variant-field）能通过 adapter 调用 transformInAction/transformOutAction
- [ ] `array-field` 的 scope projection 机制未被误改为 adapter
- [ ] detail-field / detail-view 行为不变，`valueAdaptationOwnerHelper` 公开接口不变
- [ ] `docs/architecture/value-adaptation-and-detail-field.md` 已更新反映 ValueAdapter 接口
- [ ] Phase 0 characterization tests 全部通过
