...
在本次审阅范围内，最值得参考的设计有六类。

| 设计 | 来源锚点 | 与当前 Flux 边界的适配度 | 推荐优先级 | 首个落点 |
| --- | --- | --- | --- | --- |
| 基于 schema/config 的工作台内容装配收敛，并由 host manifest 提供边界校验 | `tiny-engine/docs/extension-capabilities-overview/registry.md`, `tiny-engine/designer-demo/registry.js` | Good fit now | 高 | 先在各 host family 的 schema/config 装配与校验层收敛 |
...
## 4. 可直接参考的设计一：基于 schema/config 的工作台内容装配收敛，并由 host manifest 提供边界校验
...
- 真正显示出来的工作台内容，本来就应由 schema/config 指定：
...
- live code 也是按这条线工作的：
  - `packages/flow-designer-renderers/src/designer-page.tsx` 用 `props.helpers.render(...)` 渲染 `toolbar / inspector / dialogs`
  - `packages/flow-designer-renderers/src/designer-inspector.tsx` 先渲染 `activeNodeTypeConfig.inspector.body`，没有时才 fallback 到通用字段
  - `packages/report-designer-renderers/src/page-renderer.tsx` 也优先渲染 schema 提供的 `toolbar / fieldPanel / inspector`
...
- 实际显示内容 -> `designer-page` / `report-designer-page` 等 owner schema 的 `SchemaInput` region 与 domain config
- 视觉壳层复用 -> `WorkbenchShell`
- 更上游的默认装配 -> loader/workbench tooling layer

也就是说，Flux 不是“还缺一个 registry”；而是已经把 tiny registry 里混在一起的几类职责拆开，并且把“显示内容由 schema 指定”这件事单独定义得更清楚。

因此，Flux 真正缺的不是“再加一个工作台注册表”，而是：

- 让不同 host family 的 page schema、region 契约、domain config 与 manifest/tooling 消费路径更系统化地收敛

### 4.3 推荐的具体设计

推荐做的是**schema-first 的 host workbench 装配收敛**，而不是新的 workbench registry，也不是继续扩大 owner renderer 内部的 React fallback。
...
2. 把“实际显示什么”继续明确地放在 owner schema 与 domain config 上，而不是放进 manifest 或新的 registry。
3. 把各 workbench 的默认 `toolbar / inspector / dialogs / fieldPanel / body` 组合，视为 loader/workbench tooling 层生成的 schema/config baseline。
4. 把当前 `DesignerToolbarContent`、`DefaultInspector` 这类硬编码 React 内容视为 bootstrap fallback，而不是主 authoring 模型。
...
这里的重点不是再发明新协议，而是把“默认显示内容也应该是 schema/config”这条规则真正贯彻到 loader/tooling 层。

### 4.4 最小可实现路径

1. 不新增工作台注册表或 plugin-style workbench registry。
2. 先从 `designer-page` 开始，把默认 toolbar / inspector / dialogs 的主路径收口到 schema/config baseline，而不是继续依赖 React fallback。
3. 第二步推广到 `report-designer-page` / `spreadsheet-page` / `word-editor-page` 的对应 region。
4. 同时增加一条工具消费路径：基于 owner renderer `hostContract` 校验这些 schema 片段能否合法读取 host projection、调用 host actions。
5. 只有在没有提供 schema baseline 时，才走窄的 fallback React 内容。

### 4.5 为什么这条值得先做

因为它的收益很直接：

- workbench 实际显示内容继续保持 declarative/schema-first
- Flow/Report/Spreadsheet/Word 的 page schema 契约更一致
- 后续做领域发行版时，更容易在 loader/tooling 层生成不同 baseline，而不是改 renderer 内部逻辑
...
- 实际显示内容应继续由 `SchemaInput` / domain config 指定
...
1. 基于 owner renderer 与 host manifest 的 workbench shell 收敛
2. exporter/codegen pipeline 原型

原因：

- 这两条线都属于平台产品化增强
- 但都应该建立在前面的 material catalog / authoring contract 之上
...
> 在不打开 runtime 语义面的前提下，把工作台装配、物料目录、属性编辑、预览隔离和导出链做成正式 tooling surface。
