...
export function ReactionRenderer(props: RendererComponentProps<ReactionSchema>) {
  ...
  const compiledReaction = props.templateNode.compiledReactions?.[0] as
    | CompiledReaction
    | undefined;
  ...
  useLayoutEffect(() => {
    if (!compiledReaction) {
      throw new Error(
        `ReactionRenderer requires compiledReaction for node ${props.id}. Ensure the schema is compiled before rendering.`,
      );
    }
