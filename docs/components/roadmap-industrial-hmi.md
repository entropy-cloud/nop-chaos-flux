...
- `2026-08-05-2129-multi-audit-industrial-hmi.md` `[P2-9]`（dim 15）— `validateSymbolNode`（`validate.ts:264-280` children 递归）与 `deepEqual`（`equality.ts:17-32`）无深度限制，~10k 层嵌套（恶意/损坏 host JSON）stack overflow，fail-closed 边界缺口。建议加 `depth` 参数（cap ~100）+ 溢出返结构化错误/sentinel。
...
- `2026-08-05-2129-open-audit-industrial-hmi.md` `[P2-2]` — `equality.ts:17-32` `deepEqual` 把数组与 array-shaped 对象混同（`deepEqual([1,2],{0:1,1:2})===true`），违反自身注释「数组按 index 比较」。对 `custom`/`fillStyle` 等无类型内省字段，host 往返翻转 array↔object 形态时产 empty diff patch / 丢 serialize override。建议加 `Array.isArray(a)!==Array.isArray(b)→false` 守卫 + 数组按 index 比较分支。
- `2026-08-05-2129-open-audit-industrial-hmi.md` `[P2-3]` — `validate.ts:18-27` `checkNumberField` 接受 `NaN`/`Infinity`/`-Infinity`（`JSON.parse('1e400')→Infinity`），`x/y` 仅 `?? 0`（Infinity 留 Infinity），`strokeWidth/textSize/period/loop/deadband` 无下游 clamp。建议收紧为 `typeof n==='number' && Number.isFinite(n)`。
...
- `2026-08-05-2129-open-audit-industrial-hmi.md` `[P2-5]` — `scanLegacyAtSyntax`（`validate.ts:390-432`）只扫顶层 `config.symbols`，不递归 `scada-group` children → 组态最典型形态（group 嵌套子图元）的 legacy `@{pointId}` 方言静默不 warn。建议 walk children（镜像 `validateSymbolNode:276-278`）。
- `2026-08-05-2129-open-audit-industrial-hmi.md` `[P2-6]` — validator 只校验声明对象 surface 类型，`shadow`/animation `from,to`/`background`/declaration `scale.k,b`/`init`/binding-`scale` 子形状 malformed 全过 → 消费者读声明子字段 raw 产 NaN/undefined（`init` 与 `scale.k/b` 后果最重，静默 corrupt 点表）。建议一个 `assertShape(value, schema, scope)` helper 覆盖声明对象形状。
