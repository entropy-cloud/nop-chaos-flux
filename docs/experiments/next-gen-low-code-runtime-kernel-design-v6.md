...
## 2.1 Graph Kernel 的准确含义

`graph kernel` 不是一个营销名词，在本文里它有非常具体的含义。

它指的不是“把树换个名字叫图”，而是把原本分散在多个 runtime 子系统中的关系，统一降到一个可调度的执行底座中。

至少包括四类关系:

1. 结构关系: 节点、region、fragment、loop、surface 的实例边界
2. 数据关系: 值读取、路径命中、scope/frame、cell 归属、发布目标
3. 效果关系: action 的 `when` / `then` / `onError` / `parallel` 控制流
4. 生命周期关系: resource、reaction、validation、surface 的启动、失效、取消、结算

因此，`graph kernel` 的真正含义是:

1. runtime 的中心不再是“解释一棵 schema 树”
2. runtime 的中心也不再是“多个子系统彼此订阅”
3. runtime 的中心变成“调度一个统一的应用执行图”

## 2.2 它不是天然正确答案

本设计必须承认一件事:

1. `graph kernel` 不是天然更正确
2. “统一”本身不是目标
3. 只有当多个子系统真的共享关键不变量时，统一才有价值

只有在以下条件同时成立时，统一才值得考虑:

1. 多个子系统共享同一套依赖与失效语义
2. 多个子系统共享同一套 async ownership、取消、重试、超时语义
3. 分裂执行面已经造成明显重复机制、行为不一致或诊断困难
4. 高频性能问题来自不同子系统无法协同调度，而不是单个子系统本身设计不佳

若不满足这些条件，那么“独立演化”通常比“大一统内核”更合理。

## 2.3 可执行性判断

`graph kernel` 理论上可以落地，但它不是低风险方案。

真正困难的地方不在“能否画出图”，而在于以下闭环是否能成立:

1. 时序闭环: 谁先算、谁后算、异步结果进入哪一轮 turn
2. 所有权闭环: scope、resource、validation、surface、row instance 到底归谁
3. 失效闭环: 改一处数据后，哪些 consumer 必须重算，哪些必须保持稳定
4. 调试闭环: 为什么触发、为什么没有触发、为什么循环、为什么晚到结果被丢弃
5. 成本闭环: 简单场景不能因为统一内核而背上过高常数成本

所以 v6 的正确表述不是“更先进，因此应该采用”，而是“如果愿意承担高实现复杂度，可能换来更高的长期上限”。

## 2.4 统一的边界

这份 v6 初稿最容易被质疑的点，是统一范围过大。这里补充一个更现实的边界判断。

更适合统一到底层 substrate 的能力:

1. 表达式求值
2. 基础依赖追踪与失效传播
3. 结构化写入发布
4. action 结果分类与 effect continuation
5. async ownership、取消、重试、超时、trace id

更适合保持独立 owner family 或独立子系统演化的能力:

1. validation runtime
2. surface/dialog/drawer runtime
3. table/row 高频特化策略
4. flow designer / spreadsheet / report / word editor 等复杂域 runtime
5. React 或其他 renderer adapter

换句话说:

1. 共享“机制”比共享“语义”更值得统一
2. 共享“substrate”比共享“owner model”更值得统一
3. 领域系统不应仅因为可以接入 graph kernel，就被强行并入核心

## 2.5 更现实的替代形态: Minimal Execution Kernel

如果要把 v6 从“大一统 graph kernel”收缩成更可落地的路线，更合理的做法不是否定内核化，而是缩成 `Minimal Execution Kernel`。
...
1. form validation runtime
2. surface runtime
3. row/table runtime
4. domain module runtime

这条路线的意义在于:

1. 保留统一底层的收益
2. 避免把 owner 语义和领域语义全部压进一个大 kernel
3. 更接近真正可落地的下一代运行时演化路线
...
这里再强调一次: `surface kernel`、`form kernel`、`row arena`、`domain module contract` 不应被误读为“一定要并进同一个大而全的 kernel”。

更现实的落地方向是:

1. 它们可以共享最小执行内核提供的 substrate
2. 但仍保持各自 owner 语义和生命周期语义的独立演化
3. 只有在某块语义已经被证明跨域复用、且共享不变量足够强时，才考虑进一步收敛
...
初稿里的激进路线是把这些能力尽量收束到统一 graph kernel 上。

但更现实的版本应改写为:

1. 把共享执行机制收束到最小执行内核
2. 把 owner family 和复杂领域系统保留为独立演化层
3. 只在共享不变量足够强的地方继续向 graph kernel 收敛

但在基准数据出来前，更准确的表述是:

1. v6 对高频局部更新、统一调度、诊断可观测性有更强设计潜力
2. v6 同时引入了更高的实现复杂度和内核常数成本风险
3. 只有当最小执行内核在简单场景中能真正按需裁剪，而独立子系统又能在复杂场景中共享底层收益时，才算完成对传统方案的超越
...
1. 先实现 minimal execution kernel: compiler + path token + expression VM + dependency substrate + write publication + async governance
2. 再实现 settled turn + scheduler lanes + derived/resource/reaction 的共享失效模型
3. 再把 form、surface、row 等系统接到共享 substrate 上，但保持 owner 语义独立
4. 最后再评估是否有必要把某些高级模块继续向 graph kernel 收敛
5. 最后做 react adapter、domain module contract、devtools graph view
