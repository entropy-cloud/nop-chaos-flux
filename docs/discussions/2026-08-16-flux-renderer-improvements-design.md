# Flux 渲染器改进设计：responsive 结构断点 + collapse/checkbox/chart 增强

> 日期：2026-08-16
> 来源：`docs/analysis/sundial-ui-reproduction-analysis.md` G1/G2/G4/G7 + Sundial 复刻实测
> 状态：设计定稿（实现前评审用）
> Supersession Note（2026-08-16）：实现已完成并通过 closure audit，最终设计归档至 `docs/architecture/responsive-and-renderer-enhancements.md`；本文保留为历史讨论记录。
...
实现：line/area/bar/scatter 渲染处，当 series 声明逐点上色时用 recharts `Cell` 包裹（bar/scatter）或 `stroke/point` 逐点赋值（line/area）。pie 已有 sector 上色能力，不重复支持。
...
四个改进均涉及 renderer 契约，档位：**必须自动化**（Proof 先于 Fix）：
