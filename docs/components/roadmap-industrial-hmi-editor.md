...
- **E7. M2 连线与 undo-redo** (`planned`) <!-- 端点吸附连线 + 多选/框选 + undo-redo diff 命令栈；2026-08-07 plan `2026-08-07-0443-1` 激活 todo→planned（独立 fresh-session sub-agent draft review round 1 pass）；3 Phase 全交付——Phase 1 端点吸附连线（connection/ 4 模块 anchor-snap/connection-link/connection-adapter/connection-overlay + connection 测试句柄 + 连线 e2e）+ Phase 2 undo-redo diff 命令栈（undo-redo/ 4 模块 compute-inverse/undo-stack/operation-coalesce/undo-redo-adapter + editor-session undoStack 扩展 canUndo/canRedo 真实派生 + undo/redo 句柄 + undoRedo 测试句柄 undo/redo/getStackState/pushUndo + transform 事务语义节流起止帧 + coalesce e2e + undo-redo e2e）+ Phase 3 多选/框选 + group/ungroup（selectArea+selectKeep:true + groupSymbols/ungroupSymbols 结构 diff + 入 undo 栈 + group/ungroup e2e）；R4 严格满足（栈元素只持 forward+inverse 增量，无全量快照）+ R5 隔离不泄漏 + runtime 复用点不重复实现；workspace full-green（typecheck/build/lint/test 32/32 + 59/59，industrial 82 files / 1077 tests，coverage 90.11% branches）→ 待 E8 gate + closure-audit → done -->
