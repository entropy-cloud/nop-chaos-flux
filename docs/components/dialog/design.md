...
| `size` 预设（xs/sm/md/lg/xl/full）                              | **实现**：Flux 6 档直落 ui DialogContent 新档位（xs→xs、sm→sm、md→base、lg→md、xl→lg；未传 size 默认 base）+ `full` 走 inline 100vw/100vh；宽度经 `--dialog-size-*` token | —          | 对齐 AMIS 档位语义（C1a R9）；token 可被宿主覆盖                                 |
...
- 根节点保留 `nop-dialog` marker；host 在 DialogContent 上额外发布 `data-slot="dialog-surface"`（区别于 ui DialogContent 的 `data-slot="dialog-content"`）以及 `data-close-on-outside` / `data-close-on-esc` / `data-mobile-fullscreen` 状态 marker。
- `size` 映射（C1a 起）：Flux 6 档（xs/sm/md/lg/xl/full）直落 ui DialogContent 新档位——xs→`xs`、sm→`sm`、md→`base`、lg→`md`、xl→`lg`；未传 size 默认 `base`；`full` 走 inline `width: 100vw; height: 100vh`。宽度一律经 `--dialog-size-*` CSS 变量（`xs 375px / sm 350px / base 500px / md 800px / lg 1100px / xl 90%`），不再有 `xs→sm、sm|md→default、lg|xl|full→lg` 的拍平映射。
- `width`/`height` 显式 override：number→px，string 透传 CSS length；与 `size` 并存时显式优先。
- 顶部定位与堆叠（C1 起）：flux overlay 路径的 dialog 不再 50%/50% 垂直居中；ui `DialogContent` 新增 opt-in `topAnchored` prop（默认 false），启用时内容吸附 `top-[var(--dialog-top-offset)]`（60px）+ 仅水平居中（`translate(-50%, 0)`）；宿主按 surfaces 中 dialog 序号注入 inline `style.top: calc(var(--dialog-top-offset) + idx * var(--dialog-stack-step))`（每层 +30px，与 AMIS 阶梯叠加一致）。非 topAnchored 消费方（宿主直连 ui primitive）行为不变（仍垂直居中）。
- 遮罩/footer/标题 token 化（C1 起）：遮罩 `bg-[var(--dialog-overlay-bg)]`（默认 0.7，替换原 0.4 固定表面色）；footer 按钮 `min-width` / footer gap、标题字号、body 水平 padding、content 圆角均经 `--dialog-*` token；footer 默认移除 `border-t bg-muted/50`（对齐 AMIS footer 无边框）。
