...
- **S11. Gantt——P0 缺陷修复** (`todo`)
- **S12. Kanban——P0 缺陷修复** (`todo`)
- **S13. Calendar——P0 缺陷修复** (`todo`)
- **S14. Barcode-input——P0 缺陷修复** (`todo`)
- **S15. Gantt——P1 缺陷修复** (`todo`)
- **S16. Kanban——P1 缺陷修复** (`todo`)
- **S17. Calendar——P1 缺陷修复** (`todo`)
- **S18. Barcode-input——P1 缺陷修复** (`todo`)
- **S19. Diff-view——P1 缺陷修复** (`todo`)
- **S20. Diff-view——P2 修复与测试补充** (`todo`)
...
| ID    | Status | 内容                                                                                                                                         | 严重度 | 依赖       |
| ----- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ---------- |
| S11.1 | todo   | **zoomLevels 透传**：`gantt.tsx` `createInitialStore` 传入 `resolved.zoomLevels`，无值时种子默认 day/week/month 三档                         | P0     | S1.5       |
| S11.2 | todo   | **网格行高对齐**：`<tr>` 强制 `height: store.rowHeight`，禁止文本换行（截断），与时间线 40px 行高一致                                        | P0     | S2.1       |
| S11.3 | todo   | **`_dirty` parse 守卫移除**：`gantt-store.ts` 移除 `if (this._dirty) return`，或 parse 成功末尾复位 `_dirty=false`                           | P0     | S1.1       |
| S11.4 | todo   | **scrollTo 修正**：`scrollToToday`/`scrollToTask` 直接设 `timelineRef.scrollLeft`，仅同步 `scrollTop` 到网格                                 | P0     | S2.8       |
| S11.5 | todo   | **展开/收起接线 + 坐标重算**：`GanttGrid`/`GanttBars` 订阅 `treeRevision`；`toggleOpen` 末尾调 `computeCoordinates` 并 bump `layoutRevision` | P0     | S1.3, S2.4 |

### S12 — Kanban P0 缺陷修复

| ID    | Status | 内容                                                                                                                                                   | 严重度 | 依赖 |
| ----- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------ | ------ | ---- |
| S12.1 | todo   | **controlled 模式修正**：默认非受控（挂载时拷入 localBoardData）；仅显式 `controlled` 标志或 `data-source` 绑定时才受控；`setBoardData` 始终更新 local | P0     | S6.1 |

### S13 — Calendar P0 缺陷修复

| ID    | Status | 内容                                                                                                                                    | 严重度 | 依赖 |
| ----- | ------ | --------------------------------------------------------------------------------------------------------------------------------------- | ------ | ---- |
| S13.1 | todo   | **视图切换接线**：`activeView` 从 `useCalendarState` hook 读取用于渲染与 header；schema `view` 仅作初值（`viewOwnership:'local'` 默认） | P0     | S4.1 |
| S13.2 | todo   | **月视图事件宽度**：每格按实际并发数算宽 `100/Math.min(dayBlocks.length, effectiveMax)`，`effectiveMax` 仅用于折叠 "+N"                 | P0     | S4.4 |
| S13.3 | todo   | **月视图列数**：弃用 `getMonthDays`（含邻月溢出 28-42 天），改为 `getDateRange(getMonthStartEnd(...))` 当月天数矩阵                     | P0     | S4.1 |
| S13.4 | todo   | **虚拟器行定位**：行 div 加 `position:'absolute',left:0,right:0`，仅靠 `transform: virtualItem.start` 定位                              | P0     | S4.7 |

### S14 — Barcode-input P0 缺陷修复

| ID    | Status | 内容                                                                                                                                      | 严重度 | 依赖 |
| ----- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------- | ------ | ---- |
| S14.1 | todo   | **video 流挂接**：始终渲染 `<video>`（CSS 切换可见性），或在 `camera.isActive`/`videoRef.current` 上加响应式 effect 挂接 stream→srcObject | P0     | S8.1 |

### S15 — Gantt P1 缺陷修复

| ID     | Status | 内容                                                                                                                       | 严重度 | 依赖  |
| ------ | ------ | -------------------------------------------------------------------------------------------------------------------------- | ------ | ----- |
| S15.1  | todo   | **条宽 off-by-one**：明确 end 语义（建议 exclusive），用 `Math.max((diff+(inclusive?1:0))*cellWidth, cellWidth)`           | P1     | S1.1  |
| S15.2  | todo   | **依赖线 4 类型路由**：按 `link.type` 分支选 source/target 锚点（start=`$x`，end=`$x+$w`），处理 target 在 source 左侧绕行 | P1     | S2.5  |
| S15.3  | todo   | **周/季度刻度格式**：补 `V`(ISO 周)/`W`/`q`(季度) formatter 到 `FORMAT_TOKENS`                                             | P1     | S1.4  |
| S15.4  | todo   | **里程碑可交互**：菱形 `pointer-events:auto`；同一定位 wrapper；`linkable` 时加连线柄                                      | P1     | S2.4  |
| S15.5  | todo   | **taskBar region 透传**：将 `regions.taskBar` 透传进 `GanttBars` 替换默认 `<span>{task.text}</span>`                       | P1     | S2.4  |
| S15.6  | todo   | **网格自定义列**：`default` 改读 `(task as any)[col.name]`；`predecessor` 用 `$target`；尊重 `col.cell` region             | P1     | S2.2  |
| S15.7  | todo   | **拖拽视觉反馈**：ghost 仅 `translateX(dx)`；原条 opacity 0.3；目标列渲染 drop indicator                                   | P1     | S2.6  |
| S15.8  | todo   | **事件派发**：向拖拽 hook 传 `onCommit(taskId, changes)` 回调派发 `onTaskDragEnd`/`onLinkDragEnd`                          | P1     | S2.6  |
| S15.9  | todo   | **键盘 Left/Right 语义**：Left=收起、Right=展开；移除 per-bar 箭头→resize 冲突映射                                         | P1     | S2.10 |
| S15.10 | todo   | **键盘焦点移动**：ArrowUp/Down 后 `row.focus()`；`updateRowAria` 接入生产组件                                              | P1     | S2.10 |
| S15.11 | todo   | **顶层 chevron**：条件改为 `store.getVisibleDescendantCount(task.id) > 0`（有子即显示，非嵌套才显示）                      | P1     | S2.2  |
| S15.12 | todo   | **updateTask bump layoutRevision**：`updateTask` 内或 `computeComputedPropertiesInternal` 末尾 bump `layoutRevision`       | P1     | S1.1  |
| S15.13 | todo   | **ArrowUp/Down 冲突**：bar handler 内 `stopPropagation`，或移除 per-bar 上下箭头→日期移动绑定                              | P1     | S2.10 |
...
| ID    | Status | 内容                                                                                                                                        | 严重度 | 依赖 |
| ----- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ---- |
| S19.1 | todo   | **组件句柄实现**：`useImperativeHandle` 暴露 `toggleViewType`/`setViewType`/`expandAll`/`collapseAll`；renderer definition 注册 `reactions` | P1     | S9.4 |
| S19.2 | todo   | **renderer definition 补字段**：`fields` 数组补充 `middleContent`/`files`/`activeFileIndex`                                                 | P1     | S9.1 |
| S19.3 | todo   | **三栏 toggle 修正**：三栏模式下隐藏 toggle 按钮，或允许 `viewType` 覆盖 `isThreeColumn` 检测                                               | P1     | S9.4 |
| S19.4 | todo   | **集成测试补充**：至少一个"渲染真实组件 + 断言具体 DOM 输出"冒烟测试（行数、viewType 切换、hunk 展开）                                      | P1     | S9.4 |
...
| ID    | Status | 内容                                                                                                                   | 严重度 | 依赖  |
| ----- | ------ | ---------------------------------------------------------------------------------------------------------------------- | ------ | ----- |
| S20.1 | todo   | **清理未使用 hooks**：移除 `useRendererRuntime()`/`useRenderScope()` 的无意义调用（DV-OPS-01，共识降级 P1→P2）         | P2     | S9.4  |
| S20.2 | todo   | **三栏视图语法高亮**：三栏分支传递 `language={debouncedLang}`，`DiffThreeColumnView` 解构并使用（DV-OPS-05）           | P2     | S9.8  |
| S20.3 | todo   | **areHunkPropsEqual 补全**：比较器中添加 `prev.onHunkExpand === next.onHunkExpand`（DV-OPS-03）                        | P2     | S9.5  |
| S20.4 | todo   | **跨文件快捷键作用域**：Ctrl+↑/↓ 改为仅在 diff-view 容器获取焦点时响应，或用 `data-shortcuts` marker 限定（DV-OPS-04） | P2     | S9.10 |
| S20.5 | todo   | **3way 多冲突区逻辑**：重构 `diff-3way.ts:207-213` 为基于原始 `rows` 索引的单次遍历（DV-DISP-06）                      | P2     | S9.8  |
| S20.6 | todo   | **清理死代码导出**：删除或标记 `@deprecated` `DiffGutter`/`DiffGutterCell`/`renderFileListSidebar`（DV-DISP-04）       | P2     | S9.4  |
| S20.7 | todo   | **重写零断言测试**：重写 `diff-cross-file.test.tsx` 两个测试，断言具体行为而非 `length > 0`（DV-TEST-03）              | P2     | S9.10 |
| S20.8 | todo   | **清理 dead parameter**：移除 `buildInlineHtml` 的 `content` 参数（DV-DISP-01）                                        | P2     | S9.3  |
...
修复 Gantt 5 个 P0 阻断级缺陷：zoomLevels 未透传（时间线空白）、网格行高与时间线错位、`_dirty` 守卫导致编辑后数据冻结、scrollTo 滚错元素、展开/收起接线+坐标重算缺失。来源：`docs/analysis/2026-07-22-scheduling-display-operability-deep-analysis.md` §1。

### S12 Kanban P0 缺陷修复（todo）

修复 Kanban 1 个 P0 阻断级缺陷：`data` 传入即进入 controlled 模式致所有 mutation 空操作（拖拽/增删失效）。来源：同上 §2。
