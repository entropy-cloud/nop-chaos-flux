...
**技术选型**: 图表使用 `recharts`（已在 `@nop-chaos/ui` 中集成），不使用 `echarts`。`recharts` 是 React 原生的声明式图表库，与 React 19 生态完全对齐，避免了 ECharts 命令式实例管理的复杂度。
...
- [ ] `ChartDialog.tsx`: 图表配置弹窗
  ...
  - 数据集和字段绑定
  - 已添加图表列表管理
- [ ] 图表预览组件: 使用 `recharts` 渲染实时预览
  - 编辑时: 基于配置数据用 recharts 渲染真实图表（非占位图）
  - 依赖 `@nop-chaos/ui` 中已有的 `Chart`、`ChartTooltip`、`ChartLegend` 组件
  - 避免引入 `echarts` 及其命令式 API
- [ ] 在文档中插入图表占位图片
  - 编辑时显示 recharts 渲染的预览图
  - 渲染时后端生成实际图表图片嵌入 DOCX
