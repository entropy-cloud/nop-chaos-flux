...
| Phase                                              | 目标                                                                                                                                                                                                                                                                                                                                                                                                      | 含 improvement-analysis.md 改进项    | 退出条件                                                                                        |
| -------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ | ----------------------------------------------------------------------------------------------- |
| **P-1**：env.stream 扩充（前置，不在本包内）       | 完成 env.stream 评审 + flux-core 扩接口 + playground 默认实现 + 同步扩 `RendererEnvDecoratorHooks` 加 stream/openSocket hook                                                                                                                                                                                                                                                                              | —                                    | `env.stream` 在 playground 可用；评审 discussion 完成总结                                       |
...
| **P1**：真实 AI + 会话 + 流式 Markdown + 基础 a11y | host 提供 OpenAI/DeepSeek connector；`ai-conversations` / `ai-welcome` / `ai-prompts` / `ai-feedback`；ActionScope namespace `ai` 注册；**A-1** ChatMessageDataPart；**A-2** 流式 Markdown CJK + code fence 缓冲；**A-3** 代码块复制；**A-4** 消息时间戳；**A-5** 错误态组件；**a11y** `role="log"` + `aria-live="polite"` on ai-message-list                                                             | A-1, A-2, A-3, A-4, A-5              | 接 DeepSeek/OpenAI 真实 API 能对话；会话切换正常；外部按钮 `ai:send` 工作；流式 Markdown 不闪烁 |
| **P2**：工具调用 + 附件 + 渲染器深化               | `ai-tool-call` + `toolPlugin` 完整接入；`ai-attachments` + 多模态 content part + 拖放（**A** §4.6）；ComponentHandle 注册；**A-6** 工具卡片类型化（BubbleToolRendererMatch）；**A-7** Streamdown 适配；**A-8** 虚拟滚动（复用 `@tanstack/react-virtual`）；**A-9** useAutoScroll hook 公开；**A-10** 推理持续时间；**A-11** 流式光标动画；**A-12** 工具状态颜色；消息编辑（**A** §4.7）；LaTeX/KaTeX 评估 | A-6, A-7, A-8, A-9, A-10, A-11, A-12 | LLM 调用工具并回显结果；图片上传作为 `image_url` content part 发送；1000+ 消息性能不退化        |
| **P3**：持久化（host 实现） + 引用 + HITL          | host 提供 localStorage / IndexedDB `ConversationStorageStrategy` 实现；`useConversation` 在 host 层封装 storage 同步；**A-13** `ai-citations` 渲染器；**A-14** HITL 审批页脚（`ChatToolCallUIState.approval`）                                                                                                                                                                                            | A-13, A-14                           | 刷新页面后历史会话恢复；引用来源可悬停查看；工具调用可审批/拒绝                                 |
| **P4**：高级集成                                   | messages 序列化进 flux form 字段；`onResponseComplete` 接 data-source 联动；**A-15** `ai-voice-input`（Web Speech API 直呼）；**A-16** 消息分支；**A-17** `ai-token-usage`；`ai-suggestions`（从 P2 降级）                                                                                                                                                                                                | A-15, A-16, A-17                     | 对话历史可进入表单提交流程；语音输入工作；分支可切换                                            |
| **P6**：Tiptap 富文本（可选）                      | `@tiptap/react` 接入；@提及 / 模板 / Slash 命令；保持 `<Textarea>` 作为降级                                                                                                                                                                                                                                                                                                                               | —                                    | 富文本输入扩展通过 `senderExtensions` 字段注入                                                  |
| **P7**：MCP（可选，独立包）                        | 独立 `flux-mcp-connector` 包提供 MCP 客户端（依赖 `@modelcontextprotocol/sdk`）；本包只增加 `ai-mcp-manager` 渲染器消费它                                                                                                                                                                                                                                                                                 | A-18                                 | MCP server 启用后 LLM 能调用其工具                                                              |
...
**a11y 要求**（所有 Phase 共同遵守，来自 `improvement-analysis.md` §7）：

- `ai-message-list` 根元素加 `role="log"` + `aria-live="polite"`（P1）
- `ai-tool-call` 根元素加 `aria-label="Tool: {name}, status: {status}"`（P1）
- `ai-sender` 提交后焦点回输（P1）
- 消息列表支持 `Tab` / `Shift+Tab` 在消息间导航（P2）
- 模态审批使用焦点陷阱（P3 HITL）
- 流式文本播报动态插入 `aria-live="polite"` 占位元素（P2）
...
| `AiResponseProvider` → `AiConnector`（命名+职责更准确）                                          | FIX-2              | `engine.md` §9 全节重写          |
