...
- [ ] (Decision) 裁定包名 `@nop-chaos/flux-renderers-pivot`（graph/gantt 独立包先例）与 schema 字段集：
...
- [ ] (Fix) 创建包骨架（复制 graph 包模板）：package.json（依赖 `@visactor/vtable` 锁定 `^1.26.6`、`@nop-chaos/flux-core/flux-i18n/flux-react/ui` workspace、peer react）、tsconfig.json/tsconfig.build.json、src/index.ts（导出 `registerPivotRenderers` + definitions + types）。
- [ ] (Fix) 注册别名与引用：`vite.workspace-alias.ts` 增加包 alias + `*/styles.css` alias；根 `tsconfig.json` project references 增加条目。
- [ ] (Proof) 包级空转验证：`pnpm --filter @nop-chaos/flux-renderers-pivot typecheck` 通过（空 definitions 先落地）。

Exit Criteria:

- [ ] 新包在 workspace 中可见，`pnpm typecheck`（包级）通过。
- [ ] `PivotTableSchema` 类型定义存在，字段集与裁定一致。
...
- [ ] (Fix) 实现 `buildPivotOption(schema, resolvedData)` 纯函数（输入 schema 求值结果，输出 `PivotTableConstructorOptions`），不含任何 DOM/实例依赖。
- [ ] (Fix) theme 映射函数 `mapDesignTokensToVTableTheme()`：读取 CSS 变量（背景/边框/文字色/主题色）→ VTable Theme 结构；缺失回退默认。

Exit Criteria:

- [ ] `pivot-option.test.ts` 全绿（含降级路径断言）。
- [ ] `buildPivotOption` 为纯函数（无 DOM 依赖，可任意环境直测）。
...
- [ ] (Proof) 先写 failing 测试（mock VTable 类）：
...
- [ ] (Fix) 实现 `PivotTableRenderer`（`RendererComponentProps<PivotTableSchema>` 契约，INV-5）：
...
- [ ] (Decision) 裁定 `records` 与 `source` 双入口语义（沿用 chart `series`/`source` 裁定：source 为原始数据集优先，records 为直接数据；二者同设时 source 优先 + dev warn）。
- [ ] (Fix) `pivot-renderer-definitions.ts`：`type: 'pivot-table'`、category `data`、sourcePackage、propContracts（records/source/rowDimensions/columnDimensions/indicators/dataConfig/height/empty 等）、schemaValidator（校验 dimensions/indicators 结构，非法 dev warn 而非抛错）。
- [ ] (Follow-up) 评估 ComponentHandle 能力面（如 `refresh` 复用 data-source 语义）——先记录，不阻塞首版。

Exit Criteria:

- [ ] `pivot-renderer.test.tsx` 全绿（mock VTable 下：挂载/更新/卸载/空态/事件桥接/降级路径）。
- [ ] 无直接 store 访问；实例与交互 state 均在 renderer 内部（INV-3/INV-4 自检通过）。
...
- [ ] (Fix) `src/index.ts` 导出 `registerPivotRenderers` + `pivotRendererDefinitions` + schema types（graph 包模式）。
- [ ] (Fix) 按 `docs/references/new-renderer-introduction-audit.md` INV-1~5 逐条落地/自检（IO 边界：无新 IO 类型，数据经 props/scope；复用：表达式求值经 `helpers`；内部 state 本地化；契约守 `RendererComponentProps`）。
- [ ] (Fix) playground 新增 pivot-table 示例页：销售数据透视（region×quarter 行、category 列、sales/profit 指标、小计总计）+ 空态示例 + 主题联动（跟随 playground 主题切换）。
- [ ] (Proof) playground 示例验证：`pnpm dev` 手动走查（页面渲染、维度/指标正确、小计总计、排序点击、数据更新不闪屏、暗色主题切换）。
- [ ] (Follow-up) 检查 `pnpm check` 新增包无违规（workspace-manifest-deps 等）。

Exit Criteria:

- [ ] playground 示例页可运行（手测清单通过）。
- [ ] INV-1~5 自检清单完成，无遗留 IO/state 越界。
...
- [ ] (Fix) `docs/components/pivot-table/design.md`：schema 字段表、option 映射对照表、事件桥接清单、主题映射说明、与 table 静态透视/路径 A 的关系、Non-Goals（编辑/PivotChart/建模器）。
- [ ] (Fix) `docs/components/pivot-table/example.json`：完整可运行示例（与 playground 页一致）。
- [ ] (Fix) 更新分析报告 §3.4：标注路径 B 已落地（landed）与包名。
- [ ] (Fix) daily log 记录本计划执行摘要（`docs/logs/2026/08-09.md`，倒序）。
- [ ] (Follow-up) 记录后续候选：字段建模器（拖拽配置）、PivotChart、编辑模式、`refresh` handle。

Exit Criteria:

- [ ] design.md + example.json 存在且与 live renderer 行为一致（schema 字段逐一对应）。
- [ ] 分析报告与 daily log 已同步。
...
- [ ] `pivot-table` renderer 在 playground 可运行（维度/指标/总计/排序/数据更新/主题切换走查通过）。
- [ ] option 映射 + renderer 生命周期 + 事件桥接 focused 单测全绿。
- [ ] INV-1~5 审计通过（`new-renderer-introduction-audit.md` 清单）。
- [ ] `docs/components/pivot-table/design.md` + `example.json` 与 live baseline 一致。
- [ ] 不存在被静默降级到 deferred / follow-up 的 in-scope 行为缺口。
- [ ] 受影响的 owner docs 已同步（分析报告 §3.4、daily log）。
