...
| 倒计时            | **实现**：`countDown`（秒）/ `countDownTpl`（模板，`{timeLeft}` token，缺省 `"{timeLeft}s"`）           | —                                                                                          | 推翻原"不采纳"裁定：ERP HR 考勤打卡/SMS 验证码/B2B 验证码均需此能力；localStorage 耦合已解决——仅在有显式 `id`/`name` 时持久化，key 含 `location.pathname` 避免跨页面碰撞，无 `id`/`name` 时不持久化；timer handle 经 `setInterval` + cleanup 管理无泄漏；基于 `Date.now()` 差值避免漂移；action 成功后（`onClick` promise resolve）才启动 |
