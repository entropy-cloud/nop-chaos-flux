...
OpenAI 技术团队成员 Ryan Lopopolo 在 AI Engineer 伦敦演讲中系统讲述了 Harness Engineering（本地英文 summary 的来源日期标注为 2026-04-17），后来被微信公众号"AI 趣实验"整理成中文稿，并出现知乎二次整理。整理稿把 Ryan 的核心信息压缩为 12 个要点，第一条就是"代码正在变便宜，实施不再是软件工程里最稀缺的资源"。Ryan 本人在演讲中的原话是：

> 实现已经不再是软件工程里最稀缺的资源。代码是免费的。

Ryan 的演讲标题是 **Harness Engineering: How to Build Software When Humans Steer, Agents Execute**。"AI 趣实验"的整理稿随后把这套实践进一步提炼为具有五层定义的工程方法论。本文引用 Ryan 的观点时，以演讲字幕（整理稿第三节）和本地英文 summary 为准；整理稿第一节的 12 点总结、第二节的分析框架以及知乎文章中的场景化解释，均按二手解读处理并明确标注。

[Attractor-Guided Engineering（AGE）](https://github.com/entropy-cloud/attractor-guided-engineering-template)在 [nop-chaos-flux](https://github.com/entropy-cloud/nop-chaos-flux) 中形成了生产级实践，并在 [attractor-guided-engineering-template](https://github.com/entropy-cloud/attractor-guided-engineering-template) 中被抽取为通用模板的一部分。它的核心判断是：
...
两者都把代码库加文档当作 agent 的操作现实；"唯一事实源"是中文整理稿对 Ryan 仓库中心论的压缩表达，不是逐字原话。
...
这句话是"AI 趣实验"对 Ryan 演讲的解读，不是 Ryan 原话。但它在 Plan 76 场景下确实不够用：即使单次变更可由同一执行上下文生成并验证、且局部检查通过，整体方向仍可能错。
...
_本文基于：Ryan Lopopolo 伦敦演讲 "Harness Engineering: How to Build Software When Humans Steer, Agents Execute"（本地英文 summary 标注来源日期为 2026-04-17）、[微信公众号 AI 趣实验中文整理稿](https://mp.weixin.qq.com/s/3BvebfHJygUmXHrOROM7-w)、本地知乎整理稿；以及 AGE 在 [nop-chaos-flux](https://github.com/entropy-cloud/nop-chaos-flux) 和 [attractor-guided-engineering-template](https://github.com/entropy-cloud/attractor-guided-engineering-template) 中的实施实例。_
