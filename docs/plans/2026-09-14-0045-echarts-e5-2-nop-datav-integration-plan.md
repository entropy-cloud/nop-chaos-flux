...
- [ ] Proof（先红）：`dashboard-echarts-panel.test.tsx`——dashboard 布局含 `{ type: 'echarts', props: { option, height } }` 面板 + 注入真实 `echartsRendererDefinition`（`echarts-setup.js` 以跨包相对路径 `vi.mock`）→ 断言 setOption 收到 option；面板 props 表达式（如 height: '${panelH}'）求值生效。
- [ ] Fix：dashboard 包 devDependency 增 data 包；评估文档落地（机制/前置条件/适配点/风险边界/结论）+ design.md 锚点链接。
- [ ] Proof（转绿）：`dashboard-echarts-panel.test.tsx` 与 `dashboard-echarts-panel-failure.test.tsx` 全绿；dashboard 包测试全量回归。

Exit Criteria:

- [ ] 机制证明测试全绿（setOption 经 panel fragment 到达真实 echarts 渲染器定义）。
- [ ] source 绑定边界有行为断言 + 文档 authoring 指引；降级隔离测试证明兄弟面板不受影响。
- [ ] `integration-nop-datav.md` 落地且结论与测试证据一致。
- [ ] `pnpm --filter @nop-chaos/flux-renderers-dashboard test` 全绿。
...
- [ ] 所有 in-scope 项已落地：机制证明测试 + 评估文档
- [ ] 行为/契约结果已达成：echarts 面板经 dashboard 通用机制 setOption 到达
- [ ] 必要 focused verification 已完成：dashboard-echarts-panel 测试全绿
- [ ] 不存在被静默降级到 deferred / follow-up 的 in-scope live defect 或 contract drift
- [ ] 受影响的 owner docs 已同步（integration-nop-datav.md、design.md 锚点、roadmap E5.2 → done、daily log 收口记录）
