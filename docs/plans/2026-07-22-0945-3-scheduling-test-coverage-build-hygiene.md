...
- [ ] Analyze uncovered branches in `use-kanban-dnd.ts` (35.13% branch): add test cases for mid-drag cancellation, boundary conditions (drag to edge of board), drop on invalid target, multi-touch guard paths
- [ ] Analyze uncovered branches in `use-column-dnd.ts` (12.5% branch): add tests for column reorder edge cases, drop at start/end of column list, drag cancellation
- [ ] Assess `use-gantt-drag.ts` coverage: add tests for drag handle start/end, resize, and cancellation paths
- [ ] Verify all new tests pass and branch coverage meets >=45% threshold for each hook (or document known-unreachable branches)

Exit Criteria:

- [ ] `use-kanban-dnd.ts` branch coverage >=45%
- [ ] `use-column-dnd.ts` branch coverage >=45%
- [ ] `use-gantt-drag.ts` branch coverage >=45% (or measured improvement documented)
- [ ] All new tests pass
...
- [ ] Create `gantt.integration.test.tsx`: render `<Gantt>` with minimal mock data (3-5 tasks, 2 dependencies), assert timeline renders, task bars render with correct labels, dependency arrows render, header shows configured time scale
- [ ] Create `kanban.integration.test.tsx`: render `<KanbanBoard>` with 2-3 columns, 2-3 cards each, assert columns render in order, cards render with correct content, column headers show configured titles
- [ ] Create `calendar.integration.test.tsx`: render `<Calendar>` with mock events, assert month grid renders correct number of cells, events render at correct positions (within limits of DOM-based assertions), view switching works

Exit Criteria:

- [ ] Full integration test for each major scheduling component (Gantt, KanbanBoard, Calendar)
- [ ] Each test renders the real component (not just sub-components) and asserts DOM structure
...
- [ ] Option A (preferred if feasible): Add PostCSS/lightningcss processing step to the scheduling package build script for `calendar.css`, resolving `@apply` directives before copy-assembly. Requires adding `tailwindcss` as a devDependency and configuring PostCSS.
- [ ] Option B (fallback): Replace `@apply` directives in `calendar.css` with standard CSS equivalents (e.g., `@apply flex flex-col h-full min-h-0` → `display: flex; flex-direction: column; height: 100%; min-height: 0;`)
- [ ] Verify Calendar renders identically before and after CSS change (visual comparison or computed style assertions)
- [ ] Update `docs/architecture/styling-system.md` with guidance on CSS file handling for copy-assembled packages

Exit Criteria:

- [ ] `calendar.css` contains no `@apply` directives, OR `@apply` directives are processed during build
- [ ] Calendar component renders identically (no visual regression)
- [ ] `docs/architecture/styling-system.md` updated with build guidance
...
- [ ] DnD branch coverage meets >=45% for use-kanban-dnd, use-column-dnd, use-gantt-drag (or documented exceptions)
- [ ] Integration tests exist for Gantt, KanbanBoard, and Calendar rendering full components with DOM assertions
- [ ] `calendar.css` contains no unresolved `@apply` directives in the build output
- [ ] Calendar renders identically before and after CSS change
- [ ] All scheduling package tests pass (existing + new)
- [ ] `docs/architecture/styling-system.md` updated with CSS build guidance
- [ ] `pnpm typecheck`
- [ ] `pnpm build`
- [ ] `pnpm lint`
- [ ] `pnpm test`
