...
- [x] Re-audit the exact disposed-owner path in `applyChangesAndRevalidate(...)` against the already-established lifecycle-blocked baseline used by sibling validation entrypoints.
- [x] Define the narrowest focused regression proof that extends the existing disposed-owner blocked-result contract to `applyChangesAndRevalidate(...)`.

Exit Criteria:

- [x] The plan records why `08-01` is a result-integrity residual extending an existing blocked-result baseline rather than a reopened bootstrap/trigger plan.
- [x] Focused proof strategy is explicit for the disposed-owner path.
- [x] `docs/architecture/form-validation.md` is updated or explicitly adjudicated for disposed-owner non-clean-success semantics.
...
- [x] Change the disposed `applyChangesAndRevalidate(...)` branch so it returns an explicit lifecycle-blocked result instead of a clean-success result.
- [x] Add or update focused tests proving disposed owners do not report `ok: true` when validation never ran.

Exit Criteria:

- [x] The disposed-owner revalidation path no longer returns a clean-success result that is indistinguishable from “no validation errors”.
- [x] Focused proof is green for the fixed lifecycle-blocked branch.
- [x] `docs/architecture/form-validation.md` matches the final disposed-owner result baseline.
...
- [x] Run an independent closure audit with a fresh subagent that re-reads this plan, linked analysis, live code/tests/docs, and verification output.

Exit Criteria:

- [x] Focused verification for `08-01` has passed.
- [x] `pnpm typecheck`, `pnpm build`, `pnpm lint`, and `pnpm test` pass.
- [x] `docs/architecture/form-validation.md` matches the final baseline, or `No additional owner-doc update required in Phase 3` is explicit.
- [x] Independent closure audit confirms no remaining validation lifecycle result-integrity blocker.
- [x] This plan's statuses, checklists, closure gates, and daily log evidence are textually consistent.

## Closure Gates

- [x] The in-scope confirmed live defect (`08-01`) is fixed.
- [x] The disposed-owner revalidation path (`2026-05-17/08-01`) converges to one honest lifecycle-blocked result baseline.
- [x] Necessary focused verification exists for the disposed-owner path.
- [x] No in-scope live defect is silently downgraded to deferred/follow-up.
- [x] `docs/architecture/form-validation.md` is synced to the final disposed-owner result baseline.
...
Status Note: Completed. `applyChangesAndRevalidate(...)` now returns the shared lifecycle-blocked result baseline for disposed owners, and focused/runtime-wide verification stayed green.

Closure Audit Evidence:

- Reviewer / Agent: `ses_1c9c98c7dffeT2tkRiULA7FBJX` (`general` subagent)
- Evidence: Final independent closure audit confirmed no remaining live blocker for `08-01`; full verification stayed green via `pnpm typecheck`, `pnpm build`, `pnpm lint`, and `pnpm test` (`C:\Users\a758371\.local\share\opencode\tool-output\tool_e362d7ff6001MPuSig14CcdoVo`).
