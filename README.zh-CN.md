...
_一个面向低代码系统的 Schema 驱动运行时，建立在七个原语之上。_
...
对于带有语义 owner 的界面，Flux 也保留了一小组 owner-aware 内置动作。例如，`submitForm` 会作用于当前表单运行时，`closeSurface` 默认关闭最近的活动 surface。`closeDialog` 仍保留为面向 dialog 形态的兼容别名。当触发器需要从当前 owner 上下文之外显式定位某个实例时，仍然可以使用 `component:submit` 这类显式实例目标动作。
