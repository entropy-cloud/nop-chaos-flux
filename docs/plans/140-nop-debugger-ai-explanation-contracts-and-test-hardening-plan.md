...
- [ ] 定义 AI-first explanation 契约，而不是新增泛化快照包装器。至少包含以下稳定接口中的若干个，或等价但职责清晰的接口：`explainNodeValue(...)`、`explainNodeMeta(...)`、`explainNodeFailure(...)`、`explainNodeAsync(...)`。
- [ ] 为每个 explanation 接口定义机器可消费的结果 schema，至少包括：`kind`、`subject`、`answer`、`confidence/limitations`、`evidenceRefs`、`relatedCid/nodeId/path/requestInstanceId/interactionId`、`truncated`。
- [ ] 明确解释结果的 bounded payload 规则：默认限制 evidence 数量、scope depth、event count、async owner count，禁止返回无界大对象。
- [ ] 明确哪些字段属于稳定 automation contract，哪些字段只是 debug-safe summary；不要把 runtime 私有对象当作输出契约。
- [ ] 如果保留 `getNodeDeepDiagnostics()`，将其降级为 convenience wrapper，明确它不是本计划的主价值面。

Exit Criteria:

- [ ] explanation contracts 的主价值是回答 AI 的解释型问题，而不是再返回一份快照集合。
- [ ] 每个 explanation 结果都有稳定、边界明确、机器可消费的 schema。
- [ ] `docs/architecture/debugger-runtime.md` 明确把这些 explanation 接口列为 AI/E2E 推荐入口。
...
- [ ] 为节点值解释设计最小数据来源摘要，至少能区分：来自当前 scope、自表单 store、来自 resolved props、来自 resolved meta、来自上层 scope 可见值、当前无法可靠解释。
- [ ] 为 meta explanation 设计最小因果摘要，至少覆盖 `visible`、`hidden`、`disabled`、`label/title` 等 AI 常问字段，并说明依据是 props/meta snapshot、scope input，还是当前无法可靠证明的推断。
- [ ] 为 explanation 结果增加 `limitations` 或等价字段，明确哪些回答是“基于当前可见快照的保守解释”，避免把不完整数据伪装成确定性事实。
- [ ] 如果 live repo 现有 debug data 不足以支撑 value-origin/meta-causality，补最小只读 debug contract，而不是直接暴露 runtime 私有结构。
- [ ] 为 Node Tab 适配 explanation 结果，但保持 UI 为次要消费方，automation API 为主。

Exit Criteria:

- [ ] AI 能直接询问节点值和关键 meta 为什么是当前状态，而不是只看到一份 `scopeData` 快照后自己猜。
- [ ] explanation 结果能明确说出“已知依据”和“当前无法证明的部分”。
- [ ] 新增数据契约仍保持只读、debug-safe、边界清晰。
...
- [ ] 在现有 failure summary、interaction trace、async owner snapshot 的基础上，新增 node-scoped 的解释接口，回答“这个节点最近为什么失败/异常/不发布结果”。
- [ ] explanation 至少要覆盖：最近失败事件、关联 request instance、interaction、相关 async owner、probable cause hints、当前证据不足时的 limitation。
- [ ] 为 async explanation 增加 node-scoped query 规则，只返回与当前节点直接相关或最近相关的 owner 摘要，不返回整份全局 owner 列表。
- [ ] 让 failure/async explanation 结果引用现有 focused APIs 的证据，而不是复制一份松散的大对象。
- [ ] 为常见 AI 问题定义 scoped selectors，例如按 `cid`、`nodeId`、`path` 或最近 failure infer 的模式查询，但保持 live identity 仍以 `cid` 优先。

Exit Criteria:

- [ ] AI 能直接获得节点级失败原因摘要，而不是手工拼接 `getInteractionTrace()`、`getLatestFailedRequest()`、`getAsyncOwnerDebugSnapshot()`。
- [ ] async explanation 是 node-scoped、bounded 的，不会返回难以消费的全局大列表。
- [ ] failure/async explanation 重用现有 focused evidence，而不是制造第二套松散诊断模型。
...
- [ ] 在 playground 中增加 deterministic fixture，至少覆盖以下已知真相场景：
...
- [ ] 为 explanation contracts 编写 focused unit/controller tests，验证返回的是正确解释语义，而不是仅验证字段存在。
- [ ] 为 explanation contracts 编写 Playwright tests，使用 `page.evaluate()` 直接调用 `window.__NOP_DEBUGGER_API__` 并断言 explanation 结果与 fixture 的已知真相一致。
- [ ] 为 boundedness 编写测试，验证 evidence/event/async owner 数量受限，且结果通过 `truncated` 或等价字段显式说明裁剪。
- [ ] 为 redaction 编写测试，验证 explanation 结果引用的导出数据仍遵守脱敏边界。

Exit Criteria:

- [ ] Playwright 能证明 AI 在真实页面上发起解释型问题时，返回结果与 fixture 的已知真相一致。
- [ ] 单测和 E2E 保护 explanation 语义、boundedness、redaction，而不是只保护 API 可调用。
...
- [ ] 更新架构文档，把 explanation contracts、bounded schema、AI 推荐查询方式写成当前基线。
- [ ] 在文档中明确当前 explanation 的能力边界和 limitation 语义，避免过度承诺“完全因果解释”。
- [ ] 在 daily log 记录本计划的关键取舍：为何不再以聚合快照 API 为主，为何 explanation 结果需要 bounded schema。
- [ ] 完成后进行独立 closure audit，验证本计划实现的是 explanation contract 语义，而不是又一次接口包装。

Exit Criteria:

- [ ] 文档明确说明 explanation contracts 是 AI-first debugger 的主入口。
- [ ] 文档与 live code 对 limitation/boundedness 的描述一致。
- [ ] closure audit 明确检查“解释语义已落地”，而不是仅检查方法名存在。

## Validation Checklist

- [ ] 至少一组 explanation contracts 已落地，并能直接回答 value/meta/failure/async 解释型问题。
- [ ] explanation 结果是 machine-oriented schema，而不是随意拼接的大对象。
- [ ] explanation 结果明确区分已知依据、限制条件、相关证据引用。
- [ ] boundedness 规则已落地并有测试保护。
- [ ] redaction/export 边界对 explanation 结果仍然成立。
- [ ] deterministic playground fixtures 已落地，覆盖 value origin、meta causality、async/failure chain。
- [ ] focused unit/controller tests 已覆盖 explanation 语义。
- [ ] Playwright contract tests 已覆盖 explanation 契约。
- [ ] `docs/architecture/debugger-runtime.md` 已更新。
- [ ] `docs/logs/2026/04-25.md` 已更新。
...
- Reviewer / Agent: independent subagent review already performed for draft correction via `ses_23af1f5c3ffeUBnDr3UNrwRBl3`; final closure audit pending after implementation
- Evidence: draft review concluded the plan must prioritize AI explanation contracts over duplicative aggregation APIs

Follow-up:

- 如果本计划完成后仍缺“依赖传播全链路解释”或“更精细的值来源证明”，应拆 successor plan；不要在本计划 closure 时隐含保留未归属 debt。
