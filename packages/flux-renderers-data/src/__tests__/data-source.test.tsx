...
describe('dataRendererDefinitions data-source behavior', () => {
  it('allows formula data-sources to publish by name without dataPath', async () => {
    ...
    render(<SchemaRenderer schema={{ type: 'page', body: [{ type: 'data-source', name: 'greeting', formula: '${"hello"}' }, { type: 'text', text: '${greeting}' }] }} env={env} formulaCompiler={formulaCompiler} />);
  ...
  it('evaluates formula sources into explicit dataPath bindings', async () => {
    cleanup();
    const SchemaRenderer = createDataSchemaRenderer();
    render(<SchemaRenderer schema={{ type: 'page', body: [{ type: 'data-source', dataPath: 'total', formula: '${4 * 6}' }, { type: 'text', text: 'Total: ${total}' }] }} env={env} formulaCompiler={formulaCompiler} />);
  ...
  it('fetches data and injects into scope', async () => {
    ...
    const SchemaRenderer = createDataSchemaRenderer();
    render(<SchemaRenderer schema={{ type: 'page', body: [{ type: 'data-source', api: { url: '/api/user/1' }, dataPath: 'user' }, { type: 'text', text: 'Hello, ${user?.name}' }] }} env={{ ...env, fetcher }} formulaCompiler={formulaCompiler} />);
  ...
  it('uses initialData before fetch completes', async () => {
    ...
    const SchemaRenderer = createDataSchemaRenderer();
    render(<SchemaRenderer schema={{ type: 'page', body: [{ type: 'data-source', api: { url: '/api/user/1' }, dataPath: 'user', initialData: { name: 'Initial' } }, { type: 'text', text: 'Hello, ${user?.name}' }] }} env={{ ...env, fetcher }} formulaCompiler={formulaCompiler} />);
  ...
  it('shows error message on fetch failure', async () => {
    ...
    const notify = vi.fn();
    const SchemaRenderer = createDataSchemaRenderer();
    render(<SchemaRenderer schema={{ type: 'page', body: [{ type: 'data-source', api: { url: '/api/error' }, dataPath: 'data' }] }} env={{ ...env, fetcher, notify }} formulaCompiler={formulaCompiler} />);
  ...
  it('suppresses error notification when silent is true', async () => {
    ...
    const notify = vi.fn();
    const SchemaRenderer = createDataSchemaRenderer();
    render(<SchemaRenderer schema={{ type: 'page', body: [{ type: 'data-source', api: { url: '/api/error' }, dataPath: 'data', silent: true }] }} env={{ ...env, fetcher, notify }} formulaCompiler={formulaCompiler} />);
  ...
  it('keeps cache isolated between independent renderer roots', async () => {
    ...
    const SchemaRenderer = createDataSchemaRenderer();
    const schema = { type: 'page', body: [{ type: 'data-source', api: { url: '/api/data', cacheTTL: 60000, cacheKey: 'test-cache' }, dataPath: 'data' }, { type: 'text', text: 'Value: ${data?.value}' }] } as const;
    const { unmount } = render(<SchemaRenderer schema={schema} env={{ ...env, fetcher }} formulaCompiler={formulaCompiler} />);
    await waitFor(() => expect(screen.getByText('Value: cached')).toBeTruthy());
    const firstRenderCallCount = fetcherSpy.mock.calls.length;
    unmount();
    cleanup();
    render(<SchemaRenderer schema={schema} env={{ ...env, fetcher }} formulaCompiler={formulaCompiler} />);
    await waitFor(() => expect(screen.getByText('Value: cached')).toBeTruthy());
