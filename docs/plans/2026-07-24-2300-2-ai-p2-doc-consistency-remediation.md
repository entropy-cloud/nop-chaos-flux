...
- [ ] **§3.2 BubbleBoxRendererMatch phantom 清理**（Fix）：`renderers.md:119` 将 phantom `BubbleBoxRendererMatch` 代码块替换为 live 接口（`BubbleContentRendererMatch` + `BubbleToolRendererMatch`，从 `ai-bubble/types.ts` 提取真实签名）
- [ ] **§13 onScrollTop phantom 清理**（Fix）：`renderers.md:563` Events 表删除 phantom `onScrollTop` 行（或替换为 live event）；逐行核对 ai-message-list 的实际 events（从 `schemas.ts` AiMessageListSchema 提取）
- [ ] **§1.3 ai-chat DOM 核对**（Fix）：核对 `ai-chat.tsx` live 渲染输出与 `renderers.md:43-53`（§1.3 DOM 结构），修正不一致（如 sender 是否由 `<footer>` 包裹等）
- [ ] **§6 目录树补齐 6 文件**（Fix）：执行 `ls -R packages/flux-renderers-ai/src/` 与 design.md §6 目录树比对，补入缺失文件
- [ ] **§3.3 default renderer 计数核对**（Fix）：核对 `packages/flux-renderers-ai/src/renderers/ai-bubble/renderers/default-renderers.ts` 实际导出数，订正 renderers.md §3.3（`:139`）表格
- [ ] **§15 data-slot 速查表补齐**（Fix）：执行 `rg "data-slot=" packages/flux-renderers-ai/src/renderers/` 扫描全部 slot，与 renderers.md §15（`:655`）tree 比对，补入缺失项

Exit Criteria:

- [ ] `rg "BubbleBoxRendererMatch" docs/components/flux-renderers-ai/` 仅命中 "不存在" 注记（或零命中）
- [ ] `rg "onScrollTop" docs/components/flux-renderers-ai/` 零命中（或仅注记）
- [ ] design.md §6 目录树与 `ls -R packages/flux-renderers-ai/src/` 一致
- [ ] §3.3（`:139`）renderer 表与 `packages/flux-renderers-ai/src/renderers/ai-bubble/renderers/default-renderers.ts` 导出一致
- [ ] §15（`:655`）data-slot tree 覆盖全部 `rg "data-slot=" packages/flux-renderers-ai/src/renderers/` 扫描结果
...
- [ ] **engine.md createNativeMessageAdapter 标签订正**（Fix）：`engine.md:179` 将 "纯 TS，闭包持有 state，测试用" 订正为反映其生产默认 + 公共导出身份的描述（如 "纯 TS，闭包持有 state；`ai-chat` 的默认 adapter 创建路径，经 `index.ts` 公共导出"）
- [ ] **terminology.md 补 AI 包核心词条**（Fix）：在 `docs/references/terminology.md` 补入 `MessageEngine` / `AiConnector` / `ChatMessage` / `MessageStateAdapter` / `ReactMessageAdapter` 等 AI 包公共契约类型条目（每条含一行定义 + 指向 `engine.md`/`design.md` 的交叉引用）
- [ ] **AGENTS.md 包列表补 flux-renderers-ai**（Fix）：`AGENTS.md:9` Renderer packages 枚举中补入 `flux-renderers-ai`（AI 对话渲染器族：消息气泡、流式输出、会话管理、工具调用、HITL 等），与其他 renderer 包格式一致

Exit Criteria:

- [ ] `engine.md:179` 不再标 createNativeMessageAdapter 为 "测试用"
- [ ] `terminology.md` 含 `MessageEngine`/`AiConnector`/`ChatMessage`/`MessageStateAdapter` 条目
- [ ] `AGENTS.md:9` 包列表含 `flux-renderers-ai`
...
- [ ] renderers.md 无 phantom 引用（BubbleBoxRendererMatch / onScrollTop 等）
- [ ] design.md §6 目录树与 live 目录一致
- [ ] renderers.md §3.3 renderer 表 + §15 data-slot tree 与 live code 一致
- [ ] engine.md createNativeMessageAdapter 标签已订正
- [ ] terminology.md 含 AI 包核心词条
- [ ] AGENTS.md 包列表含 flux-renderers-ai
- [ ] 不存在被静默降级到 deferred 的 in-scope doc drift
