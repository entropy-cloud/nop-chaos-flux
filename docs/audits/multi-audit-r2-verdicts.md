...
| 14-4  | canvas-bridge.test.tsx setup 膨胀 + 固定空快照 mock             | 14-test-coverage.md        | 属实                         | successor        | registered（flow-designer 域）                            |
| 15-1  | relayoutTree 全文档 JSON.stringify ×2 变更检测                  | 15-security-performance.md | 属实（非热路径）             | successor        | registered（flow-designer 域）                            |
| 15-2  | clampShellWidth NaN 无防护 + action-provider 放行 NaN           | 15-security-performance.md | 属实（fail-closed 缺口）     | successor        | registered（flow-designer 域，confirmed defect 显式登记） |
...
| 17-2  | graph 事件 payload type 命名空间不一致                          | 17-naming.md               | 属实                         | successor        | registered（graph 域）                                    |
| 19-3  | designer JSON.parse 失败静默 null                               | 19-error-propagation.md    | 属实（reportHostIssue 可用） | successor        | registered（flow-designer 域）                            |
...
- successor 5 项已登记（本表 + plan Deferred But Adjudicated），零悬挂。
...
| 14-4           | successor                              | flow-designer 域：`canvas-bridge.test.tsx` 588 行、setup/mock `:1-212`（36%）、固定空快照 mock `:91` 属实；提取 test-support.tsx + 真实 fixture 属测试重构，归 flow-designer owner plan 链                                                    |
| 15-1           | successor                              | flow-designer 域：`tree-session-impl.ts:310-311` 两次全文档 JSON.stringify 属实；非热路径，修订计数/浅比较优化归 flow-designer owner plan 链                                                                                                  |
| 15-2           | successor（confirmed defect 显式登记） | flow-designer 域：`shell-controls.ts:23` `Math.max(NaN,min)===NaN` + `designer-action-provider.ts:447-453` typeof number 放行 NaN 属实；Number.isFinite 守卫 + `{ok:false}` 归 flow-designer owner plan 链（不降级为非阻断）                  |
...
| 17-2           | successor                              | graph 域：`graph-renderer.tsx:157` payload type 用事件字段名属实；命名空间化（graph:node-click）需与 normalizeActionEvent 'custom' 合成规则对齐，归 graph G1 plan 链                                                                          |
| 19-3           | successor                              | flow-designer 域：`designer-page-body.tsx:193-200` catch→null 属实；同包 `:219` reportHostIssue 可用；结构化失败 + monitor 上报归 flow-designer owner plan 链                                                                                 |
