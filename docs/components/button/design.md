...
| 倒计时            | **实现**：`countDown`（秒）/ `countDownTpl`（模板，`{timeLeft}` token，缺省 `"{timeLeft}s"`）           | —                                                                                          | 推翻原"不采纳"裁定：ERP HR 考勤打卡/SMS 验证码/B2B 验证码均需此能力；**持久化（2026-08-03 C1.3 修订）**：经 host 注入的 `countDownStorage` adapter（B 档 import 注入模式，`renderer-env.md` §9）——renderer 永不直调 localStorage（INV-1 硬红线）；无 adapter 时 countdown 为 session-only；仅在有显式 `id`/`name` 时持久化，key 含 `location.pathname` 避免跨页面碰撞，无 `id`/`name` 时不持久化；timer handle 经 `setInterval` + cleanup 管理无泄漏；基于 `Date.now()` 差值避免漂移；action 成功后（`onClick` promise resolve）才启动，失败（reject）不启动且不外泄 unhandled rejection |
| tooltip 位置控制  | **实现**：`tooltipPlacement: { side?, align? }`（`side` 缺省 `'top'`，透传到 `@nop-chaos/ui` Tooltip）  | —                                                                                          | 对象形式对齐 shadcn Tooltip `side`/`align`；amis `tooltipPlacement` flat enum 的 Flux 升级                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 链接模式          | **实现**：`href`（渲染为 `<a>`）/ `target`                                                              | —                                                                                          | amis 链接按钮；保持单一 `button` type，通过 `href` 切换渲染元素                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
- 导出字段：`label`、`variant`、`size`、`disabled`（基线）+ `icon?: string`、`rightIcon?: string`、`loading?: boolean | string`、`tooltip?: string`、`disabledTip?: string`、`tooltipPlacement?: { side?, align? }`、`countDown?: number`、`countDownTpl?: string`、`href?: string`、`target?: string`、`block?: boolean`、`active?: boolean | string`（E2e 新增）。
...
- `countDownTpl` 使用 `{timeLeft}` token（非 `${timeLeft}`，避免 Flux 表达式编译器把 `${...}` 当作 scope 表达式求值）。
...
- amis `reload`（定向重载）在 Flux 无字面 built-in；按目标走 `refreshSource`（source `name`）或 `component:refresh`（`componentId`/`componentName`），均**不**触发整页 `refreshTable`。详见 `docs/architecture/action-scope-and-imports.md`「amis `reload` → Flux 映射」。
