...
export function compileApiConfig(
  ...
  return {
    ...
    method: api.method !== undefined
      ? compiler.compileValue<string>(api.method, options)
      : undefined,
    data: api.data !== undefined
      ? compiler.compileValue<unknown>(api.data, options)
      : undefined,
    params: api.params !== undefined
      ? compiler.compileValue<unknown>(api.params, options)
      : undefined,
...
function compileOperationControl(
  schema: DataSourceSchema
): CompiledOperationControl | undefined {
  const control = (schema as { control?: { dedup?: string; throttle?: number; cacheTTL?: number; cacheKey?: string } }).control;
...
export function compileDataSource(
  ...
  if (isFormulaSource) {
    compiled.formula = compiler.compileValue<unknown>(
      (schema as { formula: unknown }).formula,
      { ...options, sourcePath: `${basePath}.formula` }
    );
  }
  ...
  if (schema.resultMapping !== undefined) {
    compiled.resultMapping = compiler.compileValue(
      schema.resultMapping,
      { ...options, sourcePath: `${basePath}.resultMapping` }
    ) as unknown as CompiledRuntimeValue<Record<string, string>>;
  }

  if (schema.mergeStrategy !== undefined) {
    compiled.mergeStrategy = compiler.compileValue(
      schema.mergeStrategy,
      { ...options, sourcePath: `${basePath}.mergeStrategy` }
