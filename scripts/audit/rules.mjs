...
function scanPropsDestructureInFunctionBody({ rule, relativePath, content }) {
  const results = [];
  const lines = content.split(/\r?\n/);
  const pattern = /function\s+[A-Z][A-Za-z0-9_$]*\s*\(\s*\{\s*([^}]*)\}\s*:\s*\w/g;
  let match;
  ...
  while ((match = pattern.exec(content)) !== null) {
    ...
    if (lineText.includes('RendererComponentProps') || lineText.includes('Props)')) {
      const braceOpen = content.indexOf('{', match.index);
      const braceEnd = findMatchingBrace(content, braceOpen);
      if (braceEnd < 0) continue;

      const body = content.slice(braceOpen, braceEnd + 1);
      ...
      for (const prop of destructuredProps) {
        if (!prop) continue;
        const usagePattern = new RegExp(`\\b${prop}\\b`, 'g');
        const usages = body.match(usagePattern);
        if (usages && usages.length <= 1) {
          continue;
        }
      }

      const paramCount = destructuredProps.filter(Boolean).length;
      if (paramCount > 15) {
        results.push(
          createResult(
            rule,
            ...
            line,
            lineText,
            `component has ${paramCount} destructured props (consider splitting)`,
          ),
        );
      }
    }
