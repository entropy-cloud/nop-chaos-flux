...
### F7. Styling docs and live renderer/debugger code now converge on one styling contract baseline
...
- Code evidence: `packages/flux-react/src/field-frame.tsx`, `packages/flux-renderers-data/src/table-renderer.tsx`, `packages/flux-renderers-data/src/tree-renderer.tsx`, `packages/flux-renderers-form/src/renderers/condition-builder/ValueInput.tsx`, `packages/nop-debugger/src/panel.tsx`, `packages/nop-debugger/src/panel/use-inspect-mode.ts`, `packages/nop-debugger/src/panel/network-tab.tsx`
- Why: the styling docs now align on one future baseline and the live code has been brought into closure-ready agreement for the successor-owned scope: root `nop-*` markers are semantic only, internal regions use `data-slot`, state uses `data-*`, renderer/debugger raw controls have been narrowed to the supported `@nop-chaos/ui` or domain-special cases, and focused DOM assertions now lock the shared contract across renderer families plus debugger surfaces. The remaining production raw-input exceptions are explicitly accepted by owner docs rather than treated as drift: spreadsheet canvas edit inputs stay inside the perf-first `ss-*` / inline-style / `data-*` canvas boundary, and word-editor `type="file"` / `type="color"` controls remain special semantic inputs.
...
| `docs/architecture/flux-runtime-module-boundaries.md` | consistent | The ownership map now reflects the active runtime split for `data-source`, `reaction`, and shared `surface-runtime` modules. |
...
| `docs/architecture/api-data-source.md` | consistent | Current source/reaction docs now match the live baseline: `resultMapping`, named publication, `mergeToScope`, `statusPath`, explicit `dependsOn`, scope-scoped source/reaction registries, and public debug snapshots are all implemented and covered in focused runtime/renderer tests. |
...
| `docs/architecture/renderer-markers-and-selectors.md` | consistent | The document is aligned with `styling-system.md`, and the successor-owned cleanup is now landed: extra internal `nop-tabs-*`, `nop-cb-item`, `nop-cb-group`, `nop-dialog-card`, `nop-drawer-card`, `nop-debugger--minimized`, and `nop-debugger-overlay--*` markers are gone in favor of `data-slot` or `data-*` state, while focused coverage now spans `FieldFrame`, page/container/scope-debug, table/tree/chart, the form root/tree-control family, the composite/detail/runtime-owned form editors, condition-builder `NativeSelect`, flow-designer entry surfaces, spreadsheet page root/header/body, word-editor page shell, report-designer host surfaces, and debugger minimized/overlay/filter/tree-selection state. |
...
| `docs/architecture/styling-system.md` | consistent | The normative no-implicit-layout contract is now aligned with `field-frame.md`, `renderer-markers-and-selectors.md`, and the live successor-owned code. Recent cleanup normalized report/flow roots and helper merges, removed non-semantic internal markers across the shared renderer families, replaced the remaining condition-builder raw select affordance with `NativeSelect`, and closed the final debugger drift by moving minimized/overlay state to `data-*`, replacing live debugger raw controls with `@nop-chaos/ui` primitives, and covering the resulting contract under focused tests. Accepted production exceptions are now explicit rather than ambiguous drift: spreadsheet canvas internals remain perf-first, and word-editor `file` / `color` inputs remain special semantic controls. |
| `docs/architecture/theme-compatibility.md` | consistent | The document now uses the current `.nop-theme-root` / `.fd-theme-root` terminology, points at the current playground/renderer entry anchors instead of the retired `FlowDesignerExample` path, and keeps the host-CSS-variable contract aligned with the live debugger/dialog/renderer baseline. |
