...
>
> `2026-08-08-1910-3`（A3/A4）✅ completed（A3 零缩放守卫自毁收口：`readZoomLayerScale` 显式拒 0（`&& scaleX !== 0`）→ 回退 `this.viewport.scale`（命令路径经 `clampScale` MIN_SCALE=0.1 恒非零有限），闭合 P2-8 早退守卫的「自毁环路」（守卫检测到 0 调 syncViewportFromZoomLayer 再读回 0 落地 viewport.scale=0 → viewportToWorld 除 0 产 Infinity → EventBridge world 坐标 corrupt → zoomLayer 矩阵不可逆）；`handlePluginMove` 补与 `handlePluginZoom` 同形零守卫（defense-in-depth）；A4 runtime scada-canvas 遵守 `meta.disabled`/`meta.visible` 契约（与同包编辑器画布 `scada-editor-canvas.tsx:217` 同纪律）：disabled 时不下挂图元事件回调（EventBridge 闭包经 optional chaining 早退 → dispatch 不执行、hover 覆盖物不跟随）+ wrapper `aria-disabled`/`inert`，visible:false 时 renderer 层 return null（不创建引擎）；failing-first 7 测断言 viewport.scale 非零有限 + disabled 不派发 + visible:false 不渲染，industrial 1394 tests 零回归；typecheck/build/lint 全绿；owner doc 同步 design-engine §4.4(A3) + design-renderer §5(A4)）。
