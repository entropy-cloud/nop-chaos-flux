...
| 远程异步搜索（debounce）                                                                    | **实现（S4）**：`searchSource`（`ActionSchema`）—— typing 触发 debounced (300ms) action dispatch，scope 含 `searchQuery`；AbortController 取消；结果按 `searchMergeMode`（`'append'`/`'replace'`）合并到本地 options。空查询回退到本地 options。见 §4 schema 设计、§9 数据源接入 | amis 组件级 `api`/`initFetch`                            | 请求下沉 data-source + action（X3 §1/§3）。S4 实现在 select renderer 内置 `searchSource`（镜像 input-tree 的 `searchSource` 模式），经 `useSelectRemoteSearch` hook 管理 debounce、AbortController、结果合并。                                                                                      |
...
- `placeholder` 已实现。
- **S4 远程搜索**新增字段：
  - `searchSource?: ActionSchema`：远程搜索数据源。`searchable: true` 且 `searchSource` 声明时，input 触发 debounced（300ms）action dispatch，scope 含 `searchQuery`。结果经 `searchMergeMode` 合并到 options。
  - `searchMergeMode?: 'append' | 'replace'`（默认 `'append'`）：远程结果追加到本地 options（`'append'`）或完全替换（`'replace'`）。空查询（输入清空）回退到本地 options。
...
- `searchSource`: `value`（`ActionSchema`，S4 远程搜索）
- `searchMergeMode`: `value`（`'append' | 'replace'`，S4 结果合并策略）
...
- S4 `searchSource` 作为 `ActionSchema` 接入远程搜索：typing → 300ms debounce → action dispatch（scope 含 `searchQuery`）→ 结果合并。`useSelectRemoteSearch` hook 管理 debounce、AbortController、loading/error 状态。
