...
| 自动轮询刷新                                                                     | **计划实现（E1d）**：走 data-source `interval`                                                                                                                    | amis 组件级 `api`/`initFetch`/`sendOn`/`asyncApi`/`silentPolling`/`stopAutoRefreshWhen` | 请求下沉 data-source + action，不开组件级短路径（X3 §1/§3）                                                                                                                          |
| 可折叠查询区（`filterTogglable`）                                                | **计划实现（E1d）**                                                                                                                                               | —                                                                                       | 当前 queryForm 永远内联可见                                                                                                                                                          |
| 无限滚动分页                                                                     | **计划实现（E1d）**                                                                                                                                               | —                                                                                       | 当前仅分页                                                                                                                                                                           |
| cards/list 模式                                                                  | **计划实现（E1d）**：依赖主 roadmap W1c（list）/ W2a（cards）                                                                                                     | —                                                                                       | Q2 倾向"等主 roadmap list/cards 落地后作为集成"，本表不越权裁决归属                                                                                                                  |
