...
> 过时标记（2026-04-07）：本阶段按旧的 deep-path dependency baseline 编写。其“精确 path 列表 + wildcard/full-scope 依赖 + hierarchical hit 规则”已被 `docs/plans/39-dependency-tracking-root-scope-implementation-plan.md` 取代，保留本段仅供历史回溯，不应再作为当前实现说明。
...
> 过时标记（2026-04-07）：本阶段中“`ScopeChange.paths` 与节点依赖集做交集判断”的细化描述，原本默认沿用 Phase 1 的 deep-path 语义。当前应改以 Plan 39 的 normalized-root hit 规则理解；若继续实现，请直接以 Plan 39 为准。
...
- `table.paginationOwnership` 已支持 `local`、`controlled`、`scope`。
- `table.selectionOwnership` 已支持 `local`、`controlled`、`scope`。
- `scope` 首版通过显式路径字段 `paginationStatePath` / `selectionStatePath` 落地，table 直接读写当前 render scope，而不是引入通用 table-state DSL。
- table component handle 首版已落地：`component:refresh`、`component:getSelection`、`component:setSelection`。
- `refresh` capability 已修正为保留调用方 action context，使 `refreshSource` 继续按正确 scope 解析 runtime-owned source。
- 当前仍刻意保持 `sort` / `filter` / `expand` 为 `local`。
...
- [部分过时] 依赖追踪若错误地把 dynamic/full-scope 访问当成精确 path，会导致漏刷新。该表述写于旧 deep-path baseline；当前有效风险边界以 Plan 39 的 root-binding / wildcard 退化规则为准。
