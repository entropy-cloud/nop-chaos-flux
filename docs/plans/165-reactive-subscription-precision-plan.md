...
- [ ] 分析 `useSurfaceScopeSnapshot` 当前消费方：每个 dialog/drawer 的 surface body 通过 `useSurfaceScopeSnapshot()` 获取 scope snapshot，然后用 `useSyncExternalStore` 订阅
- [ ] 将 `useSurfaceScopeSnapshot` 改为接受可选的 `paths: string[]` 参数：
  - 当提供 `paths` 时，selector 从 `readVisible()` 中只提取指定路径的值，equalityFn 使用 `shallowEqual` 比较提取结果
  - 当未提供 `paths` 时，保持全 scope 订阅行为（向后兼容）
- [ ] 添加测试：验证 dialog 打开时，修改 scope 中 dialog 未使用的路径不会触发 dialog body 重渲染
- [ ] 添加测试：验证 dialog 使用了 scope.data.a 时，修改 scope.data.b 不触发重渲染，修改 scope.data.a 触发重渲染
- [ ] 验证：现有 `dialog-host-surface.tsx` 的消费方（`dialog-host.tsx` 中 `DialogView` 和 `DrawerView`）在不提供 `paths` 参数时行为不变

Exit Criteria:

- [ ] `useSurfaceScopeSnapshot` 在提供 `paths` 时只在指定路径变化时触发重渲染
- [ ] 未提供 `paths` 时保持全 scope 订阅行为（向后兼容）
- [ ] 所有现有 dialog/drawer 测试通过
- [ ] 新增测试覆盖选择性订阅行为
- [ ] `pnpm typecheck && pnpm build && pnpm lint && pnpm test` 通过
- [ ] 确认 `docs/architecture/renderer-runtime.md` 无需更新（API 为可选扩展，无契约变更）
- [ ] `docs/logs/` 对应日期条目已更新
...
- [ ] `word-editor-page.tsx:96-110`：为 `editorRuntime` selector 添加 `shallowEqual` 作为 equalityFn
- [ ] `use-table-controls.ts:26-30`（或拆分后的 `use-table-pagination.ts`）：为 `useTablePagination` 的 scope selector 添加 `shallowEqual` equalityFn
- [ ] `crud-renderer-state.ts:232-270`：将 `useCrudRuntimeState` 单 selector（已有 equalityFn）拆分为多个细粒度 selector，每个只读取 1-2 个路径
- [ ] `crud-renderer-state.ts:272-303`：将 `defaultQuery` 对象用 `useMemo` 包裹，依赖项为实际使用的字段列表
- [ ] `word-editor-page.tsx:136-151`：将 `hostScope` 对象用 `useMemo` 包裹，依赖项为 `[savedDocument?.data, datasets, runtimeHostSummary, selection, props.path]`
- [ ] 为每处修改添加验证：render count 测试或在 playground 中使用 React DevTools Profiler 确认重渲染减少

Exit Criteria:

- [ ] 3 处 scope selector 均有 equalityFn 或已拆分为细粒度 selector
- [ ] `defaultQuery` 对象引用稳定
- [ ] word-editor `hostScope` 对象引用稳定
- [ ] 当实际数据变化时 `scope.replace` 仍被正确调用
- [ ] `pnpm typecheck && pnpm build && pnpm lint && pnpm test` 通过
- [ ] 确认 `docs/architecture/renderer-runtime.md` 无需更新（equalityFn 为内部优化，无契约变更）
- [ ] `docs/logs/` 对应日期条目已更新
...
- [ ] `page-renderer.tsx:116-128`：将 hostScope 对象用 `useMemo` 包裹，依赖项为 `[spreadsheet, props.path]`（`spreadsheet` 已由上游 `useMemo` 稳定）
- [ ] `designer-context.ts:152-160`：将 `useDesignerHostScope` 的 `useMemo` 依赖项从 `[input]` 改为 `[input.snapshot, input.config, input.core, input.path]`
- [ ] 为每处修改添加 render count 测试验证 scope.replace 调用频率降低
- [ ] 验证当实际数据变化时 `scope.replace` 仍被正确调用

Exit Criteria:

- [ ] 2 处 hostScope 对象引用稳定，不再每次渲染触发 `scope.replace`
- [ ] 当实际数据变化时 `scope.replace` 仍被正确调用
- [ ] `pnpm typecheck && pnpm build && pnpm lint && pnpm test` 通过
...
- [ ] `useSurfaceScopeSnapshot` 在提供 paths 时只订阅指定路径（P0 修复）
- [ ] 所有 scope selector 有 equalityFn（P1 修复）
- [ ] 所有 hostScope 对象引用稳定（P1 修复）
- [ ] 数据变化时 `scope.replace` 仍正确触发（无 stale data）
