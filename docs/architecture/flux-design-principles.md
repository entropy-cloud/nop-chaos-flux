...
Promotion Test 是演化的护栏：新概念必须在跨域、不可归约、语义稳定、作者可见、非实现便利、非宿主逃逸这六条全部满足时才能升格为原语。
...
| Semantic Lifecycle Entry  | Template + ScopeRef + Value + Capability + Reaction | 节点拥有的业务管道：表单提交、对话框确认、页面进入等语义操作由生命周期节点拥有，而非 UI 触发器        |
| FormRuntime / PageRuntime | 全部原语                                            | 领域特定运行时：表单验证-提交管道、页面生命周期等                                                     |

#### Action Algebra

Capability（命令派发）和 Value（求值 + 依赖追踪）组合为编译期 DAG。调度循环（`flux-action-core/action-dispatcher/action-execution.ts`）：
...
Capability 和 Resource 的执行控制维度提取为共享策略层，对 action 和 data-source 通用（`CompiledOperationControl`，`flux-core/types/compilation.ts`）：
...
#### Semantic Lifecycle Entry：表单提交管道

表单提交由五个原语组合（`flux-runtime/form-runtime-submit-flow.ts`）：

```
executeFormSubmit:
  1. Template → 防重复提交（submitting/disposed 检查）
  2. ScopeRef → touch 所有字段，展开验证模型
  3. Value → validateForm('submit')，失败走 onValidateError
  4. 子树契约 → recurse-submit / summary-gate 协调子表单
  5. Capability → 执行 submitAction（ajax 等）
  6. Action Algebra → 按 success/failure 执行 onSubmitSuccess/onSubmitError
  7. Template → 清理 submitting 状态
```
