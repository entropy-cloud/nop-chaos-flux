...
| 单元格合并（`combineNum` / colSpan-rowSpan）                                                          | **实现**                 | —          | 分组报表常规需求。amis 语义：前 N 列连续相同值的行合并 rowSpan；被合并 cell 不渲染。virtual 开启时退化为不合并（已在 §4/§12 标注）。配套 `combineFromIndex?: number`（合并起始列，`combineNum` 作用于 `[combineFromIndex, combineFromIndex+combineNum)`，与 `combineNum` 同级）。列级 `headerAlign`/`vAlign`（表头/单元格对齐类，`text-left/center/right` + `align-top/middle/bottom`）、`classNameExpr`（cell 级条件样式表达式，raw 无 `${}`，按 row scope 求值，失败降级无 className + dev warn，Failure Path `expr-eval-error`）、表级 `showHeader`（`false` 隐藏整个表头行，默认 true）均已实现                                                                                               |
...
### C1a 视觉 token 面（2026-08-09，AMIS 视觉对齐基线）

> 本小节记录 C1a 计划的 live 契约。所有关键视觉数值一律经 CSS 变量（`--table-*` / `--crud-*` / `--dialog-*`）控制，组件与样式文件内不写死魔法数字；宿主（nop-chaos-next）可覆盖 token 二次调色。

- **密度与字号**：body 12px / thead 14px（`--table-body-font-size` / `--table-header-font-size`）、行高 40px（`--table-row-height`）、单元内边距 10/11px（`--table-cell-padding-x/y`）、边列 16px（`--table-edge-padding-x`）、表头填充背景 `--table-header-bg`、表头列分隔线 `--table-header-separator-color`。实现载体：`packages/ui/src/styles/table.css`（`@import` 于 `ui/src/styles/index.css`）+ `ui/components/ui/table.tsx`（原语不再写死 `text-sm`/`p-2`/`h-10`）。
- **stripe/bordered 真实现（从死属性变活）**：表级 `data-striped` / `data-bordered` 输出由 `table.css` 规则承接——`[data-striped]` 行背景 `var(--table-striped-bg)`（默认 transparent = AMIS 默认无条纹；条纹规则带 `:not(:hover)` 限定，hover 行恒不套条纹），`[data-bordered]` 外框 + 单元格垂直分隔线走 `--border`。不再是死属性。
- **固定列背景继承语义**：`createStickyStyle`（`fixed-columns.ts`）不再写死 `hsl(var(--background))`；sticky 单元格背景透明继承（`background: inherit` AMIS 语义），行级 hover/striped/selected 色经 `--table-hover-bg`（hover 渐变）/`--table-striped-bg`/`--table-selected-bg(-strong)` 直接透传固定列。固定边缘阴影走 marker class `nop-table-sticky-edge-left/right`（仅方向最后一个固定列）+ `table.css` `::after`（`--table-fixed-edge-width` / `--table-fixed-edge-shadow(-right)`，z-index 低于 affix 表头）。
- **列宽测量回填（C1a 机制）**：`table-renderer.tsx` 挂载 `<colgroup>`（实测宽度以内联 `style` 写入 `<col>`）+ 测量器（`column-width-measure.ts`，`measureColumnWidthsFromTable` 纯函数 + `useTableColumnWidths` hook）对 `<th>` 逐个量宽回填 state；列显隐/resize 时 re-measure，测量失败（首帧不可见等）回退声明宽，无声明则不写宽度交由浏览器 auto，并 dev warn。`createFixedColumnLayout` 接受 `measuredWidths`，固定列 offset 与 cell 宽优先实测，回退声明宽，最后回退 `DEFAULT_FIXED_COLUMN_WIDTH`（160px）/控制列 40px。`use-column-resize` 拖宽为组件 local 状态（不写测量 state，属增强 Follow-up）。
- **操作列与工具栏间距**：行操作按钮高度 `--table-row-action-height`（32px）、gap `--table-row-action-gap`（10px）；CRUD 工具栏 `--crud-toolbar-gap`（10px）。
- **宿主覆盖入口**：宿主在自身 CSS 中重新定义同名 `--table-*` / `--crud-*` 变量即可整体改调（优先级高于 flux `theme-tokens` 的 `:root` 默认值）。
...
interface TableResponsiveConfig {
  ...
  defaultExpanded?: boolean; // 默认 false；true 时 expand 激活下默认展开所有 detail row（用户仍可逐行收起/展开）
}
```
