...
function filterUsersByQuery(sourceUsers: typeof users, query: string) {
  ...
  const [env] = useState<RendererEnv>(() => ({
    async fetcher<T>(api: ExecutableApiRequest, ctx: ApiRequestContext) {
      ...
      if (api.url === '/api/validate-username') {
        ...
        const username = String((api.data as Record<string, unknown> | undefined)?.username ?? '')
          .trim()
          .toLowerCase();
        const exists = directoryUsersRef.current.some(
    ...
    <main className="min-h-screen grid place-items-center p-6">
      <section className="max-w-[1100px] p-10 rounded-3xl bg-[var(--nop-hero-bg)] border border-[var(--nop-hero-border)] shadow-[var(--nop-hero-shadow)]">
        ...
        <p className="text-lg leading-relaxed text-[var(--nop-body-copy)]">
          The first execution slice is live: schema compilation, runtime evaluation, React
          rendering, forms, dialogs, actions, table operations, adaptors, monitoring, and request
          cancellation all run inside the playground.
        </p>
        <p className="mt-2.5 text-[15px] leading-relaxed text-[var(--nop-body-copy)]">
          Submit the user form to append a record into the local directory and watch the table plus
          monitor panel update in real time.
        </p>
        <p className="mt-2.5 text-[15px] leading-relaxed text-[var(--nop-body-copy)]">
          The form now includes an expression editor field at the bottom. It provides syntax
          highlighting, auto-complete for variables, and real-time validation. Try typing variable
          names like `username` or `role` and combine them with operators.
        </p>
        <p className="mt-2.5 text-[15px] leading-relaxed text-[var(--nop-body-copy)]">
          The username field now validates on blur, debounces async uniqueness checks for 500ms, and
          only reveals errors after a field has been touched. Validation timing and error visibility
          are now configured independently at the form or field level.
        </p>
        <p className="mt-2.5 text-[15px] leading-relaxed text-[var(--nop-body-copy)]">
          A first package-based debugger is now mounted as a floating panel. Drag it, minimize it,
          and reopen it from the left-bottom launcher while interacting with the schema.
        </p>
        <p className="mt-2.5 text-[15px] leading-relaxed text-[var(--nop-body-copy)]">
          AI automation can read the structured debugger API from `window.__NOP_DEBUGGER_API__` or
          the multi-instance registry at `window.__NOP_DEBUGGER_HUB__`.
        </p>
