...
| 编号       | 文件:行                                                 | 摘要                                                                                                                                                                                                                                                    | Triage                                                              |
| ---------- | ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| HCA10-P3-1 | `undo-redo-adapter.ts:232` / `operation-coalesce.ts:44` | 合并窗口时间戳源 = `Date.now()`（wall clock，非 monotonic）。系统时钟回拨（NTP 调整 / 多 tab）时 `incoming.ts - top.ts` 可能为负 → 负数 ≤500ms → 误合并跨窗口操作。实际编辑器单 tab 短窗口场景概率极低；monotonic 时钟（performance.now）需 host 接入。 | residual-adjudicated at HCA-CR（watch-only，低概率，Successor: no） |
