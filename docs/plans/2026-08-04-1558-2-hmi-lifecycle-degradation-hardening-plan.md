...
- [x] `Fix` — `use-scada-engine.ts`：observer/rafId 提为 ref；`destroy` 复用 mount cleanup 的断开逻辑（observer.disconnect + cancelAnimationFrame）；**绑定域（pipeline/animator/collector）最新实例存 ref，mount cleanup 与 `destroy` 统一释放当前域**（reload 后 unmount 不再泄漏最新 animator 时钟，M1）。
- [x] `Fix` — `use-scada-engine.ts`：`setPointValues` 注入闭包改经 `runtimeRef.current` 取最新 pipeline 写入（含 applyAttrs）。
- [x] `Fix` — `use-scada-config-sync.ts` catch 内 `prevRef.current = undefined`（full/reset 构建失败后强制下次 full 重建）。
- [x] `Fix` — `use-scada-engine.ts` `reloadBindings`：reset 前快照现存点值，`loadDeclarations` 后**经 PointStore 新增值恢复方法（如 `restoreValues(snapshot: Map<pointId, value>`），按 pointId 直接回填 entry（`entry.value = snapshot; dirty = true`，禁止经 applyValue/setPointValue 回填——`convert` 会再次施加线性 scale 造成双重换算，m2）**；`point-store.ts` 加入 Phase 1 Targets；声明不存在者丢弃，新声明用 init。
- [x] `Decision` — `syncImported`（importConfig 全量替换）语义：与 props 同步共用 `reloadBindings`——裁定 import 路径**重置为 init**（显式全量替换契约，author 意图是换画面）而 props full/diff 路径**按 id 保留**（运行期值不静默丢失）；裁定结果写进 Proof ②/④ 断言与 `design-renderer.md:163` 文档。
- [x] `Fix` — `scada-canvas.tsx`：destroy 状态面——`ScadaCanvasStatus` 增 `destroyed`（或等效 data-status），`component:destroy` 句柄路径（use-scada-handles destroy 回调）置状态；wrapper `data-status` 反映销毁。

Exit Criteria:

- [x] ⑥ 组前置回归测试全绿入库（①-⑥ 对应六条 Fix/Decision）；destroy 后 observer 断开、reload 后 unmount 无动画时钟空转、状态可见均可 repo-observable 断言。
- [x] `component:destroy` 后再调 handle 命令返回 not-mounted（既有语义不回归）；e2e/tooling 不再把已销毁画布报为 healthy（status 断言）。
- [x] `design-renderer.md:163` 同步 live baseline（含 import reset / props 保留合并语义）。
...
- [x] `Fix` — `event-bridge.ts`：各 handler（handleTap/handleDoubleTap/handlePointerMove/handlePointerLeave/handleHover）顶层 try/catch，异常经去重上报（沿用 reportOnce 式数据通道或新增可选 `onHandlerError` 选项，错误码 `handler-error`），**明确不升级画布 status**（P1-8 降级契约，m3）；`handleHover` 同符号去重（`lastHovered === symbolId` 时不重复 emit hover；hover-miss 后重置）。
- [x] `Fix` — `dirty-collector.ts`：`when:'always'` 动画启动移出 `collectStates` 独占路径——首次全量同步（`synced=false`）遍历**全部动画承载图元**（经 `engine.getSymbols()` 或 pipeline 新增 `getSymbolIds` 选项，覆盖无 states 且无绑定图元）统一 `animator.start`（幂等 start）；diff 新增的无绑定动画图元在同机制内随下次同步启动（m1）。
- [x] `Fix` — `point-store.ts` `applyValue` 订阅者循环 try/catch（单订阅者异常隔离，继续剩余循环；异常经去重上报）。

Exit Criteria:

- [x] ④ 组前置回归测试全绿入库；handler throw 后 leafer 管线无污染（后续事件可达）、status 不升级；无绑定图元 always 动画启动有断言。
- [x] hover 同符号去重有精确发射计数断言；always 动画无 states 场景有启动断言。
- [x] 包级既有 483 单测无回退（Phase 局部 typecheck + 相关 spec 复跑）。
...
- [x] `Proof` — 平台能力调研：`ExpressionCompiler.compileValue` 产物（`CompiledRuntimeValue.node`）+ 求值依赖收集（`packages/flux-formula/src/evaluate.ts:104-117` `stateNode.dependencies`，经 `createState` + `evaluateNode` 的 tracking scope 记录）——确认「复杂表达式 → 依赖路径集」的平台提取方式（probe 求值一次收集依赖 → 以依赖路径订阅 → 变更重求值）；**必须验证 probe 边界：依赖收集仅在 `exec` 成功后执行（`finalize()` 在 try/finally 之后，抛错跳过收集）——纯复杂表达式 config 无种子路径、probe scope 为空时求值抛错会走 P1-8 跳过路径**；Fix 需指定容错机制（宽容 probe scope（提供 `undefined` 兜底）或最小种子订阅）使依赖收集可达（m2-r2）；禁自研表达式解析（Cross-Cutting 复用表）。
- [x] `Proof` — 前置回归测试（TDD）：复杂表达式（`${analog.temp + 1}` 类）config 的 flux 点随 scope 数据变化而更新（useScopeSelector paths 由平台依赖收集产出），失败时按声明跳过 + 去重上报（P1-8 语义保持）。
- [x] `Fix` — `use-scada-points-bridge.ts`：`extractFluxScopePaths` 扩展——复杂表达式经平台依赖收集产出订阅路径（Probe 求值一次 + 缓存；**按 Proof 调研结论提供宽容 probe scope 或最小种子订阅使空 scope 下依赖收集可达**），`useScopeSelector` 订阅生效；求值仍走既有 `compiledCache` 单次编译路径。
- [x] `Fix` — `compiledCache`（:112）生命周期：config 变更（或绑定域重载）时清空，杜绝无界增长。

Exit Criteria:

- [x] 复杂表达式点更新回归测试全绿（focused 单测，注明订阅路径产出来源为平台依赖收集）；无表达式解析自研实现。
- [x] compiledCache 随 config 重载清空（测试断言 cache 大小归零）；长会话无界增长消除。
...
- [x] `Decision` — width/height/viewport props 变更反应：**width/height 为确定 Fix**（`use-scada-engine.ts:159-167` setSize effect deps 缺 width/height，补 deps 即生效，m10）；**viewport policy 为二选一**——(a) change 基准 effect 重应用初始视口策略（不改 diff 路径语义）或 (b) 文档化「仅初始应用」实际契约；recommend (a)（design-renderer.md §8.3 声称「width/height/viewport 变化 → 引擎命令式 API」）；裁定后 `§8.3` 同步真实语义。**裁定结果**：width/height 落地 Fix（effect deps 补 `args.width, args.height`）；viewport policy 裁定为 (b)——保持「仅 full/reset 路径应用」现状契约（diff 重应用会重置用户平移/缩放），§8.3 已同步。
- [x] `Decision` — `scada-image` `loadFailed` 消费者：保持占位渲染（视觉契约）+ 修正 `image.ts:43-47` 误导注释 + `design-symbols.md` 注记「资源加载失败渲染占位、画布级诊断后置 I16」；不新增画布级错误升级（P1-8 契约）。
...
- [x] `Fix` — `scada-errors.ts` 错误码注册表：集中定义 `ScadaErrorCode`（config-parse/config-invalid/config-build-failed/engine-create-failed/flux-compile-failed/flux-evaluate-failed/handler-error/not-visible/... 现行码整理）+ code→i18n key 映射；`scada-canvas.tsx` 错误区经 `useFluxTranslation` 渲染本地化文案，未知码 fallback 原始 message（`flux-i18n` locale 文件补 key，Cross-Cutting i18n 复用）。
- [x] `Fix` — doc 注记：`design-renderer.md` 补「缺 config 渲染最小空场景（ready）而非 loading」实际契约（行为由 plan `{1}` 收口，本项仅文档同步，不重复实现）。
- [x] `Fix` — `use-scada-handles.ts` fit/center 无 bounds 失败路径对齐 §8.5 表 `not-visible`（错误码/文案，表行在 `design-renderer.md:242`）；`design-renderer.md:242` 措辞与实际实现一致（原 :235 引用系 audit 行号漂移，m4）。
- [x] `Fix` — doc drift 同步：`design-engine.md:213-214`（引擎事件表 ready/error 行）归属回写（引擎事件表只列 symbol 事件 + render；`scada:ready`/`scada:error` 为 renderer 层 action 派发）。

Exit Criteria:

- [x] ⑤ 组前置回归测试全绿入库（①-⑤ 对应 Decision/Fix）；错误码注册表 + i18n 文案落地（locale key 在 `flux-i18n` 文件可查）。
- [x] absent config 兜底不回归（ready 空画布，非 loading，断言见 Proof ②）；fit/center `not-visible` 失败路径可断言；width/height 变更生效断言。
- [x] `design-renderer.md §8.3`（viewport 反应实际语义）/`:242`（not-visible）/`design-engine.md:213-214`/`design-symbols.md`（image 注记）与 live baseline 一致。
...
- [x] 生命周期收口：destroy 无残留观察器、注入通道写最新 pipeline、失败构建不污染基线、live 值合并保留、销毁状态可见（Phase 1 回归全绿）。
- [x] 健壮性收口：handler throw 隔离、hover 去重、always 动画无 states 可启动、订阅者异常隔离（Phase 2 回归全绿）。
- [x] 数据面收口：复杂表达式订阅生效、compiledCache 有界（Phase 3 回归全绿）。
- [x] 错误面收口：错误码注册表 + i18n、空态、not-visible、props 反应契约收敛（Phase 4 回归全绿）。
- [x] 不存在被静默降级到 deferred / follow-up 的 in-scope live defect 或 contract drift（唯一 Decision 类 deferred 见 Deferred But Adjudicated）。
- [x] 必要 focused verification 已完成；受影响的 owner docs 已同步到 live baseline（design-renderer.md §8.3/§4.3/§8.5、design-engine.md §8.1、design-symbols.md）。
...
Status Note: 独立 closure-audit 通过（fresh session，未参与执行）。全部 4 Phase items/Exit Criteria 已 repo-observable 落地；workspace typecheck/build/lint/test 全绿（FULL TURBO content-hashed cache）；industrial 包 521 tests / 38 files 全绿，coverage branches 90.09% ≥ 90% 阈值；deferred 项分类诚实。

Closure Audit Evidence:

- Auditor / Agent: 独立 fresh-session closure auditor（未参与执行链路；本 session 仅读源码/测试/文档 + 跑验证命令）
- Evidence:
  - Phase 1（生命周期）：`use-scada-engine.ts:91-118` `releaseRuntime` 共用断开逻辑（observer.disconnect + cancelAnimationFrame + `runtimeRef.current` 最新域释放，M1）；`:156-168` SL-3 注入经 runtimeRef；`use-scada-config-sync.ts:163,191` SL-4 catch 内 prevRef=undefined；`:217-220` reloadBindings snapshot/restoreValues（绕 convert，m2）；import `{preserveValues:false}`（Decision）；`scada-canvas.tsx:18,136-138,198` destroyed 状态可见（OP-4）。6 Proof 用例全绿（lifecycle-hardening.test.tsx）。
  - Phase 2（健壮性）：`event-bridge.ts:99-159` safeRun + reportHandlerError 去重（SL-5/m3）；`:134-151` hover 同符号去重（WD-4）；`dirty-collector.ts:241-256` startAlwaysAnimations 遍历 getSymbolIds（SL-1/m1，在脏检查早退前调用）；`point-store.ts:55-68,263-281` EventHub + 订阅者循环 try/catch + reportSubscriberError 去重（OP-3）。Proof 测试断言真实行为（throw 隔离、emit 计数、isPlaying）。
  - Phase 3（数据面）：`use-scada-points-bridge.ts:57-86` extractExpressionDepsViaProbe 经平台 compileValue+createState+evaluateWithState+读 state.root.dependencies.paths（WD-2，禁自研解析）；`:34-49` 宽容 probe scope（m2-r2）；`:219-221` compiledCache 随 config 变更清空（WD-3）。Proof 测试断言复杂表达式重求值 + 编译计数重置。
  - Phase 4（错误面）：`scada-errors.ts:23-77` SCADA_ERROR_CODES（12 码）+ scadaErrorI18nKey + useScadaErrorText；`scada-canvas.tsx:206` resolveErrorText 上屏；`use-scada-handles.ts:70,77` 实际 `new Error('not-visible')`（interface-vs-semantics 抽查通过，非注释壳）；i18n key 双语齐全（zh-CN:935-949、en-US:936-950）；docs 同步（design-renderer §8.3/§8.5、design-engine §8.1:214-217 ready/error 归属、design-symbols:207 image 注记）。
  - 验证命令（fresh session 实跑）：`pnpm typecheck` 32/32 ✓（FULL TURBO content-hashed cache）；`pnpm build` 32/32 ✓；`pnpm lint` 32/32 ✓；`pnpm test` 59/59 ✓；`pnpm --filter @nop-chaos/flux-renderers-industrial test` 521 tests / 38 files ✓；coverage stmts 95.55% / **branches 90.09%**（≥90% 阈值通过）/ funcs 97.77% / lines 97.58%。
  - 内部一致性：Plan Status / 4 Phase Status（均 completed）/ 各 Exit Criteria / Closure Gates 现已全勾（audit 项本审计勾选）/ Deferred 项分类诚实——彼此一致，无残留 in-scope 未勾项。
  - Deferred 复核：scada-image loadFailed 消费者分类 `watch-only residual` + P1-8 契约冲突理由 + I16 successor path，诚实 non-blocking；image.ts:44-48 注释如实声明"无画布级消费者"，author 不误信。
- Findings: 0 Blocker / 0 Major / 0 Minor（turbo cache 为 content-hashed，cached green 有效；branches 90.09% ≥ 90% 阈值通过）。

Follow-up:

- 无 plan-owned 剩余工作。Non-Blocking Follow-ups 区两项（point:change action 联动 out-of-scope；probe 求值开销 perf 观察项）维持现状，非本计划契约缺口。
