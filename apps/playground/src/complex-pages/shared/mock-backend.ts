...
// Sundial replica data lives in mock-backend-sundial.ts (file-size governance
// split, plan 460 B8); public API re-exported here for existing importers.
export {
  deleteSundialSubtask,
  filterSundialTasks,
  updateSundialTask,
} from './mock-backend-sundial.js';
export type {
  SundialSettings,
  SundialSubtask,
  SundialTask,
...
import {
  createSundialSettings,
  createSundialSubtasks,
  createSundialTasks,
...
export interface MockDatabase {
  ...
  sundialTasks: SundialTask[];
  sundialSubtasks: SundialSubtask[];
  sundialSettings: SundialSettings;
}
...
export function createMockDatabase(): MockDatabase {
  ...
  const sundialTasks = createSundialTasks();
  const sundialSubtasks = createSundialSubtasks();
  const sundialSettings = createSundialSettings();

  return { depts, tags, users, orders, orderItems, orderLogs, addresses, budgets, payments, shipments, approvalTasks, sundialTasks, sundialSubtasks, sundialSettings };
}
