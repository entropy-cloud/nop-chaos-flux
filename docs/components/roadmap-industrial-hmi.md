...
- `2026-08-05-2129-multi-audit-industrial-hmi.md` `[P2-1]`（dim 16）— `design-renderer.md §11:303-313` 实现树过期：`schemas.ts`/`renderer-definitions.ts` 误置于 `renderer/`（live 在 `src/` 顶层）；缺 `use-scada-handles.ts`/`scada-errors.ts`/`serialization/equality.ts`。建议按 live 布局重写 §11 树。
- `2026-08-05-2129-multi-audit-industrial-hmi.md` `[P2-2]`（dim 16）— `design-engine.md §9:270` 仍引用 I18 后已弃用的 `$xxx` flux 语法（live 仅认 `${expr}`，`$xxx` 走 `dollar-without-brace` Failure Path 求值 undefined）。建议改为 `${expr}` 并指向 `design-data-binding.md §4.2`。
- `2026-08-05-2129-multi-audit-industrial-hmi.md` `[P2-3]`（dim 16）— `editor-initiation.md:60` 三态清单 row 8 引用已删除的 `expression-evaluator` 模块 + 已弃用 `@{pointId}` 语法（live 已拆为 `bind-resolver.ts`+`flux-eval.ts`）。建议对齐 `design-data-binding.md §11` post-I18 模块拆分。
