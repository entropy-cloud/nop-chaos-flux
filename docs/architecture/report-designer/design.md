...
- `report-designer-page.config` 的 builder-facing formal contract 也不再是 opaque object；当前最小公开 vocabulary 至少包含 `fieldSources`、`features.fieldPanel` / `features.inspector`、以及 `inspector.body|byTarget|byProfile` 这些会直接决定 side-panel existence 的键
...
- 字段可拖拽到单元格或区域
- family-level baseline 同时要求存在非拖拽插入路径：standalone/public field row 必须保留独立、可聚焦、可 disabled 的 insert control，drag item 本身不承担 faux button 语义
- drop 时只产生标准化 designer command，不直接修改 document
- 字段面板可按组、分类、搜索、只读提示等方式配置
- 如果最终没有解析出字段源或该能力被 feature gate 关闭，则左侧工作台整体隐藏，不保留纯占位 sidebar
...
根据共享 workbench-shell 基线，inspector 也是 optional side panel：当 `designer.inspector` 未解析出可挂载的 inspector surface，或该能力被 feature gate 关闭时，右侧工作台应整体隐藏，而不是强制渲染一个空壳 fallback。
...
- inspector schema 使用固定宿主 scope 读取 canonical `selectionTarget`，同时读取 `activeSheet`、`meta`
- `activeCell`、`activeRange` 如需提供，应视为从 `selectionTarget` 或 nested spreadsheet selection 派生的便利字段，而不是高于 `selectionTarget` 的主契约
...
- row/column size editing 的 keyboard-accessible baseline 固定为 shared context menu: 从 row/column header button 通过 `Context Menu` key 或 `Shift+F10` 打开菜单，再进入带程序化标签的 row-height / column-width dialog；旧 resize handle 不承担可聚焦交互语义
...
- 当当前选择是多行或多列 header 选择时，row-height / column-width size action 必须 disabled，而不是对锚点静默生效
...
- `designer` — 主投影，包含 `selectionTarget`、`inspector`、`fieldDrag`、`preview`、`activeMeta`、report-owned `canUndo/canRedo`、`fieldSources`、`fieldSourceCount`、`fieldCount` 等
...
Current retained/removed matrix:

- retained canonical projection fields:
  - `designer.selectionTarget`
  - `designer.inspector`
  - `designer.fieldDrag`
  - `designer.preview`
  - `designer.activeMeta`
  - `designer.canUndo`
  - `designer.canRedo`
  - `designer.fieldSources`
  - `designer.fieldSourceCount`
  - `designer.fieldCount`
  - `runtime`
  - `spreadsheet`
  - `selectionTarget`
...
- retained convenience mirrors:
  - top-level `workbook`, `activeSheet`, `documentName`, `fieldCount`, `inspector`, `meta`, `preview`
  - top-level `activeSheet` mirrors the current active-sheet baseline and does not drop to `undefined` for valid `workbook` / `row` / `column` report targets while a spreadsheet active sheet still exists
- removed from the supported schema-visible boundary:
  - `ReportDesignerCore` / `SpreadsheetCore` instances
  - raw store objects and bridge-private adapters
  - canvas-only transient editing state that is not part of the published host projection
  - any alternate workbook truth surface that diverges from canonical `document.spreadsheet`
...
- `report-toolbar` 默认项现在直接声明 canonical namespaced actions，例如 `report-designer:save`、`report-designer:undo`、`report-designer:redo`、`report-designer:preview`
- host action provider 负责解析这些 canonical action，并在需要时再桥接到 report-designer 或 spreadsheet command surface；toolbar renderer 本身不再对 `report-designer:` 做二次前缀拼接
- 因此默认 toolbar、schema 自定义按钮、以及 host action provider 共享同一条 namespace/method contract，避免 `report-designer:report-designer:*` 这类 duplicated-prefix drift

导入模板后，`report-designer:importTemplate` 必须立即刷新 `fieldSources`、`inspector.resolvedSchema` 和相关 loading/error 状态，避免新文档与旧派生缓存并存。

Preview cancellation baseline:

- preview runtime adopts single-owner request sequencing: starting a newer `report-designer:preview` aborts the previous in-flight adapter call before the newer request becomes the active owner
- `report-designer:stopPreview` must abort the active preview request instead of only clearing local `preview.running`
- preview completion is gated by active request ownership, so a stale or aborted request cannot clear `preview.running` or overwrite `preview.lastResult`
- preview adapters may observe optional `args.signal` for cancellation-aware work; adapters that ignore the signal still cannot publish stale state because core-side request ownership guards the final state write

顶层便利字段 (`inspector`, `inspectorPanels`, `meta`, `documentName`, `fieldCount`) 是 `designer.*` 或 `runtime.*` 的镜像。

Current live baseline note:

- `inspectorPanels` remains a supported schema-visible convenience mirror because live host scope still publishes both `designer.inspectorPanels` and top-level `inspectorPanels`
- docs should not classify `inspectorPanels` as implementation lag while that projection remains published
- nested `spreadsheet.selection` is a structured spreadsheet-facing selection surface: `cell` publishes anchor coordinates, `range` publishes explicit range bounds, `row` / `column` publish `rows[]` / `columns[]`, and `sheet` publishes `sheetId`
...
{
  "type": "text",
  "text": "当前目标: ${selectionTarget.kind}"
}
```
